...
CPU-only validation used a fresh Visual Studio 2022 x64 Release tree with CUDA, Vulkan, developer tools and C ABI disabled. The final build, CLI `--help`, `unit` and `reference_media_decode` passed (2/2 CTest suites; 22,563 unit checks). Shader-manifest validation preserved all 54 original pinned source/binary hashes and supplied all 41 embedding-template variables.

The CUDA/Vulkan Release build also passed all 21 registered CTest suites across three runs: host/C ABI/CLI and synthetic GPU suites, checkpoint/integration suites, and CUDA dispatch suites. Cross-backend synthetic coverage performed 144,336 checks. Real-checkpoint coverage included the full Vulkan INT8 H3 path, full Qwen INT8/NVFP4 conditioner comparisons, and CUDA resident/streaming encoder consistency. The final C ABI run performed 267 checks after adding retained preflight plans. Exact H3 CUDA qualification, absent captures/fixtures, and disabled opt-in scenarios were reported as skips; the existing kernel suite still reports 11 deferred known-defect checks. Passing suites do not imply every optional scenario ran.

A two-output CUDA CLI smoke test used the local NVFP4 transformer/conditioner, FP16 video VAE and FP32 audio VAE, with 256x256 output, 22 frames, two grid points and seeds 42/43. Both MP4s were written. The second run reused cached prompt conditioning; observed total times were 13.60 and 4.96 seconds. This checks stage integration and reuse, not output quality or full-length generation parity. Explicit JSON recipe inheritance and incompatible-sampler CLI rejection also passed.

The released Viggle and TaoMate adapter compatibility tests were also run from the repository root against the available local weights: 308 checks passed. These two cases are skipped by the default build-directory host run because their fixture paths are relative to the repository root.

The C API retains its validated plan through asynchronous execution. Remaining work includes consuming that same immutable plan in cache identity helpers that still re-read metadata, completing backend-context/profiler ownership before permitting session concurrency, and introducing a second family through explicit conditioner/transformer/codec implementations. Capability and effective-plan queries should grow with those contracts while preserving the public C ABI's existing layouts.
