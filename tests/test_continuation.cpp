...
SLOPFAB_TEST(continuation_archive_roundtrip_and_validation) {
  ...
  CHECK(rejects([&] { slopfab::LatentClip::load(saved.path.string()); }));
  copy.save(saved.path.string());
  std::filesystem::resize_file(saved.path, std::filesystem::file_size(saved.path) - 4);
  CHECK(rejects([&] { slopfab::LatentClip::load(saved.path.string()); }));
  fixture.write(39, true, "future-version");
  CHECK(rejects([&] { slopfab::LatentClip::load(fixture.path.string()); }));
  fixture.write(39, false);
  auto synthetic = slopfab::LatentClip::load(fixture.path.string());
  CHECK(rejects([&] { slopfab::plan_continuation(*synthetic, 22, 17); }));
  copy.video_rows.pop_back();
  CHECK(rejects([&] { copy.save(saved.path.string()); }));
  CHECK(rejects([&] { slopfab::plan_continuation(*clip, 6, 17); }));
  CHECK(rejects([&] { slopfab::plan_continuation(*clip, 56, 17); }));
  CHECK(rejects([&] { slopfab::plan_continuation(*clip, 22, 0); }));
  CHECK(rejects([&] { slopfab::plan_continuation(*clip, 22, std::numeric_limits<int>::max()); }));
  auto invalid = slopfab::plan_continuation(*clip, 22, 17);
  invalid.overlap_audio_latents = -1;
  CHECK(rejects([&] { slopfab::join_continuation(*clip, invalid, {}, {}); }));
  const auto minimal = slopfab::plan_continuation(*clip, 5, 1);
  CHECK(minimal.extension_frames == 17 && minimal.window_frames == 22 && minimal.overlap_video_latents == 2);
}
...
SLOPFAB_TEST(continuation_guide_positions_and_planning) {
  ...
  CHECK(audio.front() == clip->audio_rows[(l.num_audio_latents - plan.continuation.overlap_audio_latents) * 32]);
  ...
  auto packed = slopfab::dit::build_ref2va_packed_sequence({1,1,1}, geometry, 17, 2, 4, plan.layout.num_audio_latents);
  ...
  packed = slopfab::dit::build_ref2va_packed_sequence({1,1,1}, geometry, 17, 2, 4, plan.layout.num_audio_latents);
  ...
  req.canvas_width = 32; req.canvas_height = 32;
  CHECK(rejects([&] { slopfab::resolve_plan(req); }));
  req.canvas_width = 0; req.canvas_height = 0; req.still_image = true;
  CHECK(rejects([&] { slopfab::resolve_plan(req); }));
}
