      ...
      for (int ss : {1, 2})
        for (int ts : {1, 2})
          for (bool down : {false, true}) {
            ...
            auto y = ops.conv3d(x.get(), w.get(), b.get(), ci, co, frames, height, width, k, ss, ts,
                                down);
            auto hy = ops.conv3d(hx.get(), w.get(), b.get(), ci, co, frames, height, width, k, ss,
                                 ts, down);
                      ...
                      for (int kt = 0; kt < k; ++kt)
                        for (int ky = 0; ky < k; ++ky)
                          for (int kx = 0; kx < k; ++kx) {
                            ...
                            expected +=
                                double(input[((ic * frames + it) * height + iy) * width + ix]) *
                                __half2float(weight[wi]);
                          }
...
SLOPFAB_TEST(reference_half_groupnorm_and_residual) {
  ...
  for (size_t i = 0; i < input.size(); ++i)
    input[i] = __float2half_rn(std::sin(float(i) * .11f));
  for (int c = 0; c < channels; ++c) {
    weight[c] = __float2half_rn(.8f);
    bias[c] = __float2half_rn(.1f);
  }
  ...
  w.copy_from_host(weight.data(), weight.size(), stream.get());
  b.copy_from_host(bias.data(), bias.size(), stream.get());
  reference_groupnorm(x.get(), w.get(), b.get(), y.get(), channels, frames, height, width,
                      stream.get());
  ...
  for (int c = 0; c < channels; ++c)
    for (int t = 0; t < frames; ++t) {
      ...
      for (int i = 0; i < plane; ++i)
        mean += __half2float(input[offset + i]) / double(plane);
      for (int i = 0; i < plane; ++i)
        variance += std::pow(__half2float(input[offset + i]) - mean, 2) / plane;
      for (int i = 0; i < plane; ++i) {
        const double v = (__half2float(input[offset + i]) - mean) / std::sqrt(variance + 1e-6) *
                             __half2float(weight[c]) +
                         __half2float(bias[c]);
        CHECK_NEAR(__half2float(actual[offset + i]),
                   v / (1 + std::exp(-v)) + __half2float(input[offset + i]), .002);
      }
    }
}
