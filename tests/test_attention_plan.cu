...
SLOPFAB_TEST(attention_plan_compact_gqa_workspace_and_execution) {
  ...
  for (int row = 0; row < 37; ++row)
    for (int head = 0; head < 8; ++head)
      for (int dim = 0; dim < 64; ++dim) {
        ...
        full_k[target] = k[source]; full_v[target] = v[source];
      }
  ...
  attention_forward(handle.h, nullptr, dq.p(), dfk.p(), dfv.p(), reference.p(),
                    original, AttentionBackend::kBlocked, reference_workspace);
  SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
  CHECK_CLOSE(reference.host(), output.host(), 0.0, "compiled compact GQA");
  Workspace empty;
  bool rejected = false;
  try { plan.forward(handle.h, nullptr, dq.p(), dk.p(), dv.p(), output.p(), empty); }
...
SLOPFAB_TEST(attention_plan_rejects_unimplemented_contracts) {
  ...
  bool rejected = false;
  try { (void)AttentionPlan::compile(descriptor, policy); }
  catch (const std::invalid_argument&) { rejected = true; }
  CHECK(rejected);
