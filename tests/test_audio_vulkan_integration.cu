...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_exact_generate_vertical_slice, "integration") {
  ...
  if (!std::filesystem::exists(video_checkpoint) || !std::filesystem::exists(audio_checkpoint)) {
    SKIP_MISSING_FIXTURE(
        "unavailable prerequisite: !std::filesystem::exists(video_checkpoint) || !std::filesystem::exists(audio_checkpoint)");
    return;
  }
  int cuda_devices = 0;
  ...
  const std::array<uint8_t, 32> expected_video_sha{0x7c, 0x1f, 0x13, 0x14, 0x92, 0xe7, 0xed, 0xda,
                                                   0xca, 0xac, 0x90, 0x69, 0xa6, 0x1b, 0x81, 0xbd,
                                                   0xd3, 0x9d, 0xe5, 0xcc, 0x96, 0x56, 0x1e, 0x67,
                                                   0x7c, 0x5e, 0xab, 0x1c, 0xdc, 0xe5, 0xe5, 0x22};
  CHECK(mapping_sha256(video_provenance.mapping_base(), video_provenance.file_size()) ==
        expected_video_sha);
  SafeTensors audio_provenance;
  audio_provenance.open(audio_checkpoint.string());
  const std::array<uint8_t, 32> expected_audio_sha{0x8e, 0x50, 0x5d, 0x95, 0xdd, 0x15, 0x61, 0xd4,
                                                   0x7a, 0xbd, 0x43, 0xd4, 0x23, 0x8f, 0xd4, 0x0d,
                                                   0x9b, 0xb1, 0xae, 0x9e, 0x14, 0x7e, 0xd0, 0xa4,
                                                   0xcb, 0xa7, 0x78, 0xd7, 0x6a, 0xe4, 0xdb, 0x48};
  CHECK(mapping_sha256(audio_provenance.mapping_base(), audio_provenance.file_size()) ==
        expected_audio_sha);
#endif
  ...
  const std::vector<float> video_rows =
      values(size_t(plan.layout.num_video_rows) * 96, 83, 1021, 1.0f / 512.0f);
  const std::vector<float> audio_rows =
      values(size_t(plan.layout.num_audio_rows) * 32, 89, 1013, 1.0f / 512.0f);
  const std::filesystem::path base = std::filesystem::temp_directory_path();
  const std::filesystem::path latent_path = base / "slopfab_cuda_vulkan_exact_latents.safetensors";
  write_safetensors(latent_path.string(),
  ...
  CHECK_MSG(cuda_result.ok, "CUDA exact generation failed: %s", cuda_result.message.c_str());
  ...
  CHECK_MSG(vk_result.ok, "Vulkan exact generation failed: %s", vk_result.message.c_str());
  ...
  check_exact(cuda_capture.audio, vk_capture.audio, "run_generate interleaved PCM");
  ...
  video::write_y4m(cuda_y4m.string(), cuda_capture.video, cuda_capture.frames, cuda_capture.height,
                   cuda_capture.width);
  video::write_y4m(vk_y4m.string(), vk_capture.video, vk_capture.frames, vk_capture.height,
                   vk_capture.width);
  audio::write_wav(cuda_wav.string(), cuda_capture.audio, cuda_capture.audio_channels,
                   cuda_capture.audio_sample_rate, audio::SampleFormat::kPcm16);
  audio::write_wav(vk_wav.string(), vk_capture.audio, vk_capture.audio_channels,
                   vk_capture.audio_sample_rate, audio::SampleFormat::kPcm16);
  ...
  const uint64_t pixel_digest = fnv64_floats(cuda_capture.video.data(), cuda_capture.video.size());
  const uint64_t pcm_digest = fnv64_floats(cuda_capture.audio.data(), cuda_capture.audio.size());
  ...
  std::printf(
      ...
      static_cast<unsigned long long>(pixel_digest), static_cast<unsigned long long>(pcm_digest),
      static_cast<unsigned long long>(y4m_digest), static_cast<unsigned long long>(wav_digest),
