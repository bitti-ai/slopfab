...
void* allocate_aligned(std::size_t bytes, std::size_t alignment) {
  ...
  if (void* result = std::aligned_alloc(alignment, rounded))
    return result;
#endif
  throw std::bad_alloc();
}
...
HostAllocationGuard::HostAllocationGuard() noexcept {
  ++allocation_guard_depth;
}

HostAllocationGuard::~HostAllocationGuard() {
  --allocation_guard_depth;
}
} // namespace slopfab::test
