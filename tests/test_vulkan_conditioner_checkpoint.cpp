...
SLOPFAB_TEST_CATEGORY(vulkan_qwen_layer0_real_l132_capture_replay, "checkpoint") {
  ...
  if (!std::filesystem::exists(checkpoint_path) ||
    ...
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(checkpoint_path) || !std::filesystem::exists(capture_path) || !std::filesystem::exists(tokenizer_path) || !Instance::available()");
    return;
  }
  ...
  if (!context.exact_causal_gqa_attention() ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: !context.exact_causal_gqa_attention() || !context.exact_fp32_vae_normalization() || !context.exact_vae_pointwise()");
    return;
  }
