...
SLOPFAB_TEST_CATEGORY(cuda_exact_vae_vit_decoder_integration, "integration") {
  ...
  if (cudaGetDeviceCount(&cuda_devices) != cudaSuccess || cuda_devices == 0) {
    SKIP_UNSUPPORTED_HARDWARE(
        "unavailable prerequisite: cudaGetDeviceCount(&cuda_devices) != cudaSuccess || cuda_devices == 0");
    return;
  }
  ...
  if (vulkan::Instance::available()) {
    ...
    if (!physical.empty() && physical.front().info().timeline_semaphore) {
      ...
      vulkan::VideoVaeDecoder vk_decoder = vulkan::VideoVaeDecoder::create(device, config);
      ...
      const double vk_load_ms = std::chrono::duration<double, std::milli>(
                                    std::chrono::steady_clock::now() - vk_load_begin)
                                    .count();
      ...
      const double vk_forward_ms = std::chrono::duration<double, std::milli>(
                                       std::chrono::steady_clock::now() - vk_forward_begin)
                                       .count();
      ...
      CHECK_MSG(one_used_delta <= one_accounted_delta + (32ull << 20),
                ...
                double(one_used_delta) / 1048576.0 - double(one_accounted_delta) / 1048576.0);
      CHECK_MSG(two_used_delta <= two_accounted_delta + (32ull << 20),
                "two-shape allocator delta exceeds accounting by %.1f MiB",
                double(two_used_delta) / 1048576.0 - double(two_accounted_delta) / 1048576.0);
      ...
      const uint64_t before_bad_descriptors = vk_decoder.descriptor_set_allocations();
      ...
      try {
        vk_decoder.forward_windows(split_latent.data(), 2, 7, 2, 2, vk_split, bad_slots.data());
      ...
      decoder.forward_windows(split_latent.data(), kSplitBatch, 7, 1, 1, cuda_split,
                              split_slots.data());
      decoder.release_host_registrations();
      vk_decoder.forward_windows(split_latent.data(), kSplitBatch, 7, 1, 1, vk_split,
                                 split_slots.data());
      ...
      const vae::DecodedVideo cuda_video =
          decoder.decode(normalized.data(), 7, 1, 1, vae::default_video_latents_mean(),
                         vae::default_video_latents_std(), decode_schedule);
      const vae::DecodedVideo vk_video =
          vk_decoder.decode(normalized.data(), 7, 1, 1, vae::default_video_latents_mean(),
                            vae::default_video_latents_std(), decode_schedule);
      ...
      video::write_y4m(cuda_y4m.string(), cuda_video.data, cuda_video.frames, cuda_video.height,
                       cuda_video.width);
      vulkan::Yuv420Converter yuv_converter;
      video::write_y4m(vk_y4m.string(), vk_video.data, vk_video.frames, vk_video.height,
                       vk_video.width, {}, &yuv_converter);
      ...
      std::printf(
          ...
          vk_load_ms, vk_forward_ms, double(vk_decoder.persistent_bytes()) / 1048576.0,
          double(final_peak) / 1048576.0, double(vk_decoder.allocator_used_bytes()) / 1048576.0,
          double(final_reserved) / 1048576.0, static_cast<unsigned long long>(final_descriptors),
  ...
  std::printf(
      ...
      load_ms, forward_ms, double(decoder.weight_bytes()) / 1048576.0, first.size(), ragged.size(),
