...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_exact_causal_gqa_attention, "synthetic") {
  ...
  cuda::launch_deterministic_causal_gqa_attention(
      ...
      reinterpret_cast<const __nv_bfloat16*>(cv.get()), reinterpret_cast<__nv_bfloat16*>(co.get()),
      sequence, query_heads, kv_heads, dim, exact_attention_scale(dim));
  ...
  cuda::launch_deterministic_causal_gqa_attention(
      ...
      reinterpret_cast<__nv_bfloat16*>(co_repeat.get()), sequence, query_heads, kv_heads, dim,
  ...
  CausalGQAAttentionPlanDesc desc{sequence, query_heads, kv_heads, dim, exact_attention_scale(dim)};
  ...
  auto run_boundary = [&](uint32_t boundary_sequence, uint32_t first_rows) {
    const size_t boundary_q_count = size_t(boundary_sequence) * query_heads * dim;
    const size_t boundary_kv_count = size_t(boundary_sequence) * kv_heads * dim;
    ...
    for (uint32_t row = 0; row < boundary_sequence; ++row) {
      for (uint32_t head = 0; head < kv_heads; ++head) {
        for (uint32_t d = 0; d < dim; ++d) {
          const size_t index = (size_t(row) * kv_heads + head) * dim + d;
          bk[index] = f32_to_bf16(row == 0 ? 0.0f : 31.0f);
          bv[index] = f32_to_bf16(row == 0 ? float(int(head) - 4) / 8.0f
    ...
    cuda::launch_deterministic_causal_gqa_attention(
        ...
        reinterpret_cast<__nv_bfloat16*>(dco.get()), boundary_sequence, query_heads, kv_heads, dim,
        exact_attention_scale(dim));
    SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DeviceTensor vq_boundary =
        vk.allocate(TensorLayout::contiguous(boundary_q_shape, 3), ScalarType::kBFloat16);
    DeviceTensor vk_boundary =
        vk.allocate(TensorLayout::contiguous(boundary_kv_shape, 3), ScalarType::kBFloat16);
    DeviceTensor vv_boundary =
        vk.allocate(TensorLayout::contiguous(boundary_kv_shape, 3), ScalarType::kBFloat16);
    DeviceTensor vo_boundary =
        vk.allocate(TensorLayout::contiguous(boundary_q_shape, 3), ScalarType::kBFloat16);
    ...
    CausalGQAAttentionPlan boundary_plan = CausalGQAAttentionPlan::create(
        vk, {boundary_sequence, query_heads, kv_heads, dim, exact_attention_scale(dim)});
    TensorBatch boundary_batch = vk.begin_batch();
    boundary_plan.record(boundary_batch, vq_boundary, vk_boundary, vv_boundary, vo_boundary, 0,
                         first_rows, 0);
    if (first_rows < boundary_sequence)
      boundary_plan.record(boundary_batch, vq_boundary, vk_boundary, vv_boundary, vo_boundary,
                           first_rows, boundary_sequence - first_rows, first_rows);
    boundary_batch.submit().wait();
    std::vector<uint16_t> boundary_got(boundary_q_count);
    vk.download_bytes(vo_boundary, boundary_got.data(), boundary_got.size() * sizeof(uint16_t));
    ...
    for (uint32_t head = 0; head < query_heads; ++head) {
      ...
      for (uint32_t d = 0; d < dim; ++d)
        CHECK(boundary_got[size_t(head) * dim + d] == bv[size_t(mapped) * dim + d]);
    }
  };
  run_boundary(1, 1);
  run_boundary(127, 63);
  ...
  DeviceTensor out_second =
      vk.allocate(TensorLayout::contiguous(q_shape, 3), ScalarType::kBFloat16);
  ...
  for (int repeat = 0; repeat < 4; ++repeat) {
    Submission a = submit_full(out), b = submit_full(out_second), c = submit_full(out);
  ...
  DeviceTensor wrong_type = vk.allocate(TensorLayout::contiguous(q_shape, 3), ScalarType::kFloat32);
  {
    TensorBatch recover = vk.begin_batch();
    bool rejected = false;
    try {
      plan.record(recover, q, k, v, wrong_type);
