...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_linear_weight_real_nvfp4_slab, "checkpoint") {
  ...
  if (!std::filesystem::exists(path) ||
      ...
      !Instance::available()) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(path) || cudaGetDeviceCount(&cuda_devices) != cudaSuccess || cuda_devices == 0 || !Instance::available()");
    return;
  }
...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_streamed_nvfp4_gemm_real_slab, "checkpoint") {
  ...
  if (physical.empty() || !physical.front().info().timeline_semaphore ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: physical.empty() || !physical.front().info().timeline_semaphore || !physical.front().info().cooperative_matrix_bf16_f32_16x16x16");
    return;
  }
  SafeTensors checkpoint;
  checkpoint.open(path.string());
