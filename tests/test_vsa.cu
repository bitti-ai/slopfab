...
namespace {
float bf(float x) {
  return slopfab::bf16_to_f32(slopfab::f32_to_bf16(x));
}
...
void verify_vsa(int dim) {
  ...
  for (int head = 0; head < heads; ++head)
    for (int t = 0; t < n; ++t) {
      ...
      for (int key = 0; key < n; ++key)
        for (int d = 0; d < dim; ++d)
          scores[key] += qp[size_t(t) * inner + head * dim + d] *
                         kp[size_t(key) * inner + head * dim + d] / std::sqrt(double(dim));
      ...
      for (int key = 0; key < n; ++key)
        mask[key] = t < tiles.prefix_tiles || key < tiles.prefix_tiles;
      for (int i = 0; i < (int(order.size()) + 4) / 5; ++i)
        mask[order[i]] = 1;
      ...
      for (int d = 0; d < dim; ++d) {
        double value = 0;
        for (int key = 0; key < n; ++key)
          value += scores[key] / total * vp[size_t(key) * inner + head * dim + d];
  ...
  for (int row = 0; row < seq; ++row)
    for (int head = 0; head < heads; ++head) {
      ...
      for (int key = 0; key < seq; ++key) {
        if (!mask[tiles.row_tiles[key]])
          continue;
        double score = 0;
        for (int d = 0; d < dim; ++d)
          score += double(__bfloat162float(q[size_t(row) * inner + head * dim + d])) *
                   __bfloat162float(k[size_t(key) * inner + head * dim + d]);
      ...
      for (int d = 0; d < dim; ++d) {
        double value = 0;
        for (int key = 0; key < seq; ++key)
          value += scores[key] / sum * __bfloat162float(v[size_t(key) * inner + head * dim + d]);
        const size_t i = size_t(row) * inner + head * dim + d;
        expected[i] = bf(float(value));
        expected_gate[i] =
            bf(expected[i] + bf(__bfloat162float(gate[i]) *
                                compressed[size_t(tiles.row_tiles[row]) * inner + head * dim + d]));
      }
    }
  Stream stream;
  DeviceBuffer<int32_t> rows(tiles.rows.size()), sizes(tiles.sizes.size()),
      row_tiles(tiles.row_tiles.size());
  ...
  DeviceBuffer<__nv_bfloat16> dq(count), dk(count), dv(count), dg(count), output(count),
      coarse(size_t(n) * inner);
  dq.copy_from_host(q.data(), count, stream.get());
  dk.copy_from_host(k.data(), count, stream.get());
  dv.copy_from_host(v.data(), count, stream.get());
  dg.copy_from_host(gate.data(), count, stream.get());
  VsaConfig config{n, tiles.prefix_tiles, heads, dim, rows.get(), sizes.get(), row_tiles.get()};
  Workspace ws;
  ws.reserve(vsa_attention_workspace_bytes(n, heads, dim));
  vsa_attention_forward(stream.get(), dq.get(), dk.get(), dv.get(), output.get(), coarse.get(),
                        config, ws);
  std::vector<__nv_bfloat16> got(count), got_coarse(size_t(n) * inner);
  output.copy_to_host(got.data(), count, stream.get());
  coarse.copy_to_host(got_coarse.data(), got_coarse.size(), stream.get());
  SLOPFAB_CUDA_CHECK(cudaStreamSynchronize(stream.get()));
  double max_error = 0, coarse_error = 0;
  for (size_t i = 0; i < count; ++i)
    max_error = std::max(max_error, double(std::fabs(expected[i] - __bfloat162float(got[i]))));
  for (size_t i = 0; i < compressed.size(); ++i)
    coarse_error =
        std::max(coarse_error, double(std::fabs(compressed[i] - __bfloat162float(got_coarse[i]))));
  ...
  for (int start = 0; start < seq; start += 37)
    vsa_add_compression(stream.get(), output.get() + size_t(start) * inner,
                        dg.get() + size_t(start) * inner, coarse.get(), start,
                        std::min(37, seq - start), config);
  output.copy_to_host(got.data(), count, stream.get());
  SLOPFAB_CUDA_CHECK(cudaStreamSynchronize(stream.get()));
  max_error = 0;
  for (size_t i = 0; i < count; ++i)
    max_error = std::max(max_error, double(std::fabs(expected_gate[i] - __bfloat162float(got[i]))));
...
SLOPFAB_TEST(vsa_real_checkpoint_forward) {
  ...
  if (!path) {
    SKIP_MISSING_FIXTURE("set SLOPFAB_VSA_CHECKPOINT for the FastH3 V2 integration test");
    return;
  }
  using namespace slopfab;
  dit::SequenceLayout layout;
  layout.num_text = 8;
  layout.num_audio_latents = 7;
  layout.num_audio_rows = 14;
  layout.num_latent_frames = 5;
  layout.latent_height = 10;
  layout.latent_width = 14;
  ...
  for (int offload : {0, 3}) {
    ...
    dit::TransformerLoadOptions options;
    options.layout = &layout;
    options.offload_blocks = offload;
    ...
    CHECK(std::all_of(ov.begin(), ov.end(), [](float x) {
      return std::isfinite(x);
    }));
    CHECK(std::all_of(oa.begin(), oa.end(), [](float x) {
      return std::isfinite(x);
    }));
    CHECK(std::any_of(ov.begin(), ov.end(), [](float x) {
      return x != 0;
    }));
    if (offload == 0) {
      resident_video = ov;
      ...
      CHECK(ov == resident_video);
      CHECK(oa == resident_audio);
    }
  }
}
