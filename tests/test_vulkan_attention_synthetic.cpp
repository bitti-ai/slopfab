...
SLOPFAB_TEST_CATEGORY(vulkan_exact_blocked_attention_single_key, "synthetic") {
  ...
  const uint16_t conversion_edges[] = {0x0000u, 0x8000u, 0x0001u, 0x007fu, 0x0080u, 0x387fu,
                                       0x3880u, 0x7f7fu, 0x7f80u, 0xff80u, 0x3f80u, 0x3f81u};
  std::copy(std::begin(conversion_edges), std::end(conversion_edges), values.begin());
  ...
  CHECK_MSG(edge_mismatch == expected.size(), "attention conversion edge %zu: %04x != %04x",
            edge_mismatch, edge_mismatch == expected.size() ? 0u : expected[edge_mismatch],
  ...
  try {
    plan.record(after_submit, third_view, out);
...

  {
    ...
    try {
      plan.record(superseded, old, out);
    } catch (const std::invalid_argument&) {
      old_rejected = true;
    }
    CHECK(old_rejected);
  ...
  try {
    plan.record(recovery, discarded, out);
  } catch (const std::invalid_argument&) {
  ...
  try {
    plan.record(recovery, recovery_view, q);
  } catch (const std::invalid_argument&) {
    alias_rejected = true;
  }
  ...
  bool thirty_third_rejected = false;
  {
    ...
    for (int i = 0; i < 31; ++i)
      plan.record(bounded, bounded_view, out);
    try {
      plan.record(bounded, bounded_view, out);
  ...
  auto run_shape = [&](uint32_t variable_sequence) {
    ...
    const TensorLayout variable_layout = TensorLayout::contiguous(variable_extent, 3);
    ...
    BlockedAttentionPlanDesc variable_desc{variable_sequence, 1, dim, exact_attention_scale(dim)};
    BlockedAttentionPlan variable_plan = BlockedAttentionPlan::create(context, variable_desc);
    ...
    PreparedAttentionView variable_inputs =
        variable_prepared.prepare(variable_batch, variable_q, variable_k, variable_v);
    variable_plan.record(variable_batch, variable_inputs, variable_out);
    variable_batch.submit().wait();
  };
  for (uint32_t variable_sequence : {3u, 17u, 5u, 33u})
    run_shape(variable_sequence);
  context.upload_bytes(q, zeros.data(), zeros.size() * sizeof(uint16_t));
  ...
  for (uint32_t variable_sequence : {33u, 5u, 17u, 3u})
    run_shape(variable_sequence);
  context.upload_bytes(q, zeros.data(), zeros.size() * sizeof(uint16_t));
  ...
  Submission dropped_wrappers;
  {
    ...
    BlockedAttentionPlanDesc drop_desc{drop_sequence, 1, dim, exact_attention_scale(dim)};
    PreparedAttentionInputs drop_prepared = PreparedAttentionInputs::create(context, drop_desc);
    BlockedAttentionPlan drop_plan = BlockedAttentionPlan::create(context, drop_desc);
    TensorBatch drop_batch = context.begin_batch();
    PreparedAttentionView drop_inputs = drop_prepared.prepare(drop_batch, drop_q, drop_k, drop_v);
...
SLOPFAB_TEST_CATEGORY(vulkan_sage_device_and_memory_selection, "synthetic") {
  ...
  auto select = [&](uint64_t budget) {
    return detail::select_sage_configuration(info, d, budget);
  };
  auto c = select(0);
...
SLOPFAB_TEST_CATEGORY(vulkan_sage_variants_and_bounded_preparation, "synthetic") {
  ...
  if (!info.timeline_semaphore || !info.shader_int8 || !info.shader_float16 ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("Sage cooperative tuples unavailable");
    return;
  }
  DeviceOptions opts;
  ...
  for (uint32_t dim : {64u, 128u}) {
    ...
    for (size_t i = 0; i < n; ++i)
      data[n + i] += i % dim < dim / 2 ? 12.0f : -8.0f;
    std::vector<uint16_t> host(n * 3), baseline;
    for (size_t i = 0; i < host.size(); ++i)
      host[i] = reference_bf16(data[i]);
    for (uint64_t budget : {0ull, 4096ull, 64ull << 20}) {
      TensorContextOptions tc;
      tc.max_batch_operators = 1;
      tc.sage_extra_workspace_bytes = budget;
      ...
      context.upload_bytes(q, host.data(), n * 2);
      context.upload_bytes(k, host.data() + n, n * 2);
      ...
      auto ranges =
          H3AttentionRanges::create(context, sequence, bounds.data(), uint32_t(bounds.size()));
      for (uint32_t kernel = 1; kernel <= 4; ++kernel) {
        if (!detail::sage_kernel_fits(info, kernel))
          continue;
        auto plan =
            H3AttentionPlan::create(context, {sequence, heads, dim, exact_attention_scale(dim),
                                              AttentionMode::kSage2, kernel});
        ...
        if (info.timestamp_valid_bits)
          timing = TimestampQuery::create(device, 4);
        ...
        if (timing.count()) {
          for (uint32_t i = 0; i < 3; ++i)
            CHECK(timing.elapsed_milliseconds(i, i + 1) >= 0);
          bool rejected = false;
          try {
            (void)timing.elapsed_milliseconds(0, 4);
          } catch (const std::invalid_argument&) {
            rejected = true;
          }
          CHECK(rejected);
        ...
        if (baseline.empty())
          baseline = actual;
        for (size_t i = 0; i < n; ++i)
          CHECK_NEAR(bf16_to_f32(actual[i]), bf16_to_f32(baseline[i]), 0.002);
        ...
        auto repeat = context.begin_batch();
        plan.record(repeat, q, k, v, out, &ranges);
        auto token = repeat.submit();
...
SLOPFAB_TEST_CATEGORY(vulkan_fast_attention_accuracy_and_lifetime, "synthetic") {
  ...
  for (auto mode : {AttentionMode::kFlash2, AttentionMode::kSage2, AttentionMode::kSol}) {
    ...
    bool rejected = false;
    try {
      (void)H3AttentionPlan::create(disabled, {1, 1, 64, 0.125f, mode});
  ...
  if (!info.cooperative_matrix_bf16_f32_16x16x16 || !info.shader_float16 ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("Fast attention cooperative matrices unavailable");
    return;
  }
  ...
  for (auto mode : {AttentionMode::kFlash2, AttentionMode::kSage2}) {
    if (!context.h3_attention_supported(mode)) {
      SKIP_UNSUPPORTED_HARDWARE("%s unavailable on %s (INT8 tuple %d)", attention_mode_name(mode),
                                info.name.c_str(), int(info.cooperative_matrix_i8_i32_16x16x32));
      continue;
    }
    ...
    for (float invalid_scale : {0.0f, -1.0f, std::numeric_limits<float>::infinity(),
      ...
      bool rejected = false;
      try {
        (void)H3AttentionPlan::create(context, {1, 1, 64, invalid_scale, mode});
      } catch (const std::invalid_argument&) {
        rejected = true;
      }
      CHECK(rejected);
    ...
    for (uint32_t dim : {64u, 128u})
      for (uint32_t sequence :
           {1u, 15u, 16u, 17u, 31u, 32u, 33u, 63u, 64u, 65u, 129u, 257u, 513u}) {
        ...
        for (size_t i = 0; i < n; ++i) {
          ...
          qf[i] = bf16_to_f32(qh[i]);
          kf[i] = bf16_to_f32(kh[i]);
          vf[i] = bf16_to_f32(vh[i]);
        }
        auto q = context.allocate(layout, ScalarType::kBFloat16);
        auto k = context.allocate(layout, ScalarType::kBFloat16);
        auto v = context.allocate(layout, ScalarType::kBFloat16);
        ...
        context.upload_bytes(q, qh.data(), n * 2);
        context.upload_bytes(k, kh.data(), n * 2);
        ...
        for (uint32_t tile = 0; tile < (sequence + 127) / 128; ++tile) {
          if (sequence >= 257 && tile % 2 == 0)
            ...
            bounds.insert(bounds.end(), {0, int32_t((sequence + 63) / 64 * 64), 0, 0});
        }
        auto ranges =
            H3AttentionRanges::create(context, sequence, bounds.data(), uint32_t(bounds.size()));
        for (bool banded : {false, true}) {
            ...
            for (uint32_t head = 0; head < heads; ++head) {
              ...
              for (uint32_t key = 0; key < sequence; ++key) {
                ...
                for (uint32_t d = 0; d < dim; ++d)
                  score += double(qf[(row * heads + head) * dim + d]) *
                           kf[(key * heads + head) * dim + d];
              ...
              for (uint32_t key = 0; key < sequence; ++key)
                for (uint32_t d = 0; d < dim; ++d)
                  expected[(row * heads + head) * dim + d] +=
                      scores[key] / sum * vf[(key * heads + head) * dim + d];
            }
          ...
          worst_l2 = std::max(worst_l2, rel);
          worst_abs = std::max(worst_abs, max_abs);
          CHECK_MSG(rel < (mode == AttentionMode::kSage2 ? 0.025 : 0.005),
                    "%s S%u D%u band%d relative L2 %.8f", attention_mode_name(mode), sequence, dim,
                    int(banded), rel);
            ...
            for (uint32_t row = 0; row < sequence; ++row)
              for (uint32_t d = 0; d < heads * dim; ++d) {
                const uint16_t expected_word = row >= 3 && row < sequence - 11
                                                   ? actual[size_t(row + 4) * heads * dim + d]
                                                   : uint16_t(0x4210);
        ...
        std::fill(qh.begin(), qh.end(), uint16_t(0));
        std::fill(kh.begin(), kh.end(), uint16_t(0));
        context.upload_bytes(q, qh.data(), n * 2);
        context.upload_bytes(k, kh.data(), n * 2);
        context.upload_bytes(v, vh.data(), n * 2);
...
SLOPFAB_TEST_CATEGORY(vulkan_attention_prepare_exhaustive_bf16, "synthetic") {
  ...
  ComputePipeline pipeline = ComputePipeline::create(
      device, load_spirv(SLOPFAB_TEST_ATTENTION_PREPARE_SPV_PATH), pipeline_options);
  ...
  Buffer upload = pool.allocate(bytes, BufferUsage::kTransferSource, MemoryUsage::kUpload);
  Buffer source = pool.allocate(bytes, BufferUsage::kTransferDestination | BufferUsage::kStorage,
                                MemoryUsage::kDevice);
  Buffer output0 = pool.allocate(bytes, BufferUsage::kStorage | BufferUsage::kTransferSource,
                                 MemoryUsage::kDevice);
  Buffer output1 = pool.allocate(bytes, BufferUsage::kStorage, MemoryUsage::kDevice);
  Buffer output2 = pool.allocate(bytes, BufferUsage::kStorage, MemoryUsage::kDevice);
  Buffer readback = pool.allocate(bytes, BufferUsage::kTransferDestination, MemoryUsage::kReadback);
  ...
  commands.barrier(output0, BufferAccess::kComputeWrite, BufferAccess::kTransferRead);
  ...
  for (uint32_t i = 0; i < patterns; ++i) {
    const uint16_t expected =
        (input[i] & 0x7fffu) > 0x7f80u ? 0x7fffu : f32_to_f16(bf16_to_f32(input[i]));
