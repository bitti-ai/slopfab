...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_dit_real_main50_capture_replay, "integration") {
  ...
  const char* replay_capture = std::getenv("SLOPFAB_REF2VA_GRAPH_REPLAY_CAPTURE");
  const bool dynamic_capture = replay_capture && replay_capture[0] != '\0';
  const std::filesystem::path checkpoint_path =
      dynamic_capture
          ? std::filesystem::path("weights/transformer/minimax_h3_ref2va_pruned_nvfp4.safetensors")
          : std::filesystem::path("weights/transformer/MiniMax_H3_FL2VA_pruned_nvfp4.safetensors");
  const std::filesystem::path capture_path =
      dynamic_capture ? std::filesystem::path(replay_capture)
                      : std::filesystem::path("tests/data/h3_main50_step0_seed424242_256.vfh3g");
  ...
  const std::array<uint8_t, 32> expected_capture_sha{
      0xbb, 0x15, 0x68, 0x92, 0x83, 0x66, 0x94, 0x96, 0xc5, 0x61, 0x4e,
      0xc3, 0x0f, 0xaa, 0xf0, 0x88, 0x12, 0x9d, 0x8c, 0xb8, 0x97, 0xcb,
      0x34, 0x0a, 0xe5, 0x36, 0x37, 0x3a, 0xd4, 0x2c, 0xb3, 0x62};
  if (!dynamic_capture)
  ...
  CHECK(header.sequence > 0 && header.hidden == 5376 && header.heads == 56 &&
        header.head_dim == 128 && header.ffn == 14336);
  CHECK(header.timesteps > 0 && header.timesteps <= 4 && header.modalities == 3 &&
        header.adaln_rank == 8 && header.layers == 50 && header.denoise_step == 0);
  if (!dynamic_capture) {
    CHECK(header.sequence == 526 && header.timesteps == 1 && header.range_values == 20);
  ...
  const std::array<uint64_t, 50> expected_boundaries{
      0xf7d651766756f015ull, 0x09afbb821887020bull, 0x779869da69c31d75ull, 0x8b76a7261ab91253ull,
      0x1df1afd29841879cull, 0x61db215dee7be558ull, 0x93bdf2ff13a3ba62ull, 0xa8a94eae981cddb7ull,
      0x23e0dd2de693a165ull, 0xdb0e0f8aa5a49972ull, 0x1ee2ee965c134dc1ull, 0x4bc93b42a31b9a63ull,
      0xed1f848054d1213cull, 0xcd0c9af21969802cull, 0x1c2dbc71eddb6bbaull, 0x4e5096ae2df1ef5full,
      0xddca1935d085d1d1ull, 0xaa8ed6a379181141ull, 0xa1ae4a76ac4cf58aull, 0xddfb16645d9e3ddaull,
      0x8d7e09064314ab3cull, 0x50f0367cff5988b9ull, 0x28be37c299d8d34dull, 0xef16ad91e90c119aull,
      0x47edfc041ca17670ull, 0x0d42e4191058937dull, 0x53ffb9a8f3a2809eull, 0x6bf28610fbf92805ull,
      0x71b964652480244dull, 0xbd860e69ea9f1bfeull, 0x3f1fe901496195cfull, 0x5b04fa6007303ccfull,
      0xb97828cd5a0c0ddfull, 0x9be29ebd17324ad8ull, 0x9593307ba6dca3faull, 0x25b1d163a816d621ull,
      0xff23adde8d9a9e50ull, 0xc779a185f7d8aa8aull, 0xf1951f02921680a6ull, 0x18b7758650a17769ull,
      0x7ffd3c6465091afbull, 0x9b99975035dcf76dull, 0x2977798f27e1a904ull, 0xdace59bec11d235aull,
      0x6a30ef6d8415272full, 0xbdb70b87dcc70527ull, 0xabd55e9b1b59d6caull, 0x2220fd508ca8a60cull,
  ...
  const std::array<uint8_t, 32> expected_ref_checkpoint_sha{
      0x8e, 0xea, 0x02, 0xf4, 0x39, 0x02, 0xe6, 0x99, 0x04, 0x99, 0x0c,
      0x44, 0x05, 0x96, 0x8d, 0x01, 0xa1, 0x3c, 0x66, 0x56, 0xaa, 0x39,
      0x2d, 0x37, 0xab, 0x80, 0x33, 0x1e, 0x79, 0xb5, 0xdf, 0x2f};
  CHECK(sha256_mapping(checkpoint.mapping_base(), checkpoint.file_size()) ==
        (dynamic_capture ? expected_ref_checkpoint_sha : expected_fl_checkpoint_sha));
#endif
  ...
  graph.record(batch, tokens, selector_tensor, code_tensor, cosine_tensor, sine_tensor, range_ptr,
               &taps);
  ...
  for (uint32_t layer = 0; layer < header.layers; ++layer) {
    ...
    if (dynamic_capture && actual_hash != header.boundary_fnv64[layer])
      std::printf("  Ref2VA S%u first main mismatch layer %u CUDA/Vulkan "
                  ...
                  static_cast<unsigned long long>(header.boundary_fnv64[layer]),
  ...
  auto repeat = [&] {
    ...
    graph.record(next, tokens, selector_tensor, code_tensor, cosine_tensor, sine_tensor, range_ptr);
    next.submit().wait();
  ...
  std::printf(
      ...
      double(graph.peak_device_bytes()) / 1048576.0, double(vk.pooled_used_bytes()) / 1048576.0,
