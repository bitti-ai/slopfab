...
SLOPFAB_TEST_CATEGORY(transformer_offloaded_blocks_match_resident, "synthetic") {
  ...
  const auto adapter_path =
      std::filesystem::temp_directory_path() / "slopfab_streamed_lora.safetensors";
  ...
  for (int block = 0; block < cfg.num_layers; ++block) {
    for (const char* suffix : {"attn.qkv_proj", "attn.out_proj", "mlp.fc1", "mlp.fc2"}) {
      ...
      adapters.push_back({name + ".lora_A.weight",
                          {4, shape[1]},
                          make_data(size_t(4 * shape[1]), 7300 + block, .05f)});
      adapters.push_back({name + ".lora_B.weight",
                          {shape[0], 4},
  ...
  auto run = [&](int offload, bool cache) {
    ...
    loras.load({{adapter_path.string(), 1}}, st);
    Transformer model;
    slopfab::dit::TransformerLoadOptions load;
    load.offload_blocks = offload;
    load.headroom_bytes = 0;
    ...
    st.close();
    loras = slopfab::LoraAdapters();
    model.set_row_chunk(128);
    if (cache) {
      slopfab::dit::BlockCacheConfig bc;
      bc.span = 3;
      bc.interval = 2;
      bc.warmup = 2;
    ...
    if (cache)
      CHECK(model.block_cache_reused() > 0);
  ...
  for (bool cache : {false, true}) {
    ...
    for (int offload : {1, 5, 6}) {
      ...
      CHECK_CLOSE(resident.first, streamed.first, 0.0,
                  "streamed blocks/LoRA preserve repeated and cached forwards");
      if (offload > 2)
        CHECK(streamed.second < resident.second);
    }
  }
...
SLOPFAB_TEST_CATEGORY(transformer_wrapped_checkpoint_matches_unwrapped, "synthetic") {
  ...
  auto evaluate = [&](const Tensors& weights, bool interleaved = false) {
    ...
    model.forward(c.video_rows.data(), c.audio_rows.data(), c.rt, result.data(),
                  result.data() + c.video_rows.size());
    CHECK(all_finite(result));
    return result;
  };
  ...
  for (auto& kv : viggle) {
    auto& tensor = kv.second;
    if (tensor.name.find(".attn.qkv_proj.weight") == std::string::npos || tensor.shape.size() != 2)
      continue;
      ...
      for (int part = 0; part < 3; ++part)
        for (int channel = 0; channel < cfg.attention_head_dim; ++channel)
          for (int col = 0; col < cols; ++col)
            tensor.data[dst++] =
                original[size_t(part * cfg.inner_dim() + head * cfg.attention_head_dim + channel) *
                             cols +
                         col];
  }
...
SLOPFAB_TEST_CATEGORY(transformer_viggle_lora_targets_reach_forward, "synthetic") {
  ...
  const auto adapter_path =
      std::filesystem::temp_directory_path() / "slopfab_viggle_cuda_lora.safetensors";
  ...
  for (const Target& target : std::vector<Target>{
           {"proj_in", "video_patch_proj", -1},
           {"proj_out", "final_layer.video_out", -1},
    ...
    const auto actual =
        evaluate(tensors, {{std::string(target.source) + ".lora_A.weight", {rank, in}, a},
    ...
    if (std::string(target.source).compare(0, 5, "proj_") == 0 ||
      ...
      for (int row = 0; row < out; ++row)
        for (int col = 0; col < in; ++col) {
          ...
          for (int r = 0; r < rank; ++r)
            delta += double(b[row * rank + r]) * a[r * in + col];
      ...
      CHECK(actual ==
            evaluate(tensors, {{std::string(target.native) + ".lora_A.weight", {rank, in}, a},
                               {std::string(target.native) + ".lora_B.weight",
                                {weight.shape[0], rank},
                                native_b}}));
    }
  }
...
SLOPFAB_TEST_CATEGORY(transformer_load_rejects_bad_shapes, "synthetic") {
  ...
  tensors["blocks.0.attn.qkv_proj.weight"].data.resize(static_cast<size_t>(2 * cfg.inner_dim()) *
                                                       cfg.hidden_size);
