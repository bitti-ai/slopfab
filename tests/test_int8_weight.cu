...
struct Fixture {
  std::string path =
      (std::filesystem::temp_directory_path() / "slopfab_int8_weight_test.safetensors").string();
  ...
  void write(int columns, const std::string& tag, bool include_scale = true) {
    ...
    if (scales.empty())
      scales = {0.000017f, 0.0023f, 0.041f};
    const size_t scale_bytes = scales.size() * sizeof(float);
    std::string header = "{\"test.weight\":{\"dtype\":\"I8\",\"shape\":[3," +
                         std::to_string(columns) + "],\"data_offsets\":[0," +
                         std::to_string(codes.size()) + "]}";
    size_t offset = codes.size();
    if (include_scale) {
      header += ",\"test.weight_scale\":{\"dtype\":\"F32\",\"shape\":[" +
                std::to_string(scales.size()) + ",1],\"data_offsets\":[" + std::to_string(offset) +
                "," + std::to_string(offset + scale_bytes) + "]}";
      offset += scale_bytes;
    }
    if (!tag.empty()) {
      header += ",\"test.comfy_quant\":{\"dtype\":\"U8\",\"shape\":[" + std::to_string(tag.size()) +
                "],\"data_offsets\":[" + std::to_string(offset) + "," +
                std::to_string(offset + tag.size()) + "]}";
    }
    header += "}";
    while (header.size() % 8)
      header += ' ';
    ...
    if (!file)
      throw std::runtime_error("could not write INT8 test checkpoint");
  }
};
...
std::vector<uint16_t> reference(const Fixture& fixture, int columns, int group, bool canonicalize) {
...
SLOPFAB_TEST(int8_vae_weights_match_dense_hadamard) {
  ...
  for (const int group : {1, 4, 16, 64, 256}) {
    for (const int columns : {group * 2, 267}) {
      ...
      for (const bool canonicalize : {false, true}) {
        ...
        bool rejected = false;
        try {
          weight.materialize(workspace.get(), expected.size() - 1, stream.get());
  ...
  slopfab::write_safetensors(fixture.path,
                             {{"test.weight", {1, 2}, {0.5f, -2.0f}, slopfab::DType::kF16}});
...
SLOPFAB_TEST(int8_vae_weights_reject_incomplete_quantization) {
  ...
  for (int failure = 0; failure < 7; ++failure) {
    ...
    if (failure == 2)
      tag = metadata(128);
    if (failure == 4)
      fixture.scales.resize(2);
    if (failure == 5)
      fixture.scales[1] = std::numeric_limits<float>::infinity();
    if (failure == 6)
      fixture.scales[1] = -0.01f;
