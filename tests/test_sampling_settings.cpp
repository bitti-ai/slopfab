...
SLOPFAB_TEST(sampling_settings_parse_and_overlay) {
  ...
  overlay_sampling_settings(settings, parse_sampling_settings(R"({"version":1,"video_sigma_shift":5})"));
  ...
  const auto embedded = sampling_settings_from_metadata({{"slopfab.sampling", R"({"version":1,"audio_sigma_shift":2})"}});
  CHECK(embedded.audio_sigma_shift == 2.0f);
  CHECK(!embedded.video_sigma_shift && !embedded.base_sigmas);
  CHECK(rejects([] { sampling_settings_from_metadata({{"slopfab.sampling", ""}}); }));
}

SLOPFAB_TEST(sampling_settings_reject_malformed_schema_and_values) {
  for (const char* invalid : {
      "", "{", "[]", "null", "{}", "{\"version\":1} trailing",
      R"({"version":2})", R"({"version":1.5})", R"({"version":"1"})",
      R"({"version":true})", R"({"version":1,"unknown":3})",
    ...
    CHECK_MSG(rejects([&] { parse_sampling_settings(invalid); }), "accepted invalid settings: %s", invalid);
  }
  for (float shift : {0.0f, -1.0f, std::numeric_limits<float>::infinity(),
                     std::numeric_limits<float>::quiet_NaN()}) {
    SamplingSettings invalid;
    invalid.video_sigma_shift = shift;
    CHECK(rejects([&] { validate_sampling_settings(invalid); }));
    CHECK(rejects([&] { FlowScheduler scheduler(shift); }));
    SamplingSettings destination;
    destination.audio_sigma_shift = 3.0f;
    CHECK(rejects([&] { overlay_sampling_settings(destination, invalid); }));
...
SLOPFAB_TEST(sampling_settings_preserve_named_schedule_arithmetic) {
  ...
  CHECK(rejects([] { sampling_schedule_defaults(static_cast<ScheduleKind>(99)); }));
}
...
SLOPFAB_TEST(sampling_settings_reject_invalid_or_collapsed_grids_atomically) {
  ...
  for (const auto& invalid : std::vector<std::vector<float>>{
      {}, {1.0f}, {1.0f, 0.1f}, {1.0f, 0.5f, 0.5f, 0.0f},
      {1.0f, std::numeric_limits<float>::quiet_NaN(), 0.0f},
      {1.0f, std::nextafter(1.0f, 0.0f), 0.0f}}) {
    CHECK(rejects([&] { scheduler.set_base_sigmas(invalid); }));
