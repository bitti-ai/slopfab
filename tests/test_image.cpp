...
SLOPFAB_TEST(reference_media_decodes_common_formats_with_ffmpeg) {
  const std::string source = "slopfab_test_media_source.ppm";
  {
    ...
    out << "P6\n3 2\n255\n";
    const char pixels[] = {char(255), 0,         0,         0,         char(255), 0,
                           0,         0,         char(255), char(255), char(255), 0,
                           0,         char(255), char(255), char(255), 0,         char(255)};
    out.write(pixels, sizeof(pixels));
  }
#ifdef _WIN32
...
SLOPFAB_TEST(reference_image_lanczos_golden) {
  ...
  for (uint8_t value : scaled.pixels)
    CHECK(value == 73);
}
...
slopfab::RGBImage resize_reference_lanczos_unmemoised(const slopfab::RGBImage& image, int width,
  ...
  for (int y = 0; y < image.height; ++y)
    for (int x = 0; x < width; ++x) {
      ...
      for (int c = 0; c < 3; ++c)
        tmp[(static_cast<size_t>(y) * width + x) * 3 + c] = rgb[c] / sum;
    }
  slopfab::RGBImage out{width, height,
  ...
  for (int y = 0; y < height; ++y)
    for (int x = 0; x < width; ++x) {
      ...
      for (int c = 0; c < 3; ++c)
        out.pixels[(static_cast<size_t>(y) * width + x) * 3 + c] =
...
SLOPFAB_TEST(reference_image_lanczos_weight_tables_are_bit_identical) {
  ...
  for (const Case& c : cases) {
    const slopfab::RGBImage src =
        noise_image(c.in_w, c.in_h, 0x9E3779B9u ^ static_cast<uint32_t>(c.out_w));
    ...
    for (size_t i = 0; i < expect.pixels.size() && i < actual.pixels.size(); ++i) {
      if (expect.pixels[i] != actual.pixels[i])
        ++differing;
    }
