...
SLOPFAB_TEST(vulkan_vsa_matches_cuda) {
  ...
  if (!vulkan::Instance::available()) { SKIP_UNSUPPORTED_HARDWARE("No Vulkan loader"); return; }
  ...
  if (physical.empty() || !physical.front().info().cooperative_matrix_bf16_f32_16x16x16 ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("VSA requires BF16 and FP16 cooperative matrices"); return;
  }
  ...
  if (!context.h3_attention_supported(AttentionMode::kFlash2)) {
    SKIP_UNSUPPORTED_HARDWARE("Vulkan VSA capabilities unavailable"); return;
  }
  cuda::Stream stream;
  for (int dim : {64, 128}) for (int scenario : {0, 1, 2}) {
    ...
    layout.num_video_rows = layout.num_latent_frames * layout.latent_height * layout.latent_width / 4;
    ...
    auto fill = [&](auto& values, unsigned seed) {
      ...
      for (size_t i = 0; i < count; ++i) values[i] = __float2bfloat16(data[i]);
    };
    fill(q, 71); fill(k, 83); fill(v, 97); fill(gate, 113);
    // Equal routing scores must select lower tile indices deterministically.
    if (scenario == 2) std::fill(q.begin(), q.end(), __float2bfloat16(0.0f));
    const uint64_t shape[] = {uint64_t(seq), uint64_t(heads), uint64_t(dim)};
    const uint64_t flat[] = {uint64_t(seq), uint64_t(heads * dim)};
    auto allocate = [&] { return context.allocate(TensorLayout::contiguous(shape, 3), ScalarType::kBFloat16); };
    ...
    context.upload_bytes(vq, q.data(), count * 2); context.upload_bytes(vk, k.data(), count * 2);
    context.upload_bytes(vv, v.data(), count * 2); context.upload_bytes(vg, gate.data(), count * 2);
    auto plan = vulkan::VsaAttentionPlan::create(context, tiles, heads, dim);
    const uint64_t workspace = (tiles.rows.size() + tiles.sizes.size() + tiles.row_tiles.size()) * 4 +
    ...
    cuda::DeviceBuffer<__nv_bfloat16> cq(count), ck(count), cv(count), cg(count), co(count), compressed(size_t(n)*heads*dim);
    cq.copy_from_host(q.data(), count, stream.get()); ck.copy_from_host(k.data(), count, stream.get());
    cv.copy_from_host(v.data(), count, stream.get()); cg.copy_from_host(gate.data(), count, stream.get());
    cuda::VsaConfig config{n, tiles.prefix_tiles, heads, dim, rows.get(), sizes.get(), row_tiles.get()};
    cuda::Workspace ws; ws.reserve(cuda::vsa_attention_workspace_bytes(n, heads, dim));
    cuda::vsa_attention_forward(stream.get(), cq.get(), ck.get(), cv.get(), co.get(), compressed.get(), config, ws);
    std::vector<__nv_bfloat16> expected(count), actual(count);
    co.copy_to_host(expected.data(), count, stream.get()); stream.synchronize();
    ...
    auto compare = [&](const char* stage) {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        maximum = std::max(maximum, std::abs(delta)); square += delta * delta;
      }
      std::printf("  D=%d scenario=%d %s max %.7g RMS %.7g\n", dim, scenario, stage, maximum, std::sqrt(square/count));
      CHECK(finite);
    ...
    invalid.rows[0] = -1;
    bool rejected = false;
    try { auto bad = vulkan::VsaAttentionPlan::create(context, invalid, heads, dim); }
    catch (const std::invalid_argument&) { rejected = true; }
    CHECK(rejected);
    auto invalid_batch = context.begin_batch();
    rejected = false;
    try { plan.record(invalid_batch, vq, vk, vv, vq); }
    catch (const std::invalid_argument&) { rejected = true; }
    CHECK(rejected);
    rejected = false;
    try { plan.add_compression(invalid_batch, vg, vo); }
    catch (const std::invalid_argument&) { rejected = true; }
    CHECK(rejected);
  }
}
