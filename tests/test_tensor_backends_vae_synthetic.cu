...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_exact_vae_vit_block_stage, "synthetic") {
  ...
  if (!context.exact_fp32_vae_normalization() || !context.exact_vae_pointwise() ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: !context.exact_fp32_vae_normalization() || !context.exact_vae_pointwise() || !context.exact_blocked_attention()");
    return;
  }
  ...
  if (!graph_context.exact_fp32_vae_normalization() ||
    ...
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: !graph_context.exact_fp32_vae_normalization() || !graph_context.exact_vae_pointwise() || !graph_context.exact_blocked_attention()");
    return;
  }
