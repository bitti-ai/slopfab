...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_qwen_vision_real_tower, "integration") {
  ...
  if (!std::filesystem::exists(checkpoint_path) ||
      ...
      !Instance::available()) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(checkpoint_path) || cudaGetDeviceCount(&cuda_devices) != cudaSuccess || cuda_devices == 0 || !Instance::available()");
    return;
  }
...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_qwen_multimodal_full50_real, "integration") {
  ...
  if (!nv_requested && !std::getenv("SLOPFAB_QWEN_MULTIMODAL_REAL")) {
    SKIP_OPT_IN("unavailable prerequisite: !nv_requested && !std::getenv(\"SLOPFAB_QWEN_MULTIMODAL_REAL\")");
    return;
  }
  const std::filesystem::path checkpoint_path =
      std::filesystem::path(SLOPFAB_TEST_SOURCE_DIR) /
...
SLOPFAB_TEST_CATEGORY(vulkan_qwen_multimodal_max_real, "integration") {
  ...
  if (!nv_requested && !cuda_authority &&
      !std::getenv("SLOPFAB_QWEN_MULTIMODAL_MAX_REAL")) {
    SKIP_OPT_IN("unavailable prerequisite: !nv_requested && !cuda_authority && !std::getenv(\"SLOPFAB_QWEN_MULTIMODAL_MAX_REAL\")");
    return;
  }
  const std::filesystem::path checkpoint_path =
      std::filesystem::path(SLOPFAB_TEST_SOURCE_DIR) /
  ...
  if (!std::filesystem::exists(checkpoint_path) || !Instance::available()) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(checkpoint_path) || !Instance::available()");
    return;
  }
  Instance instance = Instance::create();
  const auto physical = instance.enumerate_devices();
