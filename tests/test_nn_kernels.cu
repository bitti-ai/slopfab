...
SLOPFAB_TEST(nn_rmsnorm_modulate) {
  const int rows = 23;
  const int dim = 512;
...
SLOPFAB_TEST(nn_rope_tables_h3) {
  const int rows = 9;
...
SLOPFAB_TEST(nn_gather_scatter) {
  const int n = 40;
  const int dim = 96;
...
SLOPFAB_TEST(nn_sub_bf16_aliasing_and_tails) {
...
SLOPFAB_TEST(nn_dequant_f8e4m3_all_patterns) {
...
SLOPFAB_TEST(nn_quantize_f8e4m3_roundtrip) {
...
SLOPFAB_TEST(nn_dequant_i8_per_channel) {
...
SLOPFAB_TEST(nn_w4a8_weight_and_activation_decode) {
  const int rows = 2;
...
SLOPFAB_TEST(nn_convrot_matches_regular_hadamard) {
  const int group = 256;
  const int rows = 5;
...
SLOPFAB_TEST(linear_bf16_and_fp8) {
  CublasScope cb;
  const int rows = 33;
  const int in_features = 256;
...
SLOPFAB_TEST(linear_prepared_matches_per_chunk) {
  CublasScope cb;
  const int rows = 33;
...
SLOPFAB_TEST(linear_int8_convrot) {
  CublasScope cb;
  const int rows = 21;
...
SLOPFAB_TEST(nn_dequant_nvfp4_tile_shapes) {
...
SLOPFAB_TEST(nn_dequant_nf4_double_quant) {
...
SLOPFAB_TEST(nn_dequant_nf4_vector_matches_scalar) {
  const Nf4Weight w = make_nf4(129, 128);
  const size_t n = size_t(w.out) * w.in;
  CHECK(n % 8 == 0);
  DeviceBuffer<uint8_t> dp(w.packed.size()), da(w.absmax.size());
...
SLOPFAB_TEST(linear_nf4_double_quant) {
  CublasScope cb;
  const int rows = 7;
...
SLOPFAB_TEST(nn_pre_quant_scale) {
  const int rows = 17;
  const int dim = 96;
...
SLOPFAB_TEST(nvfp4_mma_operand_layout) {
...
SLOPFAB_TEST(attention_blocked) {
  CublasScope cb;
  const int seq = 512;
  const int heads = 4;
  const int head_dim = 128;
...
SLOPFAB_TEST(attention_fused_ragged_tail) {
  CublasScope cb;
  const int heads = 3;
  const int head_dim = 128;
  const int width = heads * head_dim;
...
SLOPFAB_TEST(attention_sol_pipeline_exact) {
...
SLOPFAB_TEST(attention_sol_pipeline_mixed) {
  REQUIRE_SM120_TEST("Sol TMA pipeline");
  CublasScope cb;
...
SLOPFAB_TEST(attention_sol_pipeline_real_scale_finite) {
  REQUIRE_SM120_TEST("Sol TMA pipeline");
  CublasScope cb;
...
SLOPFAB_TEST(attention_sol_pipeline_large_pooled_v) {
  REQUIRE_SM120_TEST("Sol TMA pipeline");
  CublasScope cb;
...
SLOPFAB_TEST(attention_sol_rejects_invalid_error_weights) {
  REQUIRE_SM120_TEST("Sol attention");
  slopfab::cuda::AttentionConfig cfg;
...
SLOPFAB_TEST(attention_sol_zero_error_weight_ignores_infinite_residual) {
  REQUIRE_SM120_TEST("Sol attention");
  CublasScope cb;
  const int seq=321,dim=128;
...
SLOPFAB_TEST(attention_sage2_architecture_dispatch) {
...
SLOPFAB_TEST(attention_sage2_head_dims_bit_stable) {
...
SLOPFAB_TEST(attention_fused_head_dim_64) {
  CublasScope cb;
  const int heads = 5;
  const int head_dim = 64;
  const int width = heads * head_dim;
...
SLOPFAB_TEST(attention_fused_banded) {
  CublasScope cb;
...
SLOPFAB_TEST(attention_fused_probability_precision) {
  CublasScope cb;
  const int seq = 1024;
  const int heads = 4;
  const int head_dim = 128;
...
SLOPFAB_TEST(attention_fused_gqa) {
  CublasScope cb;
  const int seq = 96;
  const int heads = 6;
...
//     satisfy.
SLOPFAB_TEST(attention_fp16_score_tile) {
  CublasScope cb;
  const int seq = 1024;
  const int heads = 4;
  const int head_dim = 128;
...
SLOPFAB_TEST(nvfp4_dequant_timings) {
  Timer timer;
  const struct {
    const char* name;
    int out_features;
...
SLOPFAB_TEST(production_shape_timings) {
  CublasScope cb;
  size_t free_bytes = 0;
...
SLOPFAB_TEST(nvfp4_mma_b_scale_operand_layout) {
...
SLOPFAB_TEST(nvfp4_rounding_reference) {
...
SLOPFAB_TEST(nvfp4_activation_quantisation) {
  const int rows = 7;
  const int dim = 64;
...
SLOPFAB_TEST(nvfp4_gemm_matches_cpu_reference) {
...
SLOPFAB_TEST(nvfp4_gemm_disk_layout) {
  REQUIRE_SM120_TEST("native NVFP4 GEMM");
...
// well-formed matrix.
SLOPFAB_TEST(nvfp4_gemm_global_scale) {
  REQUIRE_SM120_TEST("native NVFP4 GEMM");
...
SLOPFAB_TEST(nvfp4_gemm_exact_fp4_activations) {
  REQUIRE_SM120_TEST("native NVFP4 GEMM");
...
SLOPFAB_TEST(nvfp4_activation_cost) {
  REQUIRE_SM120_TEST("native NVFP4 GEMM");
...
SLOPFAB_TEST(nvfp4_activation_cost_real_weights) {
  REQUIRE_SM120_TEST("native NVFP4 GEMM");
  std::string path;
...
SLOPFAB_TEST(nvfp4_gemm_production_timings) {
...
SLOPFAB_TEST(qwen_vision_layernorm_and_gelu) {
...
SLOPFAB_TEST(qwen_vision_encode_reuses_arena_across_images) {
...
SLOPFAB_TEST(qwen_vision_merge_and_deepstack_scatter) {
