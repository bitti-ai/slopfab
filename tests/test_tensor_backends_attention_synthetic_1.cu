...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_exact_blocked_attention, "synthetic") {
  ...
  auto run = [&](uint32_t sequence, uint32_t heads, uint32_t dim,
    ...
    if (count >= 8) {
      q[0] = 0x8000u;
      q[1] = f32_to_bf16(1.0f);
      k[0] = f32_to_bf16(1.0f);
      k[1] = f32_to_bf16(-1.0f);
      v[0] = 0x8000u;
      v[1] = f32_to_bf16(1.0f);
    }
    ...
    cq.copy_from_host(q.data(), count);
    ck.copy_from_host(k.data(), count);
    cv.copy_from_host(v.data(), count);
    const float scale = dim == 64    ? 0.125f
                        : dim == 128 ? 0.0883883461356163f
    ...
    cuda::launch_prepare_deterministic_attention_inputs(
        ...
        reinterpret_cast<const __nv_bfloat16*>(cv.get()), reinterpret_cast<__half*>(cq16.get()),
        reinterpret_cast<__half*>(ck16.get()), reinterpret_cast<__half*>(cv16.get()), count);
    if (sequence == 129) {
      cuda::launch_deterministic_blocked_attention_f16(
          nullptr, reinterpret_cast<const __half*>(cq16.get()),
          reinterpret_cast<const __half*>(ck16.get()), reinterpret_cast<const __half*>(cv16.get()),
          reinterpret_cast<__nv_bfloat16*>(co.get()), sequence, heads, dim, scale, 0, 65, 0);
      cuda::launch_deterministic_blocked_attention_f16(
          nullptr, reinterpret_cast<const __half*>(cq16.get()),
          reinterpret_cast<const __half*>(ck16.get()), reinterpret_cast<const __half*>(cv16.get()),
          reinterpret_cast<__nv_bfloat16*>(co.get()), sequence, heads, dim, scale, 65, 64, 65);
    } else {
      cuda::launch_deterministic_blocked_attention_f16(
          nullptr, reinterpret_cast<const __half*>(cq16.get()),
          reinterpret_cast<const __half*>(ck16.get()), reinterpret_cast<const __half*>(cv16.get()),
    ...
    if (sequence == 129) {
      cuda::launch_deterministic_blocked_attention_f16(
          nullptr, reinterpret_cast<const __half*>(cq16.get()),
          reinterpret_cast<const __half*>(ck16.get()), reinterpret_cast<const __half*>(cv16.get()),
          reinterpret_cast<__nv_bfloat16*>(co_repeat.get()), sequence, heads, dim, scale);
      SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
    ...
    if (sequence == 129) {
      ...
      PreparedAttentionView repeat_inputs = prepared.prepare(repeat, vq, vk_tensor, vv);
    ...
    CHECK_MSG(mismatch == count,
              "CUDA/Vulkan exact attention S%u H%u D%u mismatch at %zu: %04x != %04x", sequence,
              heads, dim, mismatch, mismatch == count ? 0u : cuda_output[mismatch],
  ...
  // attention equality.
  {
    ...
    cuda::DeviceBuffer<uint16_t> source0(patterns), source1(patterns), source2(patterns),
        prepared0(patterns), prepared1(patterns), prepared2(patterns);
    ...
    try {
      cuda::launch_prepare_deterministic_attention_inputs(
          ...
          reinterpret_cast<__half*>(prepared0.get()), reinterpret_cast<__half*>(prepared1.get()),
    ...
    for (size_t i = 0; i < patterns; ++i) {
      const uint16_t expected =
          (bits[i] & 0x7fffu) > 0x7f80u ? 0x7fffu : f32_to_f16(bf16_to_f32(bits[i]));
    ...
    try {
      cuda::launch_deterministic_blocked_attention_f16(
          ...
          reinterpret_cast<__nv_bfloat16*>(prepared0.get()), 1, 1, 64, exact_attention_scale(64));
    } catch (const std::invalid_argument&) {
  ...
  run(129, 2, 128);
  {
    ...
    CHECK(std::ldexp(1.0, -17) * std::exp(-86.0) < std::numeric_limits<float>::min());
    const std::vector<uint16_t> flushed = run(sequence, heads, dim, &q, &k, &v);
    for (uint16_t bits : flushed)
      CHECK(bits == 0x0000u);
  }
  ...
  // consumers share it. Uploads/downloads are outside both timings.
  {
    ...
    cuda::DeviceBuffer<uint16_t> cq(count), ck(count), cv(count), co(count);
    cuda::DeviceBuffer<uint16_t> cq16(count), ck16(count), cv16(count);
    cq.copy_from_host(host.data(), count);
    ck.copy_from_host(host.data(), count);
    ...
    BlockedAttentionPlanDesc desc{sequence, heads, dim, exact_attention_scale(dim)};
    BlockedAttentionPlan plan = BlockedAttentionPlan::create(vk, desc);
    PreparedAttentionInputs prepared = PreparedAttentionInputs::create(vk, desc);
    ...
    std::printf(
        "  exact D72 attention S%u H16, 2 query chunks: CUDA prepare %.3f ms + attention %.3f ms, Vulkan total %.3f ms, FP16 slot %.2f MiB\n",
        sequence, cuda_prepare_ms / samples, cuda_attention_ms / samples, vulkan_total_ms / samples,
...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_exact_h3_attention, "synthetic") {
  ...
  CHECK(cuda::deterministic_h3_cuda_tuple_fits(12, 0, "NVIDIA GeForce RTX 5090", 13010, 13000, 1024,
                                               99328, board_a));
  CHECK(cuda::deterministic_h3_cuda_tuple_fits(12, 0, "NVIDIA GeForce RTX 5090", 13010, 13000, 1024,
                                               99328, board_b));
  CHECK(!cuda::deterministic_h3_cuda_tuple_fits(12, 0, "NVIDIA GeForce RTX 5080", 13010, 13000,
                                                1024, 99328, board_a));
  CHECK(!cuda::deterministic_h3_cuda_tuple_fits(10, 0, "NVIDIA GeForce RTX 5090", 13010, 13000,
                                                1024, 99328, board_a));
  CHECK(!cuda::deterministic_h3_cuda_tuple_fits(12, 0, "NVIDIA GeForce RTX 5090", 13010, 12090,
                                                1024, 99328, board_a));
  CHECK(!cuda::deterministic_h3_cuda_tuple_fits(12, 0, "NVIDIA GeForce RTX 5090", 13010, 13000, 512,
                                                99328, board_a));
  int cuda_devices = 0;
  if (cudaGetDeviceCount(&cuda_devices) != cudaSuccess || cuda_devices == 0 ||
      !Instance::available()) {
  ...
  // tile's selected prefix/frame band rather than trusting the flattened table.
  {
    ...
    const dit::BandedKeyRanges table = dit::build_banded_key_ranges(real, 9, 128, 64);
    CHECK(table.query_tile == 128 && table.num_query_tiles == 295);
    auto round_down = [](int value) {
      return value / 64 * 64;
    };
    auto round_up = [](int value) {
    ...
    for (int tile = 0; tile < table.num_query_tiles; ++tile) {
      ...
      if (q0 >= real.video_start()) {
        ...
        expected_lo1 =
            round_down(real.video_start() + std::max(0, first - 9) * real.rows_per_frame());
        expected_hi1 = std::min(
            seq_end, round_up(real.video_start() +
                              std::min(real.num_latent_frames, last + 10) * real.rows_per_frame()));
      ...
      CHECK(got[0] == expected_lo0 && got[1] == expected_hi0 && got[2] == expected_lo1 &&
            got[3] == expected_hi1);
      const int probes[] = {0,
                            real.video_start() - 1,
                            real.video_start(),
                            ...
                            real.total_rows()};
      for (int row : probes) {
        if (row < 0 || row >= real.total_rows())
          continue;
        const bool selected = (row >= got[0] && row < got[1]) || (row >= got[2] && row < got[3]);
        const bool expected = (row >= expected_lo0 && row < expected_hi0) ||
                              (row >= expected_lo1 && row < expected_hi1);
        CHECK(selected == expected);
      }
    }
    H3AttentionRanges uploaded = H3AttentionRanges::create(
        vk, real.total_rows(), table.ranges.data(), static_cast<uint32_t>(table.ranges.size()));
  ...
  hv[0] = 0x0001u;
  hv[1] = 0x807fu;
  const std::vector<int32_t> band{0, 64, 128, 192, 0, 128, 0, 0};
  const std::vector<int32_t> wide{0, 192, 0, 0, 0, 192, 0, 0};
  cuda::DeviceBuffer<uint16_t> cq(count), ck(count), cv(count), co_full(count), co_band(count),
      co_wide(count);
  ...
  cuda::launch_deterministic_h3_attention(nullptr, reinterpret_cast<const __nv_bfloat16*>(cq.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(ck.get()),
  ...
  cuda::launch_deterministic_h3_attention(nullptr, reinterpret_cast<const __nv_bfloat16*>(cq.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(ck.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(cv.get()),
                                          reinterpret_cast<__nv_bfloat16*>(co_band.get()),
                                          cband.get(), sequence, heads, dim, scale, 0, 65, 0);
  cuda::launch_deterministic_h3_attention(nullptr, reinterpret_cast<const __nv_bfloat16*>(cq.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(ck.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(cv.get()),
                                          reinterpret_cast<__nv_bfloat16*>(co_band.get()),
                                          cband.get(), sequence, heads, dim, scale, 65, 64, 65);
  cuda::launch_deterministic_h3_attention(nullptr, reinterpret_cast<const __nv_bfloat16*>(cq.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(ck.get()),
                                          reinterpret_cast<const __nv_bfloat16*>(cv.get()),
                                          reinterpret_cast<__nv_bfloat16*>(co_wide.get()),
                                          cwide.get(), sequence, heads, dim, scale);
  SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
  ...
  H3AttentionPlan plan = H3AttentionPlan::create(vk, {sequence, heads, dim, scale});
  H3AttentionRanges bands =
      H3AttentionRanges::create(vk, sequence, band.data(), static_cast<uint32_t>(band.size()));
  H3AttentionRanges wide_ranges =
      H3AttentionRanges::create(vk, sequence, wide.data(), static_cast<uint32_t>(wide.size()));
  // Touching ranges canonicalize to the same immutable table as full coverage.
  const std::vector<int32_t> touching{0, 64, 64, 192, 0, 128, 128, 192};
  ...
  const std::vector<int32_t> single_values{128, 192, 0, 0, 128, 192, 0, 0};
  H3AttentionRanges single = H3AttentionRanges::create(vk, sequence, single_values.data(),
  ...
  const std::vector<int32_t> uniform_values{0, 64, 0, 0, 0, 64, 0, 0};
  H3AttentionRanges uniform = H3AttentionRanges::create(
      vk, sequence, uniform_values.data(), static_cast<uint32_t>(uniform_values.size()));
  for (uint32_t row = 0; row < 64; ++row)
    for (uint32_t h = 0; h < heads; ++h)
      for (uint32_t d = 0; d < dim; ++d)
        hv[(size_t(row) * heads + h) * dim + d] = f32_to_bf16(row < 32 ? 1.0f : 3.0f);
  vk.upload_bytes(v, hv.data(), count * 2);
  ...
  // Vulkan consumes the same immutable table across all chunk records.
  {
    ...
    const std::vector<int32_t> r{0, 64, 128, 192, 0, 128, 192, 256, 0, 320, 0, 0};
    cuda::DeviceBuffer<uint16_t> dq(n), dk(n), dv(n), dout(n);
    cuda::DeviceBuffer<int32_t> dr(r.size());
    dq.copy_from_host(qh.data(), n);
    dk.copy_from_host(kh.data(), n);
    dv.copy_from_host(vh.data(), n);
    dr.copy_from_host(r.data(), r.size());
    cuda::launch_deterministic_h3_attention(
        ...
        reinterpret_cast<__nv_bfloat16*>(dout.get()), dr.get(), s, h, d, exact_attention_scale(d));
    SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
    ...
    H3AttentionPlan p = H3AttentionPlan::create(vk, {s, h, d, exact_attention_scale(d)});
    H3AttentionRanges vr =
        H3AttentionRanges::create(vk, s, r.data(), static_cast<uint32_t>(r.size()));
    ...
    offset.submit().wait();
    vk.download_bytes(vo, got.data(), n * 2);
    CHECK(std::memcmp(got.data(), expected.data() + size_t(128) * d, d * sizeof(uint16_t)) == 0);
    for (size_t i = d; i < n; ++i)
      CHECK(got[i] == sentinel_bits);
    ...
    const std::vector<int32_t> correction_values{0, 128, 192, 256, 0, 128, 192, 256, 0, 256, 0, 0};
    cuda::DeviceBuffer<int32_t> correction_device(correction_values.size());
    correction_device.copy_from_host(correction_values.data(), correction_values.size());
    H3AttentionRanges correction_ranges = H3AttentionRanges::create(
        vk, s, correction_values.data(), static_cast<uint32_t>(correction_values.size()));
    std::fill(qh.begin(), qh.end(), f32_to_bf16(1.0f));
    std::fill(kh.begin(), kh.end(), f32_to_bf16(0.0f));
    std::fill(vh.begin(), vh.end(), f32_to_bf16(1.0f));
    ...
    cuda::launch_deterministic_h3_attention(
        ...
        reinterpret_cast<const __nv_bfloat16*>(dk.get()),
        reinterpret_cast<const __nv_bfloat16*>(dv.get()),
        reinterpret_cast<__nv_bfloat16*>(dout.get()), correction_device.get(), s, h, d,
        exact_attention_scale(d), 0, 1, 0);
    SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
    dout.copy_to_host(expected.data(), n);
    vk.upload_bytes(vq, qh.data(), n * 2);
    vk.upload_bytes(vkey, kh.data(), n * 2);
    ...
    for (uint32_t column = 0; column < d; ++column) {
      ...
      CHECK(bf16_to_f32(got[column]) > 1.0f && bf16_to_f32(got[column]) < 3.0f);
    }
  }
  ...
  for (uint32_t adversarial_dim : {64u, 128u}) {
    ...
    const size_t adversarial_count = size_t(adversarial_sequence) * adversarial_dim;
    const uint16_t patterns[] = {0x0000u, 0x8000u, 0x0080u, 0x8080u, 0x3f80u, 0xbf80u,
                                 0x3f81u, 0xbf81u, 0x3f00u, 0xbf00u, 0x4000u, 0xc000u,
                                 0x3c00u, 0xbc00u, 0x3eabu, 0xbeabu};
    std::vector<uint16_t> aq(adversarial_count), ak(adversarial_count), av(adversarial_count);
    ...
    auto launch_adversarial = [&](cuda::DeviceBuffer<uint16_t>& selected) {
      cuda::launch_deterministic_h3_attention(
          ...
          reinterpret_cast<__nv_bfloat16*>(selected.get()), nullptr, adversarial_sequence, 1,
          adversarial_dim, exact_attention_scale(adversarial_dim));
    };
    launch_adversarial(dao);
    launch_adversarial(dar);
    SLOPFAB_CUDA_CHECK(cudaDeviceSynchronize());
    ...
    H3AttentionPlan adversarial_plan = H3AttentionPlan::create(
        vk, {adversarial_sequence, 1, adversarial_dim, exact_attention_scale(adversarial_dim)});
  ...
  auto range_rejected = [&](const std::vector<int32_t>& bad) {
    bool rejected = false;
    try {
      (void)H3AttentionRanges::create(vk, sequence, bad.data(), static_cast<uint32_t>(bad.size()));
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
    CHECK(rejected);
  ...
  DeviceTensor wrong = vk.allocate(layout, ScalarType::kFloat32);
  {
    ...
    bool rejected = false;
    try {
      plan.record(recover, q, k, v, wrong, &bands);
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
    CHECK(rejected);
    rejected = false;
    try {
      plan.record(recover, q, k, v, q, &bands);
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
    CHECK(rejected);
    rejected = false;
    try {
      plan.record(recover, q, k, v, out_full, &bands, sequence, 1, 0);
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
    CHECK(rejected);
  ...
  auto submit = [&](DeviceTensor& selected, const H3AttentionRanges* selected_ranges) {
  ...
  Submission first = submit(out_full, &bands), second_job = submit(second, &bands),
  ...
  Submission full_a = submit(out_full, nullptr), full_b = submit(second, nullptr),
  ...
  Submission retained;
  {
    ...
    H3AttentionPlan temporary_plan = H3AttentionPlan::create(vk, {sequence, heads, dim, scale});
    H3AttentionRanges temporary_ranges =
        H3AttentionRanges::create(vk, sequence, band.data(), static_cast<uint32_t>(band.size()));
