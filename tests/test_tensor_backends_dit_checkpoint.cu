...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_dit_real_block0_replay, "checkpoint") {
  ...
  if (!std::filesystem::exists(path) || !Instance::available()) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(path) || !Instance::available()");
    return;
  }
  ...
  if (!vk.exact_h3_attention() || !vk.exact_vae_pointwise() ||
      !vk.exact_fp32_vae_normalization()) {
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: !vk.exact_h3_attention() || !vk.exact_vae_pointwise() || !vk.exact_fp32_vae_normalization()");
    return;
  }
...
SLOPFAB_TEST_CATEGORY(cuda_ref2va_text_l4100_authority, "checkpoint") {
  ...
  if (!std::filesystem::exists(checkpoint_path) ||
      !std::filesystem::exists(source_capture)) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(checkpoint_path) || !std::filesystem::exists(source_capture)");
    return;
  }
