...
SLOPFAB_TEST_CATEGORY(vulkan_qwen_real_layer0_synthetic_activation, "checkpoint") {
  ...
  if (!info.timeline_semaphore || !info.shader_int64) {
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: !info.timeline_semaphore || !info.shader_int64");
    return;
  }
  ...
  if (!vk.exact_causal_gqa_attention() ||
      !vk.exact_fp32_vae_normalization() || !vk.exact_vae_pointwise()) {
    SKIP_UNSUPPORTED_HARDWARE("unavailable prerequisite: !vk.exact_causal_gqa_attention() || !vk.exact_fp32_vae_normalization() || !vk.exact_vae_pointwise()");
    return;
  }
...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_qwen_layer0_real_l132, "checkpoint") {
  ...
  if(!std::filesystem::exists(checkpoint_path)||
     ...
     cudaGetDeviceCount(&cuda_devices)!=cudaSuccess||cuda_devices==0||
     !Instance::available()) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(checkpoint_path)|| !std::filesystem::exists(tokenizer_path)|| cudaGetDeviceCount(&cuda_devices)!=cudaSuccess||cuda_devices==0|| !Instance::available()");
    return;
  }
