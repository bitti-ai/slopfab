...
int main(int argc, char** argv) {
  ...
  if (expected != 12 && expected != 13)
    return 3;

  if (slopfab_cuda_set_version("invalid") != SLOPFAB_ERR_INVALID_ARGUMENT)
    return 4;
  if (slopfab_cuda_loaded_major(nullptr) != SLOPFAB_ERR_INVALID_ARGUMENT)
    return 5;
  if (slopfab_cuda_set_version(argv[1]) != SLOPFAB_OK) {
    std::fprintf(stderr, "%s\n", slopfab_last_error());
    return 6;
  }
  if (slopfab_last_error()[0] != '\0')
    return 10;
  ...
  if (slopfab_cuda_set_version(argv[1]) != SLOPFAB_ERR_RUNTIME)
    return 9;
