...
SLOPFAB_TEST_CATEGORY(vulkan_qwen_full50_real_l132_replay, "integration") {
  ...
  context_options.max_batch_operators = std::getenv("SLOPFAB_QWEN_MULTIMODAL_REAL") ? 128 : 37;
  ...
  for (size_t layer = 0; layer < actual.size(); ++layer)
    actual[layer] = fnv64_bytes(trace.layer_residual_bf16.data() + layer * layer_elements,
  ...
  CHECK(stats.allocator_peak_nonstaging_bytes >= stats.allocator_baseline_bytes);
  CHECK(stats.peak_device_bytes >=
        stats.allocator_peak_nonstaging_bytes - stats.allocator_baseline_bytes);
  CHECK(stats.allocator_peak_used_bytes <=
        stats.allocator_peak_nonstaging_bytes + 2 * context.staging_capacity_bytes());
  ...
  if (std::getenv("SLOPFAB_QWEN_MULTIMODAL_REAL")) {
    ...
    const text::PromptEmbedding multimodal = encoder.encode(ids, {image}, &multimodal_trace);
    const double multimodal_seconds =
        std::chrono::duration<double>(std::chrono::steady_clock::now() - multimodal_begin).count();
    ...
    for (size_t layer = 0; layer < multimodal_actual.size(); ++layer)
      multimodal_actual[layer] = fnv64_bytes(multimodal_trace.layer_residual_bf16.data() +
    ...
    CHECK(multimodal.modality_tags.front() == 1 && multimodal.modality_tags.back() == 1);
    ...
    CHECK(multimodal_stats.peak_device_bytes >= multimodal_stats.allocator_peak_nonstaging_bytes -
                                                    multimodal_stats.allocator_baseline_bytes);
    CHECK(multimodal_stats.allocator_peak_used_bytes <=
          multimodal_stats.allocator_peak_nonstaging_bytes + 2 * context.staging_capacity_bytes());
    std::printf("  CUDA-off multimodal exact Qwen full50 L68 %.2f s final %016llx\n",
                multimodal_seconds, static_cast<unsigned long long>(fnv64_floats(multimodal.data)));
  }
  encoder.unload();
  context.collect();
  CHECK(context.pooled_used_bytes() >= cold_baseline);
  CHECK(context.pooled_used_bytes() <= cold_baseline + 2 * context.staging_capacity_bytes());
  std::printf(
      "  CUDA-off real exact Qwen full50 L132 %.2f s final %016llx peak/reserved %.1f/%.1f MiB descriptors %llu\n",
      seconds, static_cast<unsigned long long>(fnv64_floats(output.data)),
