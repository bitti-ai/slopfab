...
SLOPFAB_TEST(refmod_visual_packing_and_dtype) {
  ...
  for (size_t i = 0; i < z.size(); ++i) z[i] = float(i) / 16;
  for (auto dtype : {DType::kF32, DType::kF16, DType::kBF16}) {
    f.write(R"({"kind":"video","latent_h":4,"latent_w":6,"latent_t":2})", {1,24,2,4,6}, z, dtype);
    const auto mod = RefMod::load(f.path.string());
    CHECK(mod->token_count() == 12);
    const auto rows = mod->rows();
    for (int t = 0; t < 2; ++t) for (int y = 0; y < 4; ++y) for (int x = 0; x < 6; ++x)
      for (int c = 0; c < 24; ++c) {
        float expected = z[((c * 2 + t) * 4 + y) * 6 + x];
        if (dtype == DType::kBF16) expected = bf16_to_f32(f32_to_bf16(expected));
...
SLOPFAB_TEST(refmod_strength_blurs_detail_preserving_frame_and_channel_means) {
  ...
  for (size_t i = 0; i < z.size(); ++i) z[i] = float(i);
  f.write(R"({"kind":"video"})", {1,24,2,4,4}, z);
  const auto mod = RefMod::load(f.path.string());
  const auto rows = mod->rows(.25f);
  for (int t = 0; t < 2; ++t) for (int y = 0; y < 4; ++y) for (int x = 0; x < 4; ++x)
    for (int c = 0; c < 24; ++c) {
      const int offset = (c * 2 + t) * 16;
      const float expected = .25f * z[offset + y * 4 + x] + .75f * (offset + 7.5f);
      CHECK_NEAR(rows[((t * 2 + y / 2) * 2 + x / 2) * 96 + c * 4 + y % 2 * 2 + x % 2], expected, 0);
    }
  CHECK(mod->rows(0).empty());
  CHECK(rejects([&] { mod->rows(-.1f); }));
  CHECK(rejects([&] { mod->rows(NAN); }));
}
...
SLOPFAB_TEST(refmod_audio_packing_and_linear_blur) {
  ...
  for (int ch = 0; ch < 2; ++ch) for (int t = 0; t < 16; ++t) for (int c = 0; c < 32; ++c) {
...
SLOPFAB_TEST(refmod_blur_uneven_pool_cells_and_bilinear_interior) {
  ...
  for (int c = 0; c < 24; ++c) for (int y = 0; y < 18; ++y) for (int x = 0; x < 26; ++x)
    z[(c * 18 + y) * 26 + x] = float(y * y + 2 * x);
  f.write(R"({"kind":"image"})", {1,24,1,18,26}, z);
  const auto rows = RefMod::load(f.path.string())->rows(.5f);
  const auto at = [&](int y, int x) { return rows[(y / 2 * 13 + x / 2) * 96 + y % 2 * 2 + x % 2]; };
...
SLOPFAB_TEST(refmod_metadata_validation_and_owned_snapshot) {
  ...
  for (const char* meta : {R"({"kind":"bundle"})", R"({"kind":"image","latent_h":8})",
    ...
    f.write(meta);
    CHECK(rejects([&] { RefMod::load(f.path.string()); }));
  }
  f.write("", {1,24,1,4,4});
  CHECK(rejects([&] { RefMod::load(f.path.string()); }));
  auto sidecar = f.path; sidecar.replace_extension(".json");
  { std::ofstream out(sidecar); out << R"({"kind":"image","name":"legacy"})"; }
...
SLOPFAB_TEST(refmod_mixed_conditions_copies_positions_and_seed) {
  ...
  CHECK(v == v2 && a == a2);
  g2.clear(); v2.clear(); a2.clear();
  append_refmod_conditions(refs, 43, g2, v2, a2);
  CHECK(v != v2 && a == a2);
}

SLOPFAB_TEST(refmod_plan_disabled_and_prompt_cache) {
  RefModFixture f; f.write();
  GenerateRequest request; request.prompt = "a person walking";
  ...
  CHECK(resolve_plan(request).sequence_length_without_text() == base.sequence_length_without_text());
  request.refmods[0].strength = 1;
  CHECK(request.has_references() && !request.has_native_references());
  CHECK(resolve_plan(request).sequence_length_without_text() == base.sequence_length_without_text() + 8);
  CHECK(conditioning_cache_key(request) == key);
  CHECK(describe_plan(request, resolve_plan(request)).find("copies 2, tokens 8") != std::string::npos);
