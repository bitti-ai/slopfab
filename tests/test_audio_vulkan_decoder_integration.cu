...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_exact_audio_decoder_graph, "integration") {
  ...
  const std::filesystem::path checkpoint_path = "weights/vae/minimax_h3_audio_vae_fp32.safetensors";
  ...
  const uint64_t largest_host_tensor = (uint64_t(1024) * 2048 * 7 + 1024) * sizeof(float);
  ...
  CHECK(vk_decoder.staging_capacity_bytes() == uint64_t(1024) * 2048 * 7 * sizeof(float));
  CHECK(vk_decoder.load_pool_high_water_bytes() >= vk_decoder.weight_bytes());
  CHECK(vk_decoder.load_pool_high_water_bytes() <=
        vk_decoder.weight_bytes() + 2 * vk_decoder.staging_capacity_bytes() + (4ull << 20));
  ...
  std::vector<float> latent = values(size_t(2) * 32 * latent_length, 67, 607, 1.0f / 128.0f);
  vae::AudioDecodeTrace cuda_trace;
  const vae::DecodedAudio cuda_audio =
      cuda_decoder.decode(latent.data(), latent_length, &cuda_trace);
  const auto forward_begin = std::chrono::steady_clock::now();
  const vae::DecodedAudio vk_audio = vk_decoder.decode(latent.data(), latent_length);
  const double forward_ms =
      std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - forward_begin)
          .count();
  ...
  const auto cuda_wav = std::filesystem::temp_directory_path() / "slopfab_cuda_audio_exact.wav";
  const auto vk_wav = std::filesystem::temp_directory_path() / "slopfab_vulkan_audio_exact.wav";
  audio::write_wav(cuda_wav.string(), cuda_audio.samples, cuda_audio.channels,
                   cuda_audio.sample_rate, audio::SampleFormat::kPcm16);
  audio::write_wav(vk_wav.string(), vk_audio.samples, vk_audio.channels, vk_audio.sample_rate,
  ...
  const std::filesystem::path partial_path =
      std::filesystem::temp_directory_path() / "slopfab_vulkan_audio_partial_reload.safetensors";
  write_safetensors(
      partial_path.string(),
      {{"dec_in_proj.weight", {2048, 32, 1}, to_f32(checkpoint.at("dec_in_proj.weight"))},
       {"dec_in_proj.bias", {2048}, to_f32(checkpoint.at("dec_in_proj.bias"))},
       {"decoder.conv_pre.weight",
        {1024, 2048, 7},
        to_f32(checkpoint.at("decoder.conv_pre.weight"))},
       {"decoder.conv_pre.bias", {1024}, to_f32(checkpoint.at("decoder.conv_pre.bias"))}});
  const uint64_t before_failed_reload_used = vk_decoder.allocator_used_bytes();
  ...
  check_exact(cuda_audio.samples, vk_decoder.decode(latent.data(), latent_length).samples,
  ...
  std::vector<float> production =
      values(size_t(2) * 32 * production_length, 71, 613, 1.0f / 256.0f);
  const auto cuda_begin = std::chrono::steady_clock::now();
  const vae::DecodedAudio cuda_production =
      cuda_decoder.decode(production.data(), production_length);
  ...
  const vae::DecodedAudio vk_production = vk_decoder.decode(production.data(), production_length);
  const double vk_ms =
  ...
  const vae::DecodedAudio vk_small_after_production =
      vk_decoder.decode(latent.data(), latent_length);
  check_exact(cuda_audio.samples, vk_small_after_production.samples, "A3 audio decoder after A405");
  CHECK(vk_decoder.allocator_reserved_bytes() == stable_reserved);
  CHECK(vk_decoder.descriptor_set_allocations() == stable_descriptors);
  CHECK(vk_decoder.allocator_used_bytes() >= vk_decoder.peak_device_bytes());
  CHECK(vk_decoder.allocator_used_bytes() <= vk_decoder.peak_device_bytes() + (128ull << 20));
  CHECK(vk_decoder.decode_pool_high_water_bytes() >= vk_decoder.allocator_used_bytes());
  CHECK(vk_decoder.decode_pool_high_water_bytes() <=
        vk_decoder.peak_device_bytes() + 2 * vk_decoder.staging_capacity_bytes() + (4ull << 20));
  ...
  const vae::DecodedAudio vk_reloaded = vk_decoder.decode(latent.data(), latent_length, &vk_trace);
  check_exact(cuda_audio.samples, vk_reloaded.samples, "A3 after unload and reload");
  const std::array<const char*, 13> boundary_names{
      "dec_in_proj",    "conv_pre",       "stage0 average", "stage1 average", "stage2 average",
      "stage3 average", "stage4 average", "stage5 average", "stage6 average", "activation_post",
      "conv_post",      "clamp",          "interleave"};
  ...
  for (size_t index = 0; index < boundary_names.size(); ++index)
    check_exact(cuda_trace.boundaries[index], vk_trace.boundaries[index], boundary_names[index]);
