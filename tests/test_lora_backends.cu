...
struct ReducedCheckpoint {
  std::filesystem::path path =
      std::filesystem::temp_directory_path() / "slopfab_lora_reduced.safetensors";
  SafeTensors file;
  ...
  explicit ReducedCheckpoint(const SafeTensors& source) {
    ...
    for (const auto& item : source.tensors()) {
      if (item.first.compare(0, 7, "blocks.") == 0 && item.first.compare(0, 9, "blocks.0.") != 0)
        continue;
    ...
    out.close();
    if (!out)
      throw std::runtime_error("failed to write reduced LoRA test checkpoint");
...
void compare_projection(vulkan::TensorContext& vk, const LoraFactors& f, int rows, int offset,
                        int out) {
  ...
  for (size_t i = 0; i < x.size(); ++i)
    x[i] = f32_to_bf16(input[i]);
  for (size_t i = 0; i < initial.size(); ++i)
    initial[i] = f32_to_bf16(static_cast<float>(i % 13) / 32);
  ...
  for (int row = 0; row < rows; ++row) {
    for (int r = 0; r < f.rank; ++r) {
      ...
      for (int i = 0; i < f.in; ++i)
        sum = std::fma(bf16_to_f32(x[static_cast<size_t>(row) * f.in + i]), bf(f.a[r * f.in + i]),
                       sum);
      mid[r] = bf(sum);
    }
    for (int o = 0; o < out; ++o) {
  ...
  for (bool deterministic : {false, true}) {
    SLOPFAB_CUDA_CHECK(
        cudaMemcpy(cy.get(), initial.data(), initial.size() * 2, cudaMemcpyHostToDevice));
    runner.apply(&f, cx.get(), rows, cy.get(), blas, nullptr, deterministic);
    std::vector<uint16_t> actual(initial.size());
    SLOPFAB_CUDA_CHECK(
        cudaMemcpy(actual.data(), cy.get(), actual.size() * 2, cudaMemcpyDeviceToHost));
    std::vector<float> wide(actual.size());
    for (size_t i = 0; i < wide.size(); ++i)
      wide[i] = bf16_to_f32(actual[i]);
  ...
  auto vx = vk.allocate(layout({static_cast<uint64_t>(rows), static_cast<uint64_t>(f.in)}),
                        ScalarType::kBFloat16);
  // Exercise Q/K/V's actual three-dimensional output layout.
  auto vy = vk.allocate(layout({static_cast<uint64_t>(rows), 1, static_cast<uint64_t>(out)}),
                        ScalarType::kBFloat16);
...
SLOPFAB_TEST(lora_cuda_vulkan_projection_numerics) {
  ...
  if (!Instance::available() || cuda::device_count() == 0) {
    SKIP_UNSUPPORTED_HARDWARE("CUDA and Vulkan required");
    return;
  }
  ...
  for (int rank : {3, 16})
    for (int rows : {5, 64, 259}) {
      ...
      for (int offset : {0, 16, 32})
        compare_projection(vk, f, rows, offset, 16);
    }
...
SLOPFAB_TEST(lora_taomate_real_transformer_three_step_cuda_vulkan) {
  ...
  if (cuda::query_device(0).free_memory < (4ull << 30)) {
    SKIP_INSUFFICIENT_VRAM("Reduced real transformer test needs 4 GiB free");
    return;
  }
  SafeTensors base;
  base.open(path);
