...
SLOPFAB_TEST_CATEGORY(cuda_vulkan_qwen_full50_real_nvfp4_l132, "integration") {
  ...
  if (!std::filesystem::exists(checkpoint_path) ||
      ...
      !Instance::available()) {
    SKIP_MISSING_FIXTURE("unavailable prerequisite: !std::filesystem::exists(checkpoint_path) || !std::filesystem::exists(capture_path) || cudaGetDeviceCount(&cuda_devices) != cudaSuccess || cuda_devices == 0 || !Instance::available()");
    return;
  }
