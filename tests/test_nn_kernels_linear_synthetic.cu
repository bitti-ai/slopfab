...
SLOPFAB_TEST_CATEGORY(nn_dequant_f8e4m3_all_patterns, "synthetic") {
  ...
  CHECK_MSG(mismatches == 0,
            "dequant_f8e4m3: %zu of 256 patterns differ from dtype.h; first at 0x%02X", mismatches,
            first < 0 ? 0 : first);
  CHECK(nan_checked == 2);
...
SLOPFAB_TEST_CATEGORY(nn_quantize_f8e4m3_roundtrip, "synthetic") {
  ...
  for (int i = 0; i < 256; ++i) {
    ...
    if (std::isnan(v))
      continue;
    if (v == 0.0f && (i & 0x80))
      continue; // -0 encodes back as +0's pattern only by sign
    values.push_back(v);
...
SLOPFAB_TEST_CATEGORY(nn_dequant_i8_per_channel, "synthetic") {
  ...
  for (size_t i = 0; i < w.size(); ++i)
    w[i] = int8_t(int(i * 37 % 255) - 127);
  std::vector<float> scale(out_features);
  for (int o = 0; o < out_features; ++o)
    scale[o] = 1e-3f * float(o + 1);
...
SLOPFAB_TEST_CATEGORY(nn_w4a8_weight_and_activation_decode, "synthetic") {
  ...
  for (int o = 0; o < out_features; ++o) {
    for (int i = 0; i < in_features; i += 2) {
      ...
      packed[size_t(o) * (in_features / 2) + i / 2] = static_cast<int8_t>(lo | (hi << 4));
  ...
  const std::vector<uint8_t> group_scale(size_t(out_features) * in_features / group_size,
                                         0x38); // E4M3 1.0
  std::vector<float> codebook(16);
  for (int i = 0; i < 16; ++i)
    codebook[i] = static_cast<float>(i - 8);
  ...
  slopfab::cuda::launch_dequant_w4a8_weight(dpacked.get(), dgroup.get(), dcodebook.get(),
                                            dweight.get(), out_features, in_features, group_size,
                                            nullptr);
  ...
  for (size_t i = 0; i < x.size(); ++i)
    xbits[i] = slopfab::f32_to_f16(x[i]);
  ...
  slopfab::cuda::launch_quantize_w4a8_activation(reinterpret_cast<const __half*>(dx.get()),
                                                 dq.get(), dscale.get(), rows, in_features,
                                                 nullptr);
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    for (int i = 0; i < in_features; ++i) {
      const int want =
          std::max(-128, std::min(127, static_cast<int>(std::nearbyint(rotated[i] / want_scale))));
  ...
  slopfab::cuda::launch_dequant_w4a8_output(daccum.get(), dax.get(), dwx.get(), rows, out_features,
                                            nullptr);
  std::vector<float> result(accum.size());
  SLOPFAB_CUDA_CHECK(cudaMemcpy(result.data(), daccum.get(), result.size() * sizeof(float),
                                cudaMemcpyDeviceToHost));
  for (int r = 0; r < rows; ++r) {
    for (int o = 0; o < out_features; ++o) {
      const size_t i = size_t(r) * out_features + o;
      CHECK_NEAR(result[i], static_cast<float>(accum[i]) * activation_scale[r] * weight_scale[o],
                 1e-6);
    }
  }
}
...
SLOPFAB_TEST_CATEGORY(nn_convrot_matches_regular_hadamard, "synthetic") {
  ...
  for (int i = 0; i < group; ++i) {
    for (int j = 0; j < group; ++j) {
      ...
      for (int k = 0; k < group; ++k)
        acc += double(H[size_t(i) * group + k]) * H[size_t(k) * group + j];
      if (i == j) {
  ...
  for (int r = 0; r < rows; ++r) {
    for (int g = 0; g < dim / group; ++g) {
      ...
      for (int i = 0; i < group; ++i) {
        double acc = 0.0;
        for (int j = 0; j < group; ++j)
          acc += double(H[size_t(i) * group + j]) * xm[base + j];
        wm[base + i] = float(acc);
      }
    }
  }
  ...
  for (int r = 0; r < rows; ++r) {
    for (int i = 0; i < 16; ++i) {
      double acc = 0.0;
      for (int j = 0; j < 16; ++j)
        acc += double(H16[size_t(i) * 16 + j]) * x16[size_t(r) * 16 + j];
...
SLOPFAB_TEST_CATEGORY(linear_bf16_and_fp8, "synthetic") {
  ...
  // 1. dense bf16, with a bias.
  {
    ...
    for (int r = 0; r < rows; ++r) {
      for (int o = 0; o < out_features; ++o)
        want[size_t(r) * out_features + o] += bias[o];
    }
  ...
  // patterns, so the reference dequantises with dtype.h rather than assuming.
  {
    ...
    for (size_t i = 0; i < raw.size(); ++i) {
      ...
      if ((b & 0x7F) == 0x7F)
        b &= 0x7Eu; // avoid the NaN encodings
      raw[i] = b;
    }
...
SLOPFAB_TEST_CATEGORY(linear_int8_convrot, "synthetic") {
  ...
  for (int o = 0; o < out_features; ++o)
    wscale[o] = 7.675675e-4f * (1.0f + 0.01f * float(o));
  ...
  for (int r = 0; r < rows; ++r) {
    for (int g = 0; g < in_features / group; ++g) {
      ...
      for (int i = 0; i < group; ++i) {
        double acc = 0.0;
        for (int j = 0; j < group; ++j)
          acc += double(H[size_t(i) * group + j]) * x[base + j];
        xr[base + i] = float(acc);
      }
    }
  }
  ...
  for (int o = 0; o < out_features; ++o) {
    for (int i = 0; i < odd_in; ++i)
      wo[size_t(o) * odd_in + i] = 3.0f * wscale[o];
  }
...
SLOPFAB_TEST_CATEGORY(linear_nf4_double_quant, "synthetic") {
  ...
  slopfab::cuda::QuantWeight qw;
  qw.format = slopfab::cuda::QuantFormat::kNF4;
  qw.data = dp.get();
  qw.out_features = w.out;
  qw.in_features = w.in;
  qw.nf4_absmax = da.get();
  qw.nf4_quant_map = dm.get();
  qw.nf4_nested_quant_map = dnm.get();
  qw.nf4_nested_absmax = dna.get();
  qw.nf4_nested_offset = w.offset;
