...
SLOPFAB_TEST(keyframe_conv3d_single_frame_causal_slice) {
  ...
  slopfab::cuda::launch_keyframe_conv3d(dx.get(), dw.get(), db.get(), dy.get(), 1, 1, 2, 3, 3, 1,
                                        true, false, nullptr);
...
SLOPFAB_TEST(keyframe_groupnorm_silu_matches_cpu) {
  ...
  slopfab::cuda::launch_keyframe_groupnorm_silu(dx.get(), dw.get(), db.get(), dy.get(), 2, 2, 2, 1,
                                                1e-6f, nullptr);
