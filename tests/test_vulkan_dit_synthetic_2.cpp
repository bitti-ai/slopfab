...
SLOPFAB_TEST_CATEGORY(vulkan_h3_loaded_stage_cuda_off_contract, "synthetic") {
  ...
  if (!context.exact_h3_attention() || !context.exact_vae_pointwise() ||
    ...
    SKIP_UNSUPPORTED_HARDWARE(
        "unavailable prerequisite: !context.exact_h3_attention() || !context.exact_vae_pointwise() || !context.exact_fp32_vae_normalization()");
    return;
  }
  ...
  auto fixture = [&](uint32_t corruption) {
    ...
    std::vector<TensorWrite> tensors{
        ...
        {"blocks.0.attn.out_proj.weight", {h, inner}, values(size_t(h * inner), 5, 1.0f / 4096.0f)},
        // Exactly one AWQ activation pre-scale fixture.
        {"blocks.0.attn.out_proj.pre_quant_scale",
         {inner},
         values(size_t(inner), 71, 1.0f / 256.0f)},
        {"blocks.0.mlp.fc1.weight", {2 * f, h}, values(size_t(2 * f * h), 7, 1.0f / 4096.0f)},
        {"blocks.0.mlp.fc2.weight", {h, f}, values(size_t(h * f), 11, 1.0f / 4096.0f)},
        // Exactly one regular-H4 ConvRot metadata fixture.
        {"blocks.0.mlp.fc2.comfy_quant",
         {static_cast<int64_t>(metadata(corruption == 2
                                            ? "{bad fc2 metadata"
  ...
  const auto corrupt_fc2_path = base / "slopfab_h3_cuda_off_corrupt_fc2.safetensors";
  ...
  auto graph_fixture = [&](bool corrupt_last) {
    ...
    for (uint32_t layer = 0; layer < 3; ++layer) {
      ...
      for (TensorWrite& tensor : one) {
        tensor.name.replace(0, std::strlen("blocks.0"), "blocks." + std::to_string(layer));
  ...
  const auto corrupt_graph_path = base / "slopfab_h3_cuda_off_corrupt_graph.safetensors";
  ...
  auto transformer_fixture = [&](uint32_t corruption) {
    ...
    for (TensorWrite& tensor : refiner) {
      if (tensor.name.find(".adaln_proj.") != std::string::npos)
        continue;
      tensor.name.replace(0, std::strlen("blocks.0"), "token_refiner.blocks.0");
      all.push_back(std::move(tensor));
    }
    ...
    all.insert(all.end(),
               {{"adaln_t_table", {1025, 8}, values(1025 * 8, 97, 1.0f / 4096.0f)},
                ...
                {"condition_proj.bias", {h}, values(h, 103, 1.0f / 128.0f), DType::kBF16},
                ...
                {"token_refiner.final_norm.weight", {h}, std::vector<float>(h, 1.0f), DType::kBF16},
                {"final_layer.norm.weight", {h}, std::vector<float>(h, 1.0f), DType::kBF16},
                ...
                {"final_layer.video_out.bias", {video_dim}, values(video_dim, 149, 1.0f / 128.0f)},
                {"final_layer.audio_out.weight",
                 {audio_dim, h},
                 values(size_t(audio_dim * h), 151, 1.0f / 1024.0f)},
                {"final_layer.audio_out.bias",
                 {corruption == 1 ? audio_dim - 1 : audio_dim},
                 values(size_t(corruption == 1 ? audio_dim - 1 : audio_dim), 157, 1.0f / 128.0f)}});
    const char* metadata_suffix = nullptr;
    if (corruption == 3)
      metadata_suffix = ".pre_quant_scale";
    if (corruption == 4)
      metadata_suffix = ".weight_scale";
    if (corruption == 5)
      metadata_suffix = ".input_scale";
    if (corruption == 6)
      metadata_suffix = ".comfy_quant";
    if (metadata_suffix) {
      all.push_back({std::string("final_layer.adaln_proj.linear") + metadata_suffix, {1}, {1.0f}});
    }
    return all;
  };
  const auto transformer_path = base / "slopfab_h3_cuda_off_transformer.safetensors";
  ...
  write_safetensors(corrupt_transformer_path.string(), transformer_fixture(1));
  write_safetensors(corrupt_transformer_dtype_path.string(), transformer_fixture(2));
  for (uint32_t i = 0; i < corrupt_transformer_metadata_paths.size(); ++i)
    write_safetensors(corrupt_transformer_metadata_paths[i].string(), transformer_fixture(3 + i));
  SafeTensors transformer_checkpoint, corrupt_transformer, corrupt_transformer_dtype;
  ...
  for (uint32_t i = 0; i < corrupt_transformer_metadata.size(); ++i)
    corrupt_transformer_metadata[i].open(corrupt_transformer_metadata_paths[i].string());
  ...
  DeviceTensor tokens =
      context.allocate(TensorLayout::contiguous(token_shape, 2), ScalarType::kBFloat16);
  DeviceTensor selectors =
      context.allocate(TensorLayout::contiguous(selector_shape, 1), ScalarType::kInt32);
  ...
  const uint64_t code_backing_shape[] = {config.timesteps, config.adaln_rank + 1};
  DeviceTensor bad_tokens =
      context.allocate(TensorLayout::contiguous(token_backing_shape, 2), ScalarType::kBFloat16);
  DeviceTensor bad_selectors =
      context.allocate(TensorLayout::contiguous(selector_backing_shape, 1), ScalarType::kInt32);
  DeviceTensor bad_code = context.allocate(TensorLayout::contiguous(code_backing_shape, 2));
  ...
  H3AttentionRanges foreign_ranges =
      H3AttentionRanges::create(foreign_context, config.sequence, full_range, 4);
  {
    TensorBatch invalid = context.begin_batch();
    auto rejected = [&](DeviceTensor& token_arg, DeviceTensor& selector_arg, DeviceTensor& code_arg,
                        const H3AttentionRanges* ranges = nullptr) {
      bool threw = false;
      try {
        first.record(invalid, token_arg, selector_arg, code_arg, cosine, sine, scratch, ranges);
        ...
        for (uint32_t part = 0; part < 3; ++part)
          for (uint32_t channel = 0; channel < config.head_dim; ++channel)
            for (uint32_t col = 0; col < config.hidden; ++col)
              tensor.data[dst++] =
                  original[size_t((part * config.heads + head) * config.head_dim + channel) *
                               config.hidden +
                           col];
    }
  ...
  if (context.h3_attention_supported(AttentionMode::kFlash2)) {
    ...
    auto evaluate = [&](bool sparse, float gate_scale) {
      ...
      if (sparse) {
        ...
        for (uint32_t i = 0; i < config.hidden; ++i)
          gate[i * config.hidden + i] = gate_scale;
      ...
      context.upload_bytes(tokens, input.data(), input.size() * 2);
      auto batch = context.begin_batch();
      while (batch.remaining_operator_capacity() > stage.required_operators())
        batch.copy(tokens, dummy);
    ...
    bool rejected = false;
    try {
      ExactH3BlockStage::validate_checkpoint(valid, 0, vsa_config);
  ...
  for (auto mode : {AttentionMode::kFlash2, AttentionMode::kSage2}) {
    if (!context.h3_attention_supported(mode))
      continue;
    H3BlockConfig fast_config = config;
    fast_config.attention_mode = mode;
    ...
    try {
      fast_stage.record(batch, tokens, selectors, code, cosine, sine, scratch);
    } catch (const std::invalid_argument&) {
  ...
  const uint64_t graph_create_descriptors = graph_context.descriptor_set_allocations();
  ExactH3MainGraph graph = ExactH3MainGraph::create(graph_context, graph_config);
  ...
  try {
    graph.load(corrupt_graph);
  } catch (const std::exception&) {
    initial_corrupt_rejected = true;
  }
  ...
  DeviceTensor graph_tokens =
      graph_context.allocate(TensorLayout::contiguous(graph_token_shape, 2), ScalarType::kBFloat16);
  DeviceTensor graph_selectors =
      graph_context.allocate(TensorLayout::contiguous(graph_selector_shape, 1), ScalarType::kInt32);
  DeviceTensor graph_code = graph_context.allocate(TensorLayout::contiguous(graph_code_shape, 2));
  DeviceTensor graph_cosine = graph_context.allocate(TensorLayout::contiguous(graph_rope_shape, 2));
  DeviceTensor graph_sine = graph_context.allocate(TensorLayout::contiguous(graph_rope_shape, 2));
  graph_context.upload_bytes(graph_selectors, host_selectors.data(), host_selectors.size() * 4);
  ...
  for (uint32_t layer = 0; layer < graph_config.layers; ++layer)
    boundaries.push_back(graph_context.allocate(TensorLayout::contiguous(graph_token_shape, 2),
                                                ScalarType::kBFloat16));
  H3MainGraphReplayTaps graph_taps{boundaries.data(), static_cast<uint32_t>(boundaries.size())};
  DeviceTensor graph_dummy =
      graph_context.allocate(TensorLayout::contiguous(graph_token_shape, 2), ScalarType::kBFloat16);
  DeviceTensor graph_dummy_out =
      graph_context.allocate(TensorLayout::contiguous(graph_token_shape, 2), ScalarType::kBFloat16);
  ...
  CHECK(graph.required_operators(&graph_taps) == 78u);
  {
    ...
    bool rejected = false;
    try {
      graph.record(short_batch, graph_tokens, graph_selectors, graph_code, graph_cosine, graph_sine,
                   nullptr, &graph_taps);
    } catch (const std::logic_error&) {
      rejected = true;
    }
  ...
  auto run_graph = [&] {
    ...
    graph.record(batch, graph_tokens, graph_selectors, graph_code, graph_cosine, graph_sine,
                 nullptr, &graph_taps);
  ...
  for (DeviceTensor& boundary : boundaries) {
    ...
    graph_context.download_bytes(boundary, boundary_values.data(), boundary_values.size() * 2);
    boundary_hashes.push_back(digest_bf16(boundary_values));
  }
  CHECK(boundary_hashes == std::vector<uint64_t>({0xc40d66ec8f2b7f26ull, 0x70e1eaf01723020bull,
  ...
  graph_context.upload_bytes(graph_tokens, input.data(), input.size() * 2);
  {
    ...
    graph.record_layers(split, graph_tokens, graph_selectors, graph_code, graph_cosine, graph_sine,
                        0, 1, nullptr, &graph_taps);
    graph.record_layers(split, graph_tokens, graph_selectors, graph_code, graph_cosine, graph_sine,
                        1, 2, nullptr, &graph_taps);
    CHECK(split.remaining_operator_capacity() == 50u);
    split.submit().wait();
  }
  std::vector<uint16_t> split_output(input.size());
  graph_context.download_bytes(graph_tokens, split_output.data(), split_output.size() * 2);
  CHECK(split_output == graph_output);
  {
    TensorBatch invalid_span = graph_context.begin_batch();
    bool rejected = false;
    try {
      graph.record_layers(invalid_span, graph_tokens, graph_selectors, graph_code, graph_cosine,
                          graph_sine, 2, 2);
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
  ...
  graph.unload();
  CHECK(!graph.loaded() && graph.persistent_bytes() == 0u && graph.scratch_bytes() == 0u &&
        graph.peak_device_bytes() == 0u);
  ...
  graph.load(graph_checkpoint);
  CHECK(graph.persistent_bytes() == graph_persistent && graph.scratch_bytes() == graph_scratch &&
        graph.peak_device_bytes() == graph_peak);
  CHECK(run_graph() == graph_output);
  std::printf(
      "  CUDA-off H3 3-layer graph FNV64 %016llx, ops %u, persistent/scratch %.2f/%.2f MiB\n",
      static_cast<unsigned long long>(digest_bf16(graph_output)), graph.required_operators(),
      double(graph.persistent_bytes()) / 1048576.0, double(graph.scratch_bytes()) / 1048576.0);
  graph.unload();
  ...
  DeviceTensor prompt = graph_context.allocate(TensorLayout::contiguous(prompt_shape, 2));
  DeviceTensor transformer_video =
      graph_context.allocate(TensorLayout::contiguous(video_input_shape, 2));
  DeviceTensor transformer_audio =
      graph_context.allocate(TensorLayout::contiguous(audio_input_shape, 2));
  DeviceTensor transformer_video_out =
      graph_context.allocate(TensorLayout::contiguous(video_input_shape, 2));
  DeviceTensor transformer_audio_out =
      graph_context.allocate(TensorLayout::contiguous(audio_input_shape, 2));
  DeviceTensor transformer_text_idx =
      graph_context.allocate(TensorLayout::contiguous(prompt_shape, 1), ScalarType::kInt32);
  const uint64_t video_index_shape[] = {60};
  const uint64_t audio_index_shape[] = {2};
  DeviceTensor transformer_video_idx =
      graph_context.allocate(TensorLayout::contiguous(video_index_shape, 1), ScalarType::kInt32);
  DeviceTensor transformer_audio_idx =
      graph_context.allocate(TensorLayout::contiguous(audio_index_shape, 1), ScalarType::kInt32);
  DeviceTensor transformer_selectors =
      graph_context.allocate(TensorLayout::contiguous(selector_shape, 1), ScalarType::kInt32);
  DeviceTensor transformer_code = graph_context.allocate(TensorLayout::contiguous(code_shape, 2));
  DeviceTensor transformer_cosine = graph_context.allocate(TensorLayout::contiguous(rope_shape, 2));
  DeviceTensor transformer_sine = graph_context.allocate(TensorLayout::contiguous(rope_shape, 2));
  DeviceTensor transformer_video_ts =
      graph_context.allocate(TensorLayout::contiguous(video_index_shape, 1), ScalarType::kInt32);
  DeviceTensor transformer_audio_ts =
      graph_context.allocate(TensorLayout::contiguous(audio_index_shape, 1), ScalarType::kInt32);
  ...
  graph_context.upload_bytes(transformer_video_ts, zero_video_ts.data(), zero_video_ts.size() * 4);
  graph_context.upload_bytes(transformer_audio_ts, zero_audio_ts.data(), zero_audio_ts.size() * 4);
  DeviceTensor shared_forward_tap =
      graph_context.allocate(TensorLayout::contiguous(token_shape, 2), ScalarType::kBFloat16);
  ...
  const uint64_t transformer_baseline_descriptors = graph_context.descriptor_set_allocations();
  ExactH3Transformer transformer = ExactH3Transformer::create(graph_context, transformer_config);
  ...
  try {
    transformer.load(corrupt_transformer);
  } catch (const std::exception&) {
    transformer_corrupt_rejected = true;
  }
  CHECK(transformer_corrupt_rejected && !transformer.loaded());
  CHECK(graph_context.pooled_used_bytes() == transformer_baseline);
  std::vector<SafeTensors*> invalid_endpoint_archives{&corrupt_transformer_dtype};
  ...
  for (SafeTensors* invalid : invalid_endpoint_archives) {
    ...
    CHECK(graph_context.descriptor_set_allocations() == transformer_baseline_descriptors);
  }
  ...
  for (const H3TransformerTextReplayTaps* invalid :
    ...
    bool rejected = false;
    try {
      transformer.prepare_text(prompt, invalid);
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
  ...
  H3TransformerForwardReplayTaps null_forward_taps{nullptr, nullptr, &null_main_taps};
  H3MainGraphReplayTaps aliased_main_taps{&shared_forward_tap, 1};
  H3TransformerForwardReplayTaps aliased_forward_taps{&shared_forward_tap, nullptr,
                                                      &aliased_main_taps};
  H3TransformerForwardReplayTaps valid_forward_taps{&shared_forward_tap, nullptr, nullptr};
  ...
  CHECK(transformer.required_forward_operators(&valid_forward_taps) == 43u);
  {
    ...
    auto reject_forward = [&](const H3AttentionRanges* test_ranges,
      ...
      bool rejected = false;
      try {
        transformer.record_forward(transactional, transformer_video, transformer_audio,
  ...
  }
  {
    ...
    const uint32_t tapped_need = transformer.required_forward_operators(&valid_forward_taps);
    ...
    try {
      transformer.record_forward(
          one_short, transformer_video, transformer_audio, transformer_selectors, transformer_code,
          transformer_cosine, transformer_sine, transformer_video_ts, transformer_audio_ts,
          transformer_video_out, transformer_audio_out, nullptr, &valid_forward_taps);
    } catch (const std::logic_error&) {
      rejected = true;
    }
    CHECK(rejected && one_short.remaining_operator_capacity() == remaining);
    transformer.record_forward(one_short, transformer_video, transformer_audio,
                               transformer_selectors, transformer_code, transformer_cosine,
                               transformer_sine, transformer_video_ts, transformer_audio_ts,
                               transformer_video_out, transformer_audio_out);
  ...
  auto run_transformer = [&] {
    TensorBatch batch = graph_context.begin_batch();
    transformer.record_forward(batch, transformer_video, transformer_audio, transformer_selectors,
    ...
    graph_context.download(transformer_video_out, video_result.data(), video_result.size());
    graph_context.download(transformer_audio_out, audio_result.data(), audio_result.size());
  ...
  std::printf(
      "  CUDA-off H3 transformer S65 FNV64 %016llx, ops %u, persistent/scratch %.2f/%.2f MiB\n",
      static_cast<unsigned long long>(transformer_digest), transformer.required_forward_operators(),
  ...
  const uint64_t transformer_descriptors = graph_context.descriptor_set_allocations();
  bool transformer_reload_rejected = false;
  try {
    transformer.load(corrupt_transformer_metadata.back());
  } catch (const std::logic_error&) {
    transformer_reload_rejected = true;
  }
  ...
  for (auto& tensor : singularity_tensors) {
    if (tensor.name == "video_patch_proj.weight" || tensor.name == "audio_patch_proj.weight" ||
  ...
  for (const auto& target : std::vector<std::pair<std::string, std::string>>{
    ...
    write_safetensors(
        viggle_adapter_path.string(),
        {{target.first + ".lora_A.weight", {16, in}, test::make_data(size_t(16 * in), 893, .25f)},
         {target.first + ".lora_B.weight",
          {out, 16},
          test::make_data(size_t(out * 16), 981, .25f)}});
    ...
    transformer.load(transformer_checkpoint);
    transformer.prepare_text(prompt);
    CHECK_MSG(run_transformer() != transformer_output, "%s update did not reach Vulkan forward",
              target.first.c_str());
    transformer.unload();
  }
  ...
  const std::vector<int32_t> ref_text_tags{dit::kTagText, dit::kTagVideo, dit::kTagText};
  const std::vector<dit::ReferenceGeometry> ref_geometries = {
      {dit::ReferenceKind::kImage, 1, 4, 4, 0}, {dit::ReferenceKind::kAudio, 0, 0, 0, 1}};
  const dit::Ref2VAPackedSequence ref_packed =
      dit::build_ref2va_packed_sequence(ref_text_tags, ref_geometries, 1, 4, 4, 1);
  ...
  ref_config.transformer.text_rows = static_cast<uint32_t>(ref_config.layout.num_text);
  ref_config.transformer.video_rows = static_cast<uint32_t>(ref_config.indices.video.size());
  ref_config.transformer.audio_rows = static_cast<uint32_t>(ref_config.indices.audio.size());
  ...
  const uint64_t ref_validate_descriptors = denoise_context.descriptor_set_allocations();
  {
    ExactH3Denoiser ref_denoiser = ExactH3Denoiser::create(denoise_context, ref_config);
    CHECK(!ref_denoiser.loaded());
  }
  auto reject_ref_config = [&](const ExactH3DenoiseConfig& invalid) {
    bool rejected = false;
    try {
      ExactH3Denoiser candidate = ExactH3Denoiser::create(denoise_context, invalid);
    } catch (const std::invalid_argument&) {
      rejected = true;
    }
    CHECK(rejected);
    ...
    CHECK(denoise_context.descriptor_set_allocations() == ref_validate_descriptors);
  };
  ...
  std::swap(invalid_ref.indices.video.front(), invalid_ref.indices.video.back());
  reject_ref_config(invalid_ref);
  invalid_ref = ref_config;
  ...
  const uint64_t denoise_baseline_descriptors = denoise_context.descriptor_set_allocations();
  ExactH3Denoiser denoiser = ExactH3Denoiser::create(denoise_context, denoise_config);
  bool denoise_corrupt_rejected = false;
  try {
    denoiser.load(corrupt_transformer_metadata.front());
  } catch (const std::exception&) {
    denoise_corrupt_rejected = true;
  }
  ...
  CHECK(denoise_context.descriptor_set_allocations() == denoise_baseline_descriptors);
  ...
  DeviceTensor capacity_source =
      denoise_context.allocate(TensorLayout::contiguous(capacity_shape, 1));
  DeviceTensor capacity_destination =
      denoise_context.allocate(TensorLayout::contiguous(capacity_shape, 1));
  {
    ...
    CHECK(exact_capacity.remaining_operator_capacity() == denoiser.required_step_operators());
    exact_capacity.copy(capacity_source, capacity_destination);
    exact_capacity.submit().wait();
  }
  {
    ...
    bool rejected = false;
    try {
      short_capacity.require_operator_capacity(denoiser.required_step_operators());
    } catch (const std::logic_error&) {
      rejected = true;
    }
    CHECK(rejected &&
          short_capacity.remaining_operator_capacity() + 1u == denoiser.required_step_operators());
    short_capacity.copy(capacity_source, capacity_destination);
  ...
  const ExactH3DenoiseResult denoise_output = denoiser.run(denoise_video, denoise_audio);
  ...
  const uint64_t denoise_descriptors = denoise_context.descriptor_set_allocations();
  denoiser.prepare(prompt_values.data(), prompt_values.size(), video_values.data(),
                   video_values.size(), audio_values.data(), audio_values.size());
  ExactH3DenoiseResult denoise_repeat = denoiser.run(denoise_video, denoise_audio);
  ...
  denoiser.prepare(prompt_values.data(), prompt_values.size(), video_values.data(),
                   video_values.size(), audio_values.data(), audio_values.size());
  ExactH3DenoiseResult cancelled =
      denoiser.run(denoise_video, denoise_audio, [](uint32_t step, uint32_t) {
        return step != 0;
      });
  ...
  try {
    (void)denoiser.run(denoise_video, unsupported_audio);
  } catch (const std::invalid_argument&) {
    ab2_rejected = true;
  }
  ...
  CHECK(denoise_context.staging_capacity_bytes() != 0 && denoise_staging_used > denoise_baseline &&
        denoise_staging_used - denoise_baseline < denoise_persistent);
  denoiser.load(transformer_checkpoint);
  denoiser.prepare(prompt_values.data(), prompt_values.size(), video_values.data(),
                   video_values.size(), audio_values.data(), audio_values.size());
  ExactH3DenoiseResult after_reload = denoiser.run(denoise_video, denoise_audio);
  ...
  std::printf("  CUDA-off H3 denoise S65 x3 FNV64 %016llx, persistent/scratch %.2f/%.2f MiB\n",
              static_cast<unsigned long long>(denoise_digest),
              double(denoise_persistent) / 1048576.0, double(denoise_scratch) / 1048576.0);
  denoiser.unload();
  ...
  // forward/update submissions must also agree when warmup prevents reuse.
  {
    ...
    auto run_motion = [&](const ExactH3DenoiseConfig& config, bool cancel) {
      ...
      model.prepare(prompt_values.data(), prompt_values.size(), video_values.data(),
                    video_values.size(), audio_values.data(), audio_values.size());
  ...
  still_config.transformer.video_rows = static_cast<uint32_t>(still_config.layout.num_video_rows);
  ...
  ExactH3Denoiser still_denoiser = ExactH3Denoiser::create(denoise_context, still_config);
  ...
  const std::vector<float> still_video(
      video_values.begin(),
      video_values.begin() + static_cast<ptrdiff_t>(still_config.layout.num_video_rows * 4));
  still_denoiser.prepare(prompt_values.data(), prompt_values.size(), still_video.data(),
                         still_video.size(), nullptr, 0);
  const ExactH3DenoiseResult still_output = still_denoiser.run(denoise_video, denoise_audio);
  ...
  still_denoiser.prepare(prompt_values.data(), prompt_values.size(), still_video.data(),
                         still_video.size(), nullptr, 0);
  const ExactH3DenoiseResult still_repeat = still_denoiser.run(denoise_video, denoise_audio);
  ...
  const dit::Ref2VAPackedSequence still_ref_packed = dit::build_ref2va_packed_sequence(
      ref_text_tags, {{dit::ReferenceKind::kImage, 1, 4, 4, 0}}, 1, 4, 4, 0);
  ...
  ExactH3Denoiser still_ref_denoiser = ExactH3Denoiser::create(denoise_context, still_ref_config);
  still_ref_denoiser.load(transformer_checkpoint);
  const std::vector<float> still_ref_video(
      video_values.begin(),
      video_values.begin() + static_cast<ptrdiff_t>(still_ref_config.transformer.video_rows * 4));
  still_ref_denoiser.prepare(prompt_values.data(), prompt_values.size(), still_ref_video.data(),
                             still_ref_video.size(), nullptr, 0);
  const ExactH3DenoiseResult still_ref_output =
      still_ref_denoiser.run(denoise_video, denoise_audio);
  ...
  std::printf("  CUDA-off H3 S65 two-stage FNV64 %016llx, persistent/scratch %.2f/%.2f MiB\n",
  ...
  if (std::filesystem::exists(real_path)) {
    ...
    ExactH3BlockScratch real_scratch = ExactH3BlockScratch::create(context, real_config);
    ...
    const uint64_t real_code_shape[] = {real_config.timesteps, real_config.adaln_rank};
    const uint64_t real_rope_shape[] = {real_config.sequence, 96};
    DeviceTensor real_tokens =
        context.allocate(TensorLayout::contiguous(real_token_shape, 2), ScalarType::kBFloat16);
    DeviceTensor real_selectors =
        context.allocate(TensorLayout::contiguous(real_selector_shape, 1), ScalarType::kInt32);
    DeviceTensor real_code = context.allocate(TensorLayout::contiguous(real_code_shape, 2));
    DeviceTensor real_cosine = context.allocate(TensorLayout::contiguous(real_rope_shape, 2));
    DeviceTensor real_sine = context.allocate(TensorLayout::contiguous(real_rope_shape, 2));
    std::vector<uint16_t> real_input(size_t(real_config.sequence) * real_config.hidden);
    ...
    std::vector<float> real_code_values(size_t(real_config.timesteps) * real_config.adaln_rank);
    ...
    auto run_real = [&] {
      ...
      TensorBatch batch = context.begin_batch();
      real_stage.record(batch, real_tokens, real_selectors, real_code, real_cosine, real_sine,
                        real_scratch);
      batch.submit().wait();
    ...
    for (uint16_t bits : real_output) {
      real_digest ^= bits & 0xffu;
      real_digest *= 1099511628211ull;
      real_digest ^= bits >> 8;
      real_digest *= 1099511628211ull;
    }
    ...
    ExactH3MainGraph real_graph = ExactH3MainGraph::create(context, real_graph_config);
    ...
    auto run_real_graph = [&] {
      context.upload_bytes(real_tokens, real_input.data(), real_input.size() * 2);
      TensorBatch batch = context.begin_batch();
      real_graph.record(batch, real_tokens, real_selectors, real_code, real_cosine, real_sine);
      batch.submit().wait();
      std::vector<uint16_t> result(real_input.size());
      context.download_bytes(real_tokens, result.data(), result.size() * 2);
      return result;
    ...
    for (uint16_t bits : real_graph_output) {
      real_graph_digest ^= bits & 0xffu;
      real_graph_digest *= 1099511628211ull;
      real_graph_digest ^= bits >> 8;
      real_graph_digest *= 1099511628211ull;
    }
    ...
    CHECK(real_graph.persistent_bytes() == 0u && real_graph.scratch_bytes() == 0u &&
...

    {
      ...
      DeviceTensor real_prompt = context.allocate(TensorLayout::contiguous(real_prompt_shape, 2));
      DeviceTensor real_video = context.allocate(TensorLayout::contiguous(real_video_shape, 2));
      DeviceTensor real_audio = context.allocate(TensorLayout::contiguous(real_audio_shape, 2));
      DeviceTensor real_video_out = context.allocate(TensorLayout::contiguous(real_video_shape, 2));
      DeviceTensor real_audio_out = context.allocate(TensorLayout::contiguous(real_audio_shape, 2));
      DeviceTensor real_transformer_selectors =
          context.allocate(TensorLayout::contiguous(real_selector_shape, 1), ScalarType::kInt32);
      DeviceTensor real_transformer_code =
          context.allocate(TensorLayout::contiguous(real_code_shape, 2));
      DeviceTensor real_transformer_cosine =
          context.allocate(TensorLayout::contiguous(real_rope_shape, 2));
      DeviceTensor real_transformer_sine =
          context.allocate(TensorLayout::contiguous(real_rope_shape, 2));
      DeviceTensor real_video_ts =
          context.allocate(TensorLayout::contiguous(real_video_index_shape, 1), ScalarType::kInt32);
      DeviceTensor real_audio_ts =
          context.allocate(TensorLayout::contiguous(real_audio_index_shape, 1), ScalarType::kInt32);
      ...
      std::vector<int32_t> real_video_ts_values(60, 0), real_audio_ts_values(2, 0);
      ...
      context.upload_bytes(real_transformer_selectors, real_selector_values.data(),
                           real_selector_values.size() * 4);
      context.upload(real_transformer_code, real_code_values.data(), real_code_values.size());
      context.upload(real_transformer_cosine, real_cosine_values.data(), real_cosine_values.size());
      context.upload(real_transformer_sine, real_sine_values.data(), real_sine_values.size());
      ...
      ExactH3Transformer real_transformer =
          ExactH3Transformer::create(context, real_transformer_config);
      real_transformer.load(real_checkpoint);
      real_transformer.prepare_text(real_prompt);
      auto run_real_transformer = [&] {
        TensorBatch batch = context.begin_batch();
        real_transformer.record_forward(batch, real_video, real_audio, real_transformer_selectors,
      ...
      const uint64_t real_transformer_digest = fnv64_floats(real_transformer_output);
      CHECK(real_transformer_digest == 0x42764ebbb3850be4ull);
      const uint64_t real_transformer_reserved = context.reserved_bytes();
      const uint64_t real_transformer_descriptors = context.descriptor_set_allocations();
      ...
      CHECK(real_transformer.persistent_bytes() == 0u && real_transformer.scratch_bytes() == 0u);
