...
SLOPFAB_TEST(attention_descriptor_validates_grouped_query_contract) {
  ...
  d.key_value_heads = 5;
  CHECK(rejects([&] { slopfab::validate_attention_descriptor(d); }));
  d.key_value_heads = 3;
...
SLOPFAB_TEST(attention_descriptor_exact_scale_and_text_capability) {
  ...
  AttentionDescriptor d{23, 23, 64, 8, 128, AttentionLayout::kTokensHeadsChannels,
      AttentionMask::kCausal, AttentionArithmetic::kExact, exact_attention_scale(128)};
  ...
  CHECK(!supports_exact_text_attention(d, 4096));
  d.scale = 0;
  CHECK(rejects([&] { validate_attention_descriptor(d); }));
