...
SLOPFAB_TEST_CATEGORY(vulkan_h3_quantized_weight_residency, "checkpoint") {
  ...
  for (const char* filename : {"fl2va_pruned_fp8_scaled.safetensors",
    ...
    const auto path =
        std::filesystem::path(SLOPFAB_TEST_SOURCE_DIR) / "weights/transformer" / filename;
    if (!std::filesystem::exists(path)) {
    ...
    auto tensor = [&](std::initializer_list<uint64_t> shape,
      ...
      return context.allocate(
          TensorLayout::contiguous(shape.begin(), static_cast<uint32_t>(shape.size())), type);
    };
    ...
    for (size_t i = 0; i < ids.size(); ++i)
      ids[i] = int32_t(i % config.modalities);
    ...
    const uint64_t expected =
        std::strstr(filename, "int8") ? 0x730a94ce15dc8847ull : 0x3657399741e2efc5ull;
    ...
    std::printf("  %s: block persistent %llu, scratch %llu, output %016llx\n", filename,
                static_cast<unsigned long long>(stage.persistent_bytes()),
