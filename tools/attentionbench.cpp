...
template <class F> double latency(F run) {
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    run();
    ms.push_back(std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start)
                     .count());
  }
...
int main(int argc, char** argv) try {
  ...
  if (extra_mib > 65536 || kernel > 4)
    throw std::invalid_argument("Invalid extra workspace MiB or Sage kernel (0..4)");
  if (sequence < 1 || sequence > 65536 || heads < 1 || heads > 128 || (dim != 64 && dim != 128) ||
      argc > 7)
    throw std::invalid_argument(
        "Usage: slopfab_attentionbench [sequence 1..65536] [heads 1..128] [extra_MiB 0..65536] [sage_kernel 0..4] [Vulkan_device_index] [head_dim 64|128]");
  ...
  if (device_index >= physical.size())
    throw std::out_of_range("Vulkan device index");
  ...
  for (auto mode : {AttentionMode::kExact, AttentionMode::kFlash2, AttentionMode::kSage2}) {
    if (!context.h3_attention_supported(mode)) {
      std::printf("Vulkan %s unavailable\n", attention_mode_name(mode));
      continue;
    }
    auto plan = vulkan::H3AttentionPlan::create(
        context, {uint32_t(sequence), uint32_t(heads), dim, exact_attention_scale(dim), mode,
                  mode == AttentionMode::kSage2 ? kernel : 0});
    auto run = [&] {
      auto batch = context.begin_batch();
      plan.record(batch, dq, dk, dv, output);
      batch.submit().wait();
    };
    const double ms = latency(run);
    context.download_bytes(output, out.data(), n * 2);
    for (auto word : out)
      if (!std::isfinite(bf16_to_f32(word)))
        throw std::runtime_error("Nonfinite Vulkan output");
    if (mode == AttentionMode::kFlash2)
      flash = out;
    std::printf("Vulkan %-6s %9.3f ms; workspace %.3f MiB\n", attention_mode_name(mode), ms,
                double(plan.workspace_bytes()) / 1048576);
    if (mode == AttentionMode::kSage2) {
      const auto c = plan.sage_configuration();
      std::printf(
          "  Sage kernel %u: Q%u/V%u, %u threads, %u shared bytes, parallel mean %d, prepared V %d, extra %.3f MiB\n",
          c.kernel, c.query_rows, c.value_rows, c.local_size, c.shared_bytes, int(c.parallel_mean),
          int(c.prepared_value), double(c.extra_workspace_bytes) / 1048576);
      if (info.timestamp_valid_bits) {
        ...
        std::printf("  GPU timestamps unavailable on this queue\n");
    }
  }
#if SLOPFAB_WITH_CUDA
  if (device_index != 0)
    return 0; // Vulkan and CUDA enumeration orders need not agree.
  cuda::set_device(0);
  ...
  for (auto backend : {cuda::AttentionBackend::kFused, cuda::AttentionBackend::kSage2}) {
    ...
    const double ms = latency([&] {
      cuda::attention_forward(nullptr, nullptr, cq.get(), ck.get(), cv.get(), co.get(), config,
                              backend, ws);
    ...
    std::printf("CUDA   %-6s %9.3f ms; relative L2 vs Vulkan flash2 %.6f\n",
                backend == cuda::AttentionBackend::kFused ? "flash2" : "sage2", ms,
                std::sqrt(e2 / std::max(r2, 1e-30)));
  }
#endif
  return 0;
