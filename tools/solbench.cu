...
__global__ void scan_nonfinite(const __nv_bfloat16* p, size_t n, unsigned long long* result) {
  for (size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x; i < n;
       i += size_t(gridDim.x) * blockDim.x) {
    ...
    if (isfinite(a))
      atomicMax(reinterpret_cast<unsigned int*>(result + 2), __float_as_uint(a));
  }
}

__global__ void compare_outputs(const __nv_bfloat16* sol, const __nv_bfloat16* dense, size_t n,
                                double* sums, unsigned long long* bad, unsigned* max_diff) {
  __shared__ double sd[256], ss[256], sr[256], sp[256];
  __shared__ unsigned long long bs[256], br[256];
  __shared__ float md[256];
  double d2 = 0, s2 = 0, r2 = 0, dot = 0;
  unsigned long long sb = 0, rb = 0;
  float mx = 0;
  for (size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x; i < n;
       i += size_t(gridDim.x) * blockDim.x) {
    const float s = __bfloat162float(sol[i]), r = __bfloat162float(dense[i]);
    const bool sf = isfinite(s), rf = isfinite(r);
    sb += !sf;
    rb += !rf;
    if (sf && rf) {
      const double e = double(s) - r;
      d2 += e * e;
      s2 += double(s) * s;
      r2 += double(r) * r;
      dot += double(s) * r;
      mx = fmaxf(mx, fabsf(s - r));
    }
  }
  ...
  for (int k = 128; k; k >>= 1) {
    if (t < k) {
      ...
      bs[t] += bs[t + k];
      br[t] += br[t + k];
      md[t] = fmaxf(md[t], md[t + k]);
    }
    __syncthreads();
  }
  if (t == 0) {
    atomicAdd(sums, sd[0]);
    atomicAdd(sums + 1, ss[0]);
    atomicAdd(sums + 2, sr[0]);
    atomicAdd(sums + 3, sp[0]);
    atomicAdd(bad, bs[0]);
    atomicAdd(bad + 1, br[0]);
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!std::strcmp(argv[i], "--seq"))
      ...
      iterations = next();
    else if (!std::strcmp(argv[i], "--prefix"))
      prefix = next();
    else if (!std::strcmp(argv[i], "--beta")) {
      if (++i >= argc)
        return 2;
      beta = std::strtof(argv[i], nullptr);
    } else if (!std::strcmp(argv[i], "--error-k")) {
      if (++i >= argc)
        return 2;
      error_k = std::strtof(argv[i], nullptr);
    } else if (!std::strcmp(argv[i], "--error-v")) {
      if (++i >= argc)
        return 2;
      error_v = std::strtof(argv[i], nullptr);
    } else if (!std::strcmp(argv[i], "--experimental-pipeline"))
      pipeline = true;
  ...
  if (!input.empty()) {
    ...
    if (capture.tensor_elements != expected || expected > SIZE_MAX / 6)
      return 2;
  ...
  slopfab::cuda::DeviceBuffer<__nv_bfloat16> q(values), k(values), v(values), out(values),
      dense_out(values);
  if (captured.empty()) {
    ...
    SLOPFAB_CUDA_CHECK(
        cudaMemcpy(q.get(), captured.data(), values * sizeof(uint16_t), cudaMemcpyHostToDevice));
    SLOPFAB_CUDA_CHECK(cudaMemcpy(k.get(), captured.data() + values, values * sizeof(uint16_t),
                                  cudaMemcpyHostToDevice));
    SLOPFAB_CUDA_CHECK(cudaMemcpy(v.get(), captured.data() + 2 * values, values * sizeof(uint16_t),
                                  cudaMemcpyHostToDevice));
  }
  ...
  if (!save_input.empty()) {
    ...
    SLOPFAB_CUDA_CHECK(
        cudaMemcpy(captured.data(), q.get(), values * sizeof(uint16_t), cudaMemcpyDeviceToHost));
    SLOPFAB_CUDA_CHECK(cudaMemcpy(captured.data() + values, k.get(), values * sizeof(uint16_t),
                                  cudaMemcpyDeviceToHost));
    SLOPFAB_CUDA_CHECK(cudaMemcpy(captured.data() + 2 * values, v.get(), values * sizeof(uint16_t),
                                  cudaMemcpyDeviceToHost));
    slopfab::SolCaptureHeader h{{'V', 'F', 'S', 'O', 'L', 'Q', 'K', 'V'},
                                1,
                                sizeof(slopfab::SolCaptureHeader),
  ...
  SLOPFAB_CUDA_CHECK(
      cudaMemcpy(route_host, routes.get(), sizeof(route_host), cudaMemcpyDeviceToHost));
  ...
  SLOPFAB_CUDA_CHECK(
      cudaMemcpy(finite_diag.get(), finite_init, sizeof(finite_init), cudaMemcpyHostToDevice));
  scan_nonfinite<<<256, 256>>>(out.get(), values, finite_diag.get());
  unsigned long long finite_host[3]{};
  SLOPFAB_CUDA_CHECK(
      cudaMemcpy(finite_host, finite_diag.get(), sizeof(finite_host), cudaMemcpyDeviceToHost));
  ...
  double cmp[4]{};
  unsigned long long cmp_bad[2]{};
  unsigned cmp_max_bits = 0;
  SLOPFAB_CUDA_CHECK(cudaMemcpy(cmp, compare_sums.get(), sizeof(cmp), cudaMemcpyDeviceToHost));
  SLOPFAB_CUDA_CHECK(
      cudaMemcpy(cmp_bad, compare_bad.get(), sizeof(cmp_bad), cudaMemcpyDeviceToHost));
  SLOPFAB_CUDA_CHECK(
      cudaMemcpy(&cmp_max_bits, compare_max.get(), sizeof(cmp_max_bits), cudaMemcpyDeviceToHost));
  float cmp_max = 0;
  std::memcpy(&cmp_max, &cmp_max_bits, sizeof(cmp_max));
  std::printf("seq=%d heads=%d prefix=%d beta=%.3g pipeline=%d workspace=%.2f MiB\n", seq, heads,
              prefix, beta, int(pipeline), sol_bytes / 1048576.0);
  std::printf("device shared/SM=%.1f KiB registers/SM=%d\n", smem_per_sm / 1024.0, regs_per_sm);
  if (!input.empty())
    std::printf("input=%s step=%d layer=%d\n", input.c_str(), capture.denoise_step, capture.layer);
  ...
  std::printf("routes exact %.1f%%  approximate %.1f%%\n", 100.0 * route_host[0] / route_total,
              100.0 * route_host[1] / route_total);
