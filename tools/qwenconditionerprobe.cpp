...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::fprintf(stderr,
                 "usage: slopfab_qwenconditionerprobe <qwen.safetensors> <tokenizer.json>\n");
    return 2;
  }
  try {
    slopfab::text::Tokenizer tokenizer;
    tokenizer.load(argv[2]);
    ...
    encoder.load(checkpoint, cfg);
    auto t1 = std::chrono::steady_clock::now();
    auto out = encoder.encode(ids, {pixels});
    ...
    for (size_t i = 0; i < out.modality_tags.size(); ++i) {
      ...
      if (out.modality_tags[i] != want)
        throw std::runtime_error("incorrect conditioner modality tag");
    }
    long double sq = 0;
    for (float v : out.data) {
      if (!std::isfinite(v))
        throw std::runtime_error("non-finite conditioner output");
      sq += static_cast<long double>(v) * v;
    }
    const double value = std::sqrt(static_cast<double>(sq / out.data.size()));
    if (!(value > 0))
      throw std::runtime_error("degenerate conditioner output");
