...
__global__ void tma_load(const __grid_constant__ CUtensorMap map,
                         uint16_t* output) {
  ...
  if (threadIdx.x == 0) {
    ...
    asm volatile("mbarrier.init.shared::cta.b64 [%0], 1;\n"
                 ...
                 : "r"(kTileBytes), "r"(tile_addr), "l"(&map), "r"(0),
                   "r"(kStartRow)
                 : "memory");
    uint32_t ready = 0;
    do {
...
int main() {
  ...
  CUDA_CHECK(cudaMemcpy(device_source, source.data(),
                        source.size() * sizeof(uint16_t),
  ...
  CU_CHECK(cuTensorMapEncodeTiled(
      &map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2, device_source, global_dims,
  ...
  CUDA_CHECK(cudaMemcpy(output.data(), device_output, kTileBytes,
  ...
  for (int row = 0; row < kTileRows; ++row) {
    for (int col = 0; col < kTileCols; ++col) {
      ...
      if (actual != expected && mismatches++ < 8) {
        std::fprintf(stderr,
                     "mismatch [%d,%d]: got 0x%04x, expected 0x%04x\n", row,
                     col, actual, expected);
      }
    }
  }
