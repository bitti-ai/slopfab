  ...
  try {
...
#ifdef SLOPFAB_REFERENCEPROBE_VULKAN
    if (mixed) throw std::invalid_argument("FP16 probe requires CUDA");
    if (benchmark) throw std::invalid_argument("reference benchmark requires CUDA");
#endif
    ...
    }
    {
      ...
      compare(encoder.encode_reference_video(
              ...
              "video chunking/posterior/rows", mixed ? .1 : 5e-3,
              mixed ? .005 : 1e-4);
#ifndef SLOPFAB_REFERENCEPROBE_VULKAN
      if (benchmark) {
        const int width = std::stoi(argv[5]), height = std::stoi(argv[6]), count = std::stoi(argv[7]);
        if (width < 32 || height < 32 || width > 2048 || height > 2048 ||
            width % 32 || height % 32 || count < 22 || count > 345 || (count - 5) % 17)
          throw std::invalid_argument("benchmark: dimensions must be 32..2048 multiples of 32; frames must be 17*n+5, 22..345");
        ...
        for (int run = 0; run < 2; ++run) {
          const auto begin = std::chrono::steady_clock::now();
          auto rows = encoder.encode_reference_video(clip, count,
              slopfab::to_f32(golden.at("video_latent_mean")),
              slopfab::to_f32(golden.at("video_latent_std")), mixed);
          std::printf("benchmark %s %dx%d %d frames run %d: %.3f s\n", mixed ? "fp16" : "fp32",
              width, height, count, run + 1,
              std::chrono::duration<double>(std::chrono::steady_clock::now() - begin).count());
          for (float value : rows) if (!std::isfinite(value))
            throw std::runtime_error("benchmark: nonfinite reference rows");
          if (run && previous != rows) throw std::runtime_error("benchmark: repeated encode changed rows");
          previous = std::move(rows);
        }
        const auto path = std::string("reference-bench-") + (mixed ? "fp16" : "fp32") + ".bin";
        std::ofstream output(path, std::ios::binary);
        output.write(reinterpret_cast<const char*>(previous.data()), previous.size() * sizeof(float));
        if (!output) throw std::runtime_error("benchmark: failed to write rows");
