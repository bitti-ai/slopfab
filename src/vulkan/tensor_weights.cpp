...
LinearWeight LinearWeight::upload(TensorContext& context, const LinearWeightUpload& source) {
  if (!context.impl_) {
  ...
  const uint64_t elements =
      checked_multiply(source.out_features, source.in_features, "linear weight");
  ...
  case LinearWeightFormat::kInt8:
    ...
    data_type = source.format == LinearWeightFormat::kInt8 ? ScalarType::kInt8 : ScalarType::kUInt8;
    break;
  ...
  const bool scale_pair_valid =
      (source.weight_scale == nullptr) == (source.weight_scale_count == 0);
  const bool block_pair_valid = (source.block_scale == nullptr) == (source.block_scale_count == 0);
  const bool absmax_pair_valid = (source.nf4_absmax == nullptr) == (source.nf4_absmax_count == 0);
  const bool map_pair_valid =
      (source.nf4_quant_map == nullptr) == (source.nf4_quant_map_count == 0);
  const bool nested_map_pair_valid =
      (source.nf4_nested_quant_map == nullptr) == (source.nf4_nested_quant_map_count == 0);
  const bool nested_absmax_pair_valid =
      (source.nf4_nested_absmax == nullptr) == (source.nf4_nested_absmax_count == 0);
  const bool pre_scale_pair_valid =
      (source.pre_quant_scale_bf16 == nullptr) == (source.pre_quant_scale_count == 0);
  if (!scale_pair_valid || !block_pair_valid || !absmax_pair_valid || !map_pair_valid ||
      !nested_map_pair_valid || !nested_absmax_pair_valid || !pre_scale_pair_valid) {
    throw std::invalid_argument("vulkan linear weight: auxiliary pointer/count mismatch");
  }
  if (source.has_fp8_input_scale &&
      (source.format != LinearWeightFormat::kFloat8E4M3 || !std::isnormal(source.fp8_input_scale) ||
       source.fp8_input_scale <= 0.0f)) {
  ...
  if (source.pre_quant_scale_count != 0 && (source.pre_quant_scale_bf16 == nullptr ||
                                            source.pre_quant_scale_count != source.in_features)) {
    throw std::invalid_argument("vulkan linear weight: invalid AWQ pre-scale");
  }
  uint32_t power = 1;
  while (power < source.convrot_group && power <= UINT32_MAX / 4)
    power *= 4;
  if (source.convrot &&
      (source.convrot_group < 4 || source.convrot_group > 256 || power != source.convrot_group ||
  ...
  if (source.format == LinearWeightFormat::kInt8 &&
      (source.weight_scale == nullptr || source.weight_scale_count != source.out_features)) {
  ...
  if (source.format == LinearWeightFormat::kNVFloat4 &&
      (source.in_features % 64 != 0 || source.out_features % 128 != 0 ||
       source.block_scale == nullptr || source.block_scale_count != elements / 16 ||
       !std::isfinite(source.global_scale))) {
    throw std::invalid_argument("vulkan linear weight: invalid NVFP4 metadata");
  }
  const uint64_t expected_absmax =
      source.nf4_block_size == 0 ? 0 : 1 + (elements - 1) / source.nf4_block_size;
  const uint64_t expected_nested = source.nf4_nested_block_size == 0 || expected_absmax == 0
                                       ? 0
                                       : 1 + (expected_absmax - 1) / source.nf4_nested_block_size;
  if (source.format == LinearWeightFormat::kNF4 &&
      (source.nf4_block_size != 64 || source.nf4_nested_block_size != 256 ||
       ...
       source.nf4_nested_quant_map == nullptr || source.nf4_nested_quant_map_count != 256 ||
       source.nf4_nested_absmax == nullptr || source.nf4_nested_absmax_count != expected_nested ||
       !std::isfinite(source.nf4_nested_offset))) {
    throw std::invalid_argument("vulkan linear weight: invalid NF4 metadata");
  }
  const bool has_nf4_aux = source.nf4_absmax_count != 0 || source.nf4_quant_map_count != 0 ||
                           source.nf4_nested_quant_map_count != 0 ||
                           source.nf4_nested_absmax_count != 0;
  if ((source.format != LinearWeightFormat::kFloat8E4M3 &&
       source.format != LinearWeightFormat::kInt8 && source.weight_scale_count != 0) ||
      (source.format != LinearWeightFormat::kNVFloat4 && source.block_scale_count != 0) ||
      (source.format != LinearWeightFormat::kNF4 && has_nf4_aux)) {
    throw std::invalid_argument("vulkan linear weight: auxiliary metadata does not match format");
  }
  ...
  const uint64_t data_count =
      data_type == ScalarType::kFloat32                                           ? elements
      : (data_type == ScalarType::kFloat16 || data_type == ScalarType::kBFloat16) ? elements
                                                                                  : expected_bytes;
  allocate(result->data, data_type, data_count, source.data);
  if (source.weight_scale_count != 0)
    allocate(result->weight_scale, ScalarType::kFloat32, source.weight_scale_count,
             source.weight_scale);
  if (source.block_scale_count != 0)
    allocate(result->block_scale, ScalarType::kUInt8, source.block_scale_count, source.block_scale);
  if (source.format == LinearWeightFormat::kNF4) {
    allocate(result->nf4_absmax, ScalarType::kUInt8, source.nf4_absmax_count, source.nf4_absmax);
    allocate(result->nf4_quant_map, ScalarType::kFloat32, source.nf4_quant_map_count,
             source.nf4_quant_map);
    allocate(result->nf4_nested_quant_map, ScalarType::kFloat32, source.nf4_nested_quant_map_count,
             source.nf4_nested_quant_map);
    allocate(result->nf4_nested_absmax, ScalarType::kFloat32, source.nf4_nested_absmax_count,
             source.nf4_nested_absmax);
  }
  if (source.pre_quant_scale_count != 0)
    allocate(result->pre_quant_scale, ScalarType::kBFloat16, source.pre_quant_scale_count,
             source.pre_quant_scale_bf16);
  ...
  for (const auto& item : pending) {
    ...
    Buffer buffer =
        context.impl_->pool.allocate(physical, BufferUsage::kTransferSource, MemoryUsage::kUpload);
    buffer.write(0, item.values, item.logical_bytes);
    if (physical != item.logical_bytes)
      write_zero_bytes(buffer, item.logical_bytes, physical - item.logical_bytes);
  ...
  for (size_t i = 0; i < pending.size(); ++i) {
    ...
    list.barrier(staging[i], BufferAccess::kHostWrite, BufferAccess::kTransferRead, 0, physical);
...
void TensorBatch::materialize_linear_weight(const LinearWeight& weight, DeviceTensor& dense,
                                            bool fp16) {
  ...
  const uint64_t elements = checked_multiply(weight.impl_->out_features, weight.impl_->in_features,
                                             "linear materialization");
  const ScalarType output_type = fp16 ? ScalarType::kFloat16 : ScalarType::kBFloat16;
  if (dense.layout().rank != 2 || dense.layout().extent[0] != weight.impl_->out_features ||
      dense.layout().extent[1] != weight.impl_->in_features || !dense.layout().is_contiguous() ||
      output->type != output_type || elements > std::numeric_limits<uint32_t>::max()) {
    throw std::invalid_argument("vulkan linear weight: dense target must be contiguous [out,in]");
  }
  ...
  for (size_t i = 0; i + 1 < resources.size(); ++i) {
    if (resources[i].get() == output.get()) {
      throw std::invalid_argument("vulkan linear weight: dense target aliases persistent storage");
    }
  }
  ...
  const float scalar = op == 5 ? weight.impl_->global_scale : weight.impl_->nf4_nested_offset;
...
void TensorBatch::transform_linear_activation(const LinearWeight& weight, DeviceTensor& input,
  ...
  const bool matrix = shape.rank == 2 && shape.extent[1] == weight.impl_->in_features;
  ...
  if (source.get() == destination.get() || (!matrix && !heads) ||
      destination->layout.rank != shape.rank || destination->layout.extent != shape.extent ||
      !shape.is_contiguous() || !destination->layout.is_contiguous() ||
      source->type != destination->type ||
      (source->type != ScalarType::kBFloat16 && source->type != ScalarType::kFloat32) ||
  ...
  if (convrot) {
    ...
    if (group_count == 0 || group_count > impl_->owner->max_dispatch_x) {
      throw std::out_of_range("vulkan linear weight: ConvRot exceeds dispatch limits");
    }
    groups = static_cast<uint32_t>(group_count);
    const float normalization = 1.0f / std::sqrt(static_cast<float>(weight.impl_->convrot_group));
    std::memcpy(&parameters.scalar_bits, &normalization, sizeof(normalization));
  } else {
    if (!weight.impl_->pre_quant_scale) {
      throw std::invalid_argument("vulkan linear weight: AWQ pre-scale is not present");
    }
    ...
    const uint64_t invocations =
        source->type == ScalarType::kFloat32 ? elements : 1 + (elements - 1) / 2;
    impl_->owner->validate_dispatch(invocations);
...
PreparedF16Activation::PreparedF16Activation(std::shared_ptr<Impl> impl) : impl_(std::move(impl)) {
}

PreparedF16Activation::PreparedF16Activation(PreparedF16Activation&&) noexcept = default;
PreparedF16Activation& PreparedF16Activation::operator=(PreparedF16Activation&&) noexcept = default;

PreparedF16Activation::operator bool() const noexcept {
...
PreparedF16Activation PreparedF16Activation::create(TensorContext& context, uint32_t max_rows,
                                                    uint32_t in_features) {
  if (!context.impl_) {
    throw std::logic_error("vulkan gemm: moved-from tensor context");
  }
  if (max_rows == 0 || in_features == 0) {
    throw std::invalid_argument("vulkan gemm: fp16 activation dimensions must be nonzero");
  }
  ...
  result->tensor = context.allocate(TensorLayout::contiguous(shape, 2), ScalarType::kFloat16);
  return PreparedF16Activation(std::move(result));
}

PreparedF16ActivationView PreparedF16Activation::prepare(TensorBatch& batch, DeviceTensor& input,
                                                         uint32_t rows, uint32_t input_row_offset) {
  if (!impl_)
    throw std::logic_error("vulkan gemm: empty fp16 activation slot");
  if (!batch.impl_)
    throw std::logic_error("vulkan gemm: empty batch");
  if (batch.impl_->poisoned)
    throw std::logic_error("vulkan gemm: batch is poisoned");
  if (batch.impl_->owner.get() != impl_->owner.get()) {
    throw std::invalid_argument("vulkan gemm: fp16 activation slot and batch contexts differ");
  }
  auto source = impl_->owner->require(input);
  auto destination = impl_->owner->require(impl_->tensor);
  const bool valid_range = rows != 0 && rows <= impl_->max_rows && source->layout.rank == 2 &&
                           source->layout.extent[1] == impl_->in_features &&
                           input_row_offset <= source->layout.extent[0] &&
                           rows <= source->layout.extent[0] - input_row_offset;
  if (!valid_range || source->type != ScalarType::kFloat32 || !source->layout.is_contiguous() ||
      source.get() == destination.get()) {
    throw std::invalid_argument("vulkan gemm: fp16 activation source does not match the slot");
  }
  const uint64_t elements = checked_multiply(rows, impl_->in_features, "gemm fp16 prepare");
  ...
  if (gy > impl_->owner->max_dispatch_y) {
    throw std::out_of_range("vulkan gemm: fp16 preparation exceeds dispatch limits");
  }
  ...
  PreparedF16ActivationView view(impl_, batch.impl_->batch_id, rows, impl_->generation + 1);
  try {
    ...
    batch.impl_->commands.bind_compute(impl_->owner->gemm_prepare_pipeline, bindings);
    batch.impl_->commands.push_constants(&parameters, sizeof(parameters));
    batch.impl_->commands.dispatch(static_cast<uint32_t>(gx), static_cast<uint32_t>(gy));
  } catch (...) {
...
PreparedF16ActivationView::PreparedF16ActivationView(std::shared_ptr<void> slot, uintptr_t batch_id,
                                                     uint32_t rows, uint64_t generation) noexcept
    : slot_(std::move(slot)), batch_id_(batch_id), rows_(rows), generation_(generation) {
}

PreparedF16ActivationView::PreparedF16ActivationView(PreparedF16ActivationView&&) noexcept =
    default;
PreparedF16ActivationView&
PreparedF16ActivationView::operator=(PreparedF16ActivationView&&) noexcept = default;

uint32_t PreparedF16ActivationView::rows() const noexcept {
  return slot_ ? rows_ : 0;
}

PreparedF16ActivationView::operator bool() const noexcept {
...
StreamedNVFP4WeightCache::StreamedNVFP4WeightCache(std::shared_ptr<Impl> impl)
    : impl_(std::move(impl)) {
}

StreamedNVFP4WeightCache::StreamedNVFP4WeightCache(StreamedNVFP4WeightCache&&) noexcept = default;
StreamedNVFP4WeightCache&
StreamedNVFP4WeightCache::operator=(StreamedNVFP4WeightCache&&) noexcept = default;

StreamedNVFP4WeightCache::operator bool() const noexcept {
  return impl_ != nullptr;
}

StreamedNVFP4WeightCache StreamedNVFP4WeightCache::create(TensorContext& context,
                                                          uint64_t max_weight_elements) {
  if (!context.impl_) {
    throw std::logic_error("vulkan nvfp4 stream: moved-from tensor context");
  }
  if (max_weight_elements == 0 || max_weight_elements > std::numeric_limits<uint64_t>::max() / 2) {
    throw std::invalid_argument("vulkan nvfp4 stream: cache capacity is invalid");
  }
  auto result = std::make_shared<Impl>();
  result->owner = context.impl_;
  result->capacity_elements = max_weight_elements;
  const uint64_t shape[] = {max_weight_elements};
  result->dense = context.allocate(TensorLayout::contiguous(shape, 1), ScalarType::kBFloat16);
  return StreamedNVFP4WeightCache(std::move(result));
}

PreparedNVFP4WeightView StreamedNVFP4WeightCache::prepare(TensorBatch& batch,
                                                          const LinearWeight& weight,
                                                          const DenseGemmPlan& plan) {
  if (!impl_)
    throw std::logic_error("vulkan nvfp4 stream: empty cache");
  if (!batch.impl_)
    throw std::logic_error("vulkan nvfp4 stream: empty batch");
  if (!weight.impl_)
    throw std::logic_error("vulkan nvfp4 stream: empty weight");
  if (!plan.impl_)
    throw std::logic_error("vulkan nvfp4 stream: empty GEMM plan");
  if (batch.impl_->poisoned) {
    throw std::logic_error("vulkan nvfp4 stream: batch is poisoned");
  }
  if (batch.impl_->owner.get() != impl_->owner.get() ||
      plan.impl_->owner.get() != impl_->owner.get()) {
    throw std::invalid_argument("vulkan nvfp4 stream: cache, batch and plan contexts differ");
  }
  ...
  case LinearWeightFormat::kNF4:
    ...
    break;
  default:
    throw std::invalid_argument("vulkan linear stream: unsupported weight format");
  }
  const uint64_t elements = checked_multiply(weight.impl_->out_features, weight.impl_->in_features,
                                             "nvfp4 streamed weight");
  if (desc.mode != DenseGemmMode::kBFloat16 || desc.out_features != weight.impl_->out_features ||
      desc.in_features != weight.impl_->in_features || elements > impl_->capacity_elements ||
      elements > std::numeric_limits<uint32_t>::max()) {
    throw std::invalid_argument("vulkan linear stream: weight does not match the BF16 plan/cache");
  }
  ...
  for (const auto& source : sources) {
    if (source && source.get() == dense.get()) {
      throw std::invalid_argument("vulkan linear stream: cache aliases persistent weight storage");
    }
  }
  const uint64_t shape[] = {weight.impl_->out_features, weight.impl_->in_features};
  const TensorLayout layout = TensorLayout::contiguous(shape, 2);
  const uint64_t logical_bytes = checked_multiply(elements, 2, "nvfp4 streamed BF16 bytes");
  if (logical_bytes > dense->buffer.size()) {
    throw std::out_of_range("vulkan nvfp4 stream: weight exceeds the cache allocation");
  }
  PreparedNVFP4WeightView view(impl_, batch.impl_->batch_id, impl_->generation + 1,
...
PreparedNVFP4WeightView::PreparedNVFP4WeightView(std::shared_ptr<void> cache, uintptr_t batch_id,
                                                 uint64_t generation, uint32_t out_features,
                                                 uint32_t in_features, bool full_precision) noexcept
    : cache_(std::move(cache)), batch_id_(batch_id), generation_(generation),
      out_features_(out_features), in_features_(in_features), full_precision_(full_precision) {
}

PreparedNVFP4WeightView::PreparedNVFP4WeightView(PreparedNVFP4WeightView&&) noexcept = default;
PreparedNVFP4WeightView&
PreparedNVFP4WeightView::operator=(PreparedNVFP4WeightView&&) noexcept = default;
