...
std::vector<std::string> enumerate_extensions(const InstanceState& state,
  ...
  for (;;) {
    ...
    if (result != VK_INCOMPLETE)
      fail("vkEnumerateDeviceExtensionProperties", result);
  }
...
DeviceInfo inspect_device(const std::shared_ptr<InstanceState>& state, VkPhysicalDevice physical) {
  ...
  info.max_storage_buffer_bindings = std::min(properties.limits.maxPerStageDescriptorStorageBuffers,
  ...
  if (state->get_physical_device_features2 != nullptr) {
    ...
    if (info.supports_extension(VK_KHR_COOPERATIVE_MATRIX_EXTENSION_NAME)) {
      cooperative.pNext = info.supports_extension(VK_KHR_SHADER_BFLOAT16_EXTENSION_NAME)
                              ? static_cast<void*>(&bfloat16)
                              : static_cast<void*>(&features11);
    ...
    info.descriptor_binding_partially_bound = features12.descriptorBindingPartiallyBound == VK_TRUE;
    ...
    info.descriptor_indexing =
        features12.descriptorIndexing == VK_TRUE && info.runtime_descriptor_array &&
        info.descriptor_binding_partially_bound && info.descriptor_binding_variable_count &&
        info.storage_buffer_non_uniform_indexing;
    ...
    info.shader_bfloat16_cooperative_matrix = bfloat16.shaderBFloat16CooperativeMatrix == VK_TRUE;
  }
      ...
      if (state->get_cooperative_matrix_properties(physical, &count, tuples.data()) == VK_SUCCESS) {
        for (const auto& tuple : tuples) {
          info.cooperative_matrix_i8_i32_16x16x32 |=
              ...
              tuple.scope == VK_SCOPE_SUBGROUP_KHR && tuple.AType == VK_COMPONENT_TYPE_SINT8_KHR &&
          ...
          const bool common = tuple.MSize == 16 && tuple.NSize == 16 && tuple.KSize == 16 &&
          ...
          info.cooperative_matrix_bf16_f32_16x16x16 |=
              common && tuple.AType == VK_COMPONENT_TYPE_BFLOAT16_KHR &&
              tuple.BType == VK_COMPONENT_TYPE_BFLOAT16_KHR;
          info.cooperative_matrix_f16_f32_16x16x16 |=
              common && tuple.AType == VK_COMPONENT_TYPE_FLOAT16_KHR &&
...
Instance Instance::create(const InstanceOptions& options) {
  ...
  if (enumerate_version != nullptr)
    detail::check(enumerate_version(&loader_version), "vkEnumerateInstanceVersion");
  const uint32_t requested =
      options.api_version == 0 ? std::min(loader_version, VK_API_VERSION_1_3) : options.api_version;
  ...
  try {
    state->enumerate_physical_devices = detail::load_instance<PFN_vkEnumeratePhysicalDevices>(
        get, instance, "vkEnumeratePhysicalDevices");
    state->get_physical_device_properties =
        detail::load_instance<PFN_vkGetPhysicalDeviceProperties>(get, instance,
                                                                 "vkGetPhysicalDeviceProperties");
    state->get_physical_device_properties2 =
        detail::load_instance<PFN_vkGetPhysicalDeviceProperties2>(get, instance,
                                                                  "vkGetPhysicalDeviceProperties2");
    state->get_physical_device_features = detail::load_instance<PFN_vkGetPhysicalDeviceFeatures>(
        get, instance, "vkGetPhysicalDeviceFeatures");
    state->get_physical_device_features2 = reinterpret_cast<PFN_vkGetPhysicalDeviceFeatures2>(
    ...
    state->get_physical_device_memory_properties =
        detail::load_instance<PFN_vkGetPhysicalDeviceMemoryProperties>(
            get, instance, "vkGetPhysicalDeviceMemoryProperties");
    state->get_queue_family_properties =
        detail::load_instance<PFN_vkGetPhysicalDeviceQueueFamilyProperties>(
            get, instance, "vkGetPhysicalDeviceQueueFamilyProperties");
    state->enumerate_device_extensions =
        detail::load_instance<PFN_vkEnumerateDeviceExtensionProperties>(
            get, instance, "vkEnumerateDeviceExtensionProperties");
    state->create_device =
        detail::load_instance<PFN_vkCreateDevice>(get, instance, "vkCreateDevice");
    state->get_device_proc_addr =
        detail::load_instance<PFN_vkGetDeviceProcAddr>(get, instance, "vkGetDeviceProcAddr");
...
Version Instance::loader_version() const {
  if (!impl_)
    throw std::logic_error("vulkan: empty Instance");
...
std::vector<PhysicalDevice> Instance::enumerate_devices() const {
  ...
  for (;;) {
    ...
    if (result == VK_SUCCESS)
      break;
    if (result != VK_INCOMPLETE)
      detail::fail("vkEnumeratePhysicalDevices", result);
  }
...
const DeviceInfo& PhysicalDevice::info() const {
  if (!impl_)
    throw std::logic_error("vulkan: empty PhysicalDevice");
  return impl_->info;
}
...
Device PhysicalDevice::create_device(const DeviceOptions& options) const {
  ...
  require(options.enable_buffer_device_address, impl_->info.buffer_device_address,
          "bufferDeviceAddress");
  require(options.enable_descriptor_indexing, impl_->info.descriptor_indexing,
          "descriptorIndexing");
  require(options.enable_cooperative_matrix, impl_->info.cooperative_matrix, "cooperativeMatrix");
  const bool enable_bfloat16 = options.enable_cooperative_matrix &&
                               impl_->info.shader_bfloat16_type &&
  ...
  bfloat16.pNext = &features11;
  cooperative.pNext =
      enable_bfloat16 ? static_cast<void*>(&bfloat16) : static_cast<void*>(&features11);
  VkPhysicalDeviceFeatures core_features{};
  core_features.shaderInt64 = options.enable_shader_int64;
  const bool any_features =
      options.enable_shader_float16 || options.enable_shader_int8 ||
      options.enable_storage_buffer_16bit || options.enable_storage_buffer_8bit ||
      options.enable_timeline_semaphore || options.enable_buffer_device_address ||
      options.enable_descriptor_indexing || options.enable_cooperative_matrix;

  VkDeviceCreateInfo create{};
  create.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;
  create.pNext = !any_features                       ? nullptr
                 : options.enable_cooperative_matrix ? static_cast<void*>(&cooperative)
                                                     : static_cast<void*>(&features11);
  ...
  try {
    state->device_wait_idle =
        detail::load_device<PFN_vkDeviceWaitIdle>(*impl_->state, handle, "vkDeviceWaitIdle");
    state->queue_wait_idle =
        detail::load_device<PFN_vkQueueWaitIdle>(*impl_->state, handle, "vkQueueWaitIdle");
    state->create_buffer =
        detail::load_device<PFN_vkCreateBuffer>(*impl_->state, handle, "vkCreateBuffer");
    state->destroy_buffer =
        detail::load_device<PFN_vkDestroyBuffer>(*impl_->state, handle, "vkDestroyBuffer");
    state->get_buffer_requirements = detail::load_device<PFN_vkGetBufferMemoryRequirements>(
        *impl_->state, handle, "vkGetBufferMemoryRequirements");
    state->get_buffer_requirements2 = detail::load_device<PFN_vkGetBufferMemoryRequirements2>(
        *impl_->state, handle, "vkGetBufferMemoryRequirements2", false);
    state->allocate_memory =
        detail::load_device<PFN_vkAllocateMemory>(*impl_->state, handle, "vkAllocateMemory");
    state->free_memory =
        detail::load_device<PFN_vkFreeMemory>(*impl_->state, handle, "vkFreeMemory");
    state->bind_buffer_memory =
        detail::load_device<PFN_vkBindBufferMemory>(*impl_->state, handle, "vkBindBufferMemory");
    state->map_memory = detail::load_device<PFN_vkMapMemory>(*impl_->state, handle, "vkMapMemory");
    state->unmap_memory =
        detail::load_device<PFN_vkUnmapMemory>(*impl_->state, handle, "vkUnmapMemory");
    state->flush_mapped_ranges = detail::load_device<PFN_vkFlushMappedMemoryRanges>(
        *impl_->state, handle, "vkFlushMappedMemoryRanges");
    state->invalidate_mapped_ranges = detail::load_device<PFN_vkInvalidateMappedMemoryRanges>(
        *impl_->state, handle, "vkInvalidateMappedMemoryRanges");
    const auto get_queue =
        detail::load_device<PFN_vkGetDeviceQueue>(*impl_->state, handle, "vkGetDeviceQueue");
...
const DeviceInfo& Device::info() const {
  if (!impl_)
    throw std::logic_error("vulkan: empty Device");
  return impl_->info;
}
...
uint32_t Queue::family_index() const {
  if (!impl_)
    throw std::logic_error("vulkan: empty Queue");
