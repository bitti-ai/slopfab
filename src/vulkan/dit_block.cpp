...
TensorLayout vector(uint64_t n) {
...
void validate_config(const H3BlockConfig& c) {
  ...
  if (c.sequence == 0 || c.hidden == 0 || c.heads == 0 || c.head_dim != 128 || inner == 0 ||
      inner > std::numeric_limits<uint32_t>::max() || c.ffn == 0 || c.timesteps == 0 ||
      c.modalities == 0 || c.adaln_rank == 0 || !std::isnormal(c.epsilon) || c.epsilon <= 0.0f) {
...
std::vector<uint16_t> bf16_vector(const SafeTensors& st, const std::string& name, uint32_t count) {
  const TensorView& v = st.at(name);
  require_shape(v, {count}, name);
  std::vector<float> wide = to_f32(v);
  std::vector<uint16_t> result(count);
  for (uint32_t i = 0; i < count; ++i)
    result[i] = f32_to_bf16(wide[i]);
  return result;
}
...
ProjectionTag projection_tag(const SafeTensors& st, const std::string& name, uint32_t in) {
  ProjectionTag tag;
  ...
  try {
    ...
    throw std::runtime_error("Vulkan H3 block: invalid comfy_quant for '" + name + "' (" +
                             error.what() + ")");
  }
  ...
  if (const json::Value* group = root.find("convrot_groupsize")) {
    ...
    if (value <= 0 || value > UINT32_MAX)
      throw std::runtime_error("Vulkan H3 block: invalid ConvRot group for '" + name + "'");
...
void validate_projection_archive(const SafeTensors& st, const std::string& name, uint32_t out,
                                 uint32_t in, uint32_t source_out = 0, uint32_t row_offset = 0) {
  if (source_out == 0)
    source_out = out;
  ...
  const bool nvfp4 =
      w.dtype == DType::kU8 && block_scale && block_scale->dtype == DType::kF8E4M3 && in % 64 == 0;
  if (is_nf4_weight(st, name)) {
    const NF4State state = read_nf4_state(st, name, "Vulkan H3 block");
    if (state.shape != std::vector<int64_t>{source_out, in} || element_offset % 16384 != 0 ||
        elements % 16384 != 0 || w.dtype != DType::kU8 || w.nbytes != (source_elements + 1) / 2)
      throw std::runtime_error("Vulkan H3 block: invalid NF4 projection slice");
    ...
    if (absmax.dtype != DType::kU8 || absmax.nbytes != expected_absmax || map.size() != 16 ||
        nested.size() != 256 || nested_abs.size() != expected_nested ||
    ...
    require_shape(*block_scale, {source_out, static_cast<int64_t>(in / 16)},
                  name + ".weight_scale");
    ...
    case DType::kI8:
      ...
      break;
    default:
      throw std::runtime_error("Vulkan H3 block: unsupported dtype for '" + name + "'");
    }
  ...
  uint32_t power = 1;
  while (power < tag.convrot_group && power <= UINT32_MAX / 4)
    power *= 4;
  if (tag.convrot && (tag.convrot_group < 4 || tag.convrot_group > 256 ||
                      power != tag.convrot_group || in % tag.convrot_group != 0))
...
void validate_block_archive(const SafeTensors& st, const std::string& p, const H3BlockConfig& c,
                            bool with_adaln) {
  ...
  if (with_adaln) {
    const uint64_t adaln_out =
        checked_product(checked_product(c.modalities, 6, "AdaLN"), c.hidden, "AdaLN");
...
Projection load_projection(TensorContext& context, const SafeTensors& st, const LoraAdapters* loras,
                           uint32_t rows, const std::string& name, uint32_t out, uint32_t in,
  ...
  if (row_offset > source_out || out > source_out - row_offset)
    throw std::invalid_argument("Vulkan H3 block: invalid projection slice");
  const TensorView& w = st.at(name + ".weight");
  LinearWeightUpload u;
  u.out_features = out;
  u.in_features = in;
  std::vector<float> scale_storage, map_storage, nested_map_storage, nested_absmax_storage;
  ...
  if (is_nf4_weight(st, name)) {
    ...
    const TensorView& nested_map = st.at(name + ".weight.nested_quant_map");
    const TensorView& nested_absmax = st.at(name + ".weight.nested_absmax");
    map_storage = to_f32(qmap);
    nested_map_storage = to_f32(nested_map);
    ...
    u.nf4_quant_map = map_storage.data();
    u.nf4_quant_map_count = map_storage.size();
    ...
    switch (w.dtype) {
    case DType::kF32:
      u.format = LinearWeightFormat::kFloat32;
      element_bytes = 4;
      break;
    case DType::kF16:
      u.format = LinearWeightFormat::kFloat16;
      element_bytes = 2;
      break;
    case DType::kBF16:
      u.format = LinearWeightFormat::kBFloat16;
      element_bytes = 2;
      break;
    case DType::kF8E4M3:
      u.format = LinearWeightFormat::kFloat8E4M3;
      element_bytes = 1;
      break;
    case DType::kI8:
      u.format = LinearWeightFormat::kInt8;
      element_bytes = 1;
      break;
    default:
      throw std::runtime_error("Vulkan H3 block: unsupported dtype for '" + name + "'");
  ...
  if (name.size() >= 14 && name.compare(name.size() - 14, 14, ".attn.qkv_proj") == 0 &&
    ...
    if (u.format == LinearWeightFormat::kInt8) {
      const auto scales =
          dit::deinterleave_qkv_rows(st.at(name + ".weight_scale"), head_dim, row_offset, out);
      TensorView view = st.at(name + ".weight_scale");
      view.shape = {out, 1};
      view.data = scales.data();
      view.nbytes = scales.size();
  ...
  if (const TensorView* pre = st.find(name + ".pre_quant_scale")) {
    ...
    pre_scale_storage.resize(in);
    for (uint32_t i = 0; i < in; ++i)
      pre_scale_storage[i] = f32_to_bf16(wide[i]);
    u.pre_quant_scale_bf16 = pre_scale_storage.data();
    u.pre_quant_scale_count = in;
  }
  const ProjectionTag tag = projection_tag(st, name, in);
  ...
  if (loras)
    if (const auto* f = loras->find(name))
      result.lora.load(context, *f, row_offset, out, rows);
  if (loras && name.size() >= 14 && name.compare(name.size() - 14, 14, ".attn.qkv_proj") == 0) {
  ...
  if (!result.streamed()) {
    ...
    result.weight.materialize_bf16(batch, result.dense);
    batch.submit().wait();
  }
  return result;
}
...
struct ExactH3BlockScratch::Impl {
  ...
  explicit Impl(TensorContext& owner, const H3BlockConfig& c) : context(&owner), config(c) {
    const uint32_t inner = c.heads * c.head_dim;
    modulation_rows = c.timesteps * c.modalities;
    const uint64_t logical_table = checked_product(modulation_rows, c.hidden, "modulation");
    const uint64_t align_elements =
        std::max<uint64_t>(1, (owner.storage_binding_alignment() + 3) / 4);
    ...
    const uint64_t largest = std::max(
        {checked_product(inner, c.hidden, "q"), checked_product(c.hidden, inner, "out"),
         checked_product(2ull * c.ffn, c.hidden, "fc1"), checked_product(c.hidden, c.ffn, "fc2")});
    cache = StreamedNVFP4WeightCache::create(owner, largest);
    auto plan = [&](uint32_t out, uint32_t in) {
      return DenseGemmPlan::create(
          owner, {c.sequence, out, in, DenseGemmMode::kBFloat16, DenseGemmBias::kNone, false});
    };
    q_plan = plan(inner, c.hidden);
    k_plan = plan(inner, c.hidden);
    v_plan = plan(inner, c.hidden);
    out_plan = plan(c.hidden, inner);
    fc1_plan = plan(2 * c.ffn, c.hidden);
    fc2_plan = plan(c.hidden, c.ffn);
    if (c.vsa_tiles) {
      ...
      attention_plan =
          H3AttentionPlan::create(owner, {c.sequence, c.heads, c.head_dim,
                                          exact_attention_scale(c.head_dim), c.attention_mode});
    }
  }

  uint64_t reserved() const noexcept {
    return bytes(modulation) + bytes(normed) + bytes(q) + bytes(k) + bytes(v) + bytes(attention) +
           bytes(branch) + bytes(fused) + bytes(activation) + bytes(hidden_a) + bytes(hidden_b) +
           bytes(inner_a) + bytes(inner_b) + bytes(ffn_a) + bytes(ffn_b) + bytes(compress_gate) +
           vsa_plan.workspace_bytes() + lora.reserved_bytes() + cache.dense_bytes() +
           attention_plan.workspace_bytes();
  }
};
...
void ExactH3BlockStage::validate_checkpoint(const SafeTensors& st, uint32_t layer,
  ...
  if (bool(st.find(p + "attn.to_gate_compress.weight")) != bool(c.vsa_tiles))
    throw std::runtime_error(
        "Vulkan H3 block: VSA gate weights and tile geometry must be supplied together");
  if (c.vsa_tiles)
    validate_projection_archive(st, p + "attn.to_gate_compress", c.heads * c.head_dim, c.hidden);
  validate_block_archive(st, p, c, true);
}

void ExactH3BlockStage::validate_refiner_checkpoint(const SafeTensors& st, uint32_t layer,
                                                    const H3BlockConfig& c) {
  validate_config(c);
  if (c.vsa_tiles)
    throw std::invalid_argument("Vulkan H3 refiner requires dense attention");
  const std::string p = "token_refiner.blocks." + std::to_string(layer) + ".";
  validate_block_archive(st, p, c, false);
}

void ExactH3BlockStage::load(const SafeTensors& st, uint32_t layer) {
  if (!impl_)
    throw std::logic_error("Vulkan H3 block: empty stage");
  Impl& s = *impl_;
  const H3BlockConfig& c = s.config;
  const uint32_t inner = c.heads * c.head_dim;
  const std::string p = "blocks." + std::to_string(layer) + ".";
  if (bool(st.find(p + "attn.to_gate_compress.weight")) != bool(c.vsa_tiles))
    throw std::runtime_error(
  ...
  const std::vector<uint16_t> host_norm1 = bf16_vector(st, p + "norm1.weight", c.hidden);
  const std::vector<uint16_t> host_norm2 = bf16_vector(st, p + "norm2.weight", c.hidden);
  const std::vector<uint16_t> host_q_norm = bf16_vector(st, p + "attn.q_norm.weight", c.head_dim);
  const std::vector<uint16_t> host_k_norm = bf16_vector(st, p + "attn.k_norm.weight", c.head_dim);
  const uint64_t adaln_out =
      checked_product(checked_product(c.modalities, 6, "AdaLN"), c.hidden, "AdaLN");
  const TensorView& aw = st.at(p + "adaln_proj.linear.weight");
  const TensorView& ab = st.at(p + "adaln_proj.linear.bias");
  ...
  const std::vector<float> wide_w =
      adapted_adaln ? c.loras->merged_adaln_weight(st, adaln) : to_f32(aw);
  const std::vector<float> wide_b =
      adapted_adaln ? c.loras->merged_adaln_bias(st, adaln) : to_f32(ab);
  const std::string qkv = p + "attn.qkv_proj";
  validate_projection_archive(st, qkv, inner, c.hidden, 3 * inner, 0);
  validate_projection_archive(st, qkv, inner, c.hidden, 3 * inner, inner);
  validate_projection_archive(st, qkv, inner, c.hidden, 3 * inner, 2 * inner);
  ...
  next->q = load_projection(*s.context, st, c.loras, c.sequence, qkv, inner, c.hidden, 3 * inner, 0,
                            c.head_dim);
  next->k = load_projection(*s.context, st, c.loras, c.sequence, qkv, inner, c.hidden, 3 * inner,
                            inner, c.head_dim);
  next->v = load_projection(*s.context, st, c.loras, c.sequence, qkv, inner, c.hidden, 3 * inner,
                            2 * inner, c.head_dim);
  next->out =
      load_projection(*s.context, st, c.loras, c.sequence, p + "attn.out_proj", c.hidden, inner);
  next->fc1 =
      load_projection(*s.context, st, c.loras, c.sequence, p + "mlp.fc1", 2 * c.ffn, c.hidden);
...
void ExactH3BlockStage::load_refiner(const SafeTensors& st, uint32_t layer) {
  ...
  const uint64_t adaln_out =
      checked_product(checked_product(c.modalities, 6, "refiner identity AdaLN"), c.hidden,
                      "refiner identity AdaLN");
  std::vector<float> identity_w(checked_product(adaln_out, c.adaln_rank, "refiner identity weight"),
                                0.0f);
  ...
  next->adaln_w = s.context->allocate(matrix(adaln_out, c.adaln_rank));
  next->adaln_b = s.context->allocate(vector(adaln_out));
  s.context->upload_transient(next->adaln_w, identity_w.data(), identity_w.size());
  s.context->upload_transient(next->adaln_b, identity_b.data(), identity_b.size());
  s.weights = std::move(next);
}

namespace {
void ensure_transforms(TensorContext& context, const Projection& p, uint32_t rows, DeviceTensor& a,
                       DeviceTensor& b) {
...
bool transforms_ready(const Projection& p, const DeviceTensor& a, const DeviceTensor& b) {
  const bool pre_scale = p.weight.has_pre_quant_scale();
  const bool convrot = p.weight.applies_convrot();
...
DeviceTensor& transformed_input(TensorBatch& batch, Projection& p, DeviceTensor& input,
                                DeviceTensor& a, DeviceTensor& b) {
  DeviceTensor* selected = &input;
  if (p.weight.has_pre_quant_scale()) {
    p.weight.apply_pre_quant_scale(batch, *selected, a);
    selected = &a;
  }
...
void projection(TensorBatch& batch, Projection& p, const DenseGemmPlan& plan,
                StreamedNVFP4WeightCache& cache, DeviceTensor& input, DeviceTensor& output,
                DeviceTensor& transform_a, DeviceTensor& transform_b, uint32_t rows,
  ...
  auto record = [&](auto& weight) {
    ...
    if (tiled_rows != rows)
      plan.record(batch, source, weight, output, rows - tiled_rows, tiled_rows, tiled_rows);
  };
  if (p.streamed()) {
...
uint32_t projection_operators(const Projection& projection, uint32_t rows) {
  return (projection.weight.has_pre_quant_scale() ? 1u : 0u) +
         (projection.weight.applies_convrot() ? 1u : 0u) + (projection.streamed() ? 1u : 0u) +
         (rows >= 64 ? 1u : 0u) + (rows % 64 ? 1u : 0u) + projection.lora.operators(rows);
}

void validate_tap(DeviceTensor* tensor, ScalarType type, const TensorLayout& layout,
...
void ExactH3BlockStage::prepare(ExactH3BlockScratch& scratch) const {
  ...
  if (a.sequence != b.sequence || a.hidden != b.hidden || a.heads != b.heads ||
      a.head_dim != b.head_dim || a.ffn != b.ffn || a.timesteps != b.timesteps ||
      a.modalities != b.modalities || a.adaln_rank != b.adaln_rank ||
      a.attention_mode != b.attention_mode || a.vsa_tiles != b.vsa_tiles)
...
void ExactH3BlockStage::record(TensorBatch& batch, DeviceTensor& tokens, DeviceTensor& selectors,
                               DeviceTensor& code, DeviceTensor& cosine, DeviceTensor& sine,
                               ExactH3BlockScratch& scratch, const H3AttentionRanges* ranges,
                               const H3BlockReplayTaps* taps) const {
  if (!impl_ || !impl_->weights)
    throw std::logic_error("Vulkan H3 block: not loaded");
  if (!scratch.impl_ || scratch.impl_->context != impl_->context)
    throw std::invalid_argument("Vulkan H3 block: incompatible scratch");
  const H3BlockConfig& c = impl_->config;
  auto& s = *scratch.impl_;
  if (s.config.sequence != c.sequence || s.config.hidden != c.hidden || s.config.heads != c.heads ||
      s.config.head_dim != c.head_dim || s.config.ffn != c.ffn ||
      s.config.timesteps != c.timesteps || s.config.modalities != c.modalities ||
      s.config.adaln_rank != c.adaln_rank || s.config.attention_mode != c.attention_mode ||
      s.config.vsa_tiles != c.vsa_tiles)
    throw std::invalid_argument("Vulkan H3 block: scratch configuration mismatch");
  ...
  if (tv.type != ScalarType::kBFloat16 || tv.layout.rank != 2 ||
      ...
      av.type != ScalarType::kInt32 || av.layout.rank != 1 || av.layout.extent[0] != c.sequence ||
      cv.type != ScalarType::kFloat32 || cv.layout.rank != 2 ||
      cv.layout.extent[0] != c.timesteps || cv.layout.extent[1] != c.adaln_rank ||
      !tv.layout.is_contiguous() || !av.layout.is_contiguous() || !cv.layout.is_contiguous() ||
      cosv.type != ScalarType::kFloat32 || sinv.type != ScalarType::kFloat32 ||
      cosv.layout.rank != 2 || sinv.layout.rank != 2 || cosv.layout.extent[0] != c.sequence ||
      sinv.layout.extent[0] != c.sequence || cosv.layout.extent[1] != 96 ||
      sinv.layout.extent[1] != 96 || !cosv.layout.is_contiguous() || !sinv.layout.is_contiguous() ||
      cosv.resource == sinv.resource || tv.resource == cosv.resource ||
      tv.resource == sinv.resource || av.resource == tv.resource || cv.resource == tv.resource ||
  ...
  if (taps) {
    validate_tap(taps->q, ScalarType::kBFloat16, three(c.sequence, c.heads, c.head_dim), "Q",
                 tap_views);
    validate_tap(taps->k, ScalarType::kBFloat16, three(c.sequence, c.heads, c.head_dim), "K",
                 tap_views);
    validate_tap(taps->v, ScalarType::kBFloat16, three(c.sequence, c.heads, c.head_dim), "V",
                 tap_views);
    validate_tap(taps->attention, ScalarType::kBFloat16, matrix(c.sequence, c.heads * c.head_dim),
                 "attention", tap_views);
    validate_tap(taps->attention_residual, ScalarType::kBFloat16, matrix(c.sequence, c.hidden),
                 "attention residual", tap_views);
    validate_tap(taps->final_residual, ScalarType::kBFloat16, matrix(c.sequence, c.hidden),
                 "final residual", tap_views);
  }
  ...
  batch.dit_expand_adaln(w.adaln_w, w.adaln_b, code, s.modulation, c.modalities, 6, c.hidden);
  batch.rms_norm_modulate_bf16_table(tokens, w.norm1, s.modulation, s.modulation_rows, 1, 0,
                                     selectors, s.normed, c.epsilon);
  projection(batch, w.q, s.q_plan, s.cache, s.normed, s.q, s.hidden_a, s.hidden_b, c.sequence,
             *s.context, s.lora);
  projection(batch, w.k, s.k_plan, s.cache, s.normed, s.k, s.hidden_a, s.hidden_b, c.sequence,
             *s.context, s.lora);
  projection(batch, w.v, s.v_plan, s.cache, s.normed, s.v, s.hidden_a, s.hidden_b, c.sequence,
             *s.context, s.lora);
  ...
  if (taps) {
    if (taps->q)
      batch.copy(s.q, *taps->q);
    if (taps->k)
      batch.copy(s.k, *taps->k);
    if (taps->v)
      batch.copy(s.v, *taps->v);
  }
  if (c.vsa_tiles) {
    s.vsa_plan.record(batch, s.q, s.k, s.v, s.attention);
    projection(batch, w.compress_gate, s.q_plan, s.cache, s.normed, s.compress_gate, s.hidden_a,
               s.hidden_b, c.sequence, *s.context, s.lora);
  ...
  projection(batch, w.out, s.out_plan, s.cache, s.attention, s.branch, s.inner_a, s.inner_b,
             c.sequence, *s.context, s.lora);
  batch.dit_add_gated_bf16_table(tokens, s.branch, s.modulation, s.modulation_rows, 2, selectors);
  if (taps && taps->attention_residual)
    batch.copy(tokens, *taps->attention_residual);
  batch.rms_norm_modulate_bf16_table(tokens, w.norm2, s.modulation, s.modulation_rows, 4, 3,
                                     selectors, s.normed, c.epsilon);
  projection(batch, w.fc1, s.fc1_plan, s.cache, s.normed, s.fused, s.hidden_a, s.hidden_b,
             c.sequence, *s.context, s.lora);
  batch.dit_swiglu_bf16(s.fused, s.activation);
  projection(batch, w.fc2, s.fc2_plan, s.cache, s.activation, s.branch, s.ffn_a, s.ffn_b,
             c.sequence, *s.context, s.lora);
  batch.dit_add_gated_bf16_table(tokens, s.branch, s.modulation, s.modulation_rows, 5, selectors);
  if (taps && taps->final_residual)
    batch.copy(tokens, *taps->final_residual);
}

uint32_t ExactH3BlockStage::required_operators(const H3BlockReplayTaps* taps) const {
  if (!impl_ || !impl_->weights)
    throw std::logic_error("Vulkan H3 block: not loaded");
  const auto& w = *impl_->weights;
  ...
  if (taps) {
    count += taps->q != nullptr;
    count += taps->k != nullptr;
    count += taps->v != nullptr;
...
void ExactH3BlockStage::unload() noexcept {
  if (impl_)
    impl_->weights.reset();
}

bool ExactH3BlockStage::loaded() const noexcept {
  return impl_ && impl_->weights;
}

const H3BlockConfig& ExactH3BlockStage::config() const noexcept {
  return impl_->config;
}
...
uint64_t ExactH3BlockStage::peak_device_bytes(const ExactH3BlockScratch& scratch) const noexcept {
