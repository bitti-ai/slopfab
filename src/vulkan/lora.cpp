...
void LoraProjection::load(TensorContext& context, const std::vector<LoraFactors>& host,
  ...
  for (const auto& h : host) {
    ...
    f.down =
        DenseGemmPlan::create(context, {rows, f.rank, static_cast<uint32_t>(h.in),
                                        DenseGemmMode::kBFloat16, DenseGemmBias::kNone, false});
    f.up = DenseGemmPlan::create(
        context, {rows, out, f.rank, DenseGemmMode::kBFloat16, DenseGemmBias::kNone, false});
...
void LoraProjection::record(TensorBatch& batch, DeviceTensor& input, DeviceTensor& output,
  ...
  for (auto& f : factors_) {
    ...
    auto gemm = [&](DenseGemmPlan& plan, DeviceTensor& x, DeviceTensor& w, DeviceTensor& y) {
      ...
      if (tiled)
        plan.record(batch, x, w, y, tiled);
      if (tiled != rows)
        plan.record(batch, x, w, y, rows - tiled, tiled, tiled);
    };
