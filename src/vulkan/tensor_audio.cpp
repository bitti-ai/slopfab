...
void TensorBatch::audio_conv1d(DeviceTensor& input, DeviceTensor& weight, DeviceTensor* bias,
  ...
  const bool valid_bias =
      bias == nullptr || (bv->type == ScalarType::kFloat32 && bv->layout.rank == 1 &&
                          bv->layout.extent[0] == desc.out_channels && bv->layout.is_contiguous());
  const bool valid_input =
      (src->layout.rank == 3 && src->layout.extent[0] == desc.batch &&
       src->layout.extent[1] == desc.in_channels && src->layout.extent[2] == desc.length_in) ||
      (src->layout.rank == 1 && src->layout.elements() >= desc.input_elements());
  const bool valid_output =
      (dst->layout.rank == 3 && dst->layout.extent[0] == desc.batch &&
       dst->layout.extent[1] == desc.out_channels && dst->layout.extent[2] == desc.length_out) ||
      (dst->layout.rank == 1 && dst->layout.elements() >= desc.output_elements());
  if (src.get() == w.get() || src.get() == dst.get() || w.get() == dst.get() ||
      (bias != nullptr &&
       (bv.get() == src.get() || bv.get() == w.get() || bv.get() == dst.get())) ||
      src->type != ScalarType::kFloat32 || w->type != ScalarType::kFloat32 ||
      dst->type != ScalarType::kFloat32 || !valid_bias || !valid_input || w->layout.rank != 3 ||
      w->layout.extent[0] != desc.out_channels || w->layout.extent[1] != desc.in_channels ||
      w->layout.extent[2] != desc.kernel || !valid_output || !src->layout.is_contiguous() ||
  ...
  p.op = 0;
  p.batch = desc.batch;
  p.in_channels = desc.in_channels;
  p.out_channels = desc.out_channels;
  p.length_in = desc.length_in;
  p.length_out = desc.length_out;
  p.kernel = desc.kernel;
...
void TensorBatch::audio_conv_transpose1d(DeviceTensor& input, DeviceTensor& weight,
  ...
  if (src.get() == w.get() || src.get() == dst.get() || w.get() == dst.get() ||
      ...
      src->type != ScalarType::kFloat32 || w->type != ScalarType::kFloat32 ||
      dst->type != ScalarType::kFloat32 || !valid_bias || !valid_input || w->layout.rank != 3 ||
      w->layout.extent[0] != desc.in_channels || w->layout.extent[1] != desc.out_channels ||
      w->layout.extent[2] != desc.kernel || !valid_output || !src->layout.is_contiguous() ||
      !w->layout.is_contiguous() || !dst->layout.is_contiguous() ||
      desc.input_elements() > std::numeric_limits<uint32_t>::max() ||
      desc.output_elements() > std::numeric_limits<uint32_t>::max() ||
...
void TensorBatch::audio_add_inplace(DeviceTensor& input_output, DeviceTensor& branch,
                                    uint64_t live_count) {
  if (!impl_ || impl_->poisoned)
    throw std::logic_error("vulkan tensor: invalid batch");
  if (!impl_->owner->exact_audio)
  ...
  if (x.get() == y.get() || x->type != ScalarType::kFloat32 || y->type != ScalarType::kFloat32 ||
      (live_count == 0 &&
       (x->layout.extent != y->layout.extent || x->layout.rank != y->layout.rank)) ||
      capacity < count || y->layout.elements() < count || !x->layout.is_contiguous() ||
      !y->layout.is_contiguous() || count == 0 || count > std::numeric_limits<uint32_t>::max()) {
...
void TensorBatch::audio_copy_prefix(DeviceTensor& source, DeviceTensor& destination,
                                    uint64_t count) {
  if (!impl_ || impl_->poisoned)
    throw std::logic_error("vulkan tensor: invalid batch");
  if (!impl_->owner->exact_audio)
  ...
  const uint64_t bytes = checked_multiply(count, sizeof(float), "audio copy prefix");
  if (src.get() == dst.get() || count == 0 || src->type != ScalarType::kFloat32 ||
      dst->type != ScalarType::kFloat32 || !src->layout.is_contiguous() ||
      !dst->layout.is_contiguous() || src->logical_bytes < bytes || dst->logical_bytes < bytes) {
...
void TensorBatch::audio_scale_inplace(DeviceTensor& input_output, float scale,
  ...
  const uint64_t capacity = x->layout.elements();
  const uint64_t count = live_count == 0 ? capacity : live_count;
  if (x->type != ScalarType::kFloat32 || !x->layout.is_contiguous() || count == 0 ||
      capacity < count || count > std::numeric_limits<uint32_t>::max()) {
...
void TensorBatch::audio_clamp_inplace(DeviceTensor& input_output, float lower, float upper,
                                      uint64_t live_count) {
  if (!impl_ || impl_->poisoned)
    throw std::logic_error("vulkan tensor: invalid batch");
  if (!impl_->owner->exact_audio)
  ...
  if (!std::isfinite(lower) || !std::isfinite(upper) || lower > upper ||
      x->type != ScalarType::kFloat32 || !x->layout.is_contiguous() || count == 0 ||
      capacity < count || count > std::numeric_limits<uint32_t>::max()) {
...
void TensorBatch::audio_interleave(DeviceTensor& planar, DeviceTensor& interleaved, uint32_t batch,
                                   uint32_t frames) {
  if (!impl_ || impl_->poisoned)
    throw std::logic_error("vulkan tensor: invalid batch");
  if (!impl_->owner->exact_audio)
  ...
  const bool valid_source = (src->layout.rank == 3 && src->layout.extent[0] == batch &&
                             src->layout.extent[1] == 1 && src->layout.extent[2] == frames) ||
                            (src->layout.rank == 1 && src->layout.elements() >= count);
  const bool valid_destination = (dst->layout.rank == 2 && dst->layout.extent[0] == frames &&
                                  dst->layout.extent[1] == batch) ||
                                 (dst->layout.rank == 1 && dst->layout.elements() >= count);
  if (batch == 0 || frames == 0 || src.get() == dst.get() || src->type != ScalarType::kFloat32 ||
      dst->type != ScalarType::kFloat32 || !valid_source || !valid_destination ||
...
void TensorBatch::audio_snake_beta_inplace(DeviceTensor& input_output, DeviceTensor& log_alpha,
  ...
  const uint64_t count =
      checked_multiply(checked_multiply(batch, channels, "audio Snake"), length, "audio Snake");
  const bool valid_x = (x->layout.rank == 3 && x->layout.extent[0] == batch &&
  ...
  if (batch == 0 || channels == 0 || length == 0 || x.get() == alpha.get() ||
      x.get() == beta.get() || alpha.get() == beta.get() || x->type != ScalarType::kFloat32 ||
      alpha->type != ScalarType::kFloat32 || beta->type != ScalarType::kFloat32 || !valid_x ||
      alpha->layout.rank != 1 || beta->layout.rank != 1 || alpha->layout.extent[0] != channels ||
...
void TensorBatch::audio_aa_upsample_snake(DeviceTensor& input, DeviceTensor& filter,
                                          DeviceTensor& log_alpha, DeviceTensor& log_beta,
                                          DeviceTensor& output, uint32_t batch, uint32_t channels,
                                          uint32_t length_in) {
  if (!impl_ || impl_->poisoned)
    throw std::logic_error("vulkan tensor: invalid batch");
  if (!impl_->owner->exact_audio)
  ...
  auto beta = impl_->owner->require(log_beta);
  auto dst = impl_->owner->require(output);
  const uint64_t count = checked_multiply(checked_multiply(batch, channels, "audio AA upsample"),
                                          length_out, "audio AA upsample");
  const uint64_t input_count =
      checked_multiply(checked_multiply(batch, channels, "audio AA upsample input"), length_in,
  ...
  if (batch == 0 || channels == 0 || length_in == 0 || src.get() == f.get() ||
      ...
      f->layout.extent[0] != 12 || alpha->layout.rank != 1 || beta->layout.rank != 1 ||
      alpha->layout.extent[0] != channels || beta->layout.extent[0] != channels ||
      !valid_destination || !src->layout.is_contiguous() || !f->layout.is_contiguous() ||
      !alpha->layout.is_contiguous() || !beta->layout.is_contiguous() ||
      !dst->layout.is_contiguous() || count > std::numeric_limits<uint32_t>::max()) {
  ...
  p.batch = batch;
  p.out_channels = channels;
  p.length_in = length_in;
  p.length_out = length_out;
  p.count = static_cast<uint32_t>(count);
  try {
    impl_->count_operator();
    impl_->transition(src, BufferAccess::kComputeRead);
...
void TensorBatch::audio_aa_downsample(DeviceTensor& input, DeviceTensor& filter,
                                      DeviceTensor& output, uint32_t batch, uint32_t channels,
  ...
  auto f = impl_->owner->require(filter);
  auto dst = impl_->owner->require(output);
  const uint64_t count = checked_multiply(checked_multiply(batch, channels, "audio AA downsample"),
                                          length_out, "audio AA downsample");
  const uint64_t input_count =
      checked_multiply(checked_multiply(batch, channels, "audio AA downsample input"), length_in,
  ...
  const bool valid_destination =
      (dst->layout.rank == 3 && dst->layout.extent[0] == batch &&
       dst->layout.extent[1] == channels && dst->layout.extent[2] == length_out) ||
      (dst->layout.rank == 1 && dst->layout.elements() >= count);
  if (batch == 0 || channels == 0 || length_in == 0 || length_out != (length_in - 1u) / 2u + 1u ||
      src.get() == f.get() || src.get() == dst.get() || f.get() == dst.get() ||
      src->type != ScalarType::kFloat32 || f->type != ScalarType::kFloat32 ||
      dst->type != ScalarType::kFloat32 || !valid_source || f->layout.rank != 1 ||
      f->layout.extent[0] != 12 || !valid_destination || !src->layout.is_contiguous() ||
