...
void TensorBatch::dit_add_gated_bf16(DeviceTensor& residual, DeviceTensor& branch,
  ...
  if (x.get() == b.get() || x.get() == g.get() || x.get() == a.get() || b.get() == g.get() ||
      b.get() == a.get() || g.get() == a.get() || shape.rank != 2 || b->layout.rank != 2 ||
      b->layout.extent != shape.extent || g->layout.rank != 2 || g->layout.extent[1] != dim ||
      a->layout.rank != 1 || a->layout.extent[0] != rows || x->type != ScalarType::kBFloat16 ||
      b->type != ScalarType::kBFloat16 || g->type != ScalarType::kFloat32 ||
      a->type != ScalarType::kInt32 || !shape.is_contiguous() || !b->layout.is_contiguous() ||
      !g->layout.is_contiguous() || !a->layout.is_contiguous() || rows == 0 || dim == 0 ||
      mod_rows == 0 || rows > std::numeric_limits<uint32_t>::max() ||
...
void TensorBatch::dit_add_gated_bf16_table(DeviceTensor& residual, DeviceTensor& branch,
  ...
  if (gate_table >= table_count || x.get() == b.get() || x.get() == t.get() || x.get() == a.get() ||
      ...
      t->layout.extent[0] % table_count != 0 || stride < logical || a->layout.rank != 1 ||
      a->layout.extent[0] != rows || x->type != ScalarType::kBFloat16 ||
      b->type != ScalarType::kBFloat16 || t->type != ScalarType::kFloat32 ||
      a->type != ScalarType::kInt32 || !shape.is_contiguous() || !b->layout.is_contiguous() ||
      !t->layout.is_contiguous() || !a->layout.is_contiguous() || rows == 0 || dim == 0 ||
      mod_rows == 0 || rows > UINT32_MAX || dim > UINT32_MAX || count > UINT32_MAX)
  ...
  try {
    ...
    impl_->transition(x, BufferAccess::kComputeReadWrite);
    impl_->transition(b, BufferAccess::kComputeRead);
    impl_->transition(t, BufferAccess::kComputeRead);
    impl_->transition(a, BufferAccess::kComputeRead);
    impl_->dispatch_dit(p, {x, b, t, a, x}, offset);
  } catch (...) {
    impl_->poisoned = true;
    throw;
  }
...
void TensorBatch::dit_swiglu_bf16(DeviceTensor& fused, DeviceTensor& output) {
  ...
  if (src.get() == dst.get() || src->layout.rank != 2 || (doubled & 1u) != 0 || rows == 0 ||
      inner == 0 || dst->layout.rank != 2 || dst->layout.extent[0] != rows ||
      dst->layout.extent[1] != inner || src->type != ScalarType::kBFloat16 ||
      dst->type != ScalarType::kBFloat16 || !src->layout.is_contiguous() ||
...
void TensorBatch::dit_expand_adaln(DeviceTensor& weight, DeviceTensor& bias, DeviceTensor& code,
  ...
  auto c = impl_->owner->require(code);
  auto dst = impl_->owner->require(output);
  const uint64_t timesteps = c->layout.rank == 2 ? c->layout.extent[0] : 0;
  const uint64_t rank = c->layout.rank == 2 ? c->layout.extent[1] : 0;
  const uint64_t features = checked_multiply(checked_multiply(num_modality, num_param, "DiT AdaLN"),
                                             channels, "DiT AdaLN");
  const uint64_t count = checked_multiply(timesteps, features, "DiT AdaLN");
  const uint64_t table_elements = checked_multiply(
      checked_multiply(timesteps, num_modality, "DiT AdaLN table"), channels, "DiT AdaLN table");
  const bool canonical_output = dst->layout.rank == 3 && dst->layout.extent[0] == num_param &&
  ...
  const uint64_t flat_stride =
      dst->layout.rank == 1 && num_param != 0 && dst->layout.extent[0] % num_param == 0
          ? dst->layout.extent[0] / num_param
          : 0;
  const bool flat_output =
      flat_stride >= table_elements &&
      (flat_stride * sizeof(float)) % impl_->owner->storage_binding_alignment == 0;
  if (w.get() == b.get() || w.get() == c.get() || w.get() == dst.get() || b.get() == c.get() ||
      b.get() == dst.get() || c.get() == dst.get() || num_modality == 0 || num_param == 0 ||
      channels == 0 || w->layout.rank != 2 || w->layout.extent[0] != features ||
      w->layout.extent[1] != rank || b->layout.rank != 1 || b->layout.extent[0] != features ||
      c->layout.rank != 2 || timesteps == 0 || rank == 0 ||
      ...
      !w->layout.is_contiguous() || !b->layout.is_contiguous() || !c->layout.is_contiguous() ||
      !dst->layout.is_contiguous() || timesteps > std::numeric_limits<uint32_t>::max() ||
      rank > std::numeric_limits<uint32_t>::max() || count > std::numeric_limits<uint32_t>::max()) {
    throw std::invalid_argument("vulkan DiT: invalid AdaLN tensors");
  }
  TensorContext::Impl::DitParameters p;
  p.op = 2;
  p.rows = static_cast<uint32_t>(timesteps);
  p.dim = channels;
  p.num_t = static_cast<uint32_t>(timesteps);
  p.num_modality = num_modality;
  p.num_param = num_param;
...
void TensorBatch::dit_euler_step_f32(DeviceTensor& sample, DeviceTensor& velocity,
  ...
  if (x.get() == v.get() || x->type != ScalarType::kFloat32 || v->type != ScalarType::kFloat32 ||
      v->layout.extent != layout.extent || v->layout.rank != layout.rank ||
      !layout.is_contiguous() || !v->layout.is_contiguous() || layout.elements() == 0 ||
      layout.elements() > UINT32_MAX || !std::isfinite(sigma_from_timestep) ||
      sigma_from_timestep < 0.0f || sigma_from_timestep > 1.0f || !std::isfinite(ratio) ||
