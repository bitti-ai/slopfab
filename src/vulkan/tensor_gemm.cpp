...
DenseGemmPlan::operator bool() const noexcept {
  return impl_ != nullptr;
}

DenseGemmPlan DenseGemmPlan::create(TensorContext& context, const DenseGemmPlanDesc& desc) {
  if (!context.impl_)
    throw std::logic_error("vulkan gemm: moved-from context");
  switch (desc.mode) {
  ...
  if (!detail::gemm_dispatch_geometry(desc.max_rows, desc.out_features, 16, 16,
  ...
  const uint64_t weight_elements =
      checked_multiply(desc.out_features, desc.in_features, "gemm weight");
  const uint64_t input_elements = checked_multiply(desc.max_rows, desc.in_features, "gemm input");
  const uint64_t output_elements =
      checked_multiply(desc.max_rows, desc.out_features, "gemm output");
  const uint64_t input_bytes = checked_multiply(
      input_elements, desc.mode == DenseGemmMode::kBFloat16 ? 2u : 4u, "gemm input bytes");
  const uint64_t weight_bytes = checked_multiply(
      weight_elements, desc.mode == DenseGemmMode::kFloat32 ? 4u : 2u, "gemm weight bytes");
  const uint64_t output_bytes = checked_multiply(
      output_elements, desc.mode == DenseGemmMode::kBFloat16 ? 2u : 4u, "gemm output bytes");
  ...
  const bool valid_bias =
      ...
      (desc.mode == DenseGemmMode::kBFloat16 &&
       (desc.bias == DenseGemmBias::kFloat32 || desc.bias == DenseGemmBias::kBFloat16)) ||
      (desc.mode == DenseGemmMode::kFloat32 && desc.bias == DenseGemmBias::kFloat32);
  if (!valid_bias) {
...
const DenseGemmPlanDesc& DenseGemmPlan::description() const {
  if (!impl_)
    throw std::logic_error("vulkan gemm: empty plan");
  return impl_->desc;
}

void DenseGemmPlan::record(TensorBatch& batch, DeviceTensor& input, DeviceTensor& prepared_weight,
                           DeviceTensor& output, uint32_t rows, uint32_t input_row_offset,
                           uint32_t output_row_offset, DeviceTensor* bias) const {
  record_impl(batch, input, prepared_weight, output, rows, input_row_offset, output_row_offset,
              bias, false);
}
...
void DenseGemmPlan::record(TensorBatch& batch, PreparedF16ActivationView& input,
  ...
  if (!impl_)
    throw std::logic_error("vulkan gemm: empty plan");
  if (!input.slot_)
    throw std::logic_error("vulkan gemm: empty prepared fp16 view");
  if (!batch.impl_ || input.batch_id_ != batch.impl_->batch_id) {
    throw std::invalid_argument("vulkan gemm: prepared fp16 view belongs to another batch");
  }
  ...
  if (impl_->desc.mode != DenseGemmMode::kFloat16Vae || slot->owner.get() != impl_->owner.get() ||
      slot->in_features != impl_->desc.in_features) {
    throw std::invalid_argument("vulkan gemm: prepared fp16 view does not match the plan");
  }
  record_impl(batch, slot->tensor, prepared_weight, output, input.rows_, 0, output_row_offset, bias,
              true);
}
...
void DenseGemmPlan::record(TensorBatch& batch, DeviceTensor& input,
  ...
  if (!batch.impl_ || prepared_weight.batch_id_ != batch.impl_->batch_id) {
    throw std::invalid_argument("vulkan gemm: streamed NVFP4 view belongs to another batch");
  }
  auto cache = std::static_pointer_cast<StreamedNVFP4WeightCache::Impl>(prepared_weight.cache_);
  if (prepared_weight.generation_ != cache->generation ||
      prepared_weight.batch_id_ != cache->batch_id) {
    throw std::invalid_argument("vulkan gemm: streamed NVFP4 view was superseded");
  }
  if (cache->owner.get() != impl_->owner.get() || impl_->desc.mode != DenseGemmMode::kBFloat16 ||
      prepared_weight.out_features_ != impl_->desc.out_features ||
      prepared_weight.in_features_ != impl_->desc.in_features) {
    throw std::invalid_argument("vulkan gemm: streamed NVFP4 view does not match the plan");
  }
  record_impl(batch, input, cache->dense, output, rows, input_row_offset, output_row_offset, bias,
              false);
}

void DenseGemmPlan::record_impl(TensorBatch& batch, DeviceTensor& input,
                                DeviceTensor& prepared_weight, DeviceTensor& output, uint32_t rows,
                                uint32_t input_row_offset, uint32_t output_row_offset,
                                DeviceTensor* bias, bool input_is_prepared_f16) const {
  if (!impl_)
    throw std::logic_error("vulkan gemm: empty plan");
  if (!batch.impl_)
  ...
  if (desc.mode == DenseGemmMode::kFloat16Vae && !input_is_prepared_f16) {
    throw std::invalid_argument("vulkan gemm: fp16 VAE mode requires a prepared activation view");
  }
  ...
  if (bias != nullptr)
    bias_tensor = impl_->owner->require(*bias);
  if ((desc.bias == DenseGemmBias::kNone) != (bias == nullptr)) {
    throw std::invalid_argument("vulkan gemm: bias presence differs from plan");
  }
  const ScalarType input_type = input_is_prepared_f16                   ? ScalarType::kFloat16
                                : desc.mode == DenseGemmMode::kBFloat16 ? ScalarType::kBFloat16
                                                                        : ScalarType::kFloat32;
  const ScalarType weight_type = desc.mode == DenseGemmMode::kBFloat16     ? ScalarType::kBFloat16
                                 : desc.mode == DenseGemmMode::kFloat16Vae ? ScalarType::kFloat16
                                                                           : ScalarType::kFloat32;
  const ScalarType output_type =
      desc.mode == DenseGemmMode::kBFloat16 ? ScalarType::kBFloat16 : ScalarType::kFloat32;
  const ScalarType bias_type =
      desc.bias == DenseGemmBias::kBFloat16 ? ScalarType::kBFloat16 : ScalarType::kFloat32;
  const bool aliases =
      src.get() == weight.get() || src.get() == dst.get() || weight.get() == dst.get() ||
      (bias_tensor && (bias_tensor.get() == src.get() || bias_tensor.get() == weight.get() ||
                       bias_tensor.get() == dst.get()));
  const bool input_matrix = src->layout.rank == 2;
  const bool input_heads =
      src->layout.rank == 3 &&
      src->layout.extent[1] <= std::numeric_limits<uint64_t>::max() / src->layout.extent[2] &&
      src->layout.extent[1] * src->layout.extent[2] == desc.in_features;
  const bool output_matrix = dst->layout.rank == 2;
  const bool output_heads =
      dst->layout.rank == 3 &&
      dst->layout.extent[1] <= std::numeric_limits<uint64_t>::max() / dst->layout.extent[2] &&
      dst->layout.extent[1] * dst->layout.extent[2] == desc.out_features;
  const bool row_ranges_valid = (input_matrix || input_heads) && (output_matrix || output_heads) &&
  ...
  const bool bias_valid =
      !bias_tensor ||
      (bias_tensor->layout.rank == 1 && bias_tensor->layout.extent[0] == desc.out_features &&
       bias_tensor->layout.is_contiguous() && bias_tensor->type == bias_type);
  if (aliases || !row_ranges_valid || !bias_valid ||
      (input_matrix && src->layout.extent[1] != desc.in_features) || weight->layout.rank != 2 ||
      ...
      !dst->layout.is_contiguous() || src->type != input_type || weight->type != weight_type ||
      dst->type != output_type) {
    throw std::invalid_argument("vulkan gemm: tensors do not match the plan");
  }
  detail::GemmDispatchGeometry geometry;
  if (!detail::gemm_dispatch_geometry(rows, desc.out_features, 16, 16, impl_->owner->max_dispatch_x,
  ...
  if (desc.mode == DenseGemmMode::kBFloat16) {
    parameters.mode = desc.bias == DenseGemmBias::kNone      ? 0u
                      : desc.bias == DenseGemmBias::kFloat32 ? 1u
                                                             : 2u;
  ...
  const bool cooperative_shape =
      (rows % 64u) == 0u && (desc.out_features % 16u) == 0u && (desc.in_features % 16u) == 0u;
  const bool use_cooperative =
      !desc.force_scalar_order && cooperative_shape &&
      ((desc.mode == DenseGemmMode::kBFloat16 && impl_->owner->cooperative_gemm) ||
       (desc.mode == DenseGemmMode::kFloat16Vae && impl_->owner->cooperative_f16_gemm));
  if (use_cooperative) {
    if (!detail::gemm_dispatch_geometry(rows, desc.out_features, 64, 16,
                                        impl_->owner->max_dispatch_x, impl_->owner->max_dispatch_y,
                                        &geometry)) {
  ...
  try {
    ...
    if (bias_tensor)
      batch.impl_->transition(bias_tensor, BufferAccess::kComputeRead);
    ...
    batch.impl_->commands.bind_compute(use_cooperative ? (desc.mode == DenseGemmMode::kFloat16Vae
