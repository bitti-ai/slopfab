...
struct TensorBatch::Impl {
  ...
  void dispatch_vae_pointwise(VaePointwiseOperation operation,
    ...
    switch (operation) {
    case VaePointwiseOperation::kResidual:
      pipeline = &owner->vae_residual_pipeline;
      break;
    case VaePointwiseOperation::kSwiglu:
      pipeline = &owner->vae_swiglu_pipeline;
      break;
    case VaePointwiseOperation::kDenorm:
      pipeline = &owner->vae_denorm_pipeline;
      break;
    }
    if (pipeline == nullptr)
      throw std::logic_error("vulkan tensor: invalid VAE pointwise op");
    commands.bind_compute(*pipeline, owner->vae_pointwise_bindings);
    commands.push_constants(&parameters, sizeof(parameters));
    commands.dispatch(groups);
  }

  void dispatch_rope(const TensorContext::Impl::Parameters& parameters, uint32_t groups,
  ...
  void dispatch_shared_rms(ComputePipeline& pipeline,
                           const TensorContext::Impl::NormParameters& parameters, uint32_t groups,
                           const std::shared_ptr<DeviceTensor::Impl>& input,
                           const std::shared_ptr<DeviceTensor::Impl>& weight,
                           const std::shared_ptr<DeviceTensor::Impl>& output) {
    owner->ops_bindings[0].buffer = &input->buffer;
  ...
  void dispatch_shared_mod(ComputePipeline& pipeline,
                           ...
                           uint64_t scale_offset = 0, uint64_t shift_offset = 0) {
    for (size_t i = 0; i < resources.size(); ++i) {
  ...
  void dispatch_vae_rope(const TensorContext::Impl::VaeRopeParameters& parameters, uint32_t groups,
  ...
  void dispatch_audio(TensorContext::Impl::AudioParameters parameters,
                      const std::array<std::shared_ptr<DeviceTensor::Impl>, 5>& resources) {
    const uint64_t total_groups = (static_cast<uint64_t>(parameters.count) + 63ull) / 64ull;
    const uint32_t groups_x =
        static_cast<uint32_t>(std::min<uint64_t>(total_groups, owner->max_dispatch_x));
    if (groups_x == 0) {
      throw std::out_of_range("vulkan audio: device exposes no X dispatch capacity");
    }
    const uint64_t groups_y_wide = (total_groups + groups_x - 1ull) / groups_x;
    if (groups_y_wide > owner->max_dispatch_y) {
      throw std::out_of_range("vulkan audio: primitive exceeds two-dimensional dispatch limits");
    }
  ...
  void dispatch_keyframe(TensorContext::Impl::KeyframeParameters parameters,
                         const std::array<std::shared_ptr<DeviceTensor::Impl>, 4>& resources) {
    const uint64_t total_groups = (static_cast<uint64_t>(parameters.count) + 63ull) / 64ull;
    const uint32_t groups_x =
        static_cast<uint32_t>(std::min<uint64_t>(total_groups, owner->max_dispatch_x));
    if (groups_x == 0) {
      throw std::out_of_range("vulkan keyframe: device exposes no X dispatch capacity");
    }
    const uint64_t groups_y_wide = (total_groups + groups_x - 1ull) / groups_x;
    if (groups_y_wide > owner->max_dispatch_y) {
  ...
  void dispatch_dit(TensorContext::Impl::DitParameters parameters,
                    const std::array<std::shared_ptr<DeviceTensor::Impl>, 5>& resources,
  ...
  void dispatch_weight(const TensorContext::Impl::WeightParameters& parameters, uint32_t groups,
                       const std::array<std::shared_ptr<DeviceTensor::Impl>, 6>& resources) {
    for (size_t i = 0; i < resources.size(); ++i) {
  ...
  void dispatch_attention(const TensorContext::Impl::AttentionParameters& parameters,
                          const std::array<std::shared_ptr<DeviceTensor::Impl>, 4>& resources) {
    for (size_t i = 0; i < resources.size(); ++i) {
      owner->attention_bindings[i].buffer = &resources[i]->buffer;
      owner->attention_bindings[i].bytes = resources[i]->buffer.size();
    }
    commands.bind_compute(owner->attention_blocked_pipeline, owner->attention_bindings);
    commands.push_constants(&parameters, sizeof(parameters));
    commands.dispatch(parameters.rows, parameters.heads);
  }

  void dispatch_h3_attention(const TensorContext::Impl::AttentionParameters& parameters,
                             const std::array<std::shared_ptr<DeviceTensor::Impl>, 4>& resources,
                             AttentionMode mode) {
    for (size_t i = 0; i < resources.size(); ++i) {
  ...
  void
  dispatch_h3_banded_attention(const TensorContext::Impl::AttentionParameters& parameters,
                               const std::array<std::shared_ptr<DeviceTensor::Impl>, 5>& resources,
                               AttentionMode mode) {
    for (size_t i = 0; i < resources.size(); ++i) {
  ...
  void
  dispatch_attention_prepare(uint32_t words,
                             const std::array<std::shared_ptr<DeviceTensor::Impl>, 6>& resources) {
    for (size_t i = 0; i < resources.size(); ++i) {
    ...
    commands.bind_compute(owner->attention_prepare_pipeline, owner->attention_prepare_bindings);
  ...
  void record_shared_mod(bool fp32, DeviceTensor& input, DeviceTensor& weight, DeviceTensor& scale,
    ...
    if (!std::isnormal(epsilon) || epsilon <= 0.0f || aliases_parameters || shape.rank != 2 ||
        w->layout.rank != 1 || sc->layout.rank != 2 || sh->layout.rank != 2 ||
        index->layout.rank != 1 || dst->layout.rank != 2 || w->layout.extent[0] != dim ||
        sc->layout.extent[1] != dim || sh->layout.extent != sc->layout.extent ||
        index->layout.extent[0] != rows || dst->layout.extent != shape.extent ||
        w->type != ScalarType::kBFloat16 || sc->type != ScalarType::kFloat32 ||
        sh->type != ScalarType::kFloat32 || index->type != ScalarType::kInt32 ||
        src->type != (fp32 ? ScalarType::kFloat32 : ScalarType::kBFloat16) ||
        dst->type != src->type || !shape.is_contiguous() || !w->layout.is_contiguous() ||
        !sc->layout.is_contiguous() || !sh->layout.is_contiguous() ||
        !index->layout.is_contiguous() || !dst->layout.is_contiguous() ||
    ...
    try {
      ...
      if (src.get() != dst.get())
        transition(dst, BufferAccess::kComputeWrite);
      std::array<std::shared_ptr<DeviceTensor::Impl>, 6> resources{src, w, sc, sh, index, dst};
      dispatch_shared_mod(fp32 ? owner->fp32_mod_pipeline : owner->bf16_mod_pipeline, p, resources);
    } catch (...) {
      poisoned = true;
      throw;
    }
  ...
  void record_shared_mod_table(DeviceTensor& input, DeviceTensor& weight, DeviceTensor& tables,
    ...
    if (!std::isnormal(epsilon) || epsilon <= 0.0f || scale_table >= table_count ||
        shift_table >= table_count || table->layout.extent[0] % table_count != 0 ||
        stride < logical || src.get() == w.get() || src.get() == table.get() ||
        ...
        table->layout.rank != 1 || index->layout.rank != 1 || index->layout.extent[0] != rows ||
        ...
        !shape.is_contiguous() || !w->layout.is_contiguous() || !table->layout.is_contiguous() ||
        !index->layout.is_contiguous() || !dst->layout.is_contiguous() || rows > UINT32_MAX ||
        dim > kMaxExactNormDimension || mod_rows == 0 || shape.elements() > UINT32_MAX)
    ...
    try {
      ...
      transition(src, src.get() == dst.get() ? BufferAccess::kComputeReadWrite
                                             : BufferAccess::kComputeRead);
      transition(w, BufferAccess::kComputeRead);
      transition(table, BufferAccess::kComputeRead);
      transition(index, BufferAccess::kComputeRead);
      if (src.get() != dst.get())
        transition(dst, BufferAccess::kComputeWrite);
      std::array<std::shared_ptr<DeviceTensor::Impl>, 6> r{src, w, table, table, index, dst};
      dispatch_shared_mod(owner->bf16_mod_pipeline, p, r, scale_table * table_bytes,
                          shift_table * table_bytes);
    } catch (...) {
      poisoned = true;
      throw;
    }
