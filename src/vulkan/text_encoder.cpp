...
text::EncoderConfig resolved_config(const SafeTensors& checkpoint,
  ...
  if (result.format != text::WeightFormat::kAuto && result.format != detected) {
    throw std::runtime_error("Vulkan Qwen encoder: requested format differs from checkpoint");
  }
  ...
  if (!text::supports_exact_text_layer({result.max_prompt_tokens, result.hidden_size,
                                        ...
                                        result.head_dim, result.intermediate_size,
                                        result.rms_norm_eps}) ||
      result.num_layers != 50 || result.intermediate_size != 25600 || result.rope_theta != 5.0e6f ||
      result.max_prompt_tokens <= 0 || result.max_prompt_tokens > text::kMaxPromptTokens) {
    throw std::invalid_argument("Vulkan Qwen encoder: invalid production configuration");
  }
  return result;
}

uint32_t layer_operators(text::WeightFormat format, uint32_t rows) {
  const uint32_t row_gemm = (rows >= 64 ? 1u : 0u) + (rows % 64 != 0 ? 1u : 0u);
...
struct ExactQwenTextEncoder::Impl {
  struct ShapeState {
    ...
    ShapeState(TensorContext& context, const text::EncoderConfig& config, uint32_t sequence) {
      ...
      tokens = context.allocate(matrix(sequence, config.hidden_size), ScalarType::kBFloat16);
      cosine = context.allocate(matrix(sequence, config.head_dim), ScalarType::kFloat32);
      sine = context.allocate(matrix(sequence, config.head_dim), ScalarType::kFloat32);
      output = context.allocate(matrix(sequence, config.hidden_size), ScalarType::kFloat32);
    }

    uint64_t activation_bytes() const noexcept {
      return tokens.layout().bytes(tokens.type()) + cosine.layout().bytes(cosine.type()) +
             sine.layout().bytes(sine.type()) + output.layout().bytes(output.type());
    }
  };
  ...
  uint64_t external_used_bytes() const noexcept {
    ...
    const uint64_t owned = shape ? shape->scratch.reserved_bytes() + shape->activation_bytes() : 0;
  ...
  ShapeState& ensure_shape(uint32_t sequence) {
    if (shape && shape->layer_config.sequence == sequence &&
        shape->layer_config.encoder.format == config.format)
      return *shape;
...
ExactQwenTextEncoder::ExactQwenTextEncoder(std::unique_ptr<Impl> impl) : impl_(std::move(impl)) {
}

ExactQwenTextEncoder::ExactQwenTextEncoder(ExactQwenTextEncoder&&) noexcept = default;
...
void ExactQwenTextEncoder::load(const SafeTensors& checkpoint,
                                const text::EncoderConfig& requested) {
  if (!impl_)
    throw std::logic_error("Vulkan Qwen encoder: empty encoder");
  ...
  const TensorView* embedding_scale = checkpoint.find("model.embed_tokens.weight_scale");

  const bool shape_compatible = impl_->shape && impl_->config.format == next.format;
  ...
  impl_->stats.load_seconds = std::chrono::duration<double>(Clock::now() - begin).count();
}
...
const text::EncoderConfig& ExactQwenTextEncoder::config() const {
  if (!loaded())
    throw std::logic_error("Vulkan Qwen encoder: not loaded");
  return impl_->config;
}
...
text::PromptEmbedding ExactQwenTextEncoder::encode(const std::vector<int32_t>& token_ids,
  ...
  // an enabled trace adds its device-only boundary copy.
  {
    ...
    capacity.require_operator_capacity(layer_operators(impl_->config.format, sequence) + 1u +
                                       (trace != nullptr ? 1u : 0u));
  }
  ...
  if (trace != nullptr) {
    trace_device = impl_->context->allocate(
        matrix(uint64_t(sequence) * 50, impl_->config.hidden_size), ScalarType::kBFloat16);
  }
  std::vector<uint16_t> embedding;
  text::gather_embedding_rows(*impl_->embedding, impl_->embedding_scale, token_ids, embedding);
  ...
  try {
    for (uint32_t layer = 0; layer < 50; ++layer) {
      ...
      state.stage.record(batch, state.tokens, state.cosine, state.sine, state.scratch);
      if (trace != nullptr)
        batch.copy_rows(state.tokens, trace_device, 0, layer * sequence, sequence);
      if (layer == 49)
        batch.convert(state.tokens, state.output);
      batch.submit().wait();
    }
  } catch (...) {
    state.stage.unload();
  ...
  const uint64_t trace_bytes = trace_device ? trace_device.layout().bytes(trace_device.type()) : 0;
  if (trace != nullptr) {
    std::vector<uint16_t> boundaries(static_cast<size_t>(50) * sequence *
  ...
  const uint64_t observed_peak = peak_nonstaging >= allocator_baseline
                                     ? peak_nonstaging - allocator_baseline
                                     : peak_nonstaging;
  impl_->stats.peak_device_bytes =
      std::max(max_weight + state.scratch.reserved_bytes() + state.activation_bytes() + trace_bytes,
               observed_peak);
  ...
  impl_->stats.last_encode_seconds = std::chrono::duration<double>(Clock::now() - begin).count();
  return result;
}
...
text::PromptEmbedding ExactQwenTextEncoder::encode(const std::vector<int32_t>& token_ids,
  ...
  if (!loaded())
    throw std::logic_error("Vulkan Qwen encoder: not loaded");
  if (images.empty())
    return encode(token_ids, trace);
  if (token_ids.empty() || token_ids.size() > static_cast<size_t>(impl_->config.max_prompt_tokens))
  ...
  for (const auto& image : images) {
    ...
    if (image.grid.temporal <= 0 || image.grid.height <= 0 || image.grid.width <= 0 ||
        (image.grid.height & 1) != 0 || (image.grid.width & 1) != 0 || patches == 0 ||
        patches > 16384 || patches % 4 != 0 || image.rows.size() != patches * 1536)
  ...
  const text::QwenMultimodalPlan multimodal = text::qwen3vl_multimodal_plan(token_ids, grids);
  if (visual_tokens_wide == 0 || visual_tokens_wide != multimodal.image_rows.size() ||
  ...
  {
    TensorBatch capacity = impl_->context->begin_batch();
    capacity.require_operator_capacity(layer_operators(impl_->config.format, sequence) + 2u +
                                       (trace != nullptr ? 1u : 0u));
  }
  Impl::ShapeState& state = impl_->ensure_shape(sequence);
  ...
  for (auto& tensor : visual_deep)
    tensor = impl_->context->allocate(matrix(visual_tokens, impl_->config.hidden_size),
                                      ScalarType::kBFloat16);
  const uint64_t image_row_count = visual_tokens;
  DeviceTensor image_rows =
      impl_->context->allocate(TensorLayout::contiguous(&image_row_count, 1), ScalarType::kInt32);
  ...
  try {
    ...
    for (const auto& image : images) {
      ...
      vision_peak_used = std::max(vision_peak_used, vision_stats.allocator_peak_used_bytes);
      vision_peak_nonstaging =
          std::max(vision_peak_nonstaging, vision_stats.allocator_peak_nonstaging_bytes);
      vision_phase_bytes =
          std::max(vision_phase_bytes, vision_stats.scratch_bytes + vision_stats.activation_bytes +
      ...
      copy.copy_rows(impl_->vision.main_output(), visual_main, 0, visual_offset, count);
      for (uint32_t slot = 0; slot < 3; ++slot)
        copy.copy_rows(impl_->vision.deepstack_output(slot), visual_deep[slot], 0, visual_offset,
                       count);
      copy.submit().wait();
      visual_offset += count;
    }
  ...
  text::gather_embedding_rows(*impl_->embedding, impl_->embedding_scale, token_ids, embedding);
  std::vector<float> cosine, sine;
  text::qwen3vl_decoder_rope_tables(multimodal, sequence, cosine, sine, impl_->config.head_dim,
  ...
  if (trace != nullptr)
    trace_device = impl_->context->allocate(
        matrix(uint64_t(sequence) * 50, impl_->config.hidden_size), ScalarType::kBFloat16);
  uint64_t max_weight = 0;
  uint64_t peak_used = std::max(vision_peak_used, impl_->context->pooled_used_bytes());
  uint64_t peak_nonstaging =
      std::max(vision_peak_nonstaging, nonstaging_used_bytes(*impl_->context));
  try {
    for (uint32_t layer = 0; layer < 50; ++layer) {
      state.stage.unload();
      state.stage.load(*impl_->checkpoint, layer);
      ...
      state.stage.record(batch, state.tokens, state.cosine, state.sine, state.scratch);
      if (deep_slot >= 0)
        batch.vision_scatter_add_bf16(visual_deep[deep_slot], state.tokens, image_rows);
      if (trace != nullptr)
        batch.copy_rows(state.tokens, trace_device, 0, layer * sequence, sequence);
      if (layer == 49)
        batch.convert(state.tokens, state.output);
  ...
  const uint64_t visual_bytes = tensor_bytes(visual_main) + tensor_bytes(visual_deep[0]) +
                                tensor_bytes(visual_deep[1]) + tensor_bytes(visual_deep[2]) +
                                tensor_bytes(image_rows);
  impl_->stats.last_num_tokens = sequence;
  impl_->stats.max_layer_weight_bytes = max_weight;
  impl_->stats.scratch_bytes = state.scratch.reserved_bytes();
  impl_->stats.activation_bytes = state.activation_bytes() + visual_bytes;
  const uint64_t decoder_live_bytes = state.scratch.reserved_bytes() + state.activation_bytes();
  const uint64_t observed_peak = peak_nonstaging >= allocator_baseline
                                     ? peak_nonstaging - allocator_baseline
                                     : peak_nonstaging;
  impl_->stats.peak_device_bytes =
      std::max({max_weight + decoder_live_bytes + visual_bytes + trace_bytes,
                decoder_live_bytes + vision_phase_bytes + visual_bytes, observed_peak});
  impl_->stats.allocator_baseline_bytes = allocator_baseline;
  impl_->stats.allocator_peak_used_bytes = peak_used;
  impl_->stats.allocator_peak_nonstaging_bytes = peak_nonstaging;
  impl_->stats.allocator_used_bytes = impl_->context->pooled_used_bytes();
...
text::PromptEmbedding ExactQwenTextEncoder::encode(const text::Tokenizer& tokenizer,
                                                   const std::string& prompt) {
  if (prompt.empty())
    throw std::invalid_argument("Vulkan Qwen encoder: empty prompt");
