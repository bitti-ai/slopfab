...
TensorLayout layout(std::initializer_list<uint64_t> extents) {
  ...
  return TensorLayout::contiguous(shape.data(), static_cast<uint32_t>(shape.size()));
}
...
void validate_config(const vae::AudioVAEConfig& config) {
  if (config.latent_channels <= 0 || config.latent_dim <= 0 || config.decoder_dim <= 0 ||
      config.output_channels != kStereo || config.sample_rate <= 0 ||
      config.decoder_rates.size() != 7 ||
  ...
  for (size_t i = 0; i < config.decoder_rates.size(); ++i) {
    ...
    if (rate <= 0 || config.decoder_kernel_sizes[i] != expected_kernel || (channels & 1) != 0) {
...
std::string indexed(const std::string& prefix, int index, const std::string& suffix) {
...
struct AudioDecoder::Impl {
  ...
  DeviceTensor upload(const std::vector<float>& host, std::initializer_list<uint64_t> extents) {
    ...
    current_load_pool_high_water =
        std::max(current_load_pool_high_water, context.pooled_used_bytes());
  ...
  Conv load_conv(const SafeTensors& checkpoint, const std::string& name, uint32_t out_channels,
                 uint32_t in_channels, uint32_t kernel, bool transpose, bool require_bias,
                 Weights& destination) {
    const std::vector<int64_t> shape =
        transpose ? std::vector<int64_t>{in_channels, out_channels, kernel}
    ...
    current_host_loader_peak =
        std::max(current_host_loader_peak, (host.weight.size() + host.bias.size()) * sizeof(float));
    Conv result;
    result.out_channels = out_channels;
    result.in_channels = in_channels;
    result.kernel = kernel;
    result.weight = transpose ? upload(host.weight, {in_channels, out_channels, kernel})
  ...
  Activation load_activation(const SafeTensors& checkpoint, const std::string& prefix,
                             uint32_t channels, Weights& destination) {
    Activation result;
    result.channels = channels;
    auto load = [&](const std::string& name, const std::vector<int64_t>& shape,
                    std::initializer_list<uint64_t> extents) {
      std::vector<float> host = vae::load_audio_f32_tensor(checkpoint, prefix + name, shape);
      current_host_loader_peak = std::max(current_host_loader_peak, host.size() * sizeof(float));
  ...
  void record_conv(TensorBatch& batch, Conv& conv, DeviceTensor& input, DeviceTensor& output,
                   uint32_t batch_size, uint32_t length_in, uint32_t length_out, uint32_t pad,
                   uint32_t dilation) {
    vae::AudioConv1DDesc desc{batch_size, conv.in_channels, conv.out_channels,
                              length_in,  length_out,       conv.kernel,
                              pad,        dilation};
    batch.audio_conv1d(input, conv.weight, conv.bias ? &conv.bias : nullptr, output, desc);
  }

  void record_activation(TensorBatch& batch, Activation& activation, DeviceTensor& input,
                         DeviceTensor& output, uint32_t batch_size, uint32_t length) {
    batch.audio_aa_upsample_snake(input, activation.up_filter, activation.log_alpha,
                                  activation.log_beta, aa_scratch, batch_size, activation.channels,
                                  length);
    batch.audio_aa_downsample(aa_scratch, activation.down_filter, output, batch_size,
                              activation.channels, length * 2, length);
  }

  void record_block(TensorBatch& batch, AmpBlock& block, DeviceTensor& x, uint32_t batch_size,
                    uint32_t length, DeviceTensor& t1, DeviceTensor& t2) {
    ...
    for (int index = 0; index < 3; ++index) {
      ...
      record_conv(batch, block.convs1[index], t1, t2, batch_size, length, length,
                  padding(block.kernel, dilation), dilation);
      record_activation(batch, block.acts[2 * index + 1], t2, t1, batch_size, length);
      record_conv(batch, block.convs2[index], t1, t2, batch_size, length, length,
                  padding(block.kernel, 1), 1);
...
void AudioDecoder::load(const SafeTensors& checkpoint, const vae::AudioVAEConfig& config) {
  if (!impl_)
    throw std::logic_error("Vulkan audio VAE: empty decoder");
  ...
  next.conv_pre = d.load_conv(checkpoint, "decoder.conv_pre", config.decoder_dim, config.latent_dim,
                              7, false, true, next);
  ...
  for (size_t stage_index = 0; stage_index < config.decoder_rates.size(); ++stage_index) {
    ...
    stage.up =
        d.load_conv(checkpoint, up, output_channels, channels, stage.kernel, true, true, next);
    for (int block_index = 0; block_index < 3; ++block_index) {
      const int flat_index = static_cast<int>(stage_index) * 3 + block_index;
      const std::string prefix = "decoder.resblocks." + std::to_string(flat_index) + ".";
      ...
      for (int dilation_index = 0; dilation_index < 3; ++dilation_index) {
        block.convs1[dilation_index] =
            d.load_conv(checkpoint, indexed(prefix + "convs1.", dilation_index, ""),
                        output_channels, output_channels, block.kernel, false, true, next);
        block.convs2[dilation_index] =
            d.load_conv(checkpoint, indexed(prefix + "convs2.", dilation_index, ""),
                        output_channels, output_channels, block.kernel, false, true, next);
      }
      for (int activation_index = 0; activation_index < 6; ++activation_index) {
        block.acts[activation_index] =
            d.load_activation(checkpoint, indexed(prefix + "activations.", activation_index, "."),
  ...
  next.activation_post = d.load_activation(checkpoint, "decoder.activation_post.", channels, next);
  if (checkpoint.find("decoder.conv_post.bias") != nullptr) {
    throw std::runtime_error("Vulkan audio VAE: unexpected decoder.conv_post.bias");
  }
  next.conv_post = d.load_conv(checkpoint, "decoder.conv_post", 1, channels, 7, false, false, next);
  if (next.tensors != 779)
    throw std::runtime_error("Vulkan audio VAE: decoder tensor count is not 779");

  std::vector<float> next_mean =
      vae::load_audio_f32_tensor(checkpoint, "latents_mean", {config.latent_channels});
  std::vector<float> next_std =
      vae::load_audio_f32_tensor(checkpoint, "latents_std", {config.latent_channels});
  d.current_host_loader_peak = std::max(
      d.current_host_loader_peak, std::max(next_mean.size(), next_std.size()) * sizeof(float));
  d.config = config;
...
vae::DecodedAudio AudioDecoder::decode(const float* latents, int num_latents,
  ...
  const uint64_t latent_count =
      static_cast<uint64_t>(batch_size) * d.config.latent_channels * latent_length;
  DeviceTensor input = d.context.allocate(
      layout({batch_size, static_cast<uint32_t>(d.config.latent_channels), latent_length}));
  const uint32_t output_length = latent_length * static_cast<uint32_t>(d.config.total_upsample());
  DeviceTensor interleaved = d.context.allocate(layout({output_length, batch_size}));
  d.last_decode_pool_high_water =
      std::max(d.last_decode_pool_high_water, d.context.pooled_used_bytes());
  d.last_transient_bytes =
      (latent_count + static_cast<uint64_t>(output_length) * batch_size) * sizeof(float);
  ...
  if (trace != nullptr) {
    ...
    diagnostic_counts.push_back(static_cast<uint64_t>(batch_size) * d.config.latent_dim *
                                latent_length);
    diagnostic_counts.push_back(static_cast<uint64_t>(batch_size) * d.config.decoder_dim *
                                latent_length);
    uint64_t diagnostic_length = latent_length;
    for (const Stage& stage : d.weights.stages) {
      diagnostic_length *= stage.rate;
      diagnostic_counts.push_back(static_cast<uint64_t>(batch_size) * stage.up.out_channels *
                                  diagnostic_length);
    }
    diagnostic_counts.push_back(static_cast<uint64_t>(batch_size) *
                                d.weights.activation_post.channels * diagnostic_length);
    for (int index = 0; index < 3; ++index)
      diagnostic_counts.push_back(static_cast<uint64_t>(batch_size) * diagnostic_length);
  ...
  d.record_conv(commands, d.weights.conv_pre, *a, *b, batch_size, latent_length, latent_length, 3,
                1);
  capture(*b, static_cast<uint64_t>(batch_size) * d.config.decoder_dim * latent_length);
  ...
  for (Stage& stage : d.weights.stages) {
    ...
    vae::AudioConvTranspose1DDesc desc{
        batch_size,   stage.up.in_channels, stage.up.out_channels,          length, length_out,
        stage.kernel, stage.rate,           (stage.kernel - stage.rate) / 2};
    ...
    for (int block_index = 0; block_index < 3; ++block_index) {
      ...
      d.record_block(commands, stage.blocks[block_index], *destination, batch_size, length, *t1,
                     *t2);
      if (block_index != 0)
  ...
  d.record_activation(commands, d.weights.activation_post, *current, *spare, batch_size, length);
  capture(*spare, static_cast<uint64_t>(batch_size) * d.weights.activation_post.channels * length);
  d.record_conv(commands, d.weights.conv_post, *spare, *current, batch_size, length, length, 3, 1);
  ...
  const uint32_t expected_operators =
      kGraphOperators + (trace != nullptr ? static_cast<uint32_t>(diagnostic_counts.size()) : 0u);
  if (diagnostic_index != diagnostic_tensors.size() ||
  ...
  if (trace != nullptr) {
    ...
    for (size_t index = 0; index < diagnostic_tensors.size(); ++index) {
      ...
      d.context.download(diagnostic_tensors[index], trace->boundaries[index].data(),
...
uint64_t AudioDecoder::peak_device_bytes() const noexcept {
  return impl_ ? impl_->weights.bytes + impl_->activation_bytes() + impl_->last_transient_bytes : 0;
}
