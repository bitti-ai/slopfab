...
TensorContext::Impl::Impl(const Device& input, const TensorContextOptions& tensor_options)
    : max_batch_operators(tensor_options.max_batch_operators),
      ...
      pool(input, 4ull << 20), scratch(input), ops_bindings(3), vae_pointwise_bindings(4),
      norm_bindings(4), mod_bindings(6), vae_rope_bindings(7), audio_bindings(5),
      keyframe_bindings(4), dit_bindings(5), weight_bindings(6), gemm_bindings(4),
      gemm_prepare_bindings(2), attention_bindings(4), attention_h3_bindings(4),
      attention_h3_banded_bindings(5), attention_sage_bindings(6),
      attention_sage_prepare_bindings(7), attention_prepare_bindings(6),
  ...
  if (!input.info().fp32_signed_zero_inf_nan_preserve || !input.info().fp32_rounding_rte) {
  ...
  exact_vae_norm =
      detail::known_exact_vae_norm_device(input.info().vendor_id, input.info().device_id,
                                          input.info().driver_version) &&
      input.info().fp32_signed_zero_inf_nan_preserve && input.info().shader_int64_enabled;
  exact_vae_pointwise =
      detail::known_exact_vae_pointwise_device(input.info().vendor_id, input.info().device_id,
                                               input.info().driver_version) &&
      input.info().fp32_signed_zero_inf_nan_preserve && input.info().fp32_rounding_rte &&
  ...
  sage_attention =
      input.info().cooperative_matrix_enabled && input.info().shader_float16_enabled &&
      input.info().storage_buffer_16bit_enabled && input.info().compute_subgroup_shuffle &&
  ...
  prepare_pipelines(input, tensor_options.pipeline_sets | (tensor_options.enable_reference_encoder
                                                               ? TensorPipelineSet::kReference
                                                               : TensorPipelineSet::kCore));
  for (uint32_t i = 0; i < ops_bindings.size(); ++i)
    ops_bindings[i].binding = i;
  for (uint32_t i = 0; i < vae_pointwise_bindings.size(); ++i)
    vae_pointwise_bindings[i].binding = i;
  for (uint32_t i = 0; i < norm_bindings.size(); ++i)
    norm_bindings[i].binding = i;
  for (uint32_t i = 0; i < mod_bindings.size(); ++i)
    mod_bindings[i].binding = i;
...
void TensorContext::Impl::prepare_pipelines(const Device& input, TensorPipelineSet sets) {
  const uint32_t requested =
      static_cast<uint32_t>(sets) | static_cast<uint32_t>(TensorPipelineSet::kCore);
  ...
  if (has(TensorPipelineSet::kCore)) {
    const uint8_t* shader =
        full_arithmetic_exact ? detail::kTensorOpsDenormSpirv : detail::kTensorOpsSpirv;
  ...
  if (has(TensorPipelineSet::kVideo) && exact_vae_pointwise) {
    ...
    vae_residual_pipeline =
        make_pointwise(detail::kTensorVaeResidualSpirv, sizeof(detail::kTensorVaeResidualSpirv));
    vae_swiglu_pipeline =
        make_pointwise(detail::kTensorVaeSwigluSpirv, sizeof(detail::kTensorVaeSwigluSpirv));
    vae_denorm_pipeline =
        make_pointwise(detail::kTensorVaeDenormSpirv, sizeof(detail::kTensorVaeDenormSpirv));
  }
  if (has(TensorPipelineSet::kReference)) {
    ...
    std::memcpy(module.data(), detail::kTensorReferenceSpirv,
                sizeof(detail::kTensorReferenceSpirv));
  ...
  if (has(TensorPipelineSet::kCore)) {
    const uint8_t* rope_shader =
        full_arithmetic_exact ? detail::kTensorRopeDenormSpirv : detail::kTensorRopeSpirv;
    const size_t rope_shader_bytes = full_arithmetic_exact ? sizeof(detail::kTensorRopeDenormSpirv)
                                                           : sizeof(detail::kTensorRopeSpirv);
    ...
    const uint8_t* weight_shader =
        full_arithmetic_exact ? detail::kTensorWeightDenormSpirv : detail::kTensorWeightSpirv;
    ...
    const uint8_t* gemm_shader =
        full_arithmetic_exact ? detail::kTensorGemmDenormSpirv : detail::kTensorGemmSpirv;
    const size_t gemm_shader_bytes = full_arithmetic_exact ? sizeof(detail::kTensorGemmDenormSpirv)
                                                           : sizeof(detail::kTensorGemmSpirv);
    ...
    if (cooperative_gemm) {
      ...
      gemm_coop_pipeline = ComputePipeline::create(input, cooperative_spirv, cooperative_options);
    }
    {
      std::vector<uint32_t> prepare_spirv(sizeof(detail::kTensorGemmPrepareSpirv) /
                                          sizeof(uint32_t));
    ...
    if (cooperative_f16_gemm) {
      ...
      cooperative_options.local_size[0] = 128;
      cooperative_options.local_size[1] = 1;
      gemm_coop_f16_pipeline =
          ComputePipeline::create(input, cooperative_spirv, cooperative_options);
    }
  }
  ...
  auto make_norm_pipeline = [&](const uint8_t* shader, size_t shader_bytes, uint32_t bindings = 4,
                                uint32_t local_x = 256, uint32_t local_y = 1,
  ...
  if (has(TensorPipelineSet::kCore) && exact_vae_norm) {
    ...
    layer_norm_pipeline =
        make_norm_pipeline(detail::kTensorLayerNormSpirv, sizeof(detail::kTensorLayerNormSpirv));
    bf16_rms_block_pipeline = make_norm_pipeline(detail::kTensorBf16RmsBlockSpirv,
                                                 sizeof(detail::kTensorBf16RmsBlockSpirv), 3);
    bf16_rms_narrow_pipeline = make_norm_pipeline(
        detail::kTensorBf16RmsNarrowSpirv, sizeof(detail::kTensorBf16RmsNarrowSpirv), 3, 32, 8);
    bf16_layer_pipeline =
        make_norm_pipeline(detail::kTensorBf16LayerSpirv, sizeof(detail::kTensorBf16LayerSpirv));
    bf16_mod_pipeline =
        make_norm_pipeline(detail::kTensorBf16ModSpirv, sizeof(detail::kTensorBf16ModSpirv), 6);
    fp32_mod_pipeline =
        make_norm_pipeline(detail::kTensorFp32ModSpirv, sizeof(detail::kTensorFp32ModSpirv), 6);
    group_norm_pipeline =
        make_norm_pipeline(detail::kTensorGroupNormSpirv, sizeof(detail::kTensorGroupNormSpirv));
    vae_rope_pipeline =
  ...
  if (has(TensorPipelineSet::kBlockedAttention) && exact_attention) {
    ...
    attention_prepare_pipeline = make_norm_pipeline(detail::kTensorAttentionPrepareSpirv,
  ...
  if (has(TensorPipelineSet::kExactH3Attention) && exact_h3_attention) {
    ...
    attention_h3_banded_pipeline = make_norm_pipeline(
        detail::kTensorAttentionH3BandedSpirv, sizeof(detail::kTensorAttentionH3BandedSpirv), 5,
        kH3AttentionLocalSize, 1, sizeof(AttentionParameters));
  }
  if (has(TensorPipelineSet::kFlashAttention) && flash_attention) {
    attention_vsa_pipeline = make_norm_pipeline(
        detail::kTensorAttention_VSA, sizeof(detail::kTensorAttention_VSA), 6, 256, 1, 32);
    attention_vsa_prepare_pipeline =
        make_norm_pipeline(detail::kTensorAttention_VSA_PREPARE,
                           sizeof(detail::kTensorAttention_VSA_PREPARE), 8, 256, 1, 32);
    attention_flash_pipeline = make_norm_pipeline(
        detail::kTensorAttentionFlashSpirv, sizeof(detail::kTensorAttentionFlashSpirv), 4,
        kFastH3AttentionLocalSize, 1, sizeof(AttentionParameters));
    attention_flash_banded_pipeline = make_norm_pipeline(
        detail::kTensorAttentionFlashBandedSpirv, sizeof(detail::kTensorAttentionFlashBandedSpirv),
        5, kFastH3AttentionLocalSize, 1, sizeof(AttentionParameters));
  }
  if (has(TensorPipelineSet::kSageAttention) && sage_attention) {
    auto make_sage = [&](uint32_t kernel, const uint8_t* full, size_t full_bytes,
      ...
      if (!detail::sage_kernel_fits(input.info(), kernel))
        return;
      const auto c = detail::sage_kernel_configuration(kernel, input.info().subgroup_size);
      for (uint32_t variant = 0; variant < 4; ++variant) {
        const std::vector<SpecializationConstant> constants = {{0, variant & 1u},
                                                               {1, variant < 2 ? 64u : 128u}};
        attention_sage_pipelines[kernel - 1][variant] = make_norm_pipeline(
            full, full_bytes, 6, c.local_size, 1, sizeof(AttentionParameters), constants);
        attention_sage_banded_pipelines[kernel - 1][variant] = make_norm_pipeline(
            banded, banded_bytes, 6, c.local_size, 1, sizeof(AttentionParameters), constants);
      }
    };
#define SAGE_PIPELINE(K, F, B)                                                                     \
  make_sage(K, detail::F, sizeof(detail::F), detail::B, sizeof(detail::B))
    if (input.info().subgroup_size == 32) {
      ...
      SAGE_PIPELINE(4, kTensorAttention_SAGE_WIDE_FULL_SG64,
                    kTensorAttention_SAGE_WIDE_FULL_SG64_BANDED);
    }
#undef SAGE_PIPELINE
    attention_sage_prepare_pipeline = make_norm_pipeline(
