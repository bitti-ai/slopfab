...
std::vector<uint16_t> load_matrix(const SafeTensors& checkpoint,
                                  const std::string& name, uint64_t rows,
                                  uint64_t columns, bool flattened = false) {
  const TensorView& tensor = checkpoint.at(name);
...
struct VideoVaeDecoder::Impl {
  ...
  Impl(const Device& device, const vae::ViTConfig& cfg)
    ...
    if (cfg.num_layers <= 0 || cfg.dim <= 0 || cfg.in_channels <= 0 ||
        cfg.num_register <= 0 || cfg.num_suffix != cfg.num_register + 1 ||
        cfg.patch_dim() <= 0) {
  ...
  ShapeSlot& select_shape(uint32_t time, uint32_t height, uint32_t width) {
    ...
    if (shapes.size() == kShapeCache) {
      const auto oldest = std::min_element(
          ...
          [](const std::unique_ptr<ShapeSlot>& a,
             const std::unique_ptr<ShapeSlot>& b) { return a->stamp < b->stamp; });
      shapes.erase(oldest);
    }
    ...
    slot->time = time; slot->height = height; slot->width = width;
    ...
    slot->register_indices = context.allocate(vector(config.num_register),
                                               ScalarType::kInt32);
    slot->zero_index = context.allocate(vector(1), ScalarType::kInt32);
    slot->patch_indices = context.allocate(vector(slot->patches),
                                            ScalarType::kInt32);
    const vae::ViTRopeTables rope = vae::build_vit_rope_tables(
        time, height, width, static_cast<uint32_t>(config.num_suffix),
    ...
    context.upload_bytes(slot->patch_indices, patches.data(),
                         patches.size() * sizeof(int32_t));
    slot->post_prepared = PreparedF16Activation::create(
        context, slot->patches, static_cast<uint32_t>(config.in_channels));
    slot->embed_prepared = PreparedF16Activation::create(
        context, slot->patches, static_cast<uint32_t>(config.in_channels));
    slot->proj_prepared = PreparedF16Activation::create(
        context, slot->patches, static_cast<uint32_t>(config.dim));
    slot->post_plan = DenseGemmPlan::create(context, {
        slot->patches, static_cast<uint32_t>(config.in_channels),
        static_cast<uint32_t>(config.in_channels), DenseGemmMode::kFloat16Vae,
        DenseGemmBias::kNone, true});
    slot->embed_plan = DenseGemmPlan::create(context, {
        slot->patches, static_cast<uint32_t>(config.dim),
        static_cast<uint32_t>(config.in_channels), DenseGemmMode::kFloat16Vae,
        DenseGemmBias::kNone, true});
    slot->proj_plan = DenseGemmPlan::create(context, {
        slot->patches, static_cast<uint32_t>(config.patch_dim()),
        static_cast<uint32_t>(config.dim), DenseGemmMode::kFloat16Vae,
        DenseGemmBias::kNone, true});
    graph.prepare_shape(slot->sequence, slot->patches);
  ...
  Document make_document(const ShapeSlot& slot) {
    ...
    const uint64_t pixels = static_cast<uint64_t>(config.out_channels) *
        slot.time * config.patch_t * slot.height * config.patch *
        slot.width * config.patch;
  ...
  void record_document(ShapeSlot& slot, Document& doc, TensorBatch& batch) {
    ...
    PreparedF16ActivationView post = slot.post_prepared.prepare(
        batch, doc.patch, slot.patches);
    slot.post_plan.record(batch, post, post_weight, doc.quantized);
    batch.add_bias(doc.quantized, post_bias, doc.quantized);
    PreparedF16ActivationView embed = slot.embed_prepared.prepare(
        batch, doc.quantized, slot.patches);
    ...
    batch.layer_norm(doc.patch_tokens, norm_weight, norm_bias, doc.normed,
                     config.eps);
    PreparedF16ActivationView projection = slot.proj_prepared.prepare(
        batch, doc.normed, slot.patches);
    slot.proj_plan.record(batch, projection, proj_weight, doc.projected);
    batch.add_bias(doc.projected, proj_bias, doc.projected);
    batch.depth_to_space(doc.projected, doc.pixels, slot.time, slot.height,
                         slot.width, config.out_channels, config.patch_t,
                         config.patch);
  }
    ...
    for (const std::unique_ptr<ShapeSlot>& slot : shapes) {
      ...
      for (const Document& doc : slot->documents) {
        total += tensor_bytes(doc.latent) + tensor_bytes(doc.patch) +
            ...
            tensor_bytes(doc.patch_tokens) + tensor_bytes(doc.normed) +
            tensor_bytes(doc.projected) + tensor_bytes(doc.pixels);
      }
    }
    return total;
  }
...
void VideoVaeDecoder::load(const SafeTensors& checkpoint) {
  ...
  upload_matrix(d.context, d.post_weight, load_matrix(
      checkpoint, "post_quant_conv.weight", c, c, true));
  upload_vector(d.context, d.post_bias, load_vector(
      checkpoint, "post_quant_conv.bias", c));
  upload_matrix(d.context, d.embed_weight, load_matrix(
      checkpoint, "decoder.x_embedder.weight", dim, c));
  upload_vector(d.context, d.embed_bias, load_vector(
      checkpoint, "decoder.x_embedder.bias", dim));
  upload_vector(d.context, d.register_tokens, load_vector(
      checkpoint, "decoder.register_tokens", d.config.num_register * dim));
  std::vector<float> zeros(dim, 0.0f);
  upload_vector(d.context, d.zero_token, zeros);
  upload_vector(d.context, d.norm_weight, load_vector(
      checkpoint, "decoder.norm_out.weight", dim));
  upload_vector(d.context, d.norm_bias, load_vector(
      checkpoint, "decoder.norm_out.bias", dim));
  upload_matrix(d.context, d.proj_weight, load_matrix(
      checkpoint, "decoder.proj_out.weight", patch_dim, dim));
  upload_vector(d.context, d.proj_bias, load_vector(
      checkpoint, "decoder.proj_out.bias", patch_dim));
...
void VideoVaeDecoder::forward_window(const float* latent, int time, int height,
                                     int width, std::vector<float>& output) {
...
void VideoVaeDecoder::forward_windows(
    const float* latent, int batch, int time, int height, int width,
    std::vector<std::vector<float>>& output, const size_t* slots) {
  ...
  const uint64_t latent_words = static_cast<uint64_t>(d.config.in_channels) *
      shape.patches;
  ...
  for (uint32_t first = 0; first < static_cast<uint32_t>(batch);
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      d.context.download(shape.documents[first + i].pixels,
                         destination.data(), pixel_words);
    }
  }
}
