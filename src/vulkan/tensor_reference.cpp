...
void TensorBatch::group_norm_silu_f16_affine(DeviceTensor& input, DeviceTensor& weight,
  ...
  const bool group_count_overflows =
      spatial != 0 && channels_per_group > std::numeric_limits<uint64_t>::max() / spatial;
  const uint64_t group_count = group_count_overflows ? 0 : channels_per_group * spatial;
  if (!std::isnormal(epsilon) || epsilon <= 0.0f || groups == 0 || shape.rank != 3 ||
      spatial == 0 || channels == 0 || channels % groups != 0 || group_count_overflows ||
      channels > std::numeric_limits<uint32_t>::max() ||
      spatial > std::numeric_limits<uint32_t>::max() || group_count > kMaxExactNormDimension ||
      shape.elements() > std::numeric_limits<uint32_t>::max() || src.get() == w.get() ||
      src.get() == b.get() || w.get() == b.get() || dst.get() == w.get() || dst.get() == b.get() ||
      src->type != ScalarType::kFloat32 || dst->type != ScalarType::kFloat32 ||
      w->type != ScalarType::kFloat16 || b->type != ScalarType::kFloat16 || w->layout.rank != 1 ||
      b->layout.rank != 1 || dst->layout.rank != 3 || w->layout.extent[0] != channels ||
      b->layout.extent[0] != channels || dst->layout.extent != shape.extent ||
      !shape.is_contiguous() || !w->layout.is_contiguous() || !b->layout.is_contiguous() ||
...
void TensorBatch::reference_operation(DeviceTensor& input, DeviceTensor& weight, DeviceTensor& bias,
                                      DeviceTensor& output, const uint32_t* parameters,
                                      uint32_t groups, uint32_t batches, DeviceTensor* previous,
                                      DeviceTensor* earliest) {
  ...
  std::array<std::shared_ptr<DeviceTensor::Impl>, 6> tensors = {
      impl_->owner->require(input),
      impl_->owner->require(weight),
      impl_->owner->require(bias),
      impl_->owner->require(output),
  ...
  try {
    ...
    for (uint32_t i = 0; i < tensors.size(); ++i) {
      impl_->transition(tensors[i],
                        i == 3 ? BufferAccess::kComputeWrite : BufferAccess::kComputeRead);
...
void TensorBatch::keyframe_conv3d_f16(DeviceTensor& input, DeviceTensor& weight, DeviceTensor& bias,
                                      DeviceTensor& output, uint32_t in_channels,
                                      uint32_t out_channels, uint32_t input_height,
                                      uint32_t input_width, uint32_t kernel, uint32_t stride,
                                      bool reflect_padding, bool asymmetric_padding) {
  if (!impl_ || impl_->poisoned) {
    throw std::logic_error("vulkan tensor: invalid batch");
  }
  ...
  try {
    input_count =
        checked_multiply(in_channels, checked_multiply(input_height, input_width, "keyframe input"),
                         "keyframe input");
    output_count = checked_multiply(
        out_channels, checked_multiply(output_height, output_width, "keyframe output"),
        "keyframe output");
  ...
  const bool shaped_input = src->layout.rank == 3 && src->layout.extent[0] == in_channels &&
                            src->layout.extent[1] == input_height &&
                            src->layout.extent[2] == input_width;
  const bool valid_input =
      shaped_input || (src->layout.rank == 1 && src->layout.elements() >= input_count);
  const bool valid_weight = w->layout.rank == 5 && w->layout.extent[0] == out_channels &&
                            w->layout.extent[1] == in_channels && w->layout.extent[2] == kernel &&
                            w->layout.extent[3] == kernel && w->layout.extent[4] == kernel;
  const bool shaped_output = dst->layout.rank == 3 && dst->layout.extent[0] == out_channels &&
                             dst->layout.extent[1] == output_height &&
                             dst->layout.extent[2] == output_width;
  const bool valid_output =
      shaped_output || (dst->layout.rank == 1 && dst->layout.elements() >= output_count);
  if (in_channels == 0 || out_channels == 0 || input_height == 0 || input_width == 0 ||
      (kernel != 1 && kernel != 3) || (stride != 1 && stride != 2) ||
      (asymmetric_padding && (stride != 2 || reflect_padding)) || output_height == 0 ||
      output_width == 0 || output_count > std::numeric_limits<uint32_t>::max() ||
      src.get() == w.get() || src.get() == b.get() || src.get() == dst.get() ||
      w.get() == b.get() || w.get() == dst.get() || b.get() == dst.get() ||
      src->type != ScalarType::kFloat32 || dst->type != ScalarType::kFloat32 ||
      w->type != ScalarType::kFloat16 || b->type != ScalarType::kFloat16 || !valid_input ||
      !valid_weight || !valid_output || b->layout.rank != 1 ||
      b->layout.extent[0] != out_channels || src->layout.elements() < input_count ||
      w->layout.elements() != weight_count || dst->layout.elements() < output_count ||
      !src->layout.is_contiguous() || !w->layout.is_contiguous() || !b->layout.is_contiguous() ||
      !dst->layout.is_contiguous()) {
...
void TensorBatch::keyframe_group_norm_silu_f16_affine(DeviceTensor& input, DeviceTensor& weight,
                                                      DeviceTensor& bias, DeviceTensor& output,
  ...
  const uint64_t group_count = groups == 0 ? 0 : static_cast<uint64_t>(channels / groups) * spatial;
  if (!std::isnormal(epsilon) || epsilon <= 0.0f || channels == 0 || height == 0 || width == 0 ||
      groups == 0 || channels % groups != 0 || spatial > std::numeric_limits<uint32_t>::max() ||
      count > std::numeric_limits<uint32_t>::max() ||
      group_count > std::numeric_limits<uint32_t>::max() || src.get() == dst.get() ||
      src.get() == w.get() || src.get() == b.get() || dst.get() == w.get() ||
      dst.get() == b.get() || w.get() == b.get() || src->type != ScalarType::kFloat32 ||
      dst->type != ScalarType::kFloat32 || w->type != ScalarType::kFloat16 ||
      b->type != ScalarType::kFloat16 || src->layout.rank != 1 || dst->layout.rank != 1 ||
      src->layout.elements() < count || dst->layout.elements() < count || w->layout.rank != 1 ||
      b->layout.rank != 1 || w->layout.extent[0] != channels || b->layout.extent[0] != channels ||
      !src->layout.is_contiguous() || !dst->layout.is_contiguous() || !w->layout.is_contiguous() ||
      !b->layout.is_contiguous()) {
    throw std::invalid_argument("vulkan keyframe: invalid flat GroupNorm+SiLU");
  }
  if (!detail::norm_dispatch_fits(groups, impl_->owner->max_dispatch_x)) {
    throw std::out_of_range("vulkan keyframe: GroupNorm group count exceeds dispatch limits");
  }
  TensorContext::Impl::NormParameters p;
  p.rows = channels;
  p.dim = static_cast<uint32_t>(spatial);
...
void TensorBatch::keyframe_add_f32(DeviceTensor& a, DeviceTensor& b, DeviceTensor& output,
                                   uint64_t count) {
  if (!impl_ || impl_->poisoned) {
    throw std::logic_error("vulkan tensor: invalid batch");
  }
  ...
  if (count == 0 || count > std::numeric_limits<uint32_t>::max() || av.get() == bv.get() ||
      av.get() == out.get() || bv.get() == out.get() || av->type != ScalarType::kFloat32 ||
      bv->type != ScalarType::kFloat32 || out->type != ScalarType::kFloat32 ||
      av->layout.rank != 1 || bv->layout.rank != 1 || out->layout.rank != 1 ||
