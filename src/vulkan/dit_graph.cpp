...
void validate_graph_config(const H3MainGraphConfig& config) {
  ...
  if (config.block.sequence == 0 || config.block.hidden == 0 ||
      config.block.heads == 0 || config.block.head_dim != 128 ||
      uint64_t(config.block.heads) * config.block.head_dim > UINT32_MAX ||
      config.block.ffn == 0 || config.block.timesteps == 0 ||
      config.block.modalities == 0 || config.block.adaln_rank == 0)
...
ExactH3MainGraph::~ExactH3MainGraph() = default;
ExactH3MainGraph::ExactH3MainGraph(std::shared_ptr<Impl> impl)
    : impl_(std::move(impl)) {}
...
ExactH3MainGraph ExactH3MainGraph::create(
    TensorContext& context, const H3MainGraphConfig& config) {
...
void ExactH3MainGraph::record_layers(
  ...
  if (!impl_->context->owns(tokens) || !impl_->context->owns(selectors) ||
      !impl_->context->owns(code) || !impl_->context->owns(cosine) ||
      !impl_->context->owns(sine) || tv.type != ScalarType::kBFloat16 ||
      tv.layout.rank != 2 || tv.layout.extent[0] != c.sequence ||
      tv.layout.extent[1] != c.hidden || !tv.layout.is_contiguous() ||
      sv.type != ScalarType::kInt32 || sv.layout.rank != 1 ||
      sv.layout.extent[0] != c.sequence || !sv.layout.is_contiguous() ||
      cv.type != ScalarType::kFloat32 || cv.layout.rank != 2 ||
      cv.layout.extent[0] != c.timesteps ||
      cv.layout.extent[1] != c.adaln_rank || !cv.layout.is_contiguous() ||
  ...
  for (uint32_t layer = first_layer; layer < first_layer + layer_count; ++layer) {
    ...
    impl_->stages[layer].record(batch, tokens, selectors, code, cosine, sine,
                                impl_->scratch, ranges,
                                taps ? &block_tap : nullptr);
  }
}
