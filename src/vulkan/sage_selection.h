...
inline SageAttentionConfiguration sage_kernel_configuration(uint32_t kernel,
                                                            uint32_t subgroup) {
  ...
  c.shared_bytes = c.query_rows * 128 + 64 * 128 + c.value_rows * 128 * 2 +
      c.query_rows * 64 * 6 + c.query_rows * 12;
  return c;
}
...
inline SageAttentionConfiguration select_sage_configuration(
    const DeviceInfo& info, const H3AttentionPlanDesc& d, uint64_t extra_budget) {
  if (!sage_kernel_fits(info, 1)) throw std::runtime_error("vulkan Sage: compact kernel exceeds device limits");
  ...
  if (!sage_kernel_fits(info, kernel)) throw std::invalid_argument("vulkan Sage: requested kernel exceeds device limits");
  auto c = sage_kernel_configuration(kernel, info.subgroup_size);
  const uint64_t elements = uint64_t(d.sequence) * d.heads * d.head_dim;
  const uint64_t aux = uint64_t(d.heads) * (d.head_dim + 2ull * ((uint64_t(d.sequence)+15)/16)) * 4;
  c.required_workspace_bytes = elements * 2 + aux;
  const uint64_t partials = uint64_t(d.heads) * d.head_dim * ((uint64_t(d.sequence)+255)/256) * 4;
  c.parallel_mean = d.sequence >= 512 && partials <= extra_budget &&
      aux + partials <= info.max_storage_buffer_bytes && aux + partials <= info.max_allocation_bytes;
  if (c.parallel_mean) c.extra_workspace_bytes += partials;
  c.prepared_value = elements * 2 <= extra_budget - c.extra_workspace_bytes &&
      elements * 2 <= info.max_storage_buffer_bytes && elements * 2 <= info.max_allocation_bytes;
  if (c.prepared_value) c.extra_workspace_bytes += elements * 2;
  return c;
}
}  // namespace slopfab::vulkan::detail
