...
namespace {
size_t count(const DeviceTensor& t) {
  return t.layout().elements();
}
} // namespace
...
struct ReferenceEncoder::Impl {
  ...
  DeviceTensor allocate(uint64_t n) {
    if (!n || n > UINT32_MAX)
      throw std::invalid_argument("Vulkan reference: tensor size exceeds shader indexing");
  ...
  DeviceTensor& at(const std::string& n) {
    return weights.at(n);
  }
  ...
  DeviceTensor run(const DeviceTensor& x, const DeviceTensor& w, const DeviceTensor& b,
                   std::array<uint32_t, 16> p, uint64_t n, uint32_t groups = 0,
    ...
    batch.reference_operation(
        const_cast<DeviceTensor&>(x), const_cast<DeviceTensor&>(w), const_cast<DeviceTensor&>(b),
        out, p.data(), groups ? groups : uint32_t((n + 255) / 256), batches, previous, earliest);
    batch.submit().wait();
    context.collect();
    return out;
  }
  ...
  Video conv3(Video& x, const std::string& name, int ci, int co, int h, int width, int k,
              int stride = 1, int ts = 1, bool down = false) {
  ...
  DeviceTensor conv1(const DeviceTensor& x, const std::string& name, int ci, int co, int len, int k,
                     int stride = 1, int pad = 0, int dil = 1) {
    uint32_t n = (len + 2 * pad - dil * (k - 1) - 1) / stride + 1;
    return run(x, at(name + ".weight"), at(name + ".bias"),
               {1, uint32_t(ci), uint32_t(co), uint32_t(len), 0, 0, uint32_t(k), uint32_t(stride),
                0, uint32_t(pad), uint32_t(dil), n, 0, 0, 1},
               uint64_t(2) * co * n, ((n + 15) / 16) * ((co + 15) / 16), 2);
  }

  DeviceTensor norm3(const DeviceTensor& x, const std::string& name, int c, int t, int h, int w) {
    return run(x, at(name + ".weight"), at(name + ".bias"),
               {3, uint32_t(c), 0, uint32_t(t), uint32_t(h), uint32_t(w)}, count(x), 32 * t);
  }

  DeviceTensor norm(const DeviceTensor& x, const std::string& name, int rows, int dim) {
    return run(x, at(name + ".weight"), at(name + ".bias"), {4, uint32_t(dim), 0, 1}, count(x),
               rows);
  }

  DeviceTensor linear(const DeviceTensor& x, const std::string& name, int rows, int ci, int co) {
    return run(x, at(name + ".weight"), at(name + ".bias"),
               {2, uint32_t(ci), uint32_t(co), 0, 0, 0, 0, 0, 0, 0, 0, uint32_t(rows), 0, 0, 1},
               uint64_t(rows) * co, ((rows + 15) / 16) * ((co + 15) / 16));
  }

  DeviceTensor snake(const DeviceTensor& x, const std::string& name, int channels, int length) {
    return run(x, at(name), x, {5, uint32_t(channels), 0, uint32_t(length)}, count(x));
  }
  ...
  DeviceTensor attention(const DeviceTensor& q, int length) {
    auto heads =
        run(q, q, q, {9, 0, 0, uint32_t(length)}, uint64_t(2) * length * 2048, 2 * length * 8);
...
ReferenceEncoder::ReferenceEncoder(const Device& device, const SafeTensors& checkpoint, bool audio)
...
ReferenceEncoder::Impl::Impl(const Device& device, const SafeTensors& checkpoint, bool is_audio)
    : context(device,
              [] {
  ...
  if (!audio) {
    ...
    for (const auto& entry : checkpoint.tensors()) {
      if (entry.first.rfind("encoder.", 0) != 0 && entry.first.rfind("quant_conv.", 0) != 0)
        continue;
      if (entry.second.dtype != DType::kF16 && entry.second.dtype != DType::kF32 &&
  ...
  auto conv = [&](const std::string& name, int ci, int co, int k) {
    const auto weight = vae::load_audio_conv_weights(checkpoint, name, {co, ci, k}, co, true);
  ...
  for (const auto& name : {"q_bias", "zero_k_bias", "v_bias"}) {
    auto part =
        vae::load_audio_f32_tensor(checkpoint, std::string("pre_block.attn.") + name, {2048});
    bias.insert(bias.end(), part.begin(), part.end());
  }
  put("pre_block.attn.qkv.bias", bias);
}

std::vector<float> ReferenceEncoder::encode_mean(const float* stereo, int samples) {
  ...
  auto& ops = *impl_;
  auto conv = [&](const DeviceTensor& input, const std::string& n, int ci, int co, int len, int k,
                  int stride = 1, int pad = 0, int dil = 1) {
  ...
  auto linear = [&](const DeviceTensor& input, const std::string& n, int ci, int co) {
    return ops.linear(input, n, rows, ci, co);
  };
...
std::vector<float> ReferenceEncoder::encode_reference(const float* stereo, int samples) {
...
std::vector<float> ReferenceEncoder::encode_temporal_moments(const float* pixels, int frames,
                                                             int height, int width) {
  if (impl_->audio || !pixels || frames <= 0 || frames > 17 || height <= 0 || width <= 0 ||
      height % 16 || width % 16)
    throw std::invalid_argument(
  ...
  auto conv = [&](Video& x, const std::string& name, int ci, int co, int k, int ss = 1, int ts = 1,
                  bool down = false) {
  ...
  for (int t = 0; t < frames; ++t) {
    ...
    for (int c = 0; c < 3; ++c)
      std::copy_n(pixels + (size_t(c) * frames + t) * plane, plane, host.data() + c * plane);
    x.push_back(ops.upload(host));
  }
  x = conv(x, "encoder.conv_in", 3, 128, 3);
  const int channels[] = {128, 256, 256, 512, 512, 1024}, spatial[] = {2, 2, 2, 2, 1, 1},
...
std::vector<float> ReferenceEncoder::encode_reference_video(const std::vector<RGBImage>& frames,
                                                            int count,
  ...
  auto normal = vae::torch_cpu_normal_seed42(size_t(24) * latent_frames * latent_plane);
  auto latent = vae::sample_keyframe_latents(moments.data(), normal.data(),
                                             latent_frames * (h / 16), w / 16, mean, stddev);
