...
uint64_t checked_product(uint64_t left, uint64_t right, const char* what) {
  if (left != 0 && right > std::numeric_limits<uint64_t>::max() / left) {
    throw std::overflow_error(std::string("Vulkan Qwen layer: ") + what + " overflow");
  }
  return left * right;
}

bool same_model_shape(const QwenTextLayerConfig& left, const QwenTextLayerConfig& right) {
  const auto& a = left.encoder;
  const auto& b = right.encoder;
  return left.sequence == right.sequence && a.hidden_size == b.hidden_size &&
         a.num_attention_heads == b.num_attention_heads &&
         a.num_key_value_heads == b.num_key_value_heads && a.head_dim == b.head_dim &&
         a.intermediate_size == b.intermediate_size && a.rms_norm_eps == b.rms_norm_eps;
}
...
void validate_config(const QwenTextLayerConfig& config) {
  ...
  if (config.sequence == 0 || config.sequence > text::kMaxPromptTokens ||
      !text::supports_exact_text_layer({static_cast<int>(config.sequence), c.hidden_size,
                                        ...
                                        c.intermediate_size, c.rms_norm_eps}) ||
      c.num_layers <= 0 || c.intermediate_size % 256 != 0) {
...
void validate_target_layer(const SafeTensors& checkpoint, const text::EncoderConfig& config,
                           uint32_t layer, bool verify_manifest) {
  const std::string prefix = "model.layers." + std::to_string(layer) + ".";
  size_t expected_count = 0;
  for (int i = 0; i < text::kLayerTensorCount; ++i) {
    const text::TensorSpec spec =
        text::layer_tensor_spec(config, static_cast<text::LayerTensor>(i));
    if (!spec.present())
      continue;
    ++expected_count;
    ...
    const uint64_t elements =
        spec.dim1 == 0 ? static_cast<uint64_t>(spec.dim0)
                       : checked_product(static_cast<uint64_t>(spec.dim0),
                                         static_cast<uint64_t>(spec.dim1), "tensor elements");
    const uint64_t expected_bytes =
        checked_product(elements, dtype_size(spec.dtype), "tensor bytes");
    if (view.dtype != spec.dtype || view.shape != expected || view.nbytes != expected_bytes) {
  ...
  const char* canonical_metadata =
      nvfp4 ? "{\"format\": \"nvfp4\", \"full_precision_matrix_mult\": true}"
  ...
  for (const char* linear : kLinears) {
    ...
    if (metadata.dtype != DType::kU8 ||
        metadata.shape != std::vector<int64_t>{static_cast<int64_t>(canonical_metadata_bytes)} ||
        metadata.nbytes != canonical_metadata_bytes ||
        std::memcmp(metadata.data, canonical_metadata, canonical_metadata_bytes) != 0) {
      throw std::runtime_error("Vulkan Qwen layer: non-canonical comfy_quant for '" + name + "'");
    }
    if (nvfp4)
      ++expected_count;
  }
  if (nvfp4) {
    (void)text::read_global_scales(checkpoint, config, static_cast<int>(layer));
  }
  ...
  for (const auto& entry : checkpoint.tensors()) {
    ...
    for (const char* linear : kLinears) {
      ...
      expected = expected || (nvfp4 && suffix == std::string(linear) + ".weight_scale_2");
    }
    if (!expected) {
      throw std::runtime_error("Vulkan Qwen layer: unexpected target tensor '" + entry.first + "'");
    }
  }
  if (actual_count != expected_count) {
    throw std::runtime_error("Vulkan Qwen layer: incomplete target-layer tensor manifest");
  }
}

void validate_embedding_manifest(const SafeTensors& checkpoint, const text::EncoderConfig& config) {
  const TensorView& weight = checkpoint.at("model.embed_tokens.weight");
  const uint64_t elements =
      checked_product(static_cast<uint64_t>(config.vocab_size),
                      static_cast<uint64_t>(config.hidden_size), "embedding elements");
  const bool nvfp4 = config.format == text::WeightFormat::kNVFP4Awq;
  const DType expected_dtype = nvfp4 ? DType::kI8 : DType::kBF16;
  const uint64_t expected_bytes =
      checked_product(elements, dtype_size(expected_dtype), "embedding bytes");
  if (weight.dtype != expected_dtype ||
      weight.shape != std::vector<int64_t>{config.vocab_size, config.hidden_size} ||
      weight.nbytes != expected_bytes) {
    throw std::runtime_error("Vulkan Qwen encoder: invalid model.embed_tokens.weight");
  }
  if (!nvfp4)
    return;

  const TensorView& scale = checkpoint.at("model.embed_tokens.weight_scale");
  const uint64_t scale_bytes = checked_product(static_cast<uint64_t>(config.vocab_size),
                                               sizeof(float), "embedding scale bytes");
  if (scale.dtype != DType::kF32 || scale.shape != std::vector<int64_t>{config.vocab_size, 1} ||
      scale.nbytes != scale_bytes) {
    throw std::runtime_error("Vulkan Qwen encoder: invalid model.embed_tokens.weight_scale");
  }
  ...
  for (int64_t row = 0; row < config.vocab_size; ++row) {
    if (!std::isfinite(values[row]) || values[row] <= 0.0f) {
      throw std::runtime_error("Vulkan Qwen encoder: non-positive embedding weight_scale");
    }
  }

  static constexpr char kEmbeddingMetadata[] = "{\"format\": \"int8_tensorwise\"}";
  const TensorView& metadata = checkpoint.at("model.embed_tokens.comfy_quant");
  constexpr size_t kMetadataBytes = sizeof(kEmbeddingMetadata) - 1;
  if (metadata.dtype != DType::kU8 ||
      metadata.shape != std::vector<int64_t>{static_cast<int64_t>(kMetadataBytes)} ||
      metadata.nbytes != kMetadataBytes ||
      std::memcmp(metadata.data, kEmbeddingMetadata, kMetadataBytes) != 0) {
    throw std::runtime_error("Vulkan Qwen encoder: non-canonical embedding comfy_quant");
  }
}
...
Projection load_projection(TensorContext& context, const SafeTensors& checkpoint,
                           ...
                           text::LayerTensor weight_kind, text::LayerTensor scale_kind,
                           text::LayerTensor pre_scale_kind, uint32_t global_index, uint32_t out,
                           uint32_t in, const text::LayerGlobalScales& globals) {
  const std::string prefix = "model.layers." + std::to_string(layer) + ".";
  const text::TensorSpec weight_spec = text::layer_tensor_spec(config, weight_kind);
  const text::TensorSpec scale_spec = text::layer_tensor_spec(config, scale_kind);
  ...
  if (config.format == text::WeightFormat::kI8ConvRot) {
    ...
    if (pre_scale_kind != text::LayerTensor::kCount) {
      const text::TensorSpec pre_spec = text::layer_tensor_spec(config, pre_scale_kind);
      const TensorView& pre = checkpoint.at(prefix + pre_spec.suffix);
      upload.pre_quant_scale_bf16 = static_cast<const uint16_t*>(pre.data);
...
uint32_t projection_operators(const Projection& projection, uint32_t rows) {
  return 1u + row_gemm_operators(rows) + (projection.weight.has_pre_quant_scale() ? 1u : 0u) +
         (projection.weight.applies_convrot() ? 1u : 0u);
}

void validate_tap(TensorContext& context, DeviceTensor* tensor, ScalarType type,
                  const TensorLayout& layout, const char* name,
                  std::array<DeviceTensorView, 14>& views, size_t& view_count) {
  if (!tensor)
    return;
  if (!context.owns(*tensor)) {
    throw std::invalid_argument(std::string("Vulkan Qwen layer: foreign ") + name + " tap");
  }
  const DeviceTensorView view = tensor->view();
  if (view.type != type || view.layout.rank != layout.rank || view.layout.extent != layout.extent ||
      !view.layout.is_contiguous()) {
    throw std::invalid_argument(std::string("Vulkan Qwen layer: invalid ") + name + " tap");
  }
...
struct ExactQwenTextLayerScratch::Impl {
  ...
  Impl(TensorContext& owner, const QwenTextLayerConfig& c) : context(&owner), config(c) {
    const uint32_t rows = c.sequence;
    const uint32_t hidden = static_cast<uint32_t>(c.encoder.hidden_size);
    const uint32_t q_width =
        static_cast<uint32_t>(c.encoder.num_attention_heads * c.encoder.head_dim);
    const uint32_t kv_width =
        static_cast<uint32_t>(c.encoder.num_key_value_heads * c.encoder.head_dim);
    ...
    query = owner.allocate(three(rows, c.encoder.num_attention_heads, c.encoder.head_dim),
                           ScalarType::kBFloat16);
    key = owner.allocate(three(rows, c.encoder.num_key_value_heads, c.encoder.head_dim),
                         ScalarType::kBFloat16);
    value = owner.allocate(three(rows, c.encoder.num_key_value_heads, c.encoder.head_dim),
                           ScalarType::kBFloat16);
    attention = owner.allocate(three(rows, c.encoder.num_attention_heads, c.encoder.head_dim),
                               ScalarType::kBFloat16);
    ...
    query_transform = owner.allocate(three(rows, c.encoder.num_attention_heads, c.encoder.head_dim),
                                     ScalarType::kBFloat16);
    ffn_transform = owner.allocate(matrix(rows, ffn), ScalarType::kBFloat16);
    const uint64_t largest = std::max({checked_product(q_width, hidden, "Q matrix"),
    ...
    auto plan = [&](uint32_t out, uint32_t in) {
      return DenseGemmPlan::create(
          owner, {rows, out, in, DenseGemmMode::kBFloat16, DenseGemmBias::kNone, false});
    };
    ...
    attention_plan = CausalGQAAttentionPlan::create(
        owner,
        {rows, static_cast<uint32_t>(c.encoder.num_attention_heads),
         static_cast<uint32_t>(c.encoder.num_key_value_heads),
         static_cast<uint32_t>(c.encoder.head_dim), exact_attention_scale(c.encoder.head_dim)});
  }

  uint64_t reserved() const noexcept {
    return tensor_bytes(normed) + tensor_bytes(query) + tensor_bytes(key) + tensor_bytes(value) +
           tensor_bytes(attention) + tensor_bytes(branch) + tensor_bytes(gate) + tensor_bytes(up) +
           tensor_bytes(activation) + tensor_bytes(hidden_transform) +
           tensor_bytes(query_transform) + tensor_bytes(ffn_transform) + cache.dense_bytes();
  }
};
...
struct ExactQwenTextLayerStage::Impl {
  struct Weights {
    ...
    uint64_t bytes() const noexcept {
      return tensor_bytes(input_norm) + tensor_bytes(post_norm) + tensor_bytes(q_norm) +
             tensor_bytes(k_norm) + q.bytes() + k.bytes() + v.bytes() + o.bytes() + gate.bytes() +
             up.bytes() + down.bytes();
    }
  };
...
ExactQwenTextLayerScratch::ExactQwenTextLayerScratch(std::shared_ptr<Impl> impl)
    : impl_(std::move(impl)) {
}

ExactQwenTextLayerScratch::ExactQwenTextLayerScratch(ExactQwenTextLayerScratch&&) noexcept =
    default;
ExactQwenTextLayerScratch&
ExactQwenTextLayerScratch::operator=(ExactQwenTextLayerScratch&&) noexcept = default;
...
ExactQwenTextLayerStage::ExactQwenTextLayerStage(std::shared_ptr<Impl> impl)
    : impl_(std::move(impl)) {
}

ExactQwenTextLayerStage::ExactQwenTextLayerStage(ExactQwenTextLayerStage&&) noexcept = default;
ExactQwenTextLayerStage&
ExactQwenTextLayerStage::operator=(ExactQwenTextLayerStage&&) noexcept = default;
...
void ExactQwenTextLayerStage::validate_checkpoint(const SafeTensors& checkpoint, uint32_t layer,
                                                  const QwenTextLayerConfig& config) {
  validate_config(config);
...
void ExactQwenTextLayerStage::validate_archive(const SafeTensors& checkpoint,
                                               const QwenTextLayerConfig& config) {
  validate_config(config);
  text::EncoderConfig resolved = resolved_encoder(checkpoint, config);
  ...
  for (uint32_t layer = 0; layer < static_cast<uint32_t>(resolved.num_layers); ++layer) {
    validate_target_layer(checkpoint, resolved, layer, false);
  }
}

void ExactQwenTextLayerStage::load(const SafeTensors& checkpoint, uint32_t layer) {
  if (!impl_)
    throw std::logic_error("Vulkan Qwen layer: empty stage");
  ...
  const uint32_t q_width = static_cast<uint32_t>(resolved.num_attention_heads * resolved.head_dim);
  const uint32_t kv_width = static_cast<uint32_t>(resolved.num_key_value_heads * resolved.head_dim);
  ...
  next->input_norm =
      upload_norm(*impl_->context, checkpoint, prefix + "input_layernorm.weight", hidden);
  next->post_norm =
      upload_norm(*impl_->context, checkpoint, prefix + "post_attention_layernorm.weight", hidden);
  next->q_norm = upload_norm(*impl_->context, checkpoint, prefix + "self_attn.q_norm.weight",
                             resolved.head_dim);
  next->k_norm = upload_norm(*impl_->context, checkpoint, prefix + "self_attn.k_norm.weight",
                             resolved.head_dim);
...
template <typename Scratch> DeviceTensor& transform_for(Scratch& scratch, uint32_t width) {
  const uint32_t hidden = static_cast<uint32_t>(scratch.config.encoder.hidden_size);
  const uint32_t query = static_cast<uint32_t>(scratch.config.encoder.num_attention_heads *
                                               scratch.config.encoder.head_dim);
  const uint32_t ffn = static_cast<uint32_t>(scratch.config.encoder.intermediate_size);
  if (width == hidden)
    return scratch.hidden_transform;
  if (width == query)
    return scratch.query_transform;
  if (width == ffn)
    return scratch.ffn_transform;
  throw std::logic_error("Vulkan Qwen layer: missing activation transform arena");
}

template <typename Scratch>
void record_projection(TensorBatch& batch, Projection& projection, const DenseGemmPlan& plan,
                       Scratch& scratch, DeviceTensor& input, DeviceTensor& output, uint32_t rows,
                       DeviceTensor* shared_transformed_input = nullptr) {
  DeviceTensor* source = shared_transformed_input ? shared_transformed_input : &input;
  if (projection.weight.has_pre_quant_scale() && projection.weight.applies_convrot()) {
    throw std::logic_error("Vulkan Qwen layer: unsupported combined transform");
  }
  if (shared_transformed_input) {
    if (!projection.weight.applies_convrot() || projection.weight.has_pre_quant_scale()) {
      throw std::logic_error("Vulkan Qwen layer: invalid shared projection transform");
    }
  ...
  PreparedNVFP4WeightView prepared = scratch.cache.prepare(batch, projection.weight, plan);
  const uint32_t tiled = rows / 64 * 64;
  if (tiled != 0)
    plan.record(batch, *source, prepared, output, tiled);
  if (tiled != rows) {
...
void ExactQwenTextLayerStage::record(TensorBatch& batch, DeviceTensor& tokens, DeviceTensor& cosine,
  ...
  if (!impl_->context->owns(tokens) || !impl_->context->owns(cosine) ||
      !impl_->context->owns(sine) || token_view.type != ScalarType::kBFloat16 ||
      token_view.layout.rank != 2 || token_view.layout.extent[0] != rows ||
      token_view.layout.extent[1] != hidden || !token_view.layout.is_contiguous() ||
      cosine_view.type != ScalarType::kFloat32 || sine_view.type != ScalarType::kFloat32 ||
      cosine_view.layout.rank != 2 || sine_view.layout.rank != 2 ||
      cosine_view.layout.extent[0] != rows || sine_view.layout.extent[0] != rows ||
      cosine_view.layout.extent[1] != 128 || sine_view.layout.extent[1] != 128 ||
      !cosine_view.layout.is_contiguous() || !sine_view.layout.is_contiguous() ||
      token_view.resource == cosine_view.resource || token_view.resource == sine_view.resource ||
  ...
  if (taps) {
    validate_tap(*impl_->context, taps->input_norm, ScalarType::kBFloat16, matrix(rows, hidden),
                 "input norm", tap_views, tap_view_count);
    ...
    validate_tap(*impl_->context, taps->key, ScalarType::kBFloat16,
                 three(rows, c.num_key_value_heads, c.head_dim), "key", tap_views, tap_view_count);
    ...
    validate_tap(*impl_->context, taps->attention_residual, ScalarType::kBFloat16,
                 matrix(rows, hidden), "attention residual", tap_views, tap_view_count);
    validate_tap(*impl_->context, taps->post_attention_norm, ScalarType::kBFloat16,
                 matrix(rows, hidden), "post-attention norm", tap_views, tap_view_count);
    validate_tap(*impl_->context, taps->gate, ScalarType::kBFloat16, matrix(rows, ffn), "gate",
                 tap_views, tap_view_count);
    validate_tap(*impl_->context, taps->up, ScalarType::kBFloat16, matrix(rows, ffn), "up",
                 tap_views, tap_view_count);
    validate_tap(*impl_->context, taps->activation, ScalarType::kBFloat16, matrix(rows, ffn),
                 "activation", tap_views, tap_view_count);
    validate_tap(*impl_->context, taps->final_residual, ScalarType::kBFloat16, matrix(rows, hidden),
                 "final residual", tap_views, tap_view_count);
  }
  ...
  if (taps && taps->input_norm)
    batch.copy(s.normed, *taps->input_norm);
  ...
  record_projection(batch, w.q, s.q_plan, s, s.normed, s.query, rows, shared_qkv);
  record_projection(batch, w.k, s.k_plan, s, s.normed, s.key, rows, shared_qkv);
  record_projection(batch, w.v, s.v_plan, s, s.normed, s.value, rows, shared_qkv);
  batch.rms_norm_heads_bf16(s.query, w.q_norm, s.query, c.num_attention_heads, c.head_dim,
                            c.rms_norm_eps);
  batch.rms_norm_heads_bf16(s.key, w.k_norm, s.key, c.num_key_value_heads, c.head_dim,
                            c.rms_norm_eps);
  batch.rope_neox_bf16(s.query, cosine, sine);
  batch.rope_neox_bf16(s.key, cosine, sine);
  if (taps) {
    if (taps->query)
      batch.copy(s.query, *taps->query);
    if (taps->key)
      batch.copy(s.key, *taps->key);
    if (taps->value)
      batch.copy(s.value, *taps->value);
  }
  s.attention_plan.record(batch, s.query, s.key, s.value, s.attention);
  if (taps && taps->attention)
    batch.copy(s.attention, *taps->attention);
  ...
  record_projection(batch, w.gate, s.gate_plan, s, s.normed, s.gate, rows, shared_gate_up);
  record_projection(batch, w.up, s.up_plan, s, s.normed, s.up, rows, shared_gate_up);
  if (taps) {
    if (taps->gate)
      batch.copy(s.gate, *taps->gate);
    if (taps->up)
      batch.copy(s.up, *taps->up);
  }
  batch.text_swiglu_split_bf16(s.gate, s.up, s.activation);
  if (taps && taps->activation)
    batch.copy(s.activation, *taps->activation);
...
uint32_t ExactQwenTextLayerStage::required_operators(const QwenTextLayerTaps* taps) const {
  if (!impl_ || !impl_->weights) {
    throw std::logic_error("Vulkan Qwen layer: stage is not loaded");
  }
  const auto& w = *impl_->weights;
  uint32_t count = 10u + projection_operators(w.q, impl_->config.sequence) +
...
text::WeightFormat ExactQwenTextLayerStage::format() const noexcept {
  return impl_ && impl_->weights ? impl_->weights->format : text::WeightFormat::kAuto;
}
