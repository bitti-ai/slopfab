...
void TensorBatch::vision_add_positions_bf16(DeviceTensor& activation,
  ...
  const uint64_t packed = count == 0 ? 0 : 1 + (count - 1) / 2;
  if (x.get() == table.get() || x.get() == index.get() ||
      table.get() == index.get() || x->layout.rank != 2 || rows == 0 ||
      dim == 0 || table->layout.rank != 2 || table->layout.extent[1] != dim ||
      table->layout.extent[0] == 0 || index->layout.rank != 1 ||
  ...
  TensorContext::Impl::DitParameters p;
  p.op = 7; p.rows = static_cast<uint32_t>(rows);
  p.dim = static_cast<uint32_t>(dim);
...
void TensorBatch::vision_split_qkv_bf16(DeviceTensor& fused,
                                         DeviceTensor& query,
                                         DeviceTensor& key,
                                         DeviceTensor& value) {
  if (!impl_ || impl_->poisoned)
  ...
  if (src.get() == q.get() || src.get() == k.get() || src.get() == v.get() ||
      ...
      src->layout.rank != 2 || rows == 0 || dim == 0 || fused_dim % 3 != 0 ||
      src->type != ScalarType::kBFloat16 || !src->layout.is_contiguous() ||
      !output_valid(q) || !output_valid(k) || !output_valid(v) ||
      rows > UINT32_MAX || dim > UINT32_MAX || count > UINT32_MAX ||
      impl_->owner->max_batch_operators < 3 ||
      impl_->operator_count > impl_->owner->max_batch_operators - 3)
    throw std::invalid_argument("vulkan vision: invalid QKV split tensors");
  TensorContext::Impl::DitParameters p;
  p.op = 8; p.rows = static_cast<uint32_t>(rows);
  p.dim = static_cast<uint32_t>(dim); p.count = static_cast<uint32_t>(packed);
  try {
    impl_->transition(src, BufferAccess::kComputeRead);
...
void TensorBatch::vision_merge_four_bf16(DeviceTensor& input,
                                          DeviceTensor& output) {
  if (!impl_ || impl_->poisoned)
    throw std::logic_error("vulkan tensor: invalid batch");
  ...
  const bool merged_dim_overflows =
      dim > std::numeric_limits<uint64_t>::max() / 4;
  if (src.get() == dst.get() || src->layout.rank != 2 || rows == 0 ||
      (rows & 3u) != 0 || dim == 0 || dst->layout.rank != 2 ||
      merged_dim_overflows || dst->layout.extent[0] != rows / 4 ||
      dst->layout.extent[1] != dim * 4 ||
      src->type != ScalarType::kBFloat16 || dst->type != ScalarType::kBFloat16 ||
      !src->layout.is_contiguous() || !dst->layout.is_contiguous() ||
      rows > UINT32_MAX || dim > UINT32_MAX || count > UINT32_MAX)
    throw std::invalid_argument("vulkan vision: invalid merge-four tensors");
  TensorContext::Impl::DitParameters p;
  p.op = 9; p.rows = static_cast<uint32_t>(rows);
  p.dim = static_cast<uint32_t>(dim); p.count = static_cast<uint32_t>(count / 2);
  try {
    impl_->count_operator(); impl_->transition(src, BufferAccess::kComputeRead);
    impl_->transition(dst, BufferAccess::kComputeWrite);
    impl_->dispatch_dit(p, {src, src, src, src, dst});
...
void TensorBatch::vision_scatter_add_bf16(DeviceTensor& source,
                                           DeviceTensor& destination,
  ...
  TensorContext::Impl::DitParameters p;
  p.op = 10; p.rows = static_cast<uint32_t>(rows);
  p.dim = static_cast<uint32_t>(dim);
