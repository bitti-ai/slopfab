...
DeviceTensorView DeviceTensor::view() const {
  if (!impl_)
    throw std::logic_error("vulkan tensor: empty tensor");
...
DeviceTensor::operator bool() const noexcept {
...
DeviceBackend TensorWorkspace::backend() const noexcept {
  return DeviceBackend::kVulkan;
}

void TensorWorkspace::reserve(uint64_t bytes) {
  if (!impl_)
    throw std::logic_error("vulkan workspace: moved-from object");
  if (bytes <= impl_->capacity)
    return;
  if (impl_->cursor != 0)
    throw std::logic_error("vulkan workspace: reset before growth");
  Buffer replacement = impl_->pool.allocate(bytes,
...
void TensorWorkspace::reset() noexcept {
  if (impl_)
    impl_->cursor = 0;
}

uint64_t TensorWorkspace::capacity() const noexcept {
  return impl_ ? impl_->capacity : 0;
}

uint64_t TensorWorkspace::used() const noexcept {
  return impl_ ? impl_->cursor : 0;
}

uint64_t TensorWorkspace::generation() const noexcept {
...
bool TensorWorkspace::valid(const WorkspaceSpan& span) const noexcept {
  return impl_ && span.backend == DeviceBackend::kVulkan && span.context == impl_->context &&
         span.resource == impl_->buffer.native_handle() && span.generation == impl_->generation &&
         span.byte_size != 0 && span.byte_offset <= impl_->capacity &&
...
DeviceTensor TensorContext::allocate(const TensorLayout& layout, ScalarType type) {
  ...
  if (!layout.is_contiguous())
    throw std::invalid_argument("vulkan tensor: contiguous layout required");
  ...
  tensor->buffer = impl_->pool.allocate(bytes,
                                        BufferUsage::kStorage | BufferUsage::kTransferSource |
                                            BufferUsage::kTransferDestination,
                                        MemoryUsage::kDevice);
...
TensorBatch TensorContext::begin_batch() {
  if (!impl_)
    throw std::logic_error("vulkan tensor: empty context");
...
void TensorContext::upload(DeviceTensor& destination, const float* values, uint64_t count) {
  ...
  if (dst->type != ScalarType::kFloat32 || values == nullptr || count != dst->layout.elements()) {
    throw std::invalid_argument("vulkan tensor: upload element count mismatch");
  }
  upload_bytes(destination, values, count * sizeof(float));
}

void TensorContext::upload_transient(DeviceTensor& destination, const float* values,
                                     uint64_t count) {
  if (!impl_)
    throw std::logic_error("vulkan tensor: moved-from context");
  auto dst = impl_->require(destination);
...
void TensorContext::upload_bytes(DeviceTensor& destination, const void* values, uint64_t bytes) {
  if (!impl_)
    throw std::logic_error("vulkan tensor: moved-from context");
  [[maybe_unused]] auto recording_lock = impl_->acquire_recorder();
  auto dst = impl_->require(destination);
  ...
  list.barrier(impl_->upload_buffer, BufferAccess::kHostWrite, BufferAccess::kTransferRead, 0,
               physical_bytes);
...
void TensorContext::upload_transient_bytes(DeviceTensor& destination, const void* values,
                                           uint64_t bytes) {
  if (!impl_)
    throw std::logic_error("vulkan tensor: moved-from context");
  [[maybe_unused]] auto recording_lock = impl_->acquire_recorder();
  ...
  const uint64_t physical_bytes = dst->buffer.size();
  Buffer staging =
      impl_->pool.allocate(physical_bytes, BufferUsage::kTransferSource, MemoryUsage::kUpload);
  staging.write(0, values, bytes);
  if (physical_bytes != bytes)
    write_zero_bytes(staging, bytes, physical_bytes - bytes);
  CommandList list = impl_->commands.begin();
  list.barrier(staging, BufferAccess::kHostWrite, BufferAccess::kTransferRead, 0, physical_bytes);
  const bool old_has_access = dst->has_access;
  const BufferAccess old_access = dst->access;
  try {
    if (dst->has_access)
...
void TensorContext::upload_batch(const TensorUpload* uploads, uint32_t count) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    pending.push_back({tensor, uploads[i].values, uploads[i].bytes, physical, total,
                       tensor->has_access, tensor->access});
    total += physical;
  }
  [[maybe_unused]] auto recording_lock = impl_->acquire_recorder();
  ...
  for (const Pending& item : pending) {
    ...
    if (item.physical != item.bytes)
      write_zero_bytes(impl_->upload_buffer, item.offset + item.bytes, item.physical - item.bytes);
  }
  CommandList list = impl_->commands.begin();
  list.barrier(impl_->upload_buffer, BufferAccess::kHostWrite, BufferAccess::kTransferRead, 0,
               total);
  try {
    for (Pending& item : pending) {
      if (item.tensor->has_access)
        list.barrier(item.tensor->buffer, item.tensor->access, BufferAccess::kTransferWrite);
      item.tensor->has_access = true;
      item.tensor->access = BufferAccess::kTransferWrite;
      list.copy_buffer(impl_->upload_buffer, item.tensor->buffer, item.physical, item.offset, 0);
    }
    impl_->complete(std::move(list));
  } catch (...) {
    for (Pending& item : pending) {
...
void TensorContext::download(DeviceTensor& source, float* values, uint64_t count) {
  ...
  if (src->type != ScalarType::kFloat32 || values == nullptr || count != src->layout.elements()) {
    throw std::invalid_argument("vulkan tensor: download element count mismatch");
  }
  download_bytes(source, values, count * sizeof(float));
}

void TensorContext::download_bytes(DeviceTensor& source, void* values, uint64_t bytes) {
  if (!impl_)
    throw std::logic_error("vulkan tensor: moved-from context");
  [[maybe_unused]] auto recording_lock = impl_->acquire_recorder();
  auto src = impl_->require(source);
  ...
  try {
    ...
    list.barrier(impl_->readback_buffer, BufferAccess::kTransferWrite, BufferAccess::kHostRead, 0,
                 physical_bytes);
    impl_->complete(std::move(list));
  } catch (...) {
...
void TensorContext::require_full_fp32_arithmetic_exactness() const {
  if (!full_fp32_arithmetic_exactness()) {
    throw std::runtime_error("vulkan tensor: device cannot preserve fp32 subnormal inputs/results; "
...
void TensorContext::require_h3_attention(AttentionMode mode) const {
  if (!h3_attention_supported(mode))
    throw std::runtime_error(std::string("vulkan H3 attention: mode '") +
                             attention_mode_name(mode) +
                             "' is unavailable; check enabled cooperative matrix, "
                             "subgroup and arithmetic features");
  impl_->require_pipeline_set(mode == AttentionMode::kExact ? TensorPipelineSet::kExactH3Attention
                              : mode == AttentionMode::kFlash2 ? TensorPipelineSet::kFlashAttention
                                                               : TensorPipelineSet::kSageAttention);
}
...
void TensorContext::require_native_nvfp4_gemm() const {
  if (!impl_)
    throw std::logic_error("vulkan tensor: moved-from context");
  throw std::runtime_error("vulkan tensor: native NVFP4 GEMM is unavailable; this device/runtime "
...
TensorBatch::operator bool() const noexcept {
  return impl_ != nullptr;
}

uint32_t TensorBatch::remaining_operator_capacity() const noexcept {
  if (!impl_ || !impl_->owner || impl_->operator_count >= impl_->owner->max_batch_operators)
    return 0;
