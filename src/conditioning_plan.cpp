...
ConditioningSettings read_settings(const std::string& path) {
  ...
  if (it == st.metadata().end())
    return {};
  try {
    return parse_conditioning_settings(it->second);
...
#define CONDITION_FIELDS(F)                                                                        \
  F(require_prompt_embedding)                                                                      \
  F(fixed_prompt_tokens)                                                                           \
  F(image_short_edge)                                                                              \
  F(media_short_edge)                                                                              \
  F(media_max_pixels)                                                                              \
  F(references_at_target_canvas)                                                                   \
  F(canvas_from_reference_video)                                                                   \
  F(include_reference_audio)                                                                       \
  F(video_first)                                                                                   \
  F(pin_target_audio)                                                                              \
  F(max_frames)                                                                                    \
  F(require_euler)                                                                                 \
...
void validate_conditioning_request(const GenerateRequest& r, const ResolvedConditioning& s) {
  if (r.video_transition) {
    if (r.video_transition < 1 || r.video_transition > 2 || r.still_image || r.continuation ||
      ...
      throw std::invalid_argument(
          "Extend/Bridge requires exactly one/two source videos without other conditioning modes");
    for (const auto& media : r.reference_media)
      if (!media || !media->is_video() || media->duration_seconds() + 1e-9 < 22.0 / 24)
        throw std::invalid_argument(
            "Extend/Bridge source videos need at least 22 frames at 24 fps");
  }
  if ((s.video_first || s.pin_target_audio) && s.include_reference_audio)
    throw std::invalid_argument(
        "video_first and pin_target_audio require include_reference_audio=false");
  ...
  if (s.video_first) {
    if (r.still_image || r.continuation || r.has_refmods() || r.reference_image_paths.size() != 1 ||
        r.reference_media.size() != 1 || !r.reference_media.front()->is_video())
      throw std::invalid_argument(
          "video_first requires one driving video and one image without continuation or refmods");
  ...
  if (s.pin_target_audio &&
      (r.still_image || r.continuation || r.reference_media.size() != 1 ||
       !r.reference_media.front()->is_video() || !r.reference_media.front()->soundtrack()))
    throw std::invalid_argument(
        "pin_target_audio requires one video with a soundtrack, without continuation or still-image mode");
}
