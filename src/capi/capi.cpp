...
SLOPFAB_C_API void SLOPFAB_CALL slopfab_session_destroy(slopfab_session* session) { delete session; }
SLOPFAB_C_API int SLOPFAB_CALL slopfab_session_clear(slopfab_session* session) {
  if (!session) return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "session_clear: null session");
  return guarded([&] { session->value->clear(); return SLOPFAB_OK; });
}
SLOPFAB_C_API int SLOPFAB_CALL slopfab_request_set_session(
    slopfab_request* request, const slopfab_session* session) {
  if (!request) return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "set_session: null request");
  return guarded([&] {
