...
SLOPFAB_C_API int SLOPFAB_CALL slopfab_reference_video_create(double duration_seconds,
  ...
  if (!out_video)
    return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "reference video: null output pointer");
  *out_video = nullptr;
...
SLOPFAB_C_API int SLOPFAB_CALL slopfab_reference_video_append_rgb24(
  ...
  if (!video)
    return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "reference video: null handle");
  return reference_input_guarded([&] {
    video->media.append_frame(pixels, buffer_bytes, width, height, row_stride_bytes, 3,
                              timestamp_seconds);
  });
}
...
SLOPFAB_C_API int SLOPFAB_CALL slopfab_reference_video_append_rgba8(
  ...
  if (!video)
    return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "reference video: null handle");
  return reference_input_guarded([&] {
    video->media.append_frame(pixels, buffer_bytes, width, height, row_stride_bytes, 4,
                              timestamp_seconds);
  });
}
...
SLOPFAB_C_API int SLOPFAB_CALL slopfab_request_add_reference_video(
  ...
  if (!request || !video)
    return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "reference video: null handle");
  return reference_input_guarded([&] {
    attach_reference(request, video->media);
  });
}
...
SLOPFAB_C_API int SLOPFAB_CALL slopfab_request_add_reference_audio_f32(slopfab_request* request,
                                                                       ...
                                                                       int32_t sample_rate) {
  if (!request)
    return fail(SLOPFAB_ERR_INVALID_ARGUMENT, "reference audio: null request");
  return reference_input_guarded([&] {
    attach_reference(request,
                     slopfab::ReferenceMedia::audio(samples, float_count, channels, sample_rate));
  });
}
