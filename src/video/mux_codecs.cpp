...
const AVCodec* find_video_encoder(const Api& api, std::string* name) {
  ...
  const Candidate candidates[] = {{"libx264", 0},
                                  {"libopenh264", 0},
                                  {nullptr, kCodecIdH264},
                                  {"mpeg4", 0},
                                  {nullptr, kCodecIdMpeg4}};
  for (const Candidate& c : candidates) {
    const AVCodec* codec = c.by_name != nullptr ? api.avcodec_find_encoder_by_name(c.by_name)
                                                : api.avcodec_find_encoder(c.by_id);
    if (codec == nullptr)
      continue;
    bool all = false;
    const std::vector<int> pix = supported_config<int>(api, codec, kConfigPixFormat, &all);
    if (!all && !contains(pix, kPixFmtYuv420p))
      continue;
