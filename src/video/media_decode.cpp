...
DecodedVideoFrame decode_first_video_frame(const std::string& path) {
  ...
  if (!state.ok)
    throw std::runtime_error("FFmpeg media decoder unavailable: " + state.detail);
  ...
  auto cleanup = [&] {
    if (sws != nullptr)
      api.sws_freeContext(sws);
    if (packet != nullptr)
      api.av_packet_free(&packet);
    if (frame != nullptr)
      api.av_frame_free(&frame);
    if (decoder != nullptr)
      api.avcodec_free_context(&decoder);
    if (format != nullptr)
  ...
  if (rc < 0)
    return fail("cannot open input: " + err_text(api, rc));
  rc = api.avformat_find_stream_info(format, nullptr);
  if (rc < 0)
    return fail("cannot read stream information: " + err_text(api, rc));
  const AVCodec* codec = nullptr;
  const int stream_index = api.av_find_best_stream(format, kMediaTypeVideo, -1, -1, &codec, 0);
  if (stream_index < 0 || codec == nullptr)
    return fail("input has no decodable video stream");
  ...
  if (par == nullptr)
    return fail("video stream has no codec parameters");
  decoder = api.avcodec_alloc_context3(codec);
  if (decoder == nullptr)
    return fail("cannot allocate video decoder");
  rc = api.avcodec_parameters_to_context(decoder, par);
  if (rc < 0)
    return fail("cannot configure video decoder: " + err_text(api, rc));
  rc = api.avcodec_open2(decoder, codec, nullptr);
  if (rc < 0)
    return fail("cannot open video decoder: " + err_text(api, rc));
  frame = api.av_frame_alloc();
  packet = api.av_packet_alloc();
  if (frame == nullptr || packet == nullptr)
    return fail("cannot allocate decode buffers");
  ...
  for (;;) {
    ...
    if (input_done)
      return fail("video stream contains no decoded frames");
    rc = api.av_read_frame(format, packet);
    if (rc < 0) {
      input_done = true;
      rc = api.avcodec_send_packet(decoder, nullptr);
      if (rc < 0 && rc != kErrEof)
        return fail("cannot flush video decoder: " + err_text(api, rc));
      continue;
    }
  ...
  sws = api.sws_getContext(width, height, pixel_format, width, height, kPixFmtRgb24, kSwsBicubic,
                           nullptr, nullptr, nullptr);
  if (sws == nullptr)
    return fail("cannot create RGB24 converter");
  ...
  if (rc != height)
    return fail("RGB24 conversion failed");
  cleanup();
  return result;
}
