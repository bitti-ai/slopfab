...
void require(bool ok, const std::string& name) {
  if (!ok) throw std::runtime_error("tokenizer: unsupported Qwen byte-BPE contract: " + name);
}
const json::Value& field(const json::Value& value, const char* name) {
  const auto* result = value.find(name);
  if (!result) throw std::runtime_error(std::string("tokenizer: missing contract field ") + name);
  return *result;
}
void byte_level(const json::Value& value, const std::string& name) {
  ...
  for (const char* key : {"add_prefix_space", "use_regex", "trim_offsets"})
    if (const auto* v = value.find(key)) require(!v->as_bool(), name + "." + key);
}
}
void validate_tokenizer_component(const std::string& name, std::string_view text) {
  const auto value = json::parse(text);
  if (value.is_null()) return; // Historical minimal vocabulary fixtures.
  if (name == "normalizer") {
  ...
  constexpr const char* pattern = R"((?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+)";
...
void validate_tokenizer_model_field(const std::string& name, std::string_view text) {
  const auto value = json::parse(text);
  if (name == "type") require(value.as_string() == "BPE", "model.type");
  else if (name == "dropout" || name == "unk_token") require(value.is_null(), "model." + name);
