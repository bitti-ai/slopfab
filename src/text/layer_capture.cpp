...
void validate_header(const QwenLayerCaptureHeader& h) {
  ...
  if (std::memcmp(h.magic, expected, sizeof(expected)) != 0 || h.version != 1 || h.sequence == 0 ||
      h.sequence > kMaxPromptTokens || h.hidden != 5120 || h.query_heads != 64 || h.kv_heads != 8 ||
      h.head_dim != 128 || h.intermediate != 25600 || h.input_fnv64 == 0 || h.rope_fnv64 == 0 ||
      ...
      std::any_of(h.boundary_fnv64.begin(), h.boundary_fnv64.end(), [](uint64_t value) {
        return value == 0;
      })) {
...
void validate(const QwenLayerCapture& capture) {
  ...
  if (std::any_of(capture.token_ids.begin(), capture.token_ids.end(), [](int32_t id) {
        return id < 0 || id >= 151936;
      })) {
    throw std::runtime_error("Qwen capture: token id is out of range");
  }
  if (fnv64(capture.input_bf16.data(), capture.input_bf16.size() * sizeof(uint16_t)) !=
      h.input_fnv64) {
    throw std::runtime_error("Qwen capture: input digest mismatch");
  }
  uint64_t rope_hash = fnv64(capture.cosine.data(), capture.cosine.size() * sizeof(float));
  rope_hash = fnv64(capture.sine.data(), capture.sine.size() * sizeof(float), rope_hash);
  if (rope_hash != h.rope_fnv64)
    throw std::runtime_error("Qwen capture: RoPE digest mismatch");
}

template <typename T> void read_vector(std::ifstream& input, std::vector<T>& values, size_t count) {
  values.resize(count);
  input.read(reinterpret_cast<char*>(values.data()),
             static_cast<std::streamsize>(values.size() * sizeof(T)));
  if (!input)
    throw std::runtime_error("Qwen capture: truncated payload");
}

template <typename T> void write_vector(std::ofstream& output, const std::vector<T>& values) {
...
QwenLayerCapture read_qwen_layer_capture(const std::string& path) {
  ...
  if (!input)
    throw std::runtime_error("Qwen capture: cannot open " + path);
  QwenLayerCapture result;
  input.read(reinterpret_cast<char*>(&result.header), sizeof(result.header));
  if (!input)
    throw std::runtime_error("Qwen capture: truncated header");
  validate_header(result.header);
  const uint64_t token_count = result.header.sequence;
  const uint64_t input_count = checked_product(result.header.sequence, result.header.hidden);
  const uint64_t rope_count = checked_product(result.header.sequence, result.header.head_dim);
  uint64_t expected_bytes = sizeof(result.header);
  expected_bytes = checked_sum(expected_bytes, checked_product(token_count, sizeof(int32_t)));
  expected_bytes = checked_sum(expected_bytes, checked_product(input_count, sizeof(uint16_t)));
  expected_bytes =
      checked_sum(expected_bytes, checked_product(checked_product(rope_count, 2), sizeof(float)));
...
void write_qwen_layer_capture(const std::string& path, const QwenLayerCapture& capture) {
  ...
  if (!output)
    throw std::runtime_error("Qwen capture: cannot write " + path);
  output.write(reinterpret_cast<const char*>(&capture.header), sizeof(capture.header));
  ...
  if (!output)
    throw std::runtime_error("Qwen capture: write failed");
}
