...
void check(const SafeTensors& st, const std::string& name, const std::vector<int64_t>& shape) {
  ...
  if (!t)
    throw std::runtime_error("Qwen vision: missing " + name);
  uint64_t elements = 1;
  for (const int64_t extent : shape) {
    if (extent <= 0 ||
        elements > std::numeric_limits<uint64_t>::max() / static_cast<uint64_t>(extent))
  ...
  if (t->dtype != DType::kBF16 || t->shape != shape || t->nbytes != bytes || t->data == nullptr)
...
QwenVisionCheckpoint load_qwen3vl_vision_checkpoint(const SafeTensors& st) {
  ...
  if (flat)
    ...
    throw std::runtime_error("Qwen vision: visual patch embedding is absent");
  ...
  for (int i = 0; i < 27; ++i) {
    ...
    check(st, b + "norm1.weight", {1152});
    check(st, b + "norm1.bias", {1152});
    check(st, b + "norm2.weight", {1152});
    check(st, b + "norm2.bias", {1152});
    check(st, b + "attn.qkv.weight", {3456, 1152});
    check(st, b + "attn.qkv.bias", {3456});
    check(st, b + "attn.proj.weight", {1152, 1152});
    check(st, b + "attn.proj.bias", {1152});
    check(st, b + "mlp.linear_fc1.weight", {4304, 1152});
    check(st, b + "mlp.linear_fc1.bias", {4304});
    check(st, b + "mlp.linear_fc2.weight", {1152, 4304});
    check(st, b + "mlp.linear_fc2.bias", {1152});
  }
  ...
  for (const auto& kv : st.tensors())
    if (kv.first.rfind(p, 0) == 0)
      ++count;
  if (count != 351)
    throw std::runtime_error("Qwen vision: expected 351 tensors, found " + std::to_string(count));
  return {&st, p, {}};
}
...
QwenVisionPositions qwen3vl_vision_positions(const QwenImageGrid& g, int side, int merge) {
  ...
  out.learned.reserve(patches);
  out.rotary_thw.reserve(patches * 3);
  auto bucket = [side](int x, int extent) {
    // torch.linspace(0, side-1, extent).long(): conversion truncates.
    return extent == 1 ? 0
                       : static_cast<int>((static_cast<int64_t>(x) * (side - 1)) / (extent - 1));
  };
...
QwenMultimodalPlan qwen3vl_multimodal_plan(const std::vector<int32_t>& ids,
                                           const std::vector<QwenImageGrid>& grids, int32_t vs,
                                           int32_t pad, int32_t ve) {
  ...
  auto scalar = [&](size_t begin, size_t end) {
    for (size_t i = begin; i < end; ++i, ++next)
      for (int a = 0; a < 3; ++a)
        out.position_ids[static_cast<size_t>(a) * L + i] = next;
  };
  while (cursor < L) {
    ...
    if (image >= grids.size())
      throw std::runtime_error("Qwen vision: more vision blocks than grids");
    const auto& g = grids[image++];
    const size_t merged = g.merged_token_count();
    if (start + 1 + merged >= L)
      throw std::runtime_error("Qwen vision: truncated image-pad run");
    ...
    for (size_t j = 0; j < merged; ++j) {
      ...
      if (ids[row] != block_pad)
        throw std::runtime_error("Qwen vision: pad count disagrees with grid");
    ...
    if (ids[end] != ve)
      throw std::runtime_error("Qwen vision: image-pad run lacks vision_end");
    next += std::max({g.temporal, mh, mw});
    cursor = end; // vision_end is consumed by the next scalar run
  }
  if (image != grids.size())
    throw std::runtime_error("Qwen vision: fewer vision blocks than grids");
  return out;
}
...
void qwen3vl_vision_rope_tables(const QwenVisionPositions& p, std::vector<float>& cos,
  ...
  cos.resize(rows * head_dim);
  sin.resize(rows * head_dim);
...
void qwen3vl_decoder_rope_tables(const QwenMultimodalPlan& p, int tokens, std::vector<float>& cos,
                                 std::vector<float>& sin, int head_dim, float theta) {
  ...
  cos.resize(static_cast<size_t>(tokens) * head_dim);
  sin.resize(cos.size());
  ...
  for (int j = 0; j < head_dim / 2; ++j) {
    inv_freq[static_cast<size_t>(j)] = std::pow(static_cast<double>(theta), -2.0 * j / head_dim);
  }
  for (int r = 0; r < tokens; ++r)
    for (int j = 0; j < head_dim / 2; ++j) {
      ...
      const double inv = inv_freq[static_cast<size_t>(j)];
      const float a =
          static_cast<float>(p.position_ids[static_cast<size_t>(axis) * tokens + r] * inv);
      cos[static_cast<size_t>(r) * head_dim + j] = cos[static_cast<size_t>(r) * head_dim + j + 64] =
          std::cos(a);
      sin[static_cast<size_t>(r) * head_dim + j] = sin[static_cast<size_t>(r) * head_dim + j + 64] =
          std::sin(a);
    }
}
...
size_t QwenImageGrid::patch_count() const {
  ...
  if (t > std::numeric_limits<size_t>::max() / h)
    return 0;
  const size_t th = t * h;
  if (th > std::numeric_limits<size_t>::max() / w)
    return 0;
  return th * w;
}
...
QwenImageGrid qwen3vl_conditioning_grid(int width, int height) {
  ...
  if (grid.patch_count() == 0 || grid.patch_count() > kConditioningMaxPatches ||
...
size_t qwen3vl_conditioning_token_count(const std::vector<QwenImageGrid>& grids,
                                        size_t nonvision_tokens, size_t max_prompt_tokens) {
  ...
  for (const QwenImageGrid& grid : grids) {
    ...
    if (merged > max_prompt_tokens - total || size_t(2) > max_prompt_tokens - total - merged)
      throw std::runtime_error("Qwen image: conditioning exceeds max prompt tokens");
    total += merged + 2;
  }
  return total;
            ...
            for (int time = 0; time < temporal; ++time)
              for (int py = 0; py < patch; ++py)
                for (int px = 0; px < patch; ++px) {
                  ...
                  out.rows[dst++] =
                      pixels[(static_cast<size_t>(y) * width + x) * 3 + channel] / 127.5f - 1.0f;
                }
  return out;
}
...
std::vector<int32_t> qwen3vl_image_block(const std::vector<int32_t>& label_ids,
  ...
  if (merged_tokens == 0)
    throw std::runtime_error("Qwen image: empty vision block");
