...
private:
  void twist() {
    for (int i = 0; i < 624; ++i) {
      ...
      state_[i] = state_[(i + 397) % 624] ^ (mixed >> 1) ^ ((mixed & 1u) ? 0x9908b0dfu : 0u);
    }
    left_ = 624;
    next_ = 0;
  }
...
void require_conv(const SafeTensors& ckpt, const std::string& name, int out_channels,
  ...
  require_tensor(ckpt, name + ".weight", {out_channels, in_channels, kernel, kernel, kernel},
                 summary);
...
std::vector<float> sample_keyframe_latents(const float* moments, const float* normal, int height,
  ...
  if (!moments || !normal || height <= 0 || width <= 0 || latents_mean.size() != channels ||
      latents_std.size() != channels) {
  ...
  for (int c = 0; c < channels; ++c) {
    if (!(latents_std[c] > 0.0f))
      throw std::runtime_error("keyframe encoder: invalid latent std");
    for (size_t i = 0; i < voxels; ++i) {
      ...
      const float logvar =
          std::clamp(moments[static_cast<size_t>(channels) * voxels + j], -30.0f, 20.0f);
...
EncoderWeightSummary validate_keyframe_encoder_weights(const SafeTensors& checkpoint) {
  if (!checkpoint.is_open())
    throw std::runtime_error("keyframe encoder: checkpoint is not open");
  ...
  for (int level = 0; level < 6; ++level) {
    ...
    for (int block = 0; block < 2; ++block) {
      ...
      if (input != output)
        require_conv(checkpoint, prefix + ".nin_shortcut", output, input, 1, &result);
    }
    if (space_down[level] == 2)
      require_conv(checkpoint, "encoder.down." + std::to_string(level) + ".downsample.conv", output,
                   output, 3, &result);
    previous = output;
  }
...
std::vector<float> torch_cpu_normal_seed42(size_t count) {
  if (count < 16)
    throw std::runtime_error("keyframe encoder: Torch normal field must have >=16 values");
  TorchMT19937 generator(42);
  std::vector<float> out(count);
  auto uniform = [&] {
    return static_cast<float>(generator.next() & 0xFFFFFFu) / 16777216.0f;
  };
  for (float& value : out)
    value = uniform();
