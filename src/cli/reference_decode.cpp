...
std::string tool_path(const char* name, const char* executable) {
  ...
  for (const auto& directory : {exe_dir, std::filesystem::current_path() / "external" / "ffmpeg" / "bin"}) {
    const auto candidate = directory / filename;
    if (std::filesystem::is_regular_file(candidate)) return candidate.u8string();
  }
...
void run(const std::vector<std::string>& args, const Sink& sink) {
  ...
  if (stderr_handle && stderr_handle != INVALID_HANDLE_VALUE)
    DuplicateHandle(GetCurrentProcess(), stderr_handle, GetCurrentProcess(),
                    &errors.value, 0, TRUE, DUPLICATE_SAME_ACCESS);
  if (!errors.value) errors.value = CreateFileW(L"NUL", GENERIC_WRITE,
  ...
  if (!CreateProcessW(nullptr, command.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW,
    ...
    throw std::runtime_error("reference decoder: cannot launch " + args.front() + " (install FFmpeg and ffprobe)");
  ...
  try {
    ...
    DWORD count = 0;
    for (;;) {
      if (!ReadFile(reader.value, bytes.data(), static_cast<DWORD>(bytes.size()), &count, nullptr)) {
...
void run(const std::vector<std::string>& args, const Sink& sink) {
  ...
  int descriptors[2];
  if (pipe(descriptors)) throw std::runtime_error("reference decoder: cannot create pipe");
  const pid_t child = fork();
  if (child < 0) { close(descriptors[0]); close(descriptors[1]); throw std::runtime_error("reference decoder: fork failed"); }
  if (child == 0) {
    close(descriptors[0]);
  ...
  try {
    ...
    for (;;) {
      ...
      if (count < 0) throw std::runtime_error("reference decoder: pipe read failed");
      if (count == 0) break;
  ...
  if (waited < 0 || !WIFEXITED(status) || WEXITSTATUS(status) != 0)
    throw std::runtime_error("reference decoder: " + args.front() + " failed (install FFmpeg and ffprobe)");
}
#endif
...
double duration_of(const json::Value* value) {
  ...
  if (!duration || !duration->is_string() || duration->as_string() == "N/A") return 0;
...
double start_of(const json::Value* value) {
  ...
  if (!start || !start->is_string() || start->as_string() == "N/A") return 0;
  const double result = std::stod(start->as_string());
  if (!std::isfinite(result)) throw std::runtime_error("reference media: invalid stream start time");
  return result;
}
...
ReferenceMedia decode_reference_file(const std::string& path, bool video, const char* executable) {
#if !SLOPFAB_WITH_FFMPEG
  (void)path; (void)video; (void)executable;
  throw std::runtime_error("reference file decoding requires SLOPFAB_WITH_FFMPEG=ON; DLL hosts can supply decoded frames/PCM");
#else
  const auto input = std::filesystem::absolute(std::filesystem::u8path(path));
  if (!std::filesystem::is_regular_file(input)) throw std::runtime_error("reference media file not found: " + path);
  std::string metadata;
  run({tool_path("ffprobe", executable), "-v", "error", "-show_entries",
       "format=duration:stream=index,codec_type,width,height,duration,start_time,sample_rate,channels", "-of", "json", input.u8string()},
      [&](const uint8_t* bytes, size_t count) {
        if (metadata.size() + count > 1024 * 1024) throw std::runtime_error("reference decoder: excessive metadata");
  ...
  if (!streams) throw std::runtime_error("reference media: no streams");
  ...
  for (const auto& stream : streams->as_array()) {
    ...
    if (kind && kind->as_string() == "video" && !picture) picture = &stream;
    if (kind && kind->as_string() == "audio" && !sound) sound = &stream;
  }
  const auto* selected = video ? picture : sound;
  if (!selected) throw std::runtime_error(video ? "reference media: no video stream" : "reference media: no audio stream");
  double duration = duration_of(selected);
  if (duration == 0) duration = duration_of(info.find("format"));
  if (!std::isfinite(duration) || duration < 2 || duration > 15)
    throw std::runtime_error("reference media: file duration must be known and between 2 and 15 seconds; trim the source first");
  ...
  if (video) {
    ...
    if (!width_field || !height_field) throw std::runtime_error("reference media: missing video dimensions");
    ...
    if (std::ceil(duration * 24) * static_cast<double>(frame_bytes) > 2.0 * 1024 * 1024 * 1024)
      throw std::runtime_error("reference decoder: decoded video exceeds 2 GiB; resize the source first");
    std::vector<uint8_t> frame(frame_bytes);
    size_t filled = 0, index = 0;
    run({ffmpeg, "-v", "error", "-nostdin", "-noautorotate", "-i", input.u8string(),
         "-map", "0:v:0", "-an", "-vf", "setpts=PTS-STARTPTS,fps=24", "-t", std::to_string(duration),
         "-pix_fmt", "rgb24", "-f", "rawvideo", "pipe:1"},
        [&](const uint8_t* bytes, size_t count) {
          while (count) {
            const size_t take = std::min(count, frame_bytes - filled);
            std::memcpy(frame.data() + filled, bytes, take);
            filled += take; bytes += take; count -= take;
            if (filled == frame_bytes) {
              const double timestamp = static_cast<double>(index++) / 24;
              if (timestamp < duration) result.append_frame(frame.data(), frame.size(), width, height, size_t(width) * 3, 3, timestamp);
              else throw std::runtime_error("reference decoder: too many video frames");
              filled = 0;
            }
          }
        });
    if (filled) throw std::runtime_error("reference decoder: truncated RGB frame");
  }
  if (sound) {
    ...
    if (!rate_field || !channels_field) throw std::runtime_error("reference audio: missing PCM format");
    ...
    if (rate <= 0 || (channels64 != 1 && channels64 != 2))
      throw std::runtime_error("reference audio: expected a positive native rate and mono or stereo sound");
    ...
    if (start >= duration) throw std::runtime_error("reference audio: soundtrack starts after the video ends");
    ...
    const std::string audio_filter = relative_start < 0
        ? "atrim=start=" + std::to_string(-relative_start) + ",asetpts=PTS-STARTPTS"
        : "asetpts=PTS-STARTPTS";
    std::vector<uint8_t> pcm_bytes;
    run({ffmpeg, "-v", "error", "-nostdin", "-i", input.u8string(), "-map", "0:a:0",
         "-vn", "-af", audio_filter, "-t", std::to_string(audio_duration), "-f", "f32le", "pipe:1"},
        [&](const uint8_t* bytes, size_t count) {
    ...
    for (float& sample : samples) {
      if (!std::isfinite(sample)) throw std::runtime_error("reference decoder: non-finite audio sample");
      sample = std::clamp(sample, -1.0f, 1.0f);
    }
    if (video) result.set_audio(samples.data(), samples.size(), channels, rate, start);
    else result = ReferenceMedia::audio(samples.data(), samples.size(), channels, rate);
  }
  result.validate();
  return result;
#endif
