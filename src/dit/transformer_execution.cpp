...
void Transformer::Impl::build_modulation(const std::vector<float>& timesteps) {
  ...
  if (T <= 0)
    throw std::runtime_error("transformer: no distinct timesteps");

  const int code_dim =
      is_pruned_table_architecture(architecture) ? AdaLNTable::kRank : cfg.timestep_embed_dim;
  if (is_pruned_table_architecture(architecture)) {
    ...
    for (float& value : host_code)
      value = value / (1.0f + std::exp(-value));
  }
  if (d_code.size() < host_code.size())
    d_code.allocate(host_code.size());
  ...
  if (final_mod.size() < final_need)
    final_mod.allocate(final_need);
  ...
  for (size_t b = 0; b < blocks.size(); ++b) {
    if (is_pruned_table_architecture(architecture)) {
      ...
      linear.forward_f32(blocks[b].full_adaln, d_code.get(), T, mod.get() + b * per_block, ws);
    }
  }
  if (is_pruned_table_architecture(architecture)) {
...
void Transformer::Impl::run_block(const BlockWeights& b, const float* mod_base, int rows,
                                  __nv_bfloat16* x, const int32_t* adaln_idx, const float* cos,
                                  const float* sin, __nv_bfloat16* q, __nv_bfloat16* k,
                                  __nv_bfloat16* v, __nv_bfloat16* attn_out, __nv_bfloat16* normed,
                                  __nv_bfloat16* fused, __nv_bfloat16* act, __nv_bfloat16* branch,
                                  AttentionMode block_attention_mode, int layer) {
  ...
  if (layer >= 0)
    begin_block_capture(x, adaln_idx, cos, sin, rows, layer, block_attention_mode);
  if (layer >= 0)
    begin_graph_capture(x, adaln_idx, cos, sin, rows, layer, block_attention_mode);
  ...
  auto adapter_key = [&](const QuantWeight& weight) -> const void* {
    if (layer < 0 || !streamer || !streamer->contains(layer))
      return weight.data;
    ...
    for (int i = 0; i < 6; ++i)
      if (&weight == current[i])
        return logical[i]->data;
    return weight.data;
  };
  auto project = [&](const QuantWeight& weight, const __nv_bfloat16* dense,
                     const __nv_bfloat16* input, int count, __nv_bfloat16* output) {
    ...
    if (weight.bias != nullptr)
      throw std::runtime_error("transformer: exact H3 projection requires bias-free weights");
    ...
    if (weight.pre_quant_scale != nullptr) {
      __nv_bfloat16* scaled =
          ws.alloc_n<__nv_bfloat16>(static_cast<size_t>(count) * weight.in_features);
      cuda::launch_pre_quant_scale(projected_input, weight.pre_quant_scale, scaled, count,
                                   weight.in_features, stream.get());
    ...
    if (weight.convrot && weight.convrot_group > 0 &&
      ...
      __nv_bfloat16* rotated =
          ws.alloc_n<__nv_bfloat16>(static_cast<size_t>(count) * weight.in_features);
      cuda::launch_convrot(projected_input, rotated, count, weight.in_features,
                           weight.convrot_group, stream.get());
      projected_input = rotated;
    }
    const uint32_t tiled = static_cast<uint32_t>(count) / 64 * 64;
    if (tiled != 0) {
      cuda::launch_deterministic_bf16_gemm_nt(projected_input, dense, nullptr, output, tiled,
                                              static_cast<uint32_t>(weight.out_features),
                                              static_cast<uint32_t>(weight.in_features),
                                              DenseGemmBias::kNone, 0, 0, stream.get());
    }
    if (tiled != static_cast<uint32_t>(count)) {
      cuda::launch_deterministic_scalar_gemm_nt(
          projected_input, dense, nullptr, output, static_cast<uint32_t>(count) - tiled,
          static_cast<uint32_t>(weight.out_features), static_cast<uint32_t>(weight.in_features),
          DenseGemmMode::kBFloat16, DenseGemmBias::kNone, tiled, tiled, stream.get());
    }
  ...
  if (carve.chunked_attention) {
    ...
    acfg.band_ranges = layer >= 0 && d_band.size() > 0 ? d_band.get() : nullptr;
    {
      ...
      for (int start = 0; start < rows; start += chunk) {
        ...
        prof.tick("attn.qkv_proj", stream.get());
        cuda::launch_head_rmsnorm(q, b.q_norm, n, cfg.num_attention_heads, cfg.attention_head_dim,
                                  eps, stream.get());
        if (cos != nullptr) {
          // H3 rotary tables store the 96 rotated channels per row.
          const size_t rope_off = static_cast<size_t>(start) * 96;
          cuda::launch_rope_h3(q, cos + rope_off, sin + rope_off, n, cfg.num_attention_heads,
                               cfg.attention_head_dim, stream.get());
        }
        prof.tick("attn.qknorm_rope", stream.get());
        cuda::attention_forward_query_chunk(stream.get(), q, k, v, attn_out, acfg, start, n);
    ...
    if (layer < 0)
      ++attention_routes.generic_refiner;
    else
    ...
    if (block_attention_mode == AttentionMode::kNone)
      backend = AttentionBackend::kBlocked;
    if (block_attention_mode == AttentionMode::kSage2)
    ...
    if (backend == AttentionBackend::kSol && rows == layout.total_rows()) {
      ...
      acfg.sol_pipeline =
          block_attention_mode == AttentionMode::kSolExperimental || sol_pipeline_diag;
    }
    if (layer >= 0 && !sol_capture_path.empty())
      capture_sol_inputs(q, k, v, rows, layer);
    if (layer >= 0 && is_vsa()) {
      cuda::vsa_attention_forward(stream.get(), q, k, v, attn_out, vsa_compressed.get(), vsa_config,
                                  ws);
      ++attention_routes.generic_main;
    } else if (block_attention_mode == AttentionMode::kExact) {
      cuda::launch_deterministic_h3_attention(
          stream.get(), q, k, v, attn_out, acfg.band_ranges, static_cast<uint32_t>(rows),
          static_cast<uint32_t>(cfg.num_attention_heads),
          static_cast<uint32_t>(cfg.attention_head_dim), acfg.effective_scale());
      if (layer < 0)
        ++attention_routes.exact_refiner_full;
      else if (acfg.band_ranges == nullptr)
        ++attention_routes.exact_main_full;
      else
        ++attention_routes.exact_main_banded;
    } else {
      const auto attention_plan =
          cuda::AttentionPlan::compile(acfg, cfg.num_attention_heads, backend);
    ...
    if (layer >= 0)
      capture_block_attention(attn_out);
    diagnose("attention", attn_out, size_t(rows) * inner, layer);
    const char* label =
        layer >= 0 && is_vsa()                          ? "attn.vsa-h3"
        : block_attention_mode == AttentionMode::kExact ? "attn.exact"
        : backend == AttentionBackend::kFused           ? "attn.flash2"
        : backend == AttentionBackend::kSage2           ? "attn.sage2"
        : backend == AttentionBackend::kSol
            ? (block_attention_mode == AttentionMode::kSolExperimental ? "attn.sol.experimental"
                                                                       : "attn.sol")
            : "attn.none";
    ...
    if (layer >= 0 && is_vsa()) {
      ...
      for (int start = 0; start < rows; start += chunk) {
        const int n = std::min(chunk, rows - start);
        cuda::launch_rmsnorm_modulate(x + size_t(start) * hidden, b.norm1, scale_msa, shift_msa,
        ...
        cuda::vsa_add_compression(stream.get(), attn_out + size_t(start) * inner, fused,
                                  vsa_compressed.get(), start, n, vsa_config);
      }
...

    {
      ...
      for (int start = 0; start < rows; start += chunk) {
        const int n = std::min(chunk, rows - start);
        const size_t off = static_cast<size_t>(start) * hidden;
        project(b.out_proj, dout, attn_out + static_cast<size_t>(start) * inner, n, branch);
  ...
  if (layer >= 0)
    capture_block_attention_residual(x);
