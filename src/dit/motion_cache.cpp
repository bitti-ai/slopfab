...
void MotionCacheConfig::validate() const {
  if (!std::isfinite(reuse_threshold) || reuse_threshold < 0 || reuse_threshold > 1 ||
      ...
      warmup_steps < 2 || warmup_steps > 20 ||
      max_consecutive_skips < 1 || max_consecutive_skips > 10 ||
      !std::isfinite(start_percent) || !std::isfinite(end_percent) ||
      start_percent < 0 || end_percent > 1 || start_percent >= end_percent ||
      subsample_factor < 1 || subsample_factor > 32)
    throw std::invalid_argument("MotionCache: invalid threshold, motion strength, warmup, skip limit, range or subsampling");
}

MotionCache::MotionCache(const MotionCacheConfig& config, const SequenceLayout& l,
                         int video_dim, int audio_dim, int steps, float video_shift,
                         bool pin_audio)
    : config_(config), steps_(steps), frames_(l.num_latent_frames),
      channels_(video_dim / 4), start_sigma_(0), end_sigma_(0) {
  config_.validate();
  if (!enabled()) return;
  if (frames_ <= 0 || l.latent_height <= 0 || l.latent_width <= 0 ||
      l.latent_height % 2 || l.latent_width % 2 || video_dim <= 0 || video_dim % 4 ||
      audio_dim <= 0 || l.num_audio_latents < 0 ||
      int64_t(l.num_video_rows) != int64_t(frames_) * (l.latent_height / 2) * (l.latent_width / 2) ||
      int64_t(l.num_audio_rows) != 2 * int64_t(l.num_audio_latents) ||
      steps <= 0 || !std::isfinite(video_shift) || video_shift <= 0)
  ...
  spatial_ = size_t((l.latent_height + stride - 1) / stride) *
             ((l.latent_width + stride - 1) / stride);
    ...
    for (int f = 0; f < frames_; ++f)
      for (int h = 0; h < l.latent_height; h += stride)
        for (int w = 0; w < l.latent_width; w += stride) {
          const size_t row = (size_t(f) * (l.latent_height / 2) + h / 2) *
                             (l.latent_width / 2) + w / 2;
...
std::vector<float> MotionCache::sample(const float* data, const std::vector<size_t>& indices) const {
  std::vector<float> result;
  result.reserve(indices.size());
  for (size_t i : indices) result.push_back(data[i]);
  return result;
}
...
bool MotionCache::should_compute(int step, float sigma, const float* video, const float* audio) {
  bool compute = true;
  score_ = 0;
  if (enabled() && step >= config_.warmup_steps && step < steps_ - 1 &&
      sigma > end_sigma_ && sigma <= start_sigma_ && !video_residual_.empty() &&
      video_rate_ >= 0 && (audio_size_ == 0 || audio_rate_ >= 0)) {
    const float vs = video_rate_ * mean(sample(video, video_indices_), &previous_video_, weights_) / video_norm_;
    const float as = audio_size_ ? audio_rate_ * mean(sample(audio, audio_indices_), &previous_audio_) / audio_norm_ : 0;
    score_ = std::isfinite(vs) && std::isfinite(as) ? std::max(vs, as) :
             std::numeric_limits<float>::infinity();
    accumulated_ += score_;
    compute = !(accumulated_ < config_.reuse_threshold && consecutive_ < config_.max_consecutive_skips);
  }
  if (compute) {
    accumulated_ = 0;
    consecutive_ = 0;
  ...
  if (enabled() && config_.verbose)
    std::fprintf(stderr, "MotionCache: step %d %s, score %.5f, accumulated %.5f\n",
                 step + 1, compute ? "computed" : "reused", score_, accumulated_);
  return compute;
}

void MotionCache::update(float sigma, const float* video, const float* audio,
  ...
  if (frames_ > 1 && config_.motion_strength > 0) {
    ...
    for (int f = 0; f < frames_; ++f)
      for (size_t p = 0; p < spatial_; ++p) {
        ...
        for (int ch = 0; ch < channels_; ++ch) {
          ...
          delta += std::fabs((vi[i] + sigma * vo[i]) -
                             (vi[i - spatial_] + sigma * vo[i - spatial_]));
        }
    ...
    for (float& w : weights_) w /= normalization;
  }
  if (!previous_video_.empty()) {
    const float dv = mean(vi, &previous_video_, weights_);
    const float da = mean(ai, &previous_audio_);
    if (dv > epsilon) video_rate_ = mean(vo, &previous_vv_, weights_) / dv;
    if (da > epsilon) audio_rate_ = mean(ao, &previous_av_) / da;
  }
  ...
  for (size_t i = 0; i < video_size_; ++i) video_residual_[i] = vv[i] + video[i];
  for (size_t i = 0; i < audio_size_; ++i) audio_residual_[i] = av[i] + audio[i];
}

void MotionCache::reuse(const float* video, const float* audio, float* vv, float* av) const {
  if (video_residual_.empty()) throw std::logic_error("MotionCache: no residual to reuse");
  for (size_t i = 0; i < video_size_; ++i) vv[i] = video_residual_[i] - video[i];
  for (size_t i = 0; i < audio_size_; ++i) av[i] = audio_residual_[i] - audio[i];
}
}  // namespace slopfab::dit
