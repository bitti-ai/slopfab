...
struct Transformer::Impl {
  ...
  bool is_vsa() const {
    return model.compressed_attention;
  }
  ...
  bool compact_queries(AttentionMode mode) const {
    return !is_vsa() && query_chunking && mode == AttentionMode::kFlash2 &&
           ...
           row_chunk % cuda::attention_fused_query_tile() == 0 && block_capture_path.empty() &&
           graph_capture_path.empty() && transformer_capture_path.empty() &&
           sol_capture_path.empty();
  }
  ...
  void begin_block_capture(const __nv_bfloat16* residual, const int32_t* selectors,
                           const float* cosine, const float* sine, int rows, int layer,
                           AttentionMode mode);

  void capture_block_qkv(const __nv_bfloat16* q, const __nv_bfloat16* k, const __nv_bfloat16* v);
  ...
  void begin_graph_capture(const __nv_bfloat16* residual, const int32_t* selectors,
                           const float* cosine, const float* sine, int rows, int layer,
                           AttentionMode mode);
  ...
  void capture_transformer_text_stage(const __nv_bfloat16* values, int rows, int dim);

  void begin_transformer_forward_capture(const float* video_latents, const float* audio_latents,
  ...
  void finish_transformer_capture(const float* video_velocity, const float* audio_velocity);

  void capture_sol_inputs(const __nv_bfloat16* q, const __nv_bfloat16* k, const __nv_bfloat16* v,
                          int rows, int layer);
  ...
  Impl() {
    ...
    if (native != nullptr && native[0] == '1')
      linear.set_native(true);
    ...
    const char* transformer_capture_env = std::getenv("SLOPFAB_H3_TRANSFORMER_CAPTURE");
    const char* diag = std::getenv("SLOPFAB_TENSOR_DIAG");
    tensor_diag = diag != nullptr && diag[0] == '1';
    if (tensor_diag)
      d_tensor_diag.allocate(1);
    ...
    if (capture != nullptr && *capture != '\0') {
      ...
      if (step != nullptr && *step != '\0')
        sol_capture_step = std::atoi(step);
      if (layer != nullptr && *layer != '\0')
        sol_capture_layer = std::atoi(layer);
    }
    if (block_capture_env && *block_capture_env) {
      ...
      if (step && *step)
        block_capture_step = std::atoi(step);
      if (layer && *layer)
        block_capture_layer = std::atoi(layer);
    }
    if (graph_capture_env && *graph_capture_env) {
      ...
      if (step && *step)
        graph_capture_step = std::atoi(step);
    }
    if (transformer_capture_env && *transformer_capture_env) {
      ...
      if (step && *step)
        transformer_capture_step = std::atoi(step);
    }
  }
  ...
  void require_loaded(const char* what) const {
    if (!loaded())
      throw std::runtime_error(std::string("transformer: ") + what + " called before load()");
  }
  ...
  void run_block(const BlockWeights& b, const float* mod_base, int rows, __nv_bfloat16* x,
                 ...
                 __nv_bfloat16* k, __nv_bfloat16* v, __nv_bfloat16* attn_out, __nv_bfloat16* normed,
                 __nv_bfloat16* fused, __nv_bfloat16* act, __nv_bfloat16* branch,
                 AttentionMode block_attention_mode, int layer = -1);
};
} // namespace slopfab::dit
