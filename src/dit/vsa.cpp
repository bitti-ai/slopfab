...
VsaTiles build_vsa_tiles(const SequenceLayout& layout) {
  ...
  if (t <= 0 || h <= 0 || w <= 0 || layout.latent_height % 2 || layout.latent_width % 2 ||
      layout.num_text < 0 || layout.num_audio_rows < 0 || layout.num_condition_video != 0 ||
      layout.num_condition_audio != 0 || int64_t(t) * h * w != layout.num_video_rows)
