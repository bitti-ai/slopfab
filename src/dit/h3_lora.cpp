...
bool supported(const std::string& name) {
  ...
  if (name.compare(0, 7, "blocks.") == 0)
    start = 7;
  else if (name.compare(0, 21, "token_refiner.blocks.") == 0)
    start = 21;
  else
    return false;
...
bool validate_target(const SafeTensors& base, const std::string& name, const LoraFactors& f) {
  ...
  const bool rebase = adaln && base.find("adaln_t_table") &&
                      shape == std::vector<int64_t>{f.out, dit::AdaLNTable::kRank} &&
                      f.in != dit::AdaLNTable::kRank;
  if (!rebase &&
      (shape != std::vector<int64_t>{int64_t(f.out) * (split ? 3 : 1), packed ? f.in / 2 : f.in} ||
       (packed && f.in % 2)))
    throw std::runtime_error("LoRA: base model dimensions do not match " + name);
  if (adaln && (!base.find("adaln_t_table") ||
                shape != std::vector<int64_t>{f.out, dit::AdaLNTable::kRank} ||
                (w.dtype != DType::kF32 && w.dtype != DType::kF16 && w.dtype != DType::kBF16)))
    throw std::runtime_error(
        "LoRA: AdaLN updates require floating-point pruned rank-8 base projections: " + name);
  if ((name == "video_patch_proj" || name == "final_layer.video_out") &&
      (w.dtype != DType::kF32 && w.dtype != DType::kF16 && w.dtype != DType::kBF16))
...
void convert_output_basis(LoraFactors& f, bool needed) {
  if (needed) {
    if (f.out % 2)
      throw std::runtime_error("LoRA: SwiGLU output must have two equal halves");
...
void AdaLNBasis::load(const SafeTensors& base, const SafeTensors& adapter, int width) {
  ...
  if (t.shape != std::vector<int64_t>{dit::AdaLNTable::kRows, dit::AdaLNTable::kRank} ||
      e.shape != std::vector<int64_t>{dit::AdaLNTable::kRows, width})
    throw std::runtime_error(
        "LoRA: AdaLN timestep grid must match the base's 1025 rows and adapter width");
  rows = dit::AdaLNTable::kRows;
  columns = dit::AdaLNTable::kRank + 1;
  full = width;
  grid = values(e);
  table = values(t);
  ...
  for (int col = 0; col < columns; ++col) {
    for (int row = 0; row < rows; ++row)
      q[size_t(col) * rows + row] =
          col == columns - 1 ? 1 : table[size_t(row) * (columns - 1) + col];
    for (int pass = 0; pass < 2; ++pass)
      for (int j = 0; j < col; ++j) {
        double dot = 0;
        for (int row = 0; row < rows; ++row)
          dot += q[size_t(j) * rows + row] * q[size_t(col) * rows + row];
        r[size_t(j) * columns + col] += dot;
        for (int row = 0; row < rows; ++row)
          q[size_t(col) * rows + row] -= dot * q[size_t(j) * rows + row];
      }
    double norm = 0;
    for (int row = 0; row < rows; ++row)
      norm += std::pow(q[size_t(col) * rows + row], 2);
    norm = std::sqrt(norm);
    if (!(norm > 1e-12) || !std::isfinite(norm))
      throw std::runtime_error("LoRA: singular AdaLN timestep table");
    r[size_t(col) * columns + col] = norm;
    for (int row = 0; row < rows; ++row)
      q[size_t(col) * rows + row] /= norm;
  }
  map.assign(size_t(full) * columns, 0);
  for (int feature = 0; feature < full; ++feature) {
    for (int col = 0; col < columns; ++col)
      for (int row = 0; row < rows; ++row)
        map[size_t(feature) * columns + col] +=
            q[size_t(col) * rows + row] * grid[size_t(row) * full + feature];
    for (int col = columns - 1; col >= 0; --col) {
      auto& v = map[size_t(feature) * columns + col];
      for (int j = col + 1; j < columns; ++j)
        v -= r[size_t(col) * columns + j] * map[size_t(feature) * columns + j];
...
double AdaLNBasis::project(LoraFactors& f, std::vector<float>& bias) const {
  if (f.in != full)
    throw std::runtime_error("LoRA: inconsistent full AdaLN input widths");
  ...
  for (int rank = 0; rank < f.rank; ++rank) {
    ...
    for (int row = 0; row < rows; ++row) {
      ...
      for (int k = 0; k < full; ++k)
        actual += double(f.a[size_t(rank) * full + k]) * grid[size_t(row) * full + k];
      for (int col = 0; col < columns - 1; ++col)
        predicted += a[size_t(rank) * (columns - 1) + col] *
                     double(table[size_t(row) * (columns - 1) + col]);
  ...
  if (!std::isfinite(relative) || relative > 0.01)
    throw std::runtime_error(
        "LoRA: AdaLN conversion exceeds 1% relative fit error; use a matching timestep grid or a pruned adapter");
  bias.resize(f.out);
  for (int row = 0; row < f.out; ++row) {
    double sum = 0;
    for (int rank = 0; rank < f.rank; ++rank)
      sum += f.b[size_t(row) * f.rank + rank] * constant[rank];
    bias[row] = static_cast<float>(sum);
    if (!std::isfinite(bias[row]))
      throw std::runtime_error("LoRA: AdaLN bias conversion overflow");
  }
  f.in = columns - 1;
  f.a = std::move(a);
  for (float v : f.a)
    if (!std::isfinite(v))
      throw std::runtime_error("LoRA: AdaLN conversion overflow");
  return relative;
}
}
