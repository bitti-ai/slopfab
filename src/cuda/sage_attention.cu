...
size_t buffer_bytes(const AttentionConfig& c, int kvh, bool fp16_v) {
  ...
  const size_t qsn =
      static_cast<size_t>(c.num_heads) * ceil_div(c.seq_len, kQBlock) * (kQBlock / kQWarp);
...
__global__ void key_mean(const __nv_bfloat16* k, float* mean, int seq, int heads, int dim) {
  ...
  if (threadIdx.x < 32) {
    ...
    for (int dlt = 16; dlt; dlt >>= 1)
      sum += __shfl_xor_sync(0xffffffffu, sum, dlt);
    if (threadIdx.x == 0)
      mean[hd] = sum / seq;
  }
}
...
__global__ void quant_qk(const __nv_bfloat16* in, int8_t* out, float* scales, const float* mean,
                         int seq, int heads, int dim, int groups) {
  ...
  amax = warp_max(amax);
  if ((threadIdx.x & 31) == 0)
    sm[threadIdx.x >> 5] = amax;
  __syncthreads();
  if (threadIdx.x < 32) {
    amax = threadIdx.x < 8 ? sm[threadIdx.x] : 0.0f;
    amax = warp_max(amax);
    if (threadIdx.x == 0)
      sm[0] = fmaxf(amax / 127.0f, 1.0e-8f);
  }
  __syncthreads();
  const float scale = sm[0];
  if (threadIdx.x == 0)
    scales[h * groups + group] = scale;
  for (int i = threadIdx.x; i < Rows * dim; i += blockDim.x) {
    const int s = s0 + i / dim, d = i % dim;
    if (s < seq) {
      float x = __bfloat162float(in[(static_cast<size_t>(s) * heads + h) * dim + d]);
...
__global__ void quant_v(const __nv_bfloat16* v, int8_t* out, float* scales, int seq, int padded,
                        int heads, int dim) {
  const int hd = blockIdx.x;
  const int h = hd / dim, d = hd % dim;
  float amax = 0.0f;
  ...
  for (int s = threadIdx.x; s < padded; s += blockDim.x) {
    float x = s < seq ? __bfloat162float(v[(static_cast<size_t>(s) * heads + h) * dim + d]) / scale
                      : 0.0f;
...
void launch_official(cudaStream_t stream, const SageBuffers& b, __nv_bfloat16* out,
  ...
  auto kernel = qk_int_sv_f8_attn_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, D, DataType::kInt8,
                                         QuantGranularity::kPerWarp, QuantGranularity::kPerWarp,
                                         float, true, KernelOut, ComputeUnit::kCudaCore,
                                         MaskMode::kNone, false, true, false, true>;
  const size_t smem =
      std::max<size_t>(CTA_Q * D + CTA_K * D + CTA_K * D, CTA_Q * D * sizeof(__half));
  ...
  kernel<<<grid, block, smem, stream>>>(
      b.q, b.k, static_cast<int8_t*>(b.v), out, nullptr, b.qs, b.ks, b.vs, nullptr, c.seq_len,
      c.seq_len, groups, c.seq_len * c.num_heads * D, c.num_heads * D, D, c.seq_len * kvh * D,
      kvh * D, D, padded * kvh * D, padded, padded * kvh, c.seq_len * c.num_heads * D,
      c.num_heads * D, D, c.effective_scale());
...
void launch_ampere(cudaStream_t stream, const SageBuffers& b, __nv_bfloat16* out,
  ...
  constexpr int CTA_Q = 128, CTA_K = 64, WARP_Q = 32, WARP_K = 64;
  using KernelOut = nv_bfloat16;
  auto kernel = qk_int_sv_f16_attn_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, D, DataType::kInt8,
                                          QuantGranularity::kPerWarp, QuantGranularity::kPerWarp,
                                          float, false, KernelOut, ComputeUnit::kTensorCore,
  ...
  const int groups = c.num_heads / kvh;
  kernel<<<grid, block, smem, stream>>>(
      b.q, b.k, static_cast<__half*>(b.v), out, nullptr, b.qs, b.ks, nullptr, c.seq_len, c.seq_len,
      groups, c.seq_len * c.num_heads * D, c.num_heads * D, D, c.seq_len * kvh * D, kvh * D, D,
      c.seq_len * kvh * D, kvh * D, D, c.seq_len * c.num_heads * D, c.num_heads * D, D,
      c.effective_scale());
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}
...
bool sage2_supported(const AttentionConfig& cfg, int device, const char** reason) {
  ...
  static const char* kArch = "supports the shipped SM86-SM88 Ampere and SM120 Blackwell images";
  const char* why = nullptr;
  if (cfg.head_dim != 64 && cfg.head_dim != 128)
    why = kDim;
  else if (cfg.band_ranges != nullptr)
    why = kBand;
  else if (sage2_variant_for_compute_capability(device_compute_capability(device)) ==
           Sage2KernelVariant::kUnsupported)
    why = kArch;
  if (reason)
    *reason = why;
  return why == nullptr;
...
Sage2KernelVariant sage2_variant_for_compute_capability(int capability) noexcept {
  if (capability >= 86 && capability <= 88)
    return Sage2KernelVariant::kAmpereFp16;
  if (capability == 120)
    return Sage2KernelVariant::kBlackwellFp8;
...
void sage2_attention_forward(cudaStream_t stream, const __nv_bfloat16* q, const __nv_bfloat16* k,
  ...
  const bool ampere =
      sage2_variant_for_compute_capability(capability) == Sage2KernelVariant::kAmpereFp16;
  SageBuffers b =
      carve(ws.alloc(buffer_bytes(cfg, num_kv_heads, ampere)), cfg, num_kv_heads, ampere);
  const int padded = ceil_div(cfg.seq_len, kKBlock) * kKBlock;
  key_mean<<<num_kv_heads * cfg.head_dim, kThreads, 0, stream>>>(k, b.km, cfg.seq_len, num_kv_heads,
                                                                 cfg.head_dim);
  const int qgroups = ceil_div(cfg.seq_len, kQBlock) * (kQBlock / kQWarp);
  dim3 qgrid(qgroups, cfg.num_heads);
  quant_qk<kQWarp, false><<<qgrid, kThreads, 0, stream>>>(q, b.q, b.qs, nullptr, cfg.seq_len,
                                                          cfg.num_heads, cfg.head_dim, qgroups);
  const int kgroups = ceil_div(cfg.seq_len, kKBlock);
  dim3 kgrid(kgroups, num_kv_heads);
  quant_qk<kKBlock, true><<<kgrid, kThreads, 0, stream>>>(k, b.k, b.ks, b.km, cfg.seq_len,
                                                          num_kv_heads, cfg.head_dim, kgroups);
  if (ampere) {
    const size_t vn = static_cast<size_t>(cfg.seq_len) * num_kv_heads * cfg.head_dim;
    const int blocks = static_cast<int>(std::min<size_t>((vn + kThreads - 1) / kThreads, 65535));
    convert_v_f16<<<blocks, kThreads, 0, stream>>>(v, static_cast<__half*>(b.v), vn);
  } else {
    quant_v<<<num_kv_heads * cfg.head_dim, kThreads, 0, stream>>>(
        v, static_cast<int8_t*>(b.v), b.vs, cfg.seq_len, padded, num_kv_heads, cfg.head_dim);
  }
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
  if (ampere) {
    if (cfg.head_dim == 64)
      launch_ampere<64>(stream, b, out, cfg, num_kv_heads);
    else
      launch_ampere<128>(stream, b, out, cfg, num_kv_heads);
  } else {
    if (cfg.head_dim == 64)
      launch_official<64>(stream, b, out, cfg, num_kv_heads, padded);
    else
      launch_official<128>(stream, b, out, cfg, num_kv_heads, padded);
  }
}
