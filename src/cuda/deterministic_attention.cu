...
bool ranges_overlap(const void* a, uint64_t a_bytes, const void* b, uint64_t b_bytes) noexcept {
  ...
  if (begin_a > std::numeric_limits<uintptr_t>::max() - a_bytes ||
      begin_b > std::numeric_limits<uintptr_t>::max() - b_bytes)
    return true;
...
constexpr uint32_t kH3AttentionSharedLayoutBytes =
    2u * 64u * 128u * sizeof(__nv_bfloat16) + 16u * 128u * sizeof(__half) +
    64u * 64u * sizeof(float) + 64u * 64u * sizeof(__half) + 64u * 128u * sizeof(float) +
...
GridLimits cached_grid_limits() {
  ...
  std::call_once(once[device], [device] {
    ...
    int optin_shared = 0;
    SLOPFAB_CUDA_CHECK(
        cudaDeviceGetAttribute(&optin_shared, cudaDevAttrMaxSharedMemoryPerBlockOptin, device));
    ...
    const bool exact_h3_tuple = deterministic_h3_cuda_tuple_fits(
        properties.major, properties.minor, properties.name, driver_version, runtime_version,
        static_cast<uint32_t>(properties.maxThreadsPerBlock),
        static_cast<uint32_t>(
            std::max<int>(static_cast<int>(properties.sharedMemPerBlock), optin_shared)),
...
template <typename T> __device__ inline float prepared_value(const T* values, size_t index);

template <> __device__ inline float prepared_value(const __nv_bfloat16* values, size_t index) {
  return __half2float(__float2half_rn(__bfloat162float(values[index])));
}

template <> __device__ inline float prepared_value(const __half* values, size_t index) {
...
__global__ void
blocked_attention_kernel(const Input* __restrict__ query, const Input* __restrict__ key,
                         const Input* __restrict__ value, __nv_bfloat16* __restrict__ output,
  ...
  for (uint32_t key_base = 0; key_base < sequence; key_base += 128) {
    ...
    if (key_row < sequence) {
      ...
      for (uint32_t d = 0; d < head_dim; ++d) {
        score = fmaf(prepared_value(query, q_base + d), prepared_value(key, k_base + d), score);
      }
    ...
    const float correction = running_max == negative_infinity()
                                 ? 0.0f
                                 : deterministic_exp_nonpositive(running_max - next_max);
    const float exponential =
        key_row < sequence ? deterministic_exp_nonpositive(score - next_max) : 0.0f;
    __syncthreads();
    ...
    if (lane < head_dim) {
      ...
      for (uint32_t j = 0; j < 128 && key_base + j < sequence; ++j) {
        const size_t v_index =
            ((static_cast<size_t>(key_base + j) * heads + head) * head_dim) + lane;
        output_accumulator = canonicalize_subnormal(
            fmaf(score_or_probability[j], prepared_value(value, v_index), output_accumulator));
      }
    }
  ...
  if (lane < head_dim) {
    output_bits[lane] = static_cast<uint32_t>(__bfloat16_as_ushort(
        __float2bfloat16_rn(deterministic_float_divide(output_accumulator, running_sum))));
  }
  __syncthreads();
  if (lane < head_dim && (lane & 1u) == 0u) {
    const size_t output_index =
        ((static_cast<size_t>(output_row_offset + local_row) * heads + head) * head_dim) + lane;
    const uint32_t high = lane + 1 < head_dim ? output_bits[lane + 1] : 0u;
    reinterpret_cast<uint32_t*>(output)[output_index >> 1] = output_bits[lane] | (high << 16u);
  }
}
...
__global__ __launch_bounds__(1024, 1) void h3_attention_coop64_kernel(
    ...
    const int32_t* __restrict__ ranges, uint32_t sequence, uint32_t heads, uint32_t head_dim,
    float scale, uint32_t query_row_offset, uint32_t output_row_offset, uint32_t rows) {
  ...
  for (uint32_t i = tid; i < 64u * head_dim; i += blockDim.x) {
    ...
    query_stage[i] =
        query_row < sequence
            ? __float2bfloat16_rn(causal_bf16_value(
                  query[(static_cast<size_t>(query_row) * heads + head) * head_dim + d]))
    ...
    for (uint32_t key_base = range_start; key_base < range_stop; key_base += 64u) {
      for (uint32_t i = tid; i < 64u * head_dim; i += blockDim.x) {
        ...
        key_stage[i] = key_row < range_stop && key_row < sequence
                           ? __float2bfloat16_rn(causal_bf16_value(
                                 key[(static_cast<size_t>(key_row) * heads + head) * head_dim + d]))
                           : __float2bfloat16_rn(0.0f);
      }
      __syncthreads();
      if (warp < 16u) {
        ...
        for (uint32_t d_base = 0; d_base < head_dim; d_base += 16u) {
          ...
          wmma::load_matrix_sync(a, query_stage + query_tile * 16u * head_dim + d_base, head_dim);
          wmma::fragment<wmma::matrix_b, 16, 16, 16, __nv_bfloat16, wmma::col_major> b;
          wmma::load_matrix_sync(b, key_stage + key_tile * 16u * head_dim + d_base, head_dim);
          wmma::mma_sync(score_fragment, a, b, score_fragment);
        }
        wmma::store_matrix_sync(scores + query_tile * 16u * 64u + key_tile * 16u, score_fragment,
      ...
      for (uint32_t j = part * 4u; j < part * 4u + 4u; ++j) {
        ...
        const float exponential = score == negative_infinity()
                                      ? 0.0f
                                      : deterministic_exp_nonpositive(score - next_maximum[row]);
      ...
      for (uint32_t i = tid; i < 64u * head_dim; i += blockDim.x) {
        ...
        output_accumulators[i] =
            canonicalize_subnormal(__fmul_rn(output_accumulators[i], correction[output_row]));
      }
      __syncthreads();
      ...
      if (warp < total_tiles) {
        ...
        wmma::load_matrix_sync(pv_fragment,
      ...
      for (uint32_t key_part = 0; key_part < 64u; key_part += 16u) {
        for (uint32_t i = tid; i < 16u * head_dim; i += blockDim.x) {
          ...
          value_stage[i] =
              key_row < range_stop && key_row < sequence
                  ? __float2half_rn(causal_bf16_value(
                        value[(static_cast<size_t>(key_row) * heads + head) * head_dim + d]))
        ...
        if (warp < total_tiles) {
          ...
          wmma::load_matrix_sync(a, probabilities + tile_query * 16u * 64u + key_part, 64);
          wmma::fragment<wmma::matrix_b, 16, 16, 16, __half, wmma::row_major> b;
          wmma::load_matrix_sync(b, value_stage + d_tile * 16u, head_dim);
      ...
      if (warp < total_tiles) {
        const uint32_t tile_query = warp / d_tiles;
        const uint32_t d_tile = warp - tile_query * d_tiles;
        wmma::store_matrix_sync(output_accumulators + tile_query * 16u * head_dim + d_tile * 16u,
  ...
  for (uint32_t pair = tid; pair < 64u * pairs_per_row; pair += blockDim.x) {
    ...
    const uint32_t query_row = query_base + row;
    if (query_row >= query_row_offset && query_row < request_end && query_row < sequence) {
      const uint32_t output_row = output_row_offset + query_row - query_row_offset;
      const size_t output_index = (static_cast<size_t>(output_row) * heads + head) * head_dim + d;
      const uint16_t lo = __bfloat16_as_ushort(__float2bfloat16_rn(
          deterministic_float_divide(output_accumulators[row * head_dim + d], running_sum[row])));
      const uint16_t hi = __bfloat16_as_ushort(__float2bfloat16_rn(deterministic_float_divide(
          output_accumulators[row * head_dim + d + 1u], running_sum[row])));
...
template <bool Banded> void ensure_h3_shared_optin() {
  constexpr int kCachedDevices = 32;
  static std::once_flag once[kCachedDevices];
  int device = 0;
  SLOPFAB_CUDA_CHECK(cudaGetDevice(&device));
  ...
  std::call_once(once[device], [] {
    SLOPFAB_CUDA_CHECK(cudaFuncSetAttribute(h3_attention_coop64_kernel<Banded>,
...
__global__ void causal_gqa_attention_kernel(
    const __nv_bfloat16* __restrict__ query, const __nv_bfloat16* __restrict__ key,
    const __nv_bfloat16* __restrict__ value, __nv_bfloat16* __restrict__ output, uint32_t sequence,
  ...
  if (local_row >= rows || query_head >= query_heads)
    return;
  const uint32_t query_row = query_row_offset + local_row;
  const uint32_t kv_head = query_head / (query_heads / kv_heads);
  const size_t q_base = (static_cast<size_t>(query_row) * query_heads + query_head) * head_dim;
  ...
  for (uint32_t key_base = 0; key_base < causal_keys; key_base += 128) {
    ...
    if (key_row < causal_keys) {
      const size_t k_base = (static_cast<size_t>(key_row) * kv_heads + kv_head) * head_dim;
      for (uint32_t d = 0; d < head_dim; ++d) {
        const float product = canonicalize_subnormal(
            __fmul_rn(causal_bf16_value(query[q_base + d]), causal_bf16_value(key[k_base + d])));
    ...
    const float correction = running_max == negative_infinity()
                                 ...
                                 : deterministic_exp_nonpositive(running_max - next_max);
    const float exponential =
        key_row < causal_keys ? deterministic_exp_nonpositive(score - next_max) : 0.0f;
    __syncthreads();
    score_or_probability[lane] = __bfloat162float(__float2bfloat16_rn(exponential));
    reduction[lane] = exponential;
    __syncthreads();
    for (uint32_t width = 64; width != 0; width >>= 1) {
      if (lane < width)
    ...
    if (lane < head_dim) {
      ...
      for (uint32_t j = 0; j < 128 && key_base + j < causal_keys; ++j) {
        const size_t v_index =
            ((static_cast<size_t>(key_base + j) * kv_heads + kv_head) * head_dim) + lane;
        const float product = canonicalize_subnormal(
            __fmul_rn(score_or_probability[j], causal_bf16_value(value[v_index])));
        output_accumulator = canonicalize_subnormal(__fadd_rn(output_accumulator, product));
      }
    }
    running_sum = fmaf(running_sum, correction, reduction[0]);
    running_max = next_max;
  ...
  if (lane < head_dim && (lane & 1u) == 0u) {
    const size_t output_index =
        ((static_cast<size_t>(output_row_offset + local_row) * query_heads + query_head) *
         head_dim) +
        lane;
    const uint32_t high = lane + 1 < head_dim ? output_bits[lane + 1] : 0u;
    reinterpret_cast<uint32_t*>(output)[output_index >> 1] = output_bits[lane] | (high << 16u);
...
__global__ void prepare_attention_inputs_kernel(const __nv_bfloat16* query,
                                                const __nv_bfloat16* key,
                                                const __nv_bfloat16* value, __half* prepared_query,
                                                __half* prepared_key, __half* prepared_value,
                                                uint64_t elements) {
...
template <typename Input>
void launch_attention_impl(cudaStream_t stream, const Input* query, const Input* key,
  ...
  if (!query || !key || !value || !output || query == key || query == value || key == value ||
      sequence == 0 || heads == 0 || (head_dim != 64 && head_dim != 72 && head_dim != 128) ||
  ...
  if (!deterministic_attention_grid_fits(selected_rows, heads, limits.x, limits.y)) {
  ...
  blocked_attention_kernel<<<dim3(selected_rows, heads), 128, 0, stream>>>(
      query, key, value, output, sequence, heads, head_dim, scale, query_row_offset,
      output_row_offset, selected_rows);
...
bool deterministic_h3_attention_available() {
  const GridLimits limits = cached_grid_limits();
  return limits.exact_h3_tuple && limits.max_threads_per_block >= kH3AttentionThreads &&
         limits.max_shared_bytes_per_block >= kH3AttentionSharedBytes;
}

bool deterministic_h3_cuda_tuple_fits(int major, int minor, const char* model,
                                      int driver_api_version, int runtime_version,
  ...
  return major == 12 && minor == 0 && model != nullptr &&
         std::strcmp(model, "NVIDIA GeForce RTX 5090") == 0 && driver_api_version >= 13000 &&
         driver_api_version < 14000 && runtime_version == 13000 &&
         max_threads_per_block >= kH3AttentionThreads &&
         max_shared_bytes_per_block >= kH3AttentionSharedBytes;
}

bool deterministic_attention_grid_fits(uint64_t rows, uint64_t heads, uint64_t max_grid_x,
                                       uint64_t max_grid_y) noexcept {
  return rows != 0 && heads != 0 && rows <= max_grid_x && heads <= max_grid_y;
}

void launch_prepare_deterministic_attention_inputs(cudaStream_t stream, const __nv_bfloat16* query,
                                                   const __nv_bfloat16* key,
                                                   const __nv_bfloat16* value,
                                                   __half* prepared_query, __half* prepared_key,
                                                   __half* prepared_value, uint64_t elements) {
  if (!query || !key || !value || !prepared_query || !prepared_key || !prepared_value ||
      elements == 0 || elements > std::numeric_limits<uint32_t>::max()) {
    throw std::invalid_argument("deterministic attention: invalid preparation");
  }
  const uint64_t bytes = elements * sizeof(uint16_t);
  const void* buffers[] = {query, key, value, prepared_query, prepared_key, prepared_value};
...
void launch_deterministic_blocked_attention_f16(cudaStream_t stream, const __half* query,
                                                const __half* key, const __half* value,
                                                __nv_bfloat16* output, uint32_t sequence,
                                                uint32_t heads, uint32_t head_dim, float scale,
                                                uint32_t query_row_offset, uint32_t rows,
                                                uint32_t output_row_offset) {
  launch_attention_impl(stream, query, key, value, output, sequence, heads, head_dim, scale,
                        query_row_offset, rows, output_row_offset);
}

void launch_deterministic_blocked_attention(cudaStream_t stream, const __nv_bfloat16* query,
...
void launch_deterministic_h3_attention(cudaStream_t stream, const __nv_bfloat16* query,
                                       const __nv_bfloat16* key, const __nv_bfloat16* value,
                                       __nv_bfloat16* output, const int32_t* ranges,
                                       uint32_t sequence, uint32_t heads, uint32_t head_dim,
                                       float scale, uint32_t query_row_offset, uint32_t rows,
                                       uint32_t output_row_offset) {
  if (!query || !key || !value || !output || query == key || query == value || key == value ||
      sequence == 0 || heads == 0 || (head_dim != 64 && head_dim != 128) ||
      !is_exact_attention_scale(head_dim, scale) || query_row_offset > sequence) {
    throw std::invalid_argument("deterministic H3 attention: invalid tensor/configuration");
  }
  const uint32_t selected_rows = rows == 0 ? sequence - query_row_offset : rows;
  if (selected_rows == 0 || static_cast<uint64_t>(query_row_offset) + selected_rows > sequence ||
      static_cast<uint64_t>(output_row_offset) + selected_rows > sequence) {
  ...
  if (ranges_overlap(query, bytes, key, bytes) || ranges_overlap(query, bytes, value, bytes) ||
      ranges_overlap(key, bytes, value, bytes) || ranges_overlap(query, bytes, output, bytes) ||
      ranges_overlap(key, bytes, output, bytes) || ranges_overlap(value, bytes, output, bytes)) {
  ...
  if (limits.max_threads_per_block < kH3AttentionThreads ||
      limits.max_shared_bytes_per_block < kH3AttentionSharedBytes || !limits.exact_h3_tuple) {
    throw std::runtime_error("deterministic H3 attention: unqualified CUDA tuple or resources");
  }
  const uint32_t aligned_first = query_row_offset & ~63u;
  ...
  if (ranges) {
    ...
    h3_attention_coop64_kernel<true>
        <<<dim3(query_groups, heads), kH3AttentionThreads, kH3AttentionSharedBytes, stream>>>(
            query, key, value, output, ranges, sequence, heads, head_dim, scale, query_row_offset,
            output_row_offset, selected_rows);
  } else {
    ensure_h3_shared_optin<false>();
    h3_attention_coop64_kernel<false>
        <<<dim3(query_groups, heads), kH3AttentionThreads, kH3AttentionSharedBytes, stream>>>(
            query, key, value, output, nullptr, sequence, heads, head_dim, scale, query_row_offset,
            output_row_offset, selected_rows);
  }
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}

void launch_deterministic_causal_gqa_attention(cudaStream_t stream, const __nv_bfloat16* query,
                                               const __nv_bfloat16* key, const __nv_bfloat16* value,
                                               __nv_bfloat16* output, uint32_t sequence,
                                               ...
                                               uint32_t output_row_offset) {
  if (!query || !key || !value || !output || query == key || query == value || key == value ||
      sequence == 0 || query_heads == 0 || kv_heads == 0 || query_heads % kv_heads != 0 ||
      head_dim != 128 || !is_exact_attention_scale(head_dim, scale) ||
      query_row_offset > sequence) {
    throw std::invalid_argument("deterministic causal GQA attention: invalid tensor/configuration");
  }
  const uint32_t selected_rows = rows == 0 ? sequence - query_row_offset : rows;
  if (selected_rows == 0 || static_cast<uint64_t>(query_row_offset) + selected_rows > sequence ||
      static_cast<uint64_t>(output_row_offset) + selected_rows > sequence) {
    throw std::invalid_argument("deterministic causal GQA attention: invalid row range");
  }
  const uint64_t query_elements = static_cast<uint64_t>(sequence) * query_heads * head_dim;
  const uint64_t kv_elements = static_cast<uint64_t>(sequence) * kv_heads * head_dim;
  if (query_elements > std::numeric_limits<uint32_t>::max() ||
      kv_elements > std::numeric_limits<uint32_t>::max()) {
    throw std::out_of_range("deterministic causal GQA attention: uint32 indexing overflow");
  }
  ...
  if (ranges_overlap(query, query_bytes, output, query_bytes) ||
    ...
    throw std::invalid_argument("deterministic causal GQA attention: output must be distinct");
  }
  const GridLimits limits = cached_grid_limits();
  if (!deterministic_attention_grid_fits(selected_rows, query_heads, limits.x, limits.y)) {
    throw std::out_of_range("deterministic causal GQA attention: CUDA grid limit exceeded");
  }
  causal_gqa_attention_kernel<<<dim3(selected_rows, query_heads), 128, 0, stream>>>(
      query, key, value, output, sequence, query_heads, kv_heads, head_dim, scale, query_row_offset,
      output_row_offset, selected_rows);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}
