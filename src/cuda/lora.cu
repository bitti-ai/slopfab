...
size_t LoraRunner::weight_bytes() const {
  size_t result = 0;
  for (const auto& entry : factors_) for (const auto& f : entry.second)
    result += f.a.nbytes() + f.b.nbytes();
  return result;
}

void LoraRunner::apply(const void* key, const __nv_bfloat16* input, int rows,
                        __nv_bfloat16* output, cublasHandle_t blas,
                        cudaStream_t stream, bool exact) {
  const auto found = factors_.find(key);
  if (found == factors_.end()) return;
  auto gemm = [&](const __nv_bfloat16* x, const __nv_bfloat16* w,
                  __nv_bfloat16* y, int m, int n, int k) {
    if (exact) {
      const int tiled = n % 16 == 0 && k % 16 == 0 ? m / 64 * 64 : 0;
      if (tiled) launch_deterministic_bf16_gemm_nt(x, w, nullptr, y,
          tiled, n, k, DenseGemmBias::kNone, 0, 0, stream);
      if (tiled != m) launch_deterministic_scalar_gemm_nt(x, w, nullptr, y,
          m - tiled, n, k, DenseGemmMode::kBFloat16, DenseGemmBias::kNone,
          tiled, tiled, stream);
    } else {
      const float one = 1.0f, zero = 0.0f;
      SLOPFAB_CUBLAS_CHECK(cublas_gemm_ex(blas, CUBLAS_OP_T, CUBLAS_OP_N,
          n, m, k, &one, w, CUDA_R_16BF, k, x, CUDA_R_16BF, k,
          &zero, y, CUDA_R_16BF, n, CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT));
    }
  };
