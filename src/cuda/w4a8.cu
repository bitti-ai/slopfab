...
__global__ void dequant_weight_kernel(
    ...
    const float* __restrict__ codebook, int8_t* __restrict__ output,
    size_t packed_count, int packed_per_row, int in_features,
    int groups_per_row, int group_size) {
...
__global__ void quantize_activation_kernel(
    const __half* __restrict__ input, int8_t* __restrict__ output,
...
#pragma unroll
    for (int stage = 0; stage < 4; ++stage) {
      ...
      dst[tid] = 0.5f * h4(digit, src[base], src[base + stride],
                           src[base + 2 * stride], src[base + 3 * stride]);
      __syncthreads();
      float* swap = src;
      src = dst;
      dst = swap;
...
__global__ void dequant_output_kernel(int32_t* input_output,
                                      const float* activation_scale,
                                      const float* weight_scale, size_t count,
                                      int cols) {
  const size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  if (index >= count) return;
  const int col = static_cast<int>(index % cols);
  const int row = static_cast<int>(index / cols);
  const float value = static_cast<float>(input_output[index]) *
...
void launch_dequant_w4a8_weight(const int8_t* packed,
  ...
  if (out_features <= 0 || in_features <= 0 || group_size <= 0 ||
      (in_features & 1) != 0 || in_features % group_size != 0)
  ...
  dequant_weight_kernel<<<static_cast<unsigned>((count + kThreads - 1) / kThreads),
                          kThreads, 0, stream>>>(
