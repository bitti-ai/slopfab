...
namespace {
__global__ void split_qkv_kernel(const __nv_bfloat16* qkv, __nv_bfloat16* q, __nv_bfloat16* k,
                                 __nv_bfloat16* v, int hidden) {
...
__global__ void add_positions_exact_kernel(__nv_bfloat16* x, const __nv_bfloat16* table,
  ...
  x[at] = exact_vision_bf16_result(
      __fadd_rn(exact_vision_bf16(x[at]),
                exact_vision_bf16(table[static_cast<size_t>(index[row]) * hidden + column])));
}

__global__ void scatter_add_exact_kernel(const __nv_bfloat16* source, const int32_t* index,
  ...
  destination[dst] = exact_vision_bf16_result(
      __fadd_rn(exact_vision_bf16(destination[dst]), exact_vision_bf16(source[at])));
}
} // namespace

void qwen_vision_add_positions_exact(__nv_bfloat16* x, const __nv_bfloat16* table,
                                     const int32_t* index, int rows, int hidden,
                                     cudaStream_t stream) {
  if (!x || !table || !index || rows <= 0 || hidden <= 0)
    throw std::invalid_argument("qwen vision exact position: invalid input");
  const size_t count = static_cast<size_t>(rows) * hidden;
  add_positions_exact_kernel<<<static_cast<unsigned>((count + 255) / 256), 256, 0, stream>>>(
      x, table, index, rows, hidden);
...
void qwen_vision_split_qkv_exact(const __nv_bfloat16* fused, __nv_bfloat16* query,
  ...
  split_qkv_kernel<<<dim3(rows, (hidden + 255) / 256), 256, 0, stream>>>(fused, query, key, value,
                                                                         hidden);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}

void qwen_vision_scatter_add_exact(const __nv_bfloat16* source, const int32_t* index,
                                   __nv_bfloat16* destination, int rows, int hidden,
                                   cudaStream_t stream) {
  if (!source || !index || !destination || rows <= 0 || hidden <= 0)
    throw std::invalid_argument("qwen vision exact scatter: invalid input");
  const size_t count = static_cast<size_t>(rows) * hidden;
  scatter_add_exact_kernel<<<static_cast<unsigned>((count + 255) / 256), 256, 0, stream>>>(
      source, index, destination, rows, hidden);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}
...
void qwen_vision_block_forward(cublasHandle_t handle, cudaStream_t stream, LinearRunner& linear,
                               const QwenVisionBlockWeights& w, const float* cos, const float* sin,
                               __nv_bfloat16* x, int rows, QwenVisionBlockScratch s, Workspace& ws,
                               float eps) {
  ...
  qwen_vision_attention(handle, stream, s.qkv, cos, sin, s.q, s.k, s.v, s.branch, rows, heads,
                        head_dim, ws);
...
void qwen_vision_block_forward_exact(cudaStream_t stream, const QwenVisionBlockWeights& w,
  ...
  if (rows <= 0 || !x || !cos || !sin || !s.normed || !s.qkv || !s.q || !s.k || !s.v || !s.branch ||
      !s.mlp || w.qkv.format != QuantFormat::kBF16 ||
      w.attention_out.format != QuantFormat::kBF16 || w.mlp_fc1.format != QuantFormat::kBF16 ||
      w.mlp_fc2.format != QuantFormat::kBF16) {
    throw std::invalid_argument("qwen vision exact block: invalid input");
  }
  auto projection = [&](const QuantWeight& weight, const __nv_bfloat16* input,
                        __nv_bfloat16* output) {
    const uint32_t count = static_cast<uint32_t>(rows);
    const uint32_t tiled = count / 64u * 64u;
    if (tiled != 0) {
      launch_deterministic_bf16_gemm_nt(input, static_cast<const __nv_bfloat16*>(weight.data),
                                        weight.bias, output, tiled, weight.out_features,
                                        weight.in_features, DenseGemmBias::kBFloat16, 0, 0, stream);
    }
    if (tiled != count) {
      launch_deterministic_scalar_gemm_nt(input, weight.data, weight.bias, output, count - tiled,
                                          weight.out_features, weight.in_features,
                                          DenseGemmMode::kBFloat16, DenseGemmBias::kBFloat16, tiled,
                                          tiled, stream);
    }
  };
  launch_layernorm_affine(x, w.norm1_weight, w.norm1_bias, s.normed, rows, hidden, eps, stream);
  ...
  launch_deterministic_blocked_attention(stream, s.q, s.k, s.v, s.branch,
                                         static_cast<uint32_t>(rows), heads, head_dim,
                                         exact_attention_scale(head_dim));
  projection(w.attention_out, s.branch, s.normed);
  text::launch_residual_add_exact(x, s.normed, static_cast<size_t>(rows) * hidden, stream);
  launch_layernorm_affine(x, w.norm2_weight, w.norm2_bias, s.normed, rows, hidden, eps, stream);
  projection(w.mlp_fc1, s.normed, s.mlp);
  launch_gelu_tanh_exact(s.mlp, static_cast<size_t>(rows) * intermediate, stream);
...
void qwen_vision_patch_embed(LinearRunner& linear, const QuantWeight& projection,
                             ...
                             const int32_t* index, __nv_bfloat16* x, int rows, Workspace& ws,
                             cudaStream_t stream) {
...
void qwen_vision_patch_embed_exact(cudaStream_t stream, const QuantWeight& projection,
  ...
  if (rows <= 0 || !pixels || !position_table || !position_index || !output ||
      projection.format != QuantFormat::kBF16 || projection.out_features != 1152 ||
      projection.in_features != 1536 || projection.bias == nullptr ||
  ...
  if (tiled != 0)
    launch_deterministic_bf16_gemm_nt(
        pixels, static_cast<const __nv_bfloat16*>(projection.data), projection.bias, output, tiled,
        projection.out_features, projection.in_features, DenseGemmBias::kBFloat16, 0, 0, stream);
  if (tiled != count)
    launch_deterministic_scalar_gemm_nt(pixels, projection.data, projection.bias, output,
                                        count - tiled, projection.out_features,
                                        projection.in_features, DenseGemmMode::kBFloat16,
                                        DenseGemmBias::kBFloat16, tiled, tiled, stream);
  qwen_vision_add_positions_exact(output, position_table, position_index, rows, 1152, stream);
}
...
void qwen_vision_merger_forward(cudaStream_t stream, LinearRunner& linear,
                                ...
                                __nv_bfloat16* output, int rows, Workspace& ws, float eps) {
  constexpr int dim = 1152, merged_dim = 4608;
  if (rows <= 0 || rows % 4)
    throw std::runtime_error("qwen vision merger: rows not divisible by four");
  ...
  if (w.norm_before_merge) {
    ...
    launch_layernorm_affine(merged, w.norm_weight, w.norm_bias, normed, groups, merged_dim, eps,
                            stream);
    merged = normed;
  }
...
void qwen_vision_merger_forward_exact(cudaStream_t stream, const QwenVisionMergerWeights& w,
  ...
  if (rows <= 0 || rows % 4 != 0 || !x || !normed || !merged || !hidden || !output ||
      w.fc1.format != QuantFormat::kBF16 || w.fc2.format != QuantFormat::kBF16)
  ...
  auto projection = [&](const QuantWeight& weight, const __nv_bfloat16* input,
    ...
    if (tiled != 0)
      launch_deterministic_bf16_gemm_nt(input, static_cast<const __nv_bfloat16*>(weight.data),
                                        weight.bias, destination, tiled, weight.out_features,
                                        weight.in_features, DenseGemmBias::kBFloat16, 0, 0, stream);
    if (tiled != groups)
      launch_deterministic_scalar_gemm_nt(input, weight.data, weight.bias, destination,
                                          groups - tiled, weight.out_features, weight.in_features,
                                          DenseGemmMode::kBFloat16, DenseGemmBias::kBFloat16, tiled,
                                          tiled, stream);
  };
  projection(w.fc1, source, hidden);
  launch_gelu_tanh_exact(hidden, static_cast<size_t>(groups) * merged_dim, stream);
...
void qwen_vision_tower_forward(cublasHandle_t handle, cudaStream_t stream, LinearRunner& linear,
                               ...
                               const QwenVisionMergerWeights* deep, const float* cos,
                               const float* sin, __nv_bfloat16* x, int rows,
                               QwenVisionBlockScratch scratch, __nv_bfloat16* merger_normed,
                               __nv_bfloat16* merged, __nv_bfloat16* merger_hidden,
                               __nv_bfloat16* output, __nv_bfloat16** deep_out, Workspace& ws) {
  int deep_index = 0;
  for (int i = 0; i < 27; ++i) {
  ...
  qwen_vision_merger_forward(stream, linear, main_merger, x, merger_normed, merged, merger_hidden,
                             output, rows, ws);
}
