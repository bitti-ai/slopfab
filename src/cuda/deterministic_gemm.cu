...
__global__ void deterministic_bf16_gemm_kernel(const __nv_bfloat16* input,
                                               const __nv_bfloat16* weight, const void* bias,
                                               __nv_bfloat16* output, uint32_t rows,
                                               uint32_t out_features, uint32_t in_features,
                                               uint32_t bias_type, uint32_t input_row_offset,
  ...
  for (uint32_t base = 0; base < in_features; base += 16) {
    wmma::load_matrix_sync(a, input + (input_row_offset + row) * in_features + base, in_features);
  ...
  for (uint32_t pair = lane; pair < 128; pair += 32) {
    ...
    reinterpret_cast<__nv_bfloat162*>(output + (output_row_offset + row + tile_row) *
                                                   out_features)[(column + tile_column) >> 1] =
        packed;
  }
}

__global__ void deterministic_f16_gemm_kernel(const __half* input, const __half* weight,
                                              float* output, uint32_t out_features,
                                              uint32_t in_features, uint32_t output_row_offset) {
  const uint32_t warp = threadIdx.x >> 5;
  const uint32_t row = blockIdx.y * 64 + warp * 16;
  const uint32_t column = blockIdx.x * 16;
  ...
  for (uint32_t item = threadIdx.x & 31; item < 256; item += 32) {
    ...
    output[(output_row_offset + row + tile_row) * out_features + column + tile_column] =
        accumulator[warp][item];
  }
}

__global__ void deterministic_scalar_gemm_kernel(const void* input, const void* weight,
                                                 const void* bias, void* output, uint32_t rows,
                                                 uint32_t out_features, uint32_t in_features,
                                                 uint32_t mode, uint32_t bias_type,
  ...
  if (mode == static_cast<uint32_t>(DenseGemmMode::kBFloat16)) {
    ...
    if (bias_type == static_cast<uint32_t>(DenseGemmBias::kFloat32))
      ...
      sum += __bfloat162float(static_cast<const __nv_bfloat16*>(bias)[column]);
...
void launch_deterministic_bf16_gemm_nt(const __nv_bfloat16* input, const __nv_bfloat16* weight,
                                       const void* bias, __nv_bfloat16* output, uint32_t rows,
                                       uint32_t out_features, uint32_t in_features,
                                       DenseGemmBias bias_type, uint32_t input_row_offset,
                                       uint32_t output_row_offset, cudaStream_t stream) {
  if (!input || !weight || !output || rows == 0 || rows % 64 != 0 || out_features == 0 ||
      out_features % 16 != 0 || in_features == 0 || in_features % 16 != 0 ||
  ...
  if (bias_type != DenseGemmBias::kNone && bias_type != DenseGemmBias::kFloat32 &&
      bias_type != DenseGemmBias::kBFloat16) {
    throw std::invalid_argument("deterministic CUDA BF16 GEMM: invalid bias type");
  }
  deterministic_bf16_gemm_kernel<<<dim3(out_features / 16, rows / 64), 128, 0, stream>>>(
      input, weight, bias, output, rows, out_features, in_features,
      static_cast<uint32_t>(bias_type), input_row_offset, output_row_offset);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}

void launch_deterministic_f16_gemm_nt(const __half* input, const __half* weight, float* output,
                                      uint32_t rows, uint32_t out_features, uint32_t in_features,
                                      uint32_t output_row_offset, cudaStream_t stream) {
  if (!input || !weight || !output || rows == 0 || rows % 64 != 0 || out_features == 0 ||
      out_features % 16 != 0 || in_features == 0 || in_features % 16 != 0) {
    throw std::invalid_argument("deterministic CUDA F16 GEMM: invalid full-tile arguments");
  }
  deterministic_f16_gemm_kernel<<<dim3(out_features / 16, rows / 64), 128, 0, stream>>>(
      input, weight, output, out_features, in_features, output_row_offset);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}

void launch_deterministic_scalar_gemm_nt(const void* input, const void* weight, const void* bias,
                                         void* output, uint32_t rows, uint32_t out_features,
                                         uint32_t in_features, DenseGemmMode mode,
                                         DenseGemmBias bias_type, uint32_t input_row_offset,
                                         uint32_t output_row_offset, cudaStream_t stream) {
  const bool mode_valid = mode == DenseGemmMode::kBFloat16 || mode == DenseGemmMode::kFloat16Vae ||
                          mode == DenseGemmMode::kFloat32;
  const bool bias_valid =
      bias_type == DenseGemmBias::kNone ||
      (mode == DenseGemmMode::kBFloat16 &&
       (bias_type == DenseGemmBias::kFloat32 || bias_type == DenseGemmBias::kBFloat16)) ||
      (mode == DenseGemmMode::kFloat32 && bias_type == DenseGemmBias::kFloat32);
  if (!input || !weight || !output || rows == 0 || out_features == 0 || in_features == 0 ||
      !mode_valid || !bias_valid || ((bias_type == DenseGemmBias::kNone) != (bias == nullptr))) {
    throw std::invalid_argument("deterministic CUDA scalar GEMM: invalid arguments");
  }
  constexpr uint32_t tx = 16, ty = 8;
  deterministic_scalar_gemm_kernel<<<dim3((out_features + tx - 1) / tx, (rows + ty - 1) / ty),
                                     dim3(tx, ty), 0, stream>>>(
      input, weight, bias, output, rows, out_features, in_features, static_cast<uint32_t>(mode),
      static_cast<uint32_t>(bias_type), input_row_offset, output_row_offset);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
}
