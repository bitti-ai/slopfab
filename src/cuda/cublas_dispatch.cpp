...
void add_root(std::vector<std::wstring>& roots, std::wstring root) {
  while (!root.empty() && (root.back() == L'\\' || root.back() == L'/'))
    root.pop_back();
  if (root.size() >= 8 && _wcsicmp(root.c_str() + root.size() - 8, L"\\bin\\x64") == 0)
    root.resize(root.size() - 8);
  else if (root.size() >= 4 && _wcsicmp(root.c_str() + root.size() - 4, L"\\bin") == 0)
...
void load_candidate(CublasApi& api, const CudaToolkitCandidate& selected) {
  ...
  if (lt == nullptr)
    throw std::runtime_error("cuBLAS: cannot load " + narrow(lt_path) + " (Windows error " +
                             std::to_string(GetLastError()) + ")");
  HMODULE blas = LoadLibraryExW(blas_path.c_str(), nullptr, flags);
  if (blas == nullptr) {
    const DWORD error = GetLastError();
    FreeLibrary(lt);
    throw std::runtime_error("cuBLAS: cannot load " + narrow(blas_path) + " (Windows error " +
                             std::to_string(error) + ")");
  }
  try {
    ...
    api.sgemm_strided_batched =
        symbol<decltype(api.sgemm_strided_batched)>(blas, "cublasSgemmStridedBatched", blas_path);
...
void load_windows(CublasApi& api, const std::wstring& requested) {
  ...
  if (driver_status != cudaSuccess) {
    throw std::runtime_error("cuBLAS: cannot query NVIDIA driver capability without a context: " +
  ...
  for (const CudaToolkitCandidate& candidate : found) {
    ...
    if (!cuda_driver_supports_toolkit(driver_version, candidate.major)) {
      const std::string reason =
          ...
          " cuBLAS requires driver API >= " + std::to_string(candidate.major * 1000) +
  ...
  throw std::runtime_error(
      "cuBLAS: cannot initialize requested CUDA " + narrow(requested) + ": " + failures +
...
cublasStatus_t cublas_sgemm(cublasHandle_t handle, cublasOperation_t transa,
                            ...
                            float* c, int ldc) {
  return api().sgemm(handle, transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
}
...
cublasStatus_t cublas_sgemm_strided_batched(cublasHandle_t handle, cublasOperation_t transa,
  ...
  return api().sgemm_strided_batched(handle, transa, transb, m, n, k, alpha, a, lda, stride_a, b,
...
cublasStatus_t cublas_gemm_ex(cublasHandle_t handle, cublasOperation_t transa,
  ...
  return api().gemm_ex(handle, transa, transb, m, n, k, alpha, a, a_type, lda, b, b_type, ldb, beta,
                       c, c_type, ldc, compute_type, algorithm);
}
...
cublasStatus_t cublas_gemm_strided_batched_ex(
  ...
  return api().gemm_strided_batched_ex(handle, transa, transb, m, n, k, alpha, a, a_type, lda,
                                       stride_a, b, b_type, ldb, stride_b, beta, c, c_type, ldc,
                                       stride_c, batch_count, compute_type, algorithm);
}
