...
template <int VEC>
__global__ void
add_gated_kernel(__nv_bfloat16* __restrict__ x, const __nv_bfloat16* __restrict__ branch,
                 const float* __restrict__ gate, const int32_t* __restrict__ a, int dim) {
...
void launch_sub_bf16(const __nv_bfloat16* a, const __nv_bfloat16* b, __nv_bfloat16* out, size_t n,
  ...
  if (vec_n) {
    sub_bf16_kernel<8>
        <<<grid_1d(vec_n / 8, kRowThreads), kRowThreads, 0, stream>>>(a, b, out, vec_n);
    SLOPFAB_CUDA_CHECK(cudaGetLastError());
  }
  if (vec_n < n) {
