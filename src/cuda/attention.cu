...
size_t attention_workspace_bytes(const AttentionConfig& cfg, int num_kv_heads,
  ...
  if (backend == AttentionBackend::kSage2) return sage2_workspace_bytes(cfg, num_kv_heads);
...
AttentionPlan AttentionPlan::compile(const AttentionDescriptor& descriptor,
  ...
  if (descriptor.query_tokens != descriptor.key_value_tokens ||
    ...
    throw std::invalid_argument("CUDA attention: this plan supports full/banded self-attention only");
  if (descriptor.arithmetic == AttentionArithmetic::kExact)
    throw std::invalid_argument("CUDA attention: use the exact operator plan for pinned arithmetic");
  ...
  const bool approximate = policy.band_ranges != nullptr ||
      policy.backend == AttentionBackend::kSage2 || policy.backend == AttentionBackend::kSol;
  if (approximate && descriptor.arithmetic != AttentionArithmetic::kApproximate)
    throw std::invalid_argument("CUDA attention: selected execution policy requires approximate arithmetic");
  AttentionConfig cfg;
...
AttentionPlan AttentionPlan::compile(const AttentionConfig& cfg, int kv_heads,
  ...
  plan.descriptor_ = {static_cast<uint32_t>(cfg.seq_len),
      static_cast<uint32_t>(cfg.seq_len), static_cast<uint32_t>(cfg.num_heads),
      static_cast<uint32_t>(kv_heads), static_cast<uint32_t>(cfg.head_dim),
      AttentionLayout::kTokensHeadsChannels,
      cfg.band_ranges ? AttentionMask::kFrameBanded : AttentionMask::kFull,
      cfg.band_ranges || backend == AttentionBackend::kSage2 || backend == AttentionBackend::kSol
          ? AttentionArithmetic::kApproximate : AttentionArithmetic::kEquivalent,
      cfg.scale};
  ...
  switch (backend) {
    case AttentionBackend::kBlocked: break;
    case AttentionBackend::kFused:
      if (!fused_supported(cfg))
        throw std::invalid_argument("CUDA attention: fused attention supports head widths 64 and 128");
      break;
    ...
    case AttentionBackend::kSol:
      if (kv_heads != cfg.num_heads || cfg.head_dim != 128 ||
          cfg.exact_prefix < 0 || cfg.exact_prefix > cfg.seq_len ||
          !std::isfinite(cfg.sol_beta) || !std::isfinite(cfg.sol_error_k) ||
          cfg.sol_error_k < 0.0f || !std::isfinite(cfg.sol_error_v) || cfg.sol_error_v < 0.0f)
        throw std::invalid_argument("CUDA attention: invalid Sol attention contract");
      break;
    default: throw std::invalid_argument("CUDA attention: unknown backend");
  }
  plan.config_ = cfg;
...
void AttentionPlan::forward(cublasHandle_t handle, cudaStream_t stream,
  ...
  if (!query || !key || !value || !output || output == query || output == key || output == value)
    throw std::invalid_argument("CUDA attention: inputs must exist and output must not alias inputs");
  if (device_ >= 0) {
    int current = -1;
