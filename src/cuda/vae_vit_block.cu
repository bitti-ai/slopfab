...
struct BlockWeightsDevice {
  DeviceBuffer<float> norm1, norm2, scale1, scale2, qkv_bias, out_bias, w1_bias, w2_bias;
  DeviceBuffer<uint16_t> qkv_weight, out_weight, w1_weight, w2_weight;
  bool loaded = false;

  void load(const ViTBlockWeightsView& w, const ViTBlockConfig& config, cudaStream_t stream) {
    loaded = false;
    validate_weights(w);
    ...
    scale1.allocate(d);
    scale2.allocate(d);
    qkv_weight.allocate(3 * d * d);
    qkv_bias.allocate(3 * d);
    out_weight.allocate(d * d);
    out_bias.allocate(d);
    w1_weight.allocate(2 * inner * d);
    w1_bias.allocate(2 * inner);
    w2_weight.allocate(d * inner);
    w2_bias.allocate(d);
  ...
  uint64_t bytes() const noexcept {
    return norm1.nbytes() + norm2.nbytes() + scale1.nbytes() + scale2.nbytes() +
           qkv_weight.nbytes() + qkv_bias.nbytes() + out_weight.nbytes() + out_bias.nbytes() +
           w1_weight.nbytes() + w1_bias.nbytes() + w2_weight.nbytes() + w2_bias.nbytes();
  }
};

struct BlockScratchDevice {
  DeviceBuffer<float> normed, qkv, q, k, v, projected, ffn, activation;
  DeviceBuffer<uint16_t> q_bf16, k_bf16, v_bf16, attention_bf16, prepared_q, prepared_k, prepared_v,
      prepared_dim, prepared_inner;

  explicit BlockScratchDevice(const ViTBlockConfig& c) {
    const size_t rows = c.sequence, d = c.dim, inner = c.ffn_inner;
    normed.allocate(rows * d);
    qkv.allocate(rows * 3 * d);
    q.allocate(rows * d);
    k.allocate(rows * d);
    v.allocate(rows * d);
    q_bf16.allocate(rows * d);
    k_bf16.allocate(rows * d);
    v_bf16.allocate(rows * d);
    attention_bf16.allocate(rows * d);
    prepared_q.allocate(rows * d);
    prepared_k.allocate(rows * d);
    prepared_v.allocate(rows * d);
    projected.allocate(rows * d);
    ffn.allocate(rows * 2 * inner);
    activation.allocate(rows * inner);
    prepared_dim.allocate(rows * d);
    prepared_inner.allocate(rows * inner);
  }

  uint64_t bytes() const noexcept {
    return normed.nbytes() + qkv.nbytes() + q.nbytes() + k.nbytes() + v.nbytes() +
           projected.nbytes() + ffn.nbytes() + activation.nbytes() + q_bf16.nbytes() +
           k_bf16.nbytes() + v_bf16.nbytes() + attention_bf16.nbytes() + prepared_q.nbytes() +
           prepared_k.nbytes() + prepared_v.nbytes() + prepared_dim.nbytes() +
           prepared_inner.nbytes();
  }
};

void exact_gemm(DeviceBuffer<float>& input, DeviceBuffer<uint16_t>& prepared,
                const DeviceBuffer<uint16_t>& weight, DeviceBuffer<float>& output, uint32_t rows,
                uint32_t out_features, uint32_t in_features, cudaStream_t stream) {
  launch_narrow_f16(input.get(), prepared.get(), static_cast<size_t>(rows) * in_features, stream);
  launch_deterministic_scalar_gemm_nt(
      reinterpret_cast<const __half*>(prepared.get()),
      reinterpret_cast<const __half*>(weight.get()), nullptr, output.get(), rows, out_features,
      in_features, DenseGemmMode::kFloat16Vae, DenseGemmBias::kNone, 0, 0, stream);
}

void run_exact_block(const ViTBlockConfig& c, const BlockWeightsDevice& w, BlockScratchDevice& s,
                     float* tokens, const float* cosine, const float* sine, cudaStream_t stream) {
  if (!w.loaded)
    throw std::logic_error("exact CUDA VAE ViT block: weights not loaded");
  ...
  exact_gemm(s.normed, s.prepared_dim, w.qkv_weight, s.qkv, rows, 3 * d, d, stream);
  launch_split_qkv_norm_rope(s.qkv.get(), w.qkv_bias.get(), cosine, sine, s.q.get(), s.k.get(),
                             s.v.get(), rows, c.heads, c.head_dim, c.rope_dim, c.num_patches,
                             c.epsilon, stream);
  launch_heads_to_tokens_bf16(s.q.get(), reinterpret_cast<__nv_bfloat16*>(s.q_bf16.get()), rows,
                              c.heads, c.head_dim, stream);
  launch_heads_to_tokens_bf16(s.k.get(), reinterpret_cast<__nv_bfloat16*>(s.k_bf16.get()), rows,
                              c.heads, c.head_dim, stream);
  launch_heads_to_tokens_bf16(s.v.get(), reinterpret_cast<__nv_bfloat16*>(s.v_bf16.get()), rows,
                              c.heads, c.head_dim, stream);
  launch_prepare_deterministic_attention_inputs(
      ...
      reinterpret_cast<__half*>(s.prepared_q.get()), reinterpret_cast<__half*>(s.prepared_k.get()),
  ...
  launch_deterministic_blocked_attention_f16(
      ...
      reinterpret_cast<__nv_bfloat16*>(s.attention_bf16.get()), rows, c.heads, c.head_dim, 0.125f);
  launch_widen_bf16(reinterpret_cast<const __nv_bfloat16*>(s.attention_bf16.get()), s.normed.get(),
                    count, stream);
  exact_gemm(s.normed, s.prepared_dim, w.out_weight, s.projected, rows, d, d, stream);
  launch_layerscale_residual(tokens, s.projected.get(), w.out_bias.get(), w.scale1.get(), rows, d,
                             stream);
  launch_rmsnorm(tokens, w.norm2.get(), s.normed.get(), rows, d, c.epsilon, stream);
  exact_gemm(s.normed, s.prepared_dim, w.w1_weight, s.ffn, rows, 2 * inner, d, stream);
  launch_swiglu(s.ffn.get(), w.w1_bias.get(), s.activation.get(), rows, inner, stream);
  exact_gemm(s.activation, s.prepared_inner, w.w2_weight, s.projected, rows, d, inner, stream);
  launch_layerscale_residual(tokens, s.projected.get(), w.w2_bias.get(), w.scale2.get(), rows, d,
                             stream);
}
...
public:
  ...
  DeviceBackend backend() const noexcept override {
    return DeviceBackend::kCuda;
  }

  const ViTBlockConfig& config() const noexcept override {
    return config_;
  }
  ...
  void forward(const float* tokens, const float* cosine, const float* sine,
    ...
    cosine_.copy_from_host(cosine, static_cast<size_t>(rows) * config_.rope_dim, stream_.get());
    sine_.copy_from_host(sine, static_cast<size_t>(rows) * config_.rope_dim, stream_.get());

    run_exact_block(config_, weights_, scratch_, tokens_.get(), cosine_.get(), sine_.get(),
                    stream_.get());
  ...
  uint64_t peak_device_bytes() const noexcept override {
    return persistent_bytes() + scratch_.bytes() + tokens_.nbytes() + cosine_.nbytes() +
           sine_.nbytes();
  }
...
struct ExactViTBlockGraph::Impl {
  ...
  struct ScratchSlot {
    ...
    ScratchSlot() {
      SLOPFAB_CUDA_CHECK(cudaEventCreateWithFlags(&completion, cudaEventDisableTiming));
    }
  ...
  Impl(const ViTBlockConfig& c, uint32_t layers) : config(c), layer_count(layers), blocks(layers) {
    select_shape(c);
  }
...
ExactViTBlockGraph::ExactViTBlockGraph(std::unique_ptr<Impl> impl) : impl_(std::move(impl)) {
}
...
ExactViTBlockGraph ExactViTBlockGraph::create(const ViTBlockConfig& config, uint32_t layers) {
...
void ExactViTBlockGraph::load(const SafeTensors& checkpoint) {
  if (!impl_)
    throw std::logic_error("exact CUDA VAE ViT graph: empty graph");
...
void ExactViTBlockGraph::load_layer(uint32_t layer, const vae::ViTBlockWeightsView& weights) {
  if (!impl_)
    throw std::logic_error("exact CUDA VAE ViT graph: empty graph");
...
void ExactViTBlockGraph::forward_device(float* tokens, const float* cosine, const float* sine,
  ...
  for (const BlockWeightsDevice& block : impl_->blocks) {
    run_exact_block(impl_->config, block, *slot.scratch, tokens, cosine, sine, stream);
  }
  slot.mark(stream);
}

void ExactViTBlockGraph::forward_layer_device(uint32_t layer, float* tokens, const float* cosine,
                                              const float* sine, cudaStream_t stream) const {
  if (!impl_)
    throw std::logic_error("exact CUDA VAE ViT graph: empty graph");
  if (layer >= impl_->layer_count)
    throw std::out_of_range("exact CUDA VAE ViT graph: layer out of range");
...
void ExactViTBlockGraph::forward(const float* tokens, const float* cosine, const float* sine,
  ...
  if (!impl_->host) {
    ...
    impl_->host->tokens.allocate(static_cast<size_t>(impl_->config.sequence) * impl_->config.dim);
  ...
  const size_t token_count = static_cast<size_t>(impl_->config.sequence) * impl_->config.dim;
  const size_t rope_count = static_cast<size_t>(impl_->config.sequence) * impl_->config.rope_dim;
  ...
  forward_device(impl_->host->tokens.get(), impl_->host->cosine.get(), impl_->host->sine.get(),
                 impl_->stream.get());
...
uint64_t ExactViTBlockGraph::peak_device_bytes() const noexcept {
  ...
  if (impl_->host) {
    total +=
        impl_->host->tokens.nbytes() + impl_->host->cosine.nbytes() + impl_->host->sine.nbytes();
  }
  return total;
}

std::unique_ptr<vae::ExactViTBlockStage>
create_exact_vae_vit_block_stage(const vae::ViTBlockConfig& config) {
