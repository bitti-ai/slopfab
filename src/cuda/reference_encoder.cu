...
template <typename T>
__global__ void col3(const T* x, T* col, int cin, int t, int h, int w, int k, int ss, int ts,
                     bool down, int oh, int ow, int start, int columns) {
  size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (i >= size_t(cin) * k * k * k * columns)
    return;
...
__global__ void col1(const float* x, float* col, int cin, int length, int k, int stride, int pad,
                     int dil, int start, int columns) {
  size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (i >= size_t(cin) * k * columns)
    return;
  int r = int(i / columns), p = int(i % columns) + start, s = p * stride + (r % k) * dil - pad;
...
__global__ void half_bias(__half* x, const __half* b, int n, size_t count) {
  size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (i < count)
    x[i] = __float2half_rn(__half2float(x[i]) + __half2float(b[i / n]));
}

__global__ void half_add(__half* x, const __half* branch, size_t count) {
  size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (i < count)
    x[i] = __float2half_rn(__half2float(x[i]) + __half2float(branch[i]));
}
...
__global__ void transpose_kernel(const float* x, float* y, int batch, int rows, int cols) {
  size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (i >= size_t(batch) * rows * cols)
    return;
  int c = int(i % cols), r = int(i / cols) % rows, b = int(i / (size_t(rows) * cols));
  y[(size_t(b) * cols + c) * rows + r] = x[i];
}

__global__ void element(float* x, const float* p, size_t n, int channels, int length, int op) {
  size_t i = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (i >= n)
    return;
  if (op == 0)
    ...
    float v = x[i];
    x[i] = .5f * v * (1 + tanhf(.7978845608028654f * (v + .044715f * v * v * v))) * p[i];
  }
}

__global__ void norm_kernel(const float* x, const float* w, const float* b, float* y, int width) {
  int row = blockIdx.x;
  ...
  for (int c = threadIdx.x; c < width; c += 256)
    y[size_t(row) * width + c] = (x[size_t(row) * width + c] - mean) * inv * w[c] + b[c];
}

template <typename T>
__global__ void gn(const T* x, const __half* w, const __half* b, T* y, int channels, int frames,
                   int spatial) {
  int frame = blockIdx.x / 32, group = blockIdx.x % 32, cpg = channels / 32, count = cpg * spatial;
  __shared__ float a[256];
  float sum = 0;
...
__global__ void causal_attention(const float* qkv, const float* qb, const float* kb,
                                 const float* vb, float* y, int length) {
  ...
  for (int key = threadIdx.x; key <= query; key += 256) {
    ...
    for (int d = 0; d < 256; ++d) {
      ...
      dot = fmaf(qkv[(size_t(batch) * length + query) * 6144 + c] + qb[c],
                 qkv[(size_t(batch) * length + key) * 6144 + 2048 + c] + kb[c], dot);
    }
    dot *= .0625f;
    scores[key] = dot;
  ...
  for (int d = 128; d; d /= 2) {
    if (threadIdx.x < d)
      reduction[threadIdx.x] = fmaxf(reduction[threadIdx.x], reduction[threadIdx.x + d]);
    __syncthreads();
  }
  maximum = reduction[0];
  float sum = 0;
  ...
  for (int d = 128; d; d /= 2) {
    if (threadIdx.x < d)
      reduction[threadIdx.x] += reduction[threadIdx.x + d];
    __syncthreads();
  }
  sum = reduction[0];
  ...
  for (int key = 0; key <= query; ++key)
    value += scores[key] / sum * (qkv[(size_t(batch) * length + key) * 6144 + 4096 + c] + vb[c]);
...
void ReferenceEncoderOps::report_memory(const char* label) const {
  ...
  std::printf(
      "references  %s CUDA pools: activation peak %.3f GiB, scratch peak %.3f GiB, reserved %.3f GiB, %zu allocations\n",
...
ReferenceBuffer<float> ReferenceEncoderOps::conv3d(const float* x, const __half* w, const __half* b,
                                                   int ci, int co, int t, int h, int width, int k,
                                                   int ss, int ts, bool down) {
  if (!x || !w || ci <= 0 || co <= 0 || t <= 0 || h <= 0 || width <= 0 || (k != 1 && k != 3) ||
      (ss != 1 && ss != 2) || (ts != 1 && ts != 2))
    throw std::invalid_argument("reference conv3d: invalid shape");
  int oh = h / ss, ow = width / ss, ot = (t - 1) / ts + 1, K = ci * k * k * k, N = ot * oh * ow;
  ...
  memory_.sample();
  convert<<<blocks(weight.size()), 256, 0, stream_>>>(w, weight.get(), weight.size());
  if (b)
    convert<<<blocks(co), 256, 0, stream_>>>(b, bias.get(), co);
  float one = 1, zero = 0;
  for (int start = 0; start < N; start += tile) {
    int n = std::min(tile, N - start);
    col3<<<blocks(size_t(K) * n), 256, 0, stream_>>>(x, col.get(), ci, t, h, width, k, ss, ts, down,
                                                     oh, ow, start, n);
    blas_check(cublas_sgemm(blas_, CUBLAS_OP_N, CUBLAS_OP_N, n, co, K, &one, col.get(), n,
                            weight.get(), K, &zero, out.get() + start, N));
  }
  if (b)
    bias_channels<<<blocks(out.size()), 256, 0, stream_>>>(out.get(), bias.get(), N, co);
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
  return out;
}

ReferenceBuffer<__half> ReferenceEncoderOps::conv3d(const __half* x, const __half* w,
                                                    const __half* b, int ci, int co, int t, int h,
                                                    int width, int k, int ss, int ts, bool down) {
  if (!x || !w || ci <= 0 || co <= 0 || t <= 0 || h <= 0 || width <= 0 || (k != 1 && k != 3) ||
      (ss != 1 && ss != 2) || (ts != 1 && ts != 2))
    throw std::invalid_argument("reference conv3d: invalid shape");
  const int oh = h / ss, ow = width / ss, ot = (t - 1) / ts + 1, K = ci * k * k * k,
            N = ot * oh * ow;
  ...
  for (int start = 0; start < N; start += tile) {
    ...
    col3<<<blocks(size_t(K) * n), 256, 0, stream_>>>(x, col.get(), ci, t, h, width, k, ss, ts, down,
                                                     oh, ow, start, n);
    blas_check(cublas_gemm_ex(blas_, CUBLAS_OP_N, CUBLAS_OP_N, n, co, K, &one, col.get(),
                              CUDA_R_16F, n, w, CUDA_R_16F, K, &zero, out.get() + start, CUDA_R_16F,
                              N, CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  }
  if (b)
    half_bias<<<blocks(out.size()), 256, 0, stream_>>>(out.get(), b, N, out.size());
  SLOPFAB_CUDA_CHECK(cudaGetLastError());
  return out;
}

ReferenceBuffer<float> ReferenceEncoderOps::conv1d(const float* x, const float* w, const float* b,
                                                   int batch, int ci, int co, int len, int k,
  ...
  for (int bch = 0; bch < batch; ++bch) {
    ...
    for (int start = 0; start < n; start += tile) {
      ...
      col1<<<blocks(size_t(K) * count), 256, 0, stream_>>>(
          x + size_t(bch) * ci * len, col.get(), ci, len, k, stride, pad, dil, start, count);
      blas_check(cublas_sgemm(blas_, CUBLAS_OP_N, CUBLAS_OP_N, count, co, K, &one, col.get(), count,
                              w, K, &zero, y + start, n));
    }
    if (b)
...
ReferenceBuffer<float> ReferenceEncoderOps::linear(const float* x, const float* w, const float* b,
                                                   int rows, int in, int out) {
  auto y = allocate<float>(size_t(rows) * out);
  memory_.sample();
  float one = 1, zero = 0;
  blas_check(cublas_sgemm(blas_, CUBLAS_OP_T, CUBLAS_OP_N, out, rows, in, &one, w, in, x, in, &zero,
                          y.get(), out));
  if (b)
...
ReferenceBuffer<float> ReferenceEncoderOps::norm(const float* x, const float* w, const float* b,
                                                 int rows, int width) {
...
ReferenceBuffer<float> ReferenceEncoderOps::transpose(const float* x, int batch, int rows,
                                                      int cols) {
  auto y = allocate<float>(size_t(batch) * rows * cols);
  transpose_kernel<<<blocks(y.size()), 256, 0, stream_>>>(x, y.get(), batch, rows, cols);
  return y;
}

ReferenceBuffer<float> ReferenceEncoderOps::attention(const float* x, const float* qb,
                                                      const float* kb, const float* vb, int batch,
                                                      int len) {
  if (len > 600 || len <= 0)
    throw std::invalid_argument("reference audio: attention length must be 1..600");
  auto heads = allocate<float>(size_t(batch) * len * 2048);
  auto out = allocate<float>(size_t(batch) * len * 32);
  memory_.sample();
  causal_attention<<<batch * len * 8, 256, 0, stream_>>>(x, qb, kb, vb, heads.get(), len);
  pool_heads<<<blocks(out.size()), 256, 0, stream_>>>(heads.get(), out.get(), batch * len);
  return out;
}

void ReferenceEncoderOps::snake(float* x, const float* a, int b, int c, int len) {
...
void reference_groupnorm(const float* x, const __half* w, const __half* b, float* y, int c, int t,
                         int h, int width, cudaStream_t stream) {
  gn<<<32 * t, 256, 0, stream>>>(x, w, b, y, c, t, h * width);
}

void reference_groupnorm(const __half* x, const __half* w, const __half* b, __half* y, int c, int t,
                         int h, int width, cudaStream_t stream) {
  gn<<<32 * t, 256, 0, stream>>>(x, w, b, y, c, t, h * width);
}
} // namespace slopfab::cuda
