...
__global__ void conv_kernel(const float* x, const __half* w, const __half* bias, float* y, int cin,
                            int cout, int ih, int iw, int oh, int ow, int k, int stride,
                            bool reflect, int pad_left, int pad_top) {
  ...
  for (int ic = 0; ic < cin; ++ic) {
    ...
    for (int ky = 0; ky < k; ++ky) {
      ...
      if (reflect)
        sy = reflect_index(sy, ih);
      else if (sy < 0 || sy >= ih)
        continue;
      for (int kx = 0; kx < k; ++kx) {
        int sx = ox * stride + kx - pad_left;
        if (reflect)
          sx = reflect_index(sx, iw);
        else if (sx < 0 || sx >= iw)
          continue;
...
__global__ void groupnorm_kernel(const float* x, const __half* weight, const __half* bias, float* y,
  ...
  for (int d = blockDim.x / 2; d; d >>= 1) {
    if (threadIdx.x < d)
      scratch[threadIdx.x] += scratch[threadIdx.x + d];
    __syncthreads();
  }
  if (threadIdx.x == 0)
    group_mean = deterministic_divide(scratch[0], count);
  __syncthreads();
  ...
  if (threadIdx.x == 0) {
    ...
    const uint32_t base_bits = positive_float_add(__float_as_uint(variance), __float_as_uint(eps));
...
void launch_keyframe_conv3d(const float* x, const __half* weight, const __half* bias, float* y,
  ...
  if (!x || !weight || !y || cin <= 0 || cout <= 0 || ih <= 0 || iw <= 0 || (k != 1 && k != 3) ||
      (stride != 1 && stride != 2))
  ...
  const int total = cout * oh * ow;
  conv_kernel<<<(total + 255) / 256, 256, 0, stream>>>(x, weight, bias, y, cin, cout, ih, iw, oh,
                                                       ow, k, stride, reflect, pad, pad);
}
...
void launch_keyframe_groupnorm_silu(const float* x, const __half* weight, const __half* bias,
  ...
  const uint64_t spatial =
      height > 0 && width > 0 ? static_cast<uint64_t>(height) * static_cast<uint64_t>(width) : 0;
  const uint64_t total = channels > 0 ? static_cast<uint64_t>(channels) * spatial : 0;
  const uint64_t group_count =
      groups > 0 && channels > 0 ? static_cast<uint64_t>(channels / groups) * spatial : 0;
  if (!x || !weight || !bias || !y || channels <= 0 || height <= 0 || width <= 0 || groups <= 0 ||
      channels % groups || !std::isnormal(eps) || eps <= 0.0f ||
      spatial > std::numeric_limits<int>::max() || total > std::numeric_limits<uint32_t>::max() ||
  ...
  groupnorm_kernel<<<groups, 256, 0, stream>>>(x, weight, bias, y, channels,
                                               static_cast<int>(spatial), groups, eps);
}
