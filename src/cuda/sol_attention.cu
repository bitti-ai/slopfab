...
constexpr size_t SolSharedBytes =
    (B * B + B * D) * sizeof(float) + (B * B + B * D) * sizeof(__nv_bfloat16);
...
__global__ void pool_kv(const __nv_bfloat16* k, const __nv_bfloat16* v, __nv_bfloat16* km,
                        __nv_bfloat16* vm, float* vs, float* k_residual, float* v_residual, int seq,
                        int heads) {
  ...
  if (d == 0) {
    const size_t b = size_t(kb) * heads + h;
    k_residual[b] = sqrtf(kr[0] / D);
    v_residual[b] = sqrtf(vr[0] / D);
  }
}
...
__global__ void key_stats(const __nv_bfloat16* km, float* key_mean, float* key_var, int nblocks,
                          int heads) {
...
__global__ void thresholds(const __nv_bfloat16* q, const float* key_mean, const float* key_var,
                           float* tau, int seq, int heads, float scale, float beta) {
  ...
  if (t < D) {
    ...
    for (int r = qlo; r < qhi; ++r)
      x += __bfloat162float(q[size_t(r) * width + h * D + t]);
  ...
  if (t == 0)
    tau[size_t(qb) * heads + h] = scale * (mu + beta * sqrtf(fmaxf(var, 0.0f)));
}

__global__ void sol(const __grid_constant__ CUtensorMap q_map, const __nv_bfloat16* q,
                    const __nv_bfloat16* k, const __nv_bfloat16* v, const __nv_bfloat16* km,
                    const float* vs, const float* tau, __nv_bfloat16* out, int seq, int heads,
  ...
  if (t == 0) {
    ...
    asm volatile("mbarrier.init.shared::cta.b64 [%0], 1;\n"
                 ...
                 : "+r"(bar), "=l"(state)
                 : "r"(B * D * int(sizeof(__nv_bfloat16))), "r"(dst), "l"(&q_map), "r"(0), "r"(h),
                   "r"(qlo)
                 : "memory");
    uint32_t ready;
    do {
  ...
  if (t < D) {
    float x = 0;
    for (int r = 0; r < qn; ++r)
      x += __bfloat162float(staged_q[r * D + t]);
    qm[t] = x / float(qn);
  }
  if (t < B) {
    om[t] = -FLT_MAX;
  ...
  for (int od = t; od < B * D; od += Threads)
    acc[od] = 0;
  __syncthreads();
...
#pragma unroll 4
      ...
      if (route_counts)
        atomicAdd(route_counts + (routes[t] ? 0 : 1), 1ull);
    }
    __syncthreads();
...
#pragma unroll
            for (int d = 0; d < D; d += 16) {
              ...
              wmma::load_matrix_sync(b, k + size_t(klo + kr) * width + h * D + d, width);
        ...
        if (t < qn) {
          ...
          for (int kr = 0; kr < kn; ++kr)
            bm = fmaxf(bm, score[t * B + kr]);
          const float nm = fmaxf(om[t], bm);
          float sum = 0;
          for (int kr = 0; kr < kn; ++kr)
            sum += expf(score[t * B + kr] - nm);
...
#pragma unroll
            for (int kr = 0; kr < B; kr += 16) {
              ...
              wmma::load_matrix_sync(b, v + size_t(klo + kr) * width + h * D + d, width);
              wmma::mma_sync(c, a, b, c);
            }
        ...
        for (int od = t; od < qn * D; od += Threads) {
          const int qr = od / D, d = od % D;
          acc[od] =
              acc[od] * rescale[qr] + approx_weight[qr] * vs[(size_t(kb) * heads + h) * D + d];
        }
      }
      __syncthreads();
    }
...
void validate(const AttentionConfig& c) {
  if (c.seq_len <= 0 || c.num_heads <= 0)
    throw std::runtime_error("Sol-Attn: invalid shape");
  if (c.head_dim != D)
    throw std::runtime_error("Sol-Attn: requires head_dim 128");
  if (c.band_ranges)
    throw std::runtime_error("Sol-Attn: frame banding is incompatible");
  if (c.exact_prefix < 0 || c.exact_prefix > c.seq_len)
    throw std::runtime_error("Sol-Attn: invalid exact_prefix");
  if (!std::isfinite(c.sol_beta))
    throw std::runtime_error("Sol-Attn: sol_beta must be finite");
...
CUtensorMap make_q_map(const __nv_bfloat16* q, const AttentionConfig& c) {
  ...
  const CUresult status = cuTensorMapEncodeTiled(
      &map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3, const_cast<__nv_bfloat16*>(q), dims, strides, box,
      elem, CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_NONE,
...
size_t sol_attention_workspace_bytes(const AttentionConfig& c) {
  if (c.seq_len <= 0 || c.num_heads <= 0 || c.head_dim != D)
    return 0;
...
void sol_attention_forward(cudaStream_t stream, const __nv_bfloat16* q, const __nv_bfloat16* k,
  ...
  if (!pipeline_ran) {
    SLOPFAB_CUDA_CHECK(cudaFuncSetAttribute(sol, cudaFuncAttributeMaxDynamicSharedMemorySize,
                                            int(SolSharedBytes)));
  ...
  if (c.sol_phase_ms) {
    SLOPFAB_CUDA_CHECK(cudaEventRecord(phase[4], stream));
    SLOPFAB_CUDA_CHECK(cudaEventSynchronize(phase[4]));
    for (int i = 0; i < 4; ++i) {
      SLOPFAB_CUDA_CHECK(cudaEventElapsedTime(c.sol_phase_ms + i, phase[i], phase[i + 1]));
