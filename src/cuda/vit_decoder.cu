...
// Uploads a checkpoint tensor to the device as fp32.
DeviceBuffer<float> upload(const SafeTensors& ckpt, const std::string& name,
  ...
  if (expected_elems != 0 && host.size() != expected_elems) {
    throw std::runtime_error("vae: tensor '" + name + "' has " + std::to_string(host.size()) +
                             " elements, expected " + std::to_string(expected_elems));
  }
  DeviceBuffer<float> buf(host.size());
  buf.copy_from_host(host.data(), host.size());
  return buf;
}
...
struct ViTDecoder::Impl {
  ...
  // Scratch, resized on demand for the current window size.
  int cap_seq = 0;
  ...
  void run_block(const BlockWeights& b, int seq, int num_patches) {
    ...
    gemm_nn_batched(d_scores.get(), d_v.get(), d_attn.get(), seq, hd, seq, heads, score_stride,
                    head_stride, head_stride);

    cuda::launch_merge_heads(d_attn.get(), d_merged.get(), seq, heads, hd, s);
    gemm_nt(d_merged.get(), b.out_w.get(), d_normed.get(), seq, dim, dim);
    cuda::launch_add_bias(d_normed.get(), b.out_b.get(), seq, dim, s);
    cuda::launch_layerscale_residual(d_tokens.get(), d_normed.get(), b.scale1.get(), seq, dim, s);
    ...
    cuda::launch_add_bias(d_ffn.get(), b.w1_b.get(), seq, 2 * cfg.ffn_inner, s);
    cuda::launch_swiglu(d_ffn.get(), d_act.get(), seq, cfg.ffn_inner, s);
    gemm_nt(d_act.get(), b.w2.get(), d_normed.get(), seq, dim, cfg.ffn_inner);
    cuda::launch_add_bias(d_normed.get(), b.w2_b.get(), seq, dim, s);
    cuda::launch_layerscale_residual(d_tokens.get(), d_normed.get(), b.scale2.get(), seq, dim, s);
  }
};
...
void ViTDecoder::load(const SafeTensors& ckpt, const ViTConfig& config) {
  ...
  // The reference runs fp32 params; keep cuBLAS off TF32 so our GEMMs are not
  // silently lower precision than the thing we are matching.
  CUBLAS_CHECK(cublasSetMathMode(d.blas, CUBLAS_PEDANTIC_MATH));

  const int dim = config.dim;
  const int inner = config.ffn_inner;
  const int ch = config.in_channels;

  d.x_embed_w = upload(ckpt, "decoder.x_embedder.weight", static_cast<size_t>(dim) * ch);
  d.x_embed_b = upload(ckpt, "decoder.x_embedder.bias", dim);
  d.register_tokens =
      upload(ckpt, "decoder.register_tokens", static_cast<size_t>(config.num_register) * dim);
  d.norm_out_w = upload(ckpt, "decoder.norm_out.weight", dim);
  d.norm_out_b = upload(ckpt, "decoder.norm_out.bias", dim);
  d.proj_out_w =
      upload(ckpt, "decoder.proj_out.weight", static_cast<size_t>(config.patch_dim()) * dim);
  d.proj_out_b = upload(ckpt, "decoder.proj_out.bias", config.patch_dim());
  d.post_quant_w = upload(ckpt, "post_quant_conv.weight", static_cast<size_t>(ch) * ch);
  d.post_quant_b = upload(ckpt, "post_quant_conv.bias", ch);
  ...
  for (int i = 0; i < config.num_layers; ++i) {
    ...
    b.norm1 = upload(ckpt, p + "norm1.weight", dim);
    b.norm2 = upload(ckpt, p + "norm2.weight", dim);
    b.scale1 = upload(ckpt, p + "scale1", dim);
    b.scale2 = upload(ckpt, p + "scale2", dim);
    b.qkv_w = upload(ckpt, p + "attn.to_qkv.weight", static_cast<size_t>(3) * dim * dim);
    b.qkv_b = upload(ckpt, p + "attn.to_qkv.bias", static_cast<size_t>(3) * dim);
    b.out_w = upload(ckpt, p + "attn.to_out.weight", static_cast<size_t>(dim) * dim);
    b.out_b = upload(ckpt, p + "attn.to_out.bias", dim);
    b.w1 = upload(ckpt, p + "ff.w1.weight", static_cast<size_t>(2) * inner * dim);
    b.w1_b = upload(ckpt, p + "ff.w1.bias", static_cast<size_t>(2) * inner);
    b.w2 = upload(ckpt, p + "ff.w2.weight", static_cast<size_t>(dim) * inner);
    b.w2_b = upload(ckpt, p + "ff.w2.bias", dim);
  }
...
void ViTDecoder::forward_window(const float* z, int T, int H, int W, std::vector<float>& out) {
  ...
  d.ensure_scratch(seq);
  d.build_rope(T, H, W, seq, num_patches);

  // Latent arrives channel-first [C, T, H, W]; the ViT wants one token per
  // voxel, channel-last, in (t, h, w) row-major order.
  const size_t voxels = static_cast<size_t>(num_patches);
  d.d_latent.allocate(static_cast<size_t>(ch) * voxels);
  d.d_latent.copy_from_host(z, static_cast<size_t>(ch) * voxels, s);

  std::vector<float> packed(voxels * ch);
  {
    std::vector<float> host(static_cast<size_t>(ch) * voxels);
    d.d_latent.copy_to_host(host.data(), host.size(), s);
    d.stream.synchronize();
    for (size_t n = 0; n < voxels; ++n) {
      for (int c = 0; c < ch; ++c) packed[n * ch + c] = host[c * voxels + n];
    }
  }
  d.d_patch.allocate(packed.size());
  d.d_patch.copy_from_host(packed.data(), packed.size(), s);

  // post_quant_conv is a 1x1x1 Conv3d, i.e. a per-token linear map.
  DeviceBuffer<float> quantised(voxels * ch);
  d.gemm_nt(d.d_patch.get(), d.post_quant_w.get(), quantised.get(), num_patches, ch, ch);
  cuda::launch_add_bias(quantised.get(), d.post_quant_b.get(), num_patches, ch, s);

  // x_embedder: Linear(24 -> 2048)
  d.gemm_nt(quantised.get(), d.x_embed_w.get(), d.d_tokens.get(), num_patches, dim, ch);
  ...
  out.resize(pixels);
  d.d_pixels.copy_to_host(out.data(), pixels, s);
  d.stream.synchronize();
}
