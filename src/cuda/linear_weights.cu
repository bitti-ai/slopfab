...
__device__ inline uint8_t f32_to_f8_e4m3_dev(float f) {
  ...
  int e = ilogbf(a);
  if (e < -6)
    e = -6; // below the smallest normal the exponent stops moving
  ...
  if (q < 8)
    return static_cast<uint8_t>(sign | q); // subnormal, exponent 0
...
__device__ inline void store8(__nv_bfloat16* p, const float* in) {
  store8_bf16(p, in);
}

__device__ inline void store8(__half* p, const float* in) {
  store8_half(p, in);
}
...
__global__ void dequant_nvfp4_kernel(const uint8_t* __restrict__ src,
  ...
  for (int o = static_cast<int>(blockIdx.y); o < out_features; o += static_cast<int>(gridDim.y)) {
...
template <typename T>
__global__ void
dequant_nf4_vec_kernel(const uint8_t* __restrict__ src, const uint8_t* __restrict__ absmax,
                       const float* __restrict__ quant_map,
                       const float* __restrict__ nested_quant_map,
                       const float* __restrict__ nested_absmax, int block_shift, int nested_shift,
                       float nested_offset, T* __restrict__ dst, size_t packs) {
  __shared__ float lut[16];
  if (threadIdx.x < 16)
    lut[threadIdx.x] = quant_map[threadIdx.x];
  __syncthreads();
  const size_t p = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  if (p >= packs)
    return;
  ...
  const float scale =
      nested_quant_map[absmax[scale_index]] * nested_absmax[scale_index >> nested_shift] +
      nested_offset;
...
__global__ void
dequant_nf4_kernel(const uint8_t* __restrict__ src, const uint8_t* __restrict__ absmax,
                   const float* __restrict__ quant_map, const float* __restrict__ nested_quant_map,
                   const float* __restrict__ nested_absmax, int block_size, int nested_block_size,
                   float nested_offset, __nv_bfloat16* __restrict__ dst, size_t n) {
  ...
  if (even + 1 < n)
    dst[even + 1] = __float2bfloat16(quant_map[packed & 0x0f] * scale);
}
...
__global__ void dequant_nf4_f16_kernel(const uint8_t* __restrict__ src,
  ...
  if (even + 1 < n)
    dst[even + 1] = __float2half_rn(quant_map[packed & 0x0f] * scale);
}
...
template <typename T> __device__ inline float load_as_f32(const T* p);

template <> __device__ inline float load_as_f32<float>(const float* p) {
  return *p;
}

template <> __device__ inline float load_as_f32<__nv_bfloat16>(const __nv_bfloat16* p) {
  return __bfloat162float(*p);
}

template <typename T> __device__ inline void store_from_f32(T* p, float v);

template <> __device__ inline void store_from_f32<float>(float* p, float v) {
  *p = v;
}

template <> __device__ inline void store_from_f32<__nv_bfloat16>(__nv_bfloat16* p, float v) {
  *p = __float2bfloat16(v);
}

// Element i of a group sits at word `i + i/4` of that group's slice.
__device__ inline int convrot_pad(int i) {
  return i + (i >> 2);
}

template <typename T>
__global__ void convrot_kernel(const T* __restrict__ in, T* __restrict__ out, int group, int stages,
                               float norm, int groups_per_block, size_t total_groups) {
...
__global__ void pre_quant_scale_kernel(const T* __restrict__ src,
                                       const __nv_bfloat16* __restrict__ scale, T* __restrict__ dst,
                                       int dim) {
  const int col = static_cast<int>(blockIdx.y * blockDim.x + threadIdx.x);
  if (col >= dim)
    return;
...
void launch_pre_quant_scale_impl(const T* src, const __nv_bfloat16* scale, T* dst, int rows,
  ...
  if (rows <= 0 || dim <= 0)
    return;
  if (scale == nullptr)
    throw std::runtime_error("pre_quant_scale: null scale");
...
void add_bias(void* y, bool y_is_f32, const QuantWeight& w, int rows, cudaStream_t stream) {
  ...
  if (y_is_f32) {
    ...
    case QuantFormat::kBF16:
      add_bias_kernel<<<grid, kThreads, 0, stream>>>(yf, static_cast<const __nv_bfloat16*>(w.bias),
                                                     cols);
      break;
    default:
    ...
    case QuantFormat::kF32:
      ...
      break;
    case QuantFormat::kBF16:
      add_bias_kernel<<<grid, kThreads, 0, stream>>>(yb, static_cast<const __nv_bfloat16*>(w.bias),
                                                     cols);
      break;
    default:
      throw std::runtime_error("linear: unsupported bias format");
...
const __nv_bfloat16* materialise_bf16(const QuantWeight& w, Workspace& ws, cudaStream_t stream) {
  ...
  case QuantFormat::kNF4:
    ...
    launch_dequant_nf4(static_cast<const uint8_t*>(w.data), w.nf4_absmax, w.nf4_quant_map,
                       w.nf4_nested_quant_map, w.nf4_nested_absmax, w.nf4_block_size,
                       w.nf4_nested_block_size, w.nf4_nested_offset, dst, w.out_features,
                       w.in_features, stream);
    break;
...
void pre_quant_scale_f32(const float* src, const __nv_bfloat16* scale, float* dst, int rows,
                         int dim, cudaStream_t stream) {
...
void launch_convrot_impl(const T* in, T* out, int rows, int dim, int group, cudaStream_t stream) {
  if (rows <= 0 || dim <= 0)
    return;
  if (dim % group != 0) {
    throw std::runtime_error("convrot: dim " + std::to_string(dim) +
                             " is not a multiple of group " + std::to_string(group));
  }
  ...
  const int shared = groups_per_block * (group + group / 4) * static_cast<int>(sizeof(float));
