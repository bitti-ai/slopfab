...
constexpr size_t SmemBytes =
    B * D * sizeof(__nv_bfloat16) + B * B * sizeof(float) + 4 * 16 * 16 * sizeof(__nv_bfloat16);
...
__device__ uint64_t issue(const CUtensorMap* map, void* dst, int h, int row, uint64_t* barrier) {
  ...
  asm volatile("mbarrier.init.shared::cta.b64 [%0], 1;\n"
               ...
               : "+r"(bar), "=l"(state)
               : "r"(B * D * int(sizeof(__nv_bfloat16))), "r"(ptr), "l"(map), "r"(0), "r"(h),
                 "r"(row)
               : "memory");
  return state;
#else
...
__global__ __launch_bounds__(Threads, 1) void exact_pipeline(
    ...
    const __grid_constant__ CUtensorMap vmap, __nv_bfloat16* out, const __nv_bfloat16* km,
    const __nv_bfloat16* vm, const float* vs, const float* k_residual, const float* v_residual,
    const float* tau, int seq, int heads, int prefix, float scale, float error_k, float error_v,
  ...
  wmma::fragment<wmma::matrix_a, 16, 16, 16, __nv_bfloat16, wmma::row_major> qr0, qr1, qr2, qr3,
      qr4, qr5, qr6, qr7;
  if (t < B) {
    old_m[t] = -FLT_MAX;
    denom[t] = 0.0f;
  ...
  for (int kb = t; kb < blocks; kb += Threads) {
    ...
    if (error_k > 0.0f) {
      ...
      if (error_v > 0.0f)
        hetero *= 1.0f + error_v * v_residual[bi];
    ...
    if (route_counts)
      atomicAdd(route_counts + (take ? 0 : 1), 1ull);
  }
  __syncthreads();
...
#define SLOPFAB_QK_STEP(QR, OFF)                                                                   \
  do {                                                                                             \
    wmma::fragment<wmma::matrix_b, 16, 16, 16, __nv_bfloat16, wmma::col_major> b;                  \
    wmma::load_matrix_sync(b, kv + kr * D + (OFF), D);                                             \
    wmma::mma_sync(c, (QR), b, c);                                                                 \
  } while (false)
      SLOPFAB_QK_STEP(qr0, 0);
      SLOPFAB_QK_STEP(qr1, 16);
      SLOPFAB_QK_STEP(qr2, 32);
      SLOPFAB_QK_STEP(qr3, 48);
      SLOPFAB_QK_STEP(qr4, 64);
      SLOPFAB_QK_STEP(qr5, 80);
      SLOPFAB_QK_STEP(qr6, 96);
      SLOPFAB_QK_STEP(qr7, 112);
  ...
  wmma::fragment<wmma::accumulator, 16, 16, 16, float> o0, o1, o2, o3, o4, o5, o6, o7;
#define SLOPFAB_INIT_O(C, OFF)                                                                     \
  do {                                                                                             \
    auto& frag = (C);                                                                              \
    const int d = (OFF), lane = t & 31, qr0 = warp * 16;                                           \
    for (unsigned i = 0; i < frag.num_elements; ++i) {                                             \
      const int row = qr0 + lane / 4 + int((i / 2) % 2) * 8;                                       \
      const int col = d + (lane % 4) * 2 + int(i % 2) + int(i / 4) * 8;                            \
      frag.x[i] =                                                                                  \
          (row < qn && approx_count)                                                               \
              ? __bfloat162float(out[size_t(qlo + row) * heads * D + h * D + col]) * denom[row]    \
              : 0.0f;                                                                              \
    }                                                                                              \
  } while (false)
  SLOPFAB_INIT_O(o0, 0);
  SLOPFAB_INIT_O(o1, 16);
  SLOPFAB_INIT_O(o2, 32);
  SLOPFAB_INIT_O(o3, 48);
  SLOPFAB_INIT_O(o4, 64);
  SLOPFAB_INIT_O(o5, 80);
  SLOPFAB_INIT_O(o6, 96);
  SLOPFAB_INIT_O(o7, 112);
#undef SLOPFAB_INIT_O
  ...
  for (int ordinal = 0; ordinal < exact_count; ++ordinal) {
    ...
    if (t < qn) {
      ...
      float sum = 0;
      for (int j = 0; j < B; ++j)
        sum += fast_exp(score[t * B + j] - nm);
      ratio[t] = rs;
      denom[t] = denom[t] * rs + sum;
      old_m[t] = nm;
    }
    __syncthreads();
    if (t == 0) {
...
#undef SLOPFAB_SCALE_O
    for (int j = 0; j < B; j += 16) {
      ...
      for (int p = (t & 31); p < 256; p += 32) {
        const int row = warp * 16 + p / 16, col = j + p % 16;
        ps[p] = (row < qn && col < kn)
                    ? __float2bfloat16_rn(fast_exp(score[row * B + col] - old_m[row]))
                    : __float2bfloat16_rn(0);
      }
      __syncwarp();
      wmma::fragment<wmma::matrix_a, 16, 16, 16, __nv_bfloat16, wmma::row_major> a;
      wmma::load_matrix_sync(a, ps, 16);
#define SLOPFAB_PV_STEP(C, OFF)                                                                    \
  do {                                                                                             \
    const int d = (OFF);                                                                           \
    wmma::fragment<wmma::matrix_b, 16, 16, 16, __nv_bfloat16, wmma::row_major> b;                  \
    ...
    wmma::mma_sync((C), a, b, (C));                                                                \
      ...
      SLOPFAB_PV_STEP(o0, 0);
      SLOPFAB_PV_STEP(o1, 16);
      SLOPFAB_PV_STEP(o2, 32);
      SLOPFAB_PV_STEP(o3, 48);
      SLOPFAB_PV_STEP(o4, 64);
      SLOPFAB_PV_STEP(o5, 80);
      SLOPFAB_PV_STEP(o6, 96);
      SLOPFAB_PV_STEP(o7, 112);
#undef SLOPFAB_PV_STEP
  ...
  const int lane = t & 31;
#define SLOPFAB_STORE_O(C, OFF)                                                                    \
  do {                                                                                             \
    const int qr0 = warp * 16, d = (OFF);                                                          \
    auto& frag = (C);                                                                              \
    for (unsigned i = 0; i < frag.num_elements; ++i) {                                             \
      const int row = qr0 + lane / 4 + int((i / 2) % 2) * 8;                                       \
      const int col = d + (lane % 4) * 2 + int(i % 2) + int(i / 4) * 8;                            \
  ...
  SLOPFAB_STORE_O(o0, 0);
  SLOPFAB_STORE_O(o1, 16);
  SLOPFAB_STORE_O(o2, 32);
  SLOPFAB_STORE_O(o3, 48);
  SLOPFAB_STORE_O(o4, 64);
  SLOPFAB_STORE_O(o5, 80);
  SLOPFAB_STORE_O(o6, 96);
  SLOPFAB_STORE_O(o7, 112);
...
bool map_for(const __nv_bfloat16* p, const AttentionConfig& c, CUtensorMap* m) {
  *m = CUtensorMap{};
  ...
  const CUresult status = cuTensorMapEncodeTiled(
      m, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3, const_cast<__nv_bfloat16*>(p), dims, strides, box,
...
bool sol_pipeline_forward(cudaStream_t stream, const __nv_bfloat16* q, const __nv_bfloat16* k,
                          const __nv_bfloat16* v, const __nv_bfloat16* km, const __nv_bfloat16* vm,
                          const float* vs, const float* k_residual, const float* v_residual,
                          const float* tau, __nv_bfloat16* out, const AttentionConfig& c) {
  if (c.head_dim != D || (c.seq_len + B - 1) / B > MaxBlocks)
    return false;
  ...
  if (!map_for(q, c, &q_map) || !map_for(k, c, &k_map) || !map_for(v, c, &v_map))
    return false;
  SLOPFAB_CUDA_CHECK(cudaFuncSetAttribute(
      exact_pipeline, cudaFuncAttributeMaxDynamicSharedMemorySize, int(SmemBytes)));
