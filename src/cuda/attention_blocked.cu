...
#define SLOPFAB_SOFTMAX_CASE(n)                                                                    \
  case n:                                                                                          \
    online_softmax_kernel<n><<<blocks, kSoftmaxThreads, shared, stream>>>(tile, acc, m_run, l_run, \
                                                                          key_block, head_dim);    \
    break;
  switch (chunks) {
  ...
  default:
    online_softmax_kernel<0>
        <<<blocks, kSoftmaxThreads, shared, stream>>>(tile, acc, m_run, l_run, key_block, head_dim);
    break;
  }
...
#pragma unroll
  for (int i = 0; i < 4; ++i)
    h[i] = __float22half2_rn(__bfloat1622float2(in[i]));
    ...
    for (int k0 = 0; k0 < S; k0 += bk_max) {
      ...
      if (G == 1) {
        SLOPFAB_CUBLAS_CHECK(cublas_gemm_strided_batched_ex(
            ...
            &zero, tile_buf, CUDA_R_16F, bk, static_cast<long long>(bq) * bk, H, CUBLAS_COMPUTE_32F,
            CUBLAS_GEMM_DEFAULT));
      } else {
      ...
      if (G == 1) {
        SLOPFAB_CUBLAS_CHECK(cublas_gemm_strided_batched_ex(
            handle, CUBLAS_OP_N, CUBLAS_OP_N, D, bq, bk, &one, v16 + static_cast<size_t>(k0) * kvld,
            CUDA_R_16F, kvld, D, tile_buf, CUDA_R_16F, bk, static_cast<long long>(bq) * bk, pv_beta,
