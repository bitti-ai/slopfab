...
Sha256Digest sha256_file(const std::string& path) {
  ...
  if (!input)
    throw std::runtime_error("SHA-256 cannot open " + path);
  ...
  while (input) {
    ...
    if (count > 0 && BCryptHashData(hash, chunk.data(), static_cast<ULONG>(count), 0) < 0) {
      fail("SHA-256 update failed");
    }
  }
  if (!input.eof())
    fail("SHA-256 read failed");
  Sha256Digest result{};
  ...
  while (input) {
    ...
    const std::streamsize count = input.gcount();
    if (count > 0)
      hash.update(chunk.data(), static_cast<size_t>(count));
  }
  if (!input.eof())
    throw std::runtime_error("SHA-256 read failed for " + path);
  return hash.finish();
#endif
}
