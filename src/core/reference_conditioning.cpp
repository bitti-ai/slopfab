...
std::vector<float> normalize_audio(const ReferenceAudio& audio, double duration, int n) {
  ...
  if (audio.start_seconds >= duration) return out;
  const int offset =
...
ReferenceConditionOptions animate_reference_options(int width, int height) {
  ...
  if (int64_t(width) * height > INT32_MAX) throw std::invalid_argument("Animate canvas too large");
...
std::vector<float> target_audio_rows(const std::vector<float>& encoded, int target_latents) {
  if (target_latents <= 0 || encoded.size() % 64 ||
      encoded.size() / 64 < size_t(target_latents))
  ...
  for (int c = 0; c < 2; ++c)
    std::copy_n(encoded.data() + c * source_channel, target_channel, out.data() + c * target_channel);
  return out;
}
...
void order_animate_references(std::vector<dit::ReferenceGeometry>& geometry,
  ...
  if (geometry.size() != 2 || geometry[0].kind != dit::ReferenceKind::kImage ||
      ...
      geometry[1].audio_rows() || video_rows.size() !=
          size_t(geometry[0].video_rows() + geometry[1].video_rows()) * 96)
...
ReferenceConditionPlan reference_condition_plan(const ReferenceMedia& reference,
                                                double target_seconds, const ReferenceConditionOptions& options) {
  reference.validate();
...
PreparedReference prepare_reference_condition(const ReferenceMedia& reference,
                                              ...
                                              bool prepare_vae, const ReferenceConditionOptions& options) {
