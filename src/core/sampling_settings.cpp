...
SamplingSettings parse_sampling_settings(std::string_view text) {
  ...
  const auto scalar = [](const json::Value& entry) {
    ...
    if (!std::isfinite(value) || std::abs(value) > std::numeric_limits<float>::max())
      throw std::runtime_error("sampling settings: number must be finite and representable as float32");
  ...
  for (const auto& [key, value] : root.as_object()) {
    ...
    if (key == "video_sigma_shift") settings.video_sigma_shift = scalar(value);
      ...
      for (const auto& sigma : value.as_array()) settings.base_sigmas->push_back(scalar(sigma));
    } else {
...
SamplingSettings sampling_schedule_defaults(sampler::ScheduleKind schedule) {
  SamplingSettings settings;
  switch (schedule) {
    case sampler::ScheduleKind::kDefault: return settings;
