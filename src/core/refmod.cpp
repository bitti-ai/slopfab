...
std::shared_ptr<const RefMod> RefMod::load(const std::string& path) {
  ...
  if (embedded == file.metadata().end())
    embedded = file.metadata().find("audio_refmod_meta");
  if (embedded != file.metadata().end()) {
    ...
    if (!in)
      throw std::runtime_error("refmod needs refmod_meta metadata or a .json sidecar: " + path);
  ...
  if (kind == "audio") {
    ...
    if (s.size() != 5 || s[0] != 1 || s[1] != 24 || s[3] % 2 || s[4] % 2 ||
      ...
      throw std::runtime_error(
          "visual refmod latent must be [1,24,T,H,W], with even H/W and T=1 for images");
  ...
  mod->path_ = path;
  mod->name_ = meta.find("name") ? meta.find("name")->as_string()
  ...
  if (strength < 1) {
    ...
    const auto round = [&](float x) {
      return dtype_ == DType::kF16    ? f16_to_f32(f32_to_f16(x))
             : dtype_ == DType::kBF16 ? bf16_to_f32(f32_to_bf16(x))
                                      : x;
    };
    for (size_t offset = 0; offset < z.size(); offset += plane) {
      ...
      for (size_t i = 0; i < plane; ++i)
        z[offset + i] =
            round(round(strength * z[offset + i]) + round((1 - strength) * round(blurred[i])));
    }
  }
  if (!audio)
    return patchify_reference_video(z.data(), geometry_.num_latent_frames, geometry_.latent_height,
                                    geometry_.latent_width);
  const int t = geometry_.num_audio_latents;
  std::vector<float> packed(z.size());
  for (int ch = 0; ch < 2; ++ch)
    for (int i = 0; i < t; ++i)
      for (int c = 0; c < 32; ++c)
        packed[(static_cast<size_t>(ch) * t + i) * 32 + c] =
            z[(static_cast<size_t>(c) * 2 + ch) * t + i];
  return packed;
}
...
void validate_refmods(const std::vector<RefModReference>& refs) {
  ...
  for (const auto& ref : refs) {
    if (!ref.mod)
      throw std::invalid_argument("refmod: null reference");
    check_strength(ref.strength);
    if (ref.copies < 1 || ref.copies > 10)
      throw std::invalid_argument("refmod copies must be between 1 and 10");
    if (ref.enabled())
      tokens += static_cast<int64_t>(ref.mod->token_count()) * ref.copies;
    if (tokens > std::numeric_limits<int>::max() / 96)
      throw std::invalid_argument(
          "refmods exceed the packed sequence size limit; reduce copies or references");
  }
}

void append_refmod_conditions(const std::vector<RefModReference>& refs, uint64_t seed,
  ...
  for (const auto& ref : refs) {
    ...
    for (int copy = 0; copy < ref.copies; ++copy) {
      ...
      if (g.kind != dit::ReferenceKind::kAudio) {
        const auto noise =
            sampler::video_noise(seed ^ (0x9e3779b97f4a7c15ULL * (geometry.size() + 1)),
        ...
        sampler::FlowScheduler::scale_noise(rows.data(), packed.data(), .999f, rows.size(),
                                            rows.data());
      }
