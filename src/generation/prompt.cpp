...
bool prepare_multimodal_prompt(const GenerateRequest& request, const RunOptions& options,
  ...
  if (options.source == LatentSource::kDenoise && !reference_images.empty() &&
    ...
    try {
      ...
      if (prompt_ids.size() > std::numeric_limits<size_t>::max() - nonvision_tokens)
        throw std::overflow_error("reference conditioning token overflow");
      nonvision_tokens += prompt_ids.size();
      const size_t total =
          text::qwen3vl_conditioning_token_count(reference_conditioning_grids, nonvision_tokens);
      ...
      for (size_t i = 0; i < labels.size(); ++i) {
        ...
        reference_conditioning_ids.insert(reference_conditioning_ids.end(), block.begin(),
                                          block.end());
      }
      reference_conditioning_ids.insert(reference_conditioning_ids.end(), prompt_ids.begin(),
                                        prompt_ids.end());
  ...
  if (!prepared_media.empty() && options.prompt_embedding_path.empty()) {
    ...
    if (!reference_images.empty())
      reference_conditioning_ids.resize(reference_conditioning_ids.size() - prompt_ids.size());
    ...
    for (const auto& media : prepared_media) {
      if (media.plan.audio_samples)
        emit_text("<Audio " + std::to_string(++audio_number) + ">: ");
      ...
      for (int pair = 0; pair < sampled; pair += 2) {
        ...
        emit_text("<" + std::to_string(tenths / 10) + "." + std::to_string(tenths % 10) +
                  " seconds>");
        ...
        media_qwen_pairs.push_back(
            text::qwen3vl_patchify_resized_rgb_pair(first_rgb, second_rgb, rw, rh));
      }
    }
    reference_conditioning_ids.insert(reference_conditioning_ids.end(), prompt_ids.begin(),
                                      prompt_ids.end());
...
bool encode_h3_prompt(const GenerateRequest& request, const RunOptions& options,
                      ReusedGenerationModels& reuse, PromptInputs& inputs,
                      text::PromptEmbedding& prompt, RunResult& result) {
  auto& fixed_prompt = inputs.fixed_prompt;
  const auto& reference_identities = inputs.reference_identities;
  const auto& reference_images = inputs.reference_images;
  auto& reference_conditioning_grids = inputs.reference_conditioning_grids;
  ...
  if (!options.prompt_embedding_path.empty()) {
    ...
    if (options.inference_backend == DeviceBackend::kCuda) {
      ...
      vulkan::ExactQwenTextEncoder encoder = vulkan::ExactQwenTextEncoder::create(context);
      encoder.load(encoder_file);
      prompt = qwen_images.empty() ? encoder.encode(ids) : encoder.encode(ids, qwen_images);
      if (options.verbose) {
        const auto& stats = encoder.stats();
        std::printf("conditioner Vulkan exact peak/reserved %.2f/%.2f GiB, %llu descriptors\n",
                    static_cast<double>(stats.peak_device_bytes) / (1024.0 * 1024.0 * 1024.0),
