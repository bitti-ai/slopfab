...
#define VIDFAB_CUBLAS_CHECK(expr) \
  ::vidfab::cuda::cublas_check((expr), #expr, __FILE__, __LINE__)
...
inline void gemm_nt_batched(cublasHandle_t h, const float* A, const float* B, float* C, int M,
  ...
  const float alpha = 1.0f;
  const float beta = 0.0f;
  VIDFAB_CUBLAS_CHECK(cublasSgemmStridedBatched(h, CUBLAS_OP_T, CUBLAS_OP_N, N, M, K, &alpha, B, K,
                                                strideB, A, K, strideA, &beta, C, N, strideC,
                                                batch));
}
