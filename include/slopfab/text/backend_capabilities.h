...
inline bool supports_exact_text_layer(const ExactTextLayerShape& shape) noexcept {
  if (shape.tokens <= 0 || shape.hidden != 5120 || shape.intermediate <= 0 ||
      shape.rms_norm_epsilon != 1.0e-6f)
    return false;
  AttentionDescriptor attention{static_cast<uint32_t>(shape.tokens),
                                static_cast<uint32_t>(shape.tokens),
                                static_cast<uint32_t>(shape.query_heads),
                                static_cast<uint32_t>(shape.key_value_heads),
                                static_cast<uint32_t>(shape.head_dim),
                                AttentionLayout::kTokensHeadsChannels,
                                AttentionMask::kCausal,
                                AttentionArithmetic::kExact,
                                exact_attention_scale(shape.head_dim)};
