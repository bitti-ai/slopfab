...
struct BlockOffloadPlan {
  ...
  size_t slots() const { return std::min<size_t>(count, 2); }
};
...
inline BlockOffloadPlan plan_block_offload(const std::vector<size_t>& blocks,
                                           size_t fixed, size_t budget,
                                           int forced_count = -1) {
  ...
  for (size_t b : blocks) p.resident_bytes = add(p.resident_bytes, b);
  for (;;) {
    ...
    if ((forced_count < 0 || p.count == static_cast<size_t>(forced_count)) &&
        p.device_bytes <= budget) return p;
    if (p.count == blocks.size() ||
        (forced_count >= 0 && p.count == static_cast<size_t>(forced_count)))
      throw std::runtime_error("transformer offload: weights and transfer buffers exceed the available VRAM budget");
