...
class RefMod {
 public:
  ...
  const dit::ReferenceGeometry& geometry() const { return geometry_; }
  int token_count() const { return geometry_.video_rows() + geometry_.audio_rows(); }
...
struct RefModReference {
  ...
  bool enabled() const { return mod && strength > 0.0f; }
};

void validate_refmods(const std::vector<RefModReference>& references);
void append_refmod_conditions(const std::vector<RefModReference>& references,
                              uint64_t seed,
