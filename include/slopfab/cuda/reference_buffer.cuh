...
class ReferenceBufferPool {
 public:
  ...
  Lease acquire(size_t bytes) {
    ...
    if (best == blocks_.size()) {
      ...
      for (size_t i = 0; i < blocks_.size(); ++i) if (!blocks_[i].used) {
        reserved_ -= blocks_[i].data.size();
        blocks_[i].data.reset();
        best = i;
      }
      if (best == blocks_.size()) blocks_.emplace_back();
 ...
 private:
  struct Block { DeviceBuffer<uint8_t> data; size_t used = 0; };
...
template <typename T> class ReferenceBuffer {
 public:
  ...
  ReferenceBuffer(ReferenceBuffer&& other) noexcept { swap(other); }
  ...
  void reset() noexcept {
    ...
    pointer_ = nullptr; pool_ = nullptr; count_ = 0;
  }
  T* get() const { return pointer_; }
  size_t size() const { return count_; }
  void copy_from_host(const T* src, size_t count, cudaStream_t stream) {
    if (count > count_) throw std::out_of_range("reference upload size");
    SLOPFAB_CUDA_CHECK(cudaMemcpyAsync(pointer_, src, count * sizeof(T), cudaMemcpyHostToDevice, stream));
  }
  void copy_to_host(T* dst, size_t count, cudaStream_t stream) const {
    if (count > count_) throw std::out_of_range("reference download size");
    SLOPFAB_CUDA_CHECK(cudaMemcpyAsync(dst, pointer_, count * sizeof(T), cudaMemcpyDeviceToHost, stream));
  }
 private:
  void swap(ReferenceBuffer& other) noexcept {
    std::swap(pointer_, other.pointer_); std::swap(pool_, other.pool_);
    std::swap(slot_, other.slot_); std::swap(count_, other.count_);
  }
  T* pointer_ = nullptr;
