...
__device__ inline uint64_t round_shift_even(uint64_t value, unsigned shift) {
  ...
  return quotient +
         static_cast<uint64_t>(remainder > halfway || (remainder == halfway && (quotient & 1u)));
}

__device__ inline int highest_bit(uint32_t value) {
  int result = 0;
  for (uint32_t scan = value; (scan >>= 1u) != 0u;)
    ++result;
  return result;
}
...
__device__ inline uint32_t positive_float_div_uint(uint32_t bits, uint32_t divisor) {
  ...
  if (subnormal_shift >= 0)
    numerator <<= subnormal_shift;
  else if (-subnormal_shift < 32)
    denominator <<= -subnormal_shift;
  else
    return 0u;
...
__device__ inline uint32_t positive_float_add(uint32_t a_bits, uint32_t b_bits) {
  ...
  rounded += static_cast<uint64_t>(remainder > 4u || (remainder == 4u && (rounded & 1u)));
  if (rounded == (1ull << 24)) {
    rounded >>= 1u;
    ++ae;
  }
  return static_cast<uint32_t>(ae + 127) << 23u | static_cast<uint32_t>(rounded) & 0x007fffffu;
}
...
__device__ inline float deterministic_rsqrt(float value) {
  ...
  if (magnitude < 0x00800000u) {
    int leading = 0;
    for (uint32_t scan = significand; (scan >>= 1u) != 0u;)
      ++leading;
...
#pragma unroll
  ...
  int top = 0;
  for (uint64_t scan = estimate_q30; (scan >>= 1u) != 0u;)
    ++top;
  ...
  const uint32_t output_bits = static_cast<uint32_t>(output_exponent + 127) << 23u |
                               static_cast<uint32_t>(rounded) & 0x007fffffu;
  return __uint_as_float(output_bits);
}

__device__ inline float deterministic_norm_rsqrt(float sum, uint32_t divisor, float epsilon) {
...
__device__ inline float deterministic_float_divide(float numerator_value, float denominator_value);
...
__device__ inline float deterministic_exp(float value) {
  ...
  if (value <= 0.0f)
    return deterministic_exp_nonpositive(value);
  if (value >= 87.0f)
    return __uint_as_float(0x7f800000u);
  return deterministic_float_divide(1.0f, deterministic_exp_nonpositive(-value));
}
...
__device__ inline float deterministic_sin(float value) {
  ...
  const int quadrant =
      scaled >= 0.0f ? static_cast<int>(scaled + 0.5f) : static_cast<int>(scaled - 0.5f);
  float reduced = fmaf(-static_cast<float>(quadrant), 3.141592502593994140625f, value);
  reduced = fmaf(-static_cast<float>(quadrant), 1.5099579909783764e-7f, reduced);
...
__device__ inline float deterministic_snake(float value, float log_alpha, float log_beta) {
  value = canonicalize_pointwise_float(value);
  if ((__float_as_uint(value) & 0x7fffffffu) > 0x7f800000u)
    return __uint_as_float(0x7fc00000u);
  const float alpha = deterministic_exp(canonicalize_pointwise_float(log_alpha));
  const float beta = deterministic_exp(canonicalize_pointwise_float(log_beta));
  ...
  const float denominator =
      __uint_as_float(positive_float_add(__float_as_uint(beta), __float_as_uint(1.0e-9f)));
  const float periodic = (__float_as_uint(beta) & 0x7fffffffu) == 0x7f800000u
                             ? 0.0f
                             : deterministic_float_divide(square, denominator);
...
__device__ inline uint64_t deterministic_round_quotient_even(uint64_t numerator,
  ...
  if (remainder > complement || (remainder == complement && (quotient & 1ull) != 0ull)) {
    ++quotient;
  }
  return quotient;
}
...
__device__ inline float deterministic_float_divide(float numerator_value, float denominator_value) {
  ...
  if (numerator_magnitude < 0x00800000u)
  ...
  uint64_t significand = deterministic_round_quotient_even(static_cast<uint64_t>(a) << shift,
                                                           static_cast<uint64_t>(b));
  if (significand == (1ull << 24u)) {
    significand >>= 1u;
    ++exponent;
  }
  if (exponent >= 255)
  ...
  if (exponent <= 0) {
    ...
    if (lost > halfway || (lost == halfway && (base & 1ull) != 0ull))
      ++base;
...
__device__ inline float deterministic_silu(float value) {
  ...
  return (result_bits & 0x7fffffffu) < 0x00800000u ? __uint_as_float(result_bits & 0x80000000u)
                                                   : result;
}
...
__device__ inline float deterministic_pointwise_silu(float value) {
  ...
  if (value < 0.0f) {
    ...
    const float exponential = deterministic_exp_nonpositive(-value);
    result = deterministic_float_divide(
        value, __uint_as_float(positive_float_add(__float_as_uint(exponential), 0x3f800000u)));
  }
  return canonicalize_pointwise_float(result);
}
...
__device__ inline float deterministic_pointwise_gelu_tanh(float value) {
  ...
  const float inner = canonicalize_pointwise_float(__fadd_rn(value, __fmul_rn(0.044715f, cubic)));
  const float angle = canonicalize_pointwise_float(__fmul_rn(0.7978845608028654f, inner));
  ...
  if (angle_magnitude == 0x7f800000u) {
    ...
    const float exponential = deterministic_exp_nonpositive(__fmul_rn(-2.0f, fabsf(angle)));
    const float numerator = canonicalize_pointwise_float(__fadd_rn(1.0f, -exponential));
    const float denominator = canonicalize_pointwise_float(__fadd_rn(1.0f, exponential));
    tanh_value = deterministic_float_divide(numerator, denominator);
    if ((angle_bits & 0x80000000u) != 0u)
      tanh_value = -tanh_value;
  }
  const float half_value = canonicalize_pointwise_float(__fmul_rn(0.5f, value));
  const float one_plus_tanh = canonicalize_pointwise_float(__fadd_rn(1.0f, tanh_value));
  return canonicalize_pointwise_float(__fmul_rn(half_value, one_plus_tanh));
}
