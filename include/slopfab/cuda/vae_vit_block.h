...
public:
  ...
  static ExactViTBlockGraph create(const vae::ViTBlockConfig& config, uint32_t layers);
  ...
  void forward_layer_device(uint32_t layer, float* tokens, const float* cosine, const float* sine,
                            cudaStream_t stream) const;
