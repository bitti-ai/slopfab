...
class ReferenceEncoderOps {
 public:
  ...
  ReferenceBuffer<float> conv3d(const float* x, const __half* weight,
  ...
  ReferenceBuffer<float> conv1d(const float* x, const float* weight,
                             ...
                             int dilation);
  ReferenceBuffer<float> linear(const float* x, const float* weight,
                             const float* bias, int rows, int in, int out);
  ReferenceBuffer<float> norm(const float* x, const float* weight,
                           const float* bias, int rows, int width);
  ReferenceBuffer<float> transpose(const float* x, int batch, int rows,
                                int columns);
  ReferenceBuffer<float> attention(const float* qkv, const float* qb,
